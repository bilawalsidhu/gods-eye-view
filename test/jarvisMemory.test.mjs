...
test('Persistent memory system operations', async (t) => {
  await t.test(
    'executeTool routes remember, recall, and memory_stats',
    async () => {
  ...
  await t.test(
    'setMemory stores key with category, tags and timestamps',
    async () => {
      const res = await setMemory(
        'test_recon_target',
      ...
      assert.deepEqual(mem.test_recon_target.tags, [
        'satellite',
        'recon',
        'tokyo',
      ]);
  ...
  await t.test(
    'searchMemory retrieves by key, content, tags, or category',
    async () => {
  ...
  await t.test(
    'deleteMemory removes existing key and returns status',
    async () => {
      ...
      const delResNonExistent = await deleteMemory(
        'definitely_non_existent_key_999',
      );
