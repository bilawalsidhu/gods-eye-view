...
- Historic Fires scaffolding: `node scripts/fire-event-scaffold.mjs --name <incident> --year <YYYY> [--state US-XX] [--write]` looks an incident up in NIFC Open Data (WFIGS from 2020, the interagency perimeter history before), lists the candidates, takes the largest (or `--pick N`), derives the day window from the published discovery/containment dates, pads the perimeter extent into a bbox, picks the archive sources that existed that year and prints a validated `config/fire_events.json` entry; `--write` appends it. Anything not taken from the record (estimated dates, empty region/summary/references) is flagged. Pure helpers in `src/data/fireEventScaffold.js` with tests.

- Historic Fires official perimeters: each registered event may name a whitelisted NIFC Open Data source (WFIGS Interagency Perimeters or Interagency Fire Perimeter History) with structured filters; `/api/fire-history/<id>/perimeter` builds the query, keeps the largest matching polygon, simplifies it to ~50 m and caches it permanently. The layer draws it as a ground-clamped pale outline after the detections load (best effort, superseded by a newer selection), and the panel card reads `OFFICIAL PERIMETER · <ha> HA · <source> · AS OF <date>` with an NIFC link in Sources. Camp Fire, Lahaina and Park Fire are wired.

- Historic Fires voice control and share links: `control_fire_history` (list / select / replay / pause / reset / speed / seek / focus / status) drives the layer by voice and enables it itself; event names, years and ids are matched from the registered catalog and results carry the event, detection count, daily peak and clock so confirmations come from evidence. The selected event is now a layer param (`eventId`, share token `y` + option `e`): chips, the panel roster and voice all settle through the manager, so share links restore the chosen event. Tool inventory pins re-derived (28 → 29 tools).

- Historic Fires context panel: the layer mounts a `HISTORIC FIRES` section into the Context panel (above Radio) while enabled — event roster, event card (region, window, burned area, detection count, summary), a per-day detections chart that carries the replay cursor, a seek slider, a 0.5×–4× speed slider, FOCUS / ▶ REPLAY SPREAD / ↺ ALL, and https reference links from the event config. Markup builders are pure (`src/layers/fireHistory/panel.js`) and escape all config/feed text.

- Historic Fires replay: a ▶ REPLAY / ❚❚ PAUSE / ↺ ALL / speed chip set in the layer row plays an event's detections in event time (6 event-hours per second at 1×; 0.5×–4×). Pending detections hide, fresh ones flare for 12 event-hours, older ones cool into the burn scar; the row reads `REPLAY · <UTC clock> · <speed>×` and `shown / total`. The clock, phases and chips are pure functions in `src/layers/fireHistory/replay.js`; the layer exposes `toggleReplay`, `resetReplay`, `setReplaySpeed`, `seekReplay` and `getReplayState()` for the upcoming context panel and voice tools.

- Add the Historic Fires data layer: archived NASA FIRMS detections for registered past wildfires (`config/fire_events.json`), rendered as depth-test-free points colored early → late across the event window, with per-event row chips, a progress legend, camera framing on selection, analyst records, and a per-event timeline exposed for the upcoming replay panel. The `/api/fire-history` proxy fetches only registered events in ≤5-day windows and caches complete events permanently; keyless it lists events and reports KEY REQUIRED.
