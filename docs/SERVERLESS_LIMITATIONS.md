...
| --- | --- | --- |
| `/api/ais-live` (+ `/track`) | `server/providers/vessels/ais-live.js` `aisLiveProxy()`'s `configureServer()` starts an (unref'd) `setInterval` THE MOMENT it is called — independent of any request — whose tick opens a persistent outbound WebSocket to `wss://stream.aisstream.io` when `AISSTREAM_API_KEY` is set, and accumulates an in-memory vessel cache. A Vercel Function instance can be frozen between invocations and killed at any time, cannot hold a long-lived WebSocket, and AISStream allows only ONE connection per key — a fleet of function instances would fight over it. `app.js` never calls this plugin's `.configureServer()` at all in serverless mode (the plugin object itself is inert to construct — see the code comment in `app.js` — so simply never mounting it is sufficient; nothing needs to be torn down). | A long-running Node process (`npm run dev`, `vite preview` with the provider plugins, or a small container running either) that keeps the socket and in-memory store alive across requests. |
...
| 06:56:38Z | `GET /api/ais-live` | 501 | `{"error":"unavailable_in_serverless","feature":"ais-live","message":"Live vessel relay (AISStream WebSocket) is unavailable in the serverless deployment"}` |
