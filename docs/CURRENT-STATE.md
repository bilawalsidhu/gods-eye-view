...
- **A key-gated layer names its key on the control** (#143). Wildfires is the one DATA LAYERS row a provider key gates, and keyless it names the key rather than only reporting a state: the row's toggle tooltip reads `Needs FIRMS_MAP_KEY — add it in Provider Settings`. Two halves, so either can be reused: a layer may declare `requiresKeyId` (a `KEY_SETUP_KEYS` id from `src/keySetupCore.mjs`, mirroring `showInTogglePanel`), which `DataLayerManager.getAll()` publishes on the row; and it reports `stats.keyRequired` while that key is absent. `layerKeyRequirementTooltip()` (exported from `src/data/manager.js`) turns the pair into text, and `DataLayerManager._syncToggleButton` writes it onto the control — so a layer that needs no key, or already has one, carries no title. An unnamed or unknown registry id yields NO tooltip on purpose: guidance naming the wrong env var sends the operator to the wrong provider. `stats.keyRequired` is also what `src/loadingFeedback.js` already reads, so the keyless state is now a declared missing-optional-key rather than a generic feed error.
