...
- Both track-backfill routes carry an always-on per-client limiter of 30 requests/minute (90 global), because their 60s per-key cache bounds memory rather than spend: a caller varying `icao24`/`hex` produces a fresh key and a fresh upstream call each time. `/api/opensky-track` additionally reads `/api/opensky`'s 429 credit cooldown and, while it is active, serves any cached track and otherwise answers `429` with the remaining cooldown instead of spending 4 more credits against an account that is already out of budget.
