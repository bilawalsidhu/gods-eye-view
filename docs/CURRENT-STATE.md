# OnDemand Spatial Intelligence Current State
...
`ondemand-spatial-intelligence/layers/military` exports `createMilitaryFlightLayer`. It uses the
...
`ondemand-spatial-intelligence/aircraft` exports the existing shared classification, icon,
...
`ondemand-spatial-intelligence/layers/flights` exports `createCivilFlightLayer`. Each instance
...
through `ondemand-spatial-intelligence/sources/live`. The standalone adapters use the existing
...
`ondemand-spatial-intelligence/scenes` exports the same director used by the standalone app.
...
under `ondemand-spatial-intelligence/server/providers/`. Portable terrain mechanics, traffic tile
math and GBFS source rules are available under `ondemand-spatial-intelligence/sources/`.
...
The Node-only `ondemand-spatial-intelligence/server/providers/space` export supplies factories;
...
`ondemand-spatial-intelligence/build/vite` is a Node-only export for explicit browser build
...
`ondemand-spatial-intelligence/server/providers/live` is a Node-only entry for the existing
...
`ondemand-spatial-intelligence/sources/adsb-lol` export. Provider URLs, local credentials, cache
...
- A successful Pinokio install writes the owner-only `pinokio/.installed`
  ...
  running server with a captured ready URL exposes Open OnDemand Spatial Intelligence.
