...
- **Scene context** (`get_entity_context`): selected entity from the context store, or visible entities ranked by distance to the view target (≤100km altitude); plus basemap context — view-target picking, 7-point viewport sampling, view-scale classification (global/continental/regional/metro/city/local), reverse geocoding (center ≤750km, viewport samples ≤3,000km), Google Places Nearby via `/api/google/nearby-places` (≤25km), known-landmark matching against `CITY_POIS`, coarse country inference fallback. Context assembly is capped at 1.5s with cache fallbacks; caches are deduped in-flight.
  ...
  - **Satellites and local infrastructure join the analyst record seam** (2026-09-12). `getAnalystRecords()` on the CelesTrak satellites layer and on `createLocalGeoJsonLayer` (datacenters/dams) snapshots in-memory rows for `analyst_query`, so voice can answer counts and nearest queries the same way it already does for flights, ships, fires, and earthquakes. Core catalog rows are emitted before dense Starlink extras when the 2,000-record cap truncates. Compact items carry `noradId`/`satelliteClass` for satellites and `operator`/`river`/`output`/`capacity` for infrastructure. On-demand only — zero per-frame cost.
