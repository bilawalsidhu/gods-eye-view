...
| CCTV | Austin + Caltrans (CA) + TfL London + Fintraffic (FI) Open Data + Street View fallback | `src/data/cctv.js` | `/api/cctv` | 10s (active) |
...
- **CCTV v2 foundation:** a pitched
  ...
  on the main road spine. `CCTV_MAX_SOURCES` defaults to **1,200** (was 900) so the four packs fit
  at their own caps; the merge truncates by position, so a lower global cap starves the last pack.
