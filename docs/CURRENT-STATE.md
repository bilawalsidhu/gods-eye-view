...
- **Diagnostics**: client events are posted to `/api/realtime/debug-log`. Server storage is disabled unless `GEV_DEBUG_LOG=1`; enabled logs are redacted, size-limited, and written to a private system temporary directory outside the checkout (the console prints the path). See SECURITY.md for limits and cleanup. The last 30 errors persist in `localStorage` (`gev-realtime-errors`); use `window.__gevVoiceCommands.getDiagnostics()` in the console.
...
- Network access: direct loopback requests only in development and preview. LAN and reverse-proxy access are refused even with a wildcard bind; see SECURITY.md.
...
- Voice debug log: set `GEV_DEBUG_LOG=1`, then tail the private temporary file whose path the server prints on its first log write. Storage is capped at 8 MiB per process; see SECURITY.md.
