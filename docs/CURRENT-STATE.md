...
The optional **Weather** layer is the only data layer that owns Cesium imagery rather than entities, and the only one with no keyless path. It offers thirty Vaisala Xweather raster layers in two groups and draws only what is selected: sparse overlays — radar, lightning, warnings, cyclone tracks, storm cells, fronts, isobars, wind direction — stack freely above at most one continuous field, since fields paint every pixel and a second would hide the first. Radar alone is selected by default. A `WEATHER` panel in the right rail picks the layers, refreshes on demand or on an optional timer, and shows the running monthly tile count beside those controls; it is the layer's control surface, so it appears only while the layer is switched on. Refreshing asks the proxy for tiles newer than the moment it was asked, since the cache is tuned for spend and outlives the weather. Each layer is a `UrlTemplateImageryProvider` pointed at the app's own `/api/xweather` proxy; the credentials never reach the browser, because the vendor puts both halves in the upstream URL path. `GET /api/xweather/status` reports `{hasKey, monthCount, budget, month, refreshMs}`, and the proxy refuses any layer outside the catalogue allowlist before reading the key. The layer draws into whichever imagery collection is being rendered — the globe's, or a photoreal tileset's — and asks `MapSourceController` which that is rather than reading scene state. Each handle records the collection it went into and is only ever removed from that one, so a stack switch moves the imagery without orphaning any, and neither the base map at index 0 of the globe's collection nor the tileset's own tenants are ever disturbed. Within a collection the layer inserts at each spec's rung, so overlays stay above fields whatever order they refresh in. The layer row counts what is drawn (`6 ON`), and reads `NONE` when the selection is empty, which is a choice rather than a fault. A photoreal map stack hides the globe, so the layer re-homes onto the tileset and keeps drawing; the round trip redraws from the held frames with the same freshness floor, so it costs no read and, on a metered source, no bill. Draped imagery is far more constrained than globe imagery: Cesium composites it per model primitive and truncates past ten inputs per primitive, silently and from the top of the stack, so the layer draws at most `MAX_DRAPED_SPECS` at a time, spending that budget highest rung first — the sparse overlays worth seeing over a photograph rather than a field smeared across every facade. The row then reads `2 OF 4 ON` instead of claiming four, and the panel marks the rows being held back. A tileset never asks for a frame when its imagery changes, so the layer wakes the scene itself after every change to a collection. The selection travels in share links as one packed field of per-layer codes. Layer state and the existing voice layer tools include `weather`.
