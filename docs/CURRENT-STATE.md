...
### Repeaters (September 2026)

- DATA ▸ **Repeaters** (`ham-repeaters`, `src/layers/hamRepeaters/`) plots amateur-radio FM and D-STAR repeaters around the view. It is a directory, not a coverage map, and the panel, the marker card and the voice answer all say so: `distanceKm` is ground distance from the search centre and nothing in the layer models terrain, antennas, power or whether a repeater can actually be worked.
- **Camera-driven, never global.** `updateInterval` is `0` — there is no poll. The layer watches `camera.moveEnd`, debounces 1,500 ms (`MOVE_END_DEBOUNCE_MS`) and loads the repeaters around the new view only below `HEIGHT_GATE_M` = 1,500 km; above the gate it renders nothing and the panel reads `Fly below 1500 km or press LOAD HERE`. The radius follows the view span (`viewSpanKm` → `radiusForSpanKm`) clamped to 15–300 km, and `cameraFetchPlan` skips a fetch whose centre and radius are close enough to the last one, so panning inside a loaded area costs no requests. LOAD HERE in the panel bypasses the gate with the operator's own radius (25/50/100/200/300 km).
- **One provider-neutral row schema.** `src/sources/hamRepeaters.js` is pure and holds the rules: `REPEATER_ROW_KEYS` (the frozen row shape), band edges for 6m/2m/1.25m/70cm/23cm, colours by mode, the normalisers for both HamRig tables, filtering, sorting, nearest-N, the label/details/provenance strings and the camera-plan maths. The layer depends only on a source object with `getRepeaters({ lat, lon, radiusKm, band, kind, limit }, { signal })`; a second adapter (RepeaterBook under its own approved access, or OSM via Overpass) is a provider module plus a `sourceLabel`, with no change to the layer, the panel or the voice tool.
- **Server proxy.** `GET /api/ham-repeaters/nearby` (`server/providers/ham-repeaters/`) is the only path the browser talks to. It validates every parameter with its own message (`radiusKm must be <= 500`, `band must be one of all, 6m, 2m, 1.25m, 70cm, 23cm`, …), rounds the search to a 0.1° cell for the cache key, coalesces in-flight requests, keeps 256 cells for 10 minutes and serves a stale cell for up to ten times that when upstream fails, answers `Cache-Control: no-store`, and merges the two HamRig feeds first-wins by id with `partial` and per-feed `errors` when only one answers. 23cm is filtered locally because the upstream FM table has no such band filter. The D-STAR directory's sponsor e-mail and other contact fields are dropped on the server. `HAMRIG_ENABLED=0` makes the route answer 503, `HAM_REPEATERS_HAMRIG_FM=0` serves D-STAR only, and `HAMRIG_BASE_URL` must be https outside loopback.
- **Provenance is per row and always visible.** Every row carries `source`, `sourceLabel`, `confidence` and `recordUpdatedAt`. HamRig's FM table is a historic import whose upstream provenance is not cleared, so those rows are `unverified` and the upstream record ids are never relayed; the scraped D-STAR listings are `reported` with `statusKnown: false`. The panel row and the selected marker's third label line read `<source> · <confidence> · record <date>`, and a status is rendered as the directory last listed it, never as a live check. HamRig's login-only DL3EL route (`/api/repeaters/for-location`) has no code path here at all.
- **Panel and voice.** `#ham-repeaters-panel` (`src/ui/hamRepeatersControls.js` + `hamRepeatersPresentation.js`, share token `h`, Utilities group, not a Context companion) holds the enable toggle, radius, band and mode selects, LOAD HERE, the area line (gate, PARTIAL, STALE, errors) and the nearest-first list. The voice tool `show_ham_repeaters` takes a place or the current view, a band, a mode and a radius, switches the layer on itself, frames the results and returns them with their source, confidence and the distance caveat; German "Relais" and "Umsetzer" route to it, and it is explicitly not Internet Radio (`control_radio`).
- Regression surface: `src/sources/hamRepeaters.test.mjs` (rules and normalisers against both fixtures), `src/data/hamRepeatersProxy.test.mjs` (routing, validation table, caching, stale/partial, env and base-URL hardening), `src/layers/hamRepeaters/index.test.mjs` (layer lifecycle, gate, selection), `src/ui/hamRepeatersControls.test.mjs` (panel wiring), `src/voice/hamRepeaterActions.test.mjs` (the voice action), and the live gate `scripts/qa-ham-repeaters.mjs`.
