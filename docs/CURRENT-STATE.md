...
The optional **Weather** layer is the only data layer that owns Cesium imagery rather than entities. Like Live Vessels and Active Fires it has no keyless path; unlike them it is metered per tile past its free allowance. It offers thirty Vaisala Xweather raster layers in two groups and draws only what is selected: sparse overlays — radar, lightning, warnings, cyclone tracks, storm cells, fronts, isobars, wind direction — stack freely above at most one continuous field, since fields paint every pixel and a second would hide the first. Radar alone is selected by default. A `WEATHER` panel in the right rail picks the layers, refreshes on demand or on an optional timer, and shows the running monthly tile count beside those controls; it is the layer's control surface, so it appears only while the layer is switched on. Refreshing asks the proxy for tiles newer than the moment it was asked, since the cache is tuned for spend and outlives the weather. Each layer is a `UrlTemplateImageryProvider` pointed at the app's own `/api/xweather` proxy; the credentials never reach the browser, because the vendor puts both halves in the upstream URL path. `GET /api/xweather/status` reports `{hasKey, monthCount, budget, month, refreshMs}`, and the proxy refuses any layer outside the catalogue allowlist before reading the key. `MapSourceController` still owns the base map at index 0; this layer inserts at each spec's rung and removes only handles it added, so overlays stay above fields whatever order they refresh in. The layer row counts what is drawn (`6 ON`), and reads `NONE` when the selection is empty, which is a choice rather than a fault. A photoreal map stack hides the globe and every imagery layer with it, so the row then reports `UNAVAILABLE · GLOBE HIDDEN IN 3D`, and returning redraws from the held frames without re-fetching — which on a metered source also means without re-billing. The selection travels in share links as one packed field of per-layer codes. Layer state and the existing voice layer tools include `weather`.
