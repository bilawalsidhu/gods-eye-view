...
Transit feeds may register network resources beside their vehicle feed. MBTA registers a route catalog (V3 `route_patterns`, typical pattern per route and direction) and an alert feed (`Alerts_enhanced.json`), served as `/api/transit/routes/<id>` and `/api/transit/alerts/<id>` by the same portable request service, with their own cache windows, a stop-position lookup for stop-level alerts, and shared admission and backoff. The layer's network part (`src/layers/transit/network.js`) owns the route ground polylines, the disruption outline, the stop markers and their cards; selection owns one card at a time across vehicles, routes and stops; the row's `busLines` / `stopAlerts` params hide bus lines and stop markers. Alerts are normalized to bounded text and never rendered as HTML; alert links are not passed to the browser.
