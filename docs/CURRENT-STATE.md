...
### Web Receivers (September 2026)

- The layer sits in the Data Layers panel's Utilities group beside Radio, registered as `web-receivers` (share token `2`, `enabled-only`; panel `web-receivers-panel`, share token `w`). A Context companion like Radio: toggling it never exits Context.
- DATA ▸ **Web Receivers** (`src/layers/webReceivers/`) maps internet-controllable SDR receivers — KiwiSDR, WebSDR, OpenWebRX — and tunes them from the globe. `createWebReceiversLayer({ services, source })` owns its state (`state.js`), markers and cluster styling (`rendering.js`), picking (`interaction.js`), the UI snapshot and subscription (`presentation.js`), catalog acquisition (`ingestion.js`: one shared request, generation-guarded so a disable discards a late result) and the query surface (`queries.js`: `find` ranks online → coverage → preferred family → free slots → distance and highlights the results; `tune` and `showSpectrum` build the family-specific URL and record `lastTune`; `resolveReceiver` matches id, name, site or host). `src/app/layers/webReceivers.js` supplies ground, picking and render services; `src/standalone/layerSources.js` supplies `createWebReceiversSource()` (`getCatalog`).
- **Directory:** `server/providers/web-receivers/` (`/api/web-receivers/catalog`, registered in `server/providers/local.js` after Radio) merges Receiverbook's map data with the community KiwiSDR feed by receiver URL (`directory.js`), caches for 30 minutes with a seven-day stale fallback (`catalog.js`), pins outbound requests to validated public addresses and refuses redirects; no client-supplied URLs. websdr.org's list is deliberately not used (no-reuse notice). Rows carry `type`, `bands` (published for KiwiSDR, inferred from labels otherwise), `users`/`usersMax`, `online`, `antenna`, `sources`, `coverage`. The browser re-validates every row (`model.js`).
- **Rules:** `src/sources/webReceivers.js` is the portable half shared by the proxy, the layer, the panel and voice — the band table and label parser, frequency parsing and formatting, mode normalisation and per-band defaults, `buildTuneUrl` (KiwiSDR `?f=kHz+mode+zoom`, WebSDR `?tune=`, OpenWebRX `#freq=Hz,mod=`), `buildSpectrumUrl` (a zoomed, muted KiwiSDR spectrum; the other families are tuned to the range centre with audio on and a `note`), coverage answers (`true`/`false`/`null` when unpublished), `receiverMatchesFilter` and `rankWebReceivers`.
- **Panel (`#web-receivers-panel`, left stack after CCTV; `src/ui/webReceiversControls.js` + `webReceiversPresentation.js`, wired by `_initWebReceiversPanel` in the shell):** the enable toggle runs through the user-facing Context action; family/band filters; the receiver card; frequency (kHz) + mode + TUNE/OPEN; the SPECTRUM row (from/to kHz + SHOW); and a dock that embeds the tuned receiver page in an iframe (minimize keeps it running, close unloads it, disabling the layer closes it). On an https origin a plain-http receiver cannot be embedded; the dock says so and offers NEW TAB. GEV reclaims keyboard focus for eight seconds after a receiver page loads so push-to-talk keeps working; a deliberate click into the dock ends the guard.
- **Voice:** `find_web_receivers` (place, coordinates or the current view; optional frequency in kHz, band, family; enables the layer, highlights and frames the results), `tune_web_receiver` (selected / by id or name / nearest covering the frequency; dock or tab) and `show_rf_spectrum` (startKhz/stopKhz or centerKhz+spanKhz; selected / named / nearest receiver covering the whole range, KiwiSDRs preferred). Schemas in `src/voice/actionSchemas.js`, wording in `server/providers/openai/toolDescriptions.js` and `instructions.js`, execution in `src/voice/gevActions.js`. All return `covers`; the spectrum tool also returns `muted`. `set_layer_visibility` and `set_panel_open` accept `web-receivers` / `web-receivers-panel` and the usual aliases (SDR, KiwiSDR, WebSDR, OpenWebRX).
- **Proof:** `npm test` covers the rules (`src/sources/webReceivers.test.mjs`), the proxy (`src/data/webReceiversProxy.test.mjs`), the layer (`src/layers/webReceivers/*.test.mjs`), the panel controller (`src/ui/webReceiversControls.test.mjs`) and the voice handlers (`src/voice/webReceiverActions.test.mjs`); `node scripts/qa-web-receivers.mjs --url http://localhost:4173` drives the live layer, panel, dock and voice tools headlessly without contacting any receiver.
