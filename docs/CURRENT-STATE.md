...
- **Token flow**: browser fetches a short-lived client secret from `/api/realtime/token`; the Vite middleware holds the OpenAI credential and posts the full session config (instructions, tool schemas, VAD, truncation) to `api.openai.com/v1/realtime/client_secrets`. SDP exchange goes directly to `api.openai.com/v1/realtime/calls` with the ephemeral token.
- **Voice auth lanes** (`server/providers/openai/codex-auth.js`): `GEV_PREFER_CODEX_OAUTH=true` pin → `OPENAI_API_KEY` → the operator's local Codex CLI login (`$CODEX_HOME/auth.json` or `~/.codex/auth.json`, ChatGPT token mode). The Codex lane mints on the ChatGPT subscription instead of a metered key; the vendor file is read-only (never refreshed or rewritten here), an expired token fails closed with a `codex login` hint, and a token without an `exp` claim anchors its expiry estimate on the auth.json mtime (+1h). A blank `OPENAI_API_KEY` falls through — both lanes mint the same secret for the same provider, so there is no billing ambiguity. Responses echo the lane in `X-GEV-Voice-Auth` (`env` / `codex-oauth`). Ported from hermes-talk's `talk_auth.py` (itself a port of OpenClaw PR #100671).
...
- **Degradation**: without `OPENAI_API_KEY` or a usable Codex CLI login, `/api/realtime/token` returns 503 and the mic button surfaces the error; the rest of the app is unaffected.
...
- **Auth lanes match the voice token endpoint.** Without `OPENAI_API_KEY` (or with `GEV_PREFER_CODEX_OAUTH=true`), the summary rides the local Codex CLI login against the ChatGPT backend (`chatgpt.com/backend-api/codex/responses` — the Codex CLI's own endpoint, model `OPENAI_HUD_SUMMARY_CODEX_MODEL`, default `gpt-5.6-luna`): strict wire shape (`store:false`, SSE stream, list-form input, `ChatGPT-Account-Id` header from the JWT). A `usage_limit_reached` 429 or any upstream error still degrades to the free deterministic summary. Responses echo the lane in `X-GEV-Hud-Auth`.
