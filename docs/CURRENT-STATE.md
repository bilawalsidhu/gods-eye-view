...
### Civic Offices layer (September 2026)

- `src/data/civicOffices.js` renders `amenity=police`, `amenity=townhall` and `office=government` for the current viewport through `/api/overpass`, debounced on camera `moveEnd`. Classification is by declared order, so a feature tagged both `amenity=police` and `office=government` is a police station rather than whichever tag was read first.
- The layer's honesty problem is specific to it: a missing aircraft is obviously missing, but an unmapped district renders identically to a district with no office, so a reader takes the gap for a fact about the world rather than about OpenStreetMap. `CONTRIBUTING.md` forbids presenting public-data inference as authoritative intelligence, and here that obligation lands almost entirely on the row's meta line. It is built from measurements, not from a disclaimer: Overpass's `out count` gives the number in view (shown alongside the drawn count whenever the two differ, because a silently truncated list reads as a complete one) and `osm3s.timestamp_osm_base` gives the answering mirror's snapshot date, which lags — measured at 2026-05-06 and 2026-06-01 on two mirrors during development. No count is accumulated across viewports, and a response without a count reports `total: null` rather than reusing the rendered length.
- Above a 2° viewport the layer clears and says `zoom in to load — too wide to map`. Reporting `0 in view` over a continent would be a false statement about the world; leaving the last city's offices drawn over it would be worse.
- Names go through `worldOverlay` (`createCivicOfficeOverlayEntry`), not a Cesium entity `label`, so they are placed by the same collision arbiter as every other on-globe label.
