...
- GBFS response size is capped and `/api/gbfs` is throttled with the same
  90-per-client/300-global one-minute limiter as `/api/overpass`; CCTV health map is bounded.
