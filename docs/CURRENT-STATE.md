...
- Runtime entry: `createStandaloneTools` (`src/standalone/tools.js`) calls `initDrawTool({ viewer, annotations })` after `initAnnotations` and defers its `destroy()`, so disposing the shell returns every DOM listener, the Cesium handler, the preview data source, the borrowed double-click and the `window.__gevDrawTool` handle. That handle (`setActive`, `setShape`, `addVertex(lon, lat)`, `finish`, `cancel`, `clearAll`, `diagnostics`) is the no-mouse test seam.
...
- **A vertex is a lon/lat, and the mark is DRAPED.** Clicks are picked through `pickWorldFromScreen` (exported from the resolver) so the live preview hugs the surface under the pointer, but the world renderer draws areas and routes with `clampToGround` and `CESIUM_3D_TILE` classification. The height under the click is therefore dropped at finish, in one place (`finishSpec`), rather than being carried downstream and discarded silently: what you get is the outline you drew, lying on the photoreal surface beneath it.
...
- Pointer ownership: while a session is open the tool holds the single pointer claim (`claimPointer('draw')`, `src/data/inputOwnership.js`) and **every** ambient scene selection handler — the shared tracking click gesture behind flights, military and CCTV, plus vessels, satellites, launches, fires, installations, bikeshare, radio, submarine cables, the bundled GeoJSON layers and the CCTV calibration gizmo — returns before picking. A vertex over an aircraft is a vertex, not a selection. The claim is never stolen and is released by identity, so a late teardown cannot free a successor's claim. The viewer's stock LEFT_DOUBLE_CLICK action is removed for the session and restored on exit.
- Pure half: `src/annotations/drawMode.js` (session, minimum vertices, double-click de-dup at 0.5 m, vertex ceiling, degenerate-geometry refusal, length/area, hint text). Regression surface: `src/annotations/drawMode.test.mjs`, `src/annotations/drawTool.test.mjs`, `src/data/inputOwnership.test.mjs`, and the rendered harness `scripts/qa-draw-tool.mjs`.
