# READY_TO_PASTE — OnDemand dashboard blocks for OnDemand Spatial (2026-09-20)

Generated 2026-09-20 (UTC) by script (`build.py`, run in the agent environment; no git state touched) from the repository at tip `0310089` (branch `ondemand-serverless`): the tool definition `docs/ondemand-workflows/tools/earthquake_search.openapi.json` (OpenAPI 3.0.3, one path `/api/sources/earthquakes`), the registration pack `docs/audit/dashboard-registration-pack.md` (§2 agent fields + system prompt, §4 skill table, §5 click-by-click steps, §6 ID paste table, §7 verification) and the nine `SKILL.md` bodies `docs/ondemand-skills/ondemand-spatial-*.md`. It supersedes `docs/registration/READY_TO_PASTE.md` (2026-09-19), whose layout it mirrors; every block below was regenerated from those sources, not copied from the predecessor. The only edit to a source artefact is `servers` in §1 (see the next section); the click-path steps in §1.1 and the connective sentences are authored by the script from the pack. Verbatim regions sit between `<!-- prettier-ignore-start -->` / `<!-- prettier-ignore-end -->` HTML comments (invisible when rendered) so that Prettier leaves them byte-identical to their sources.

## Base URL used in §1 — read this first

- `servers[0].url` = **`https://ondemand-eand-spatial.vercel.app`** — the canonical production URL of Vercel project `ondemand-eand-spatial` (team `schoolhack-web-team`). It is a **PLACEHOLDER until a production deployment exists**: at 2026-09-20T03:26:50Z it (and the aliases `https://ondemand-eand-spatial-schoolhack-web-team.vercel.app` and `https://ondemand-eand-spatial-git-main-schoolhack-web-team.vercel.app`) answered HTTP 404 `DEPLOYMENT_NOT_FOUND`. The `vercel` CLI is blocked in the agent environment (exit 126), so the production deploy must be run by a human/CI: `vercel --prod --token $VERCEL_TOKEN` from the repo tip.
- `servers[1].url` = **`https://ondemand-eand-spatial-j8s1kk2zd-schoolhack-web-team.vercel.app`** — the durable READY Vercel deployment of commit `0677f16` (same `/api/sources/earthquakes` route). Verified 2026-09-20T03:26:51Z: `GET /` → 200, `GET /api/ondemand/health` → 200 ("healthy"), `GET /api/sources/earthquakes?latitude=30.25146&longitude=-97.7533&maxradiuskm=500&limit=1` → 200 (`count` 1, first event `tx2026smwdrk`). Use it for the dashboard **Test and validate** step until production answers 200, then move it out of `servers`.
- `https://ondemand-eand-spatial-opal.vercel.app` answered 200 on `/` but 404 on both API routes (it hosts a different app) — do **NOT** use it.

<!-- prettier-ignore-start -->

## 1. `earthquake_search` — exact OpenAPI JSON (paste into *My Plugins → Create Plugins → OpenAPI schema*)

<!-- prettier-ignore-end -->

Identity: `operationId: earthquake_search`, 1 operation (`GET /api/sources/earthquakes`) with 14 optional query parameters, no authentication (leave the configuration/auth fields empty). sha256 of the compact JSON (`json.dumps(separators=(",", ":"), ensure_ascii=False)`): `776c968a3761393414eddbcfc94ecc82c93fbf126a9e326291e7d11b3f53da6e`; sha256 of the pretty block exactly as fenced below: `898180f1c6d0ccf908cb0679ebbb81bc8e7c3e7173631358a75f0b2a7aa4efb6`.

<!-- prettier-ignore-start -->

| Field | Value |
|---|---|
| Plugin name | `earthquake_search` |
| Description | Search recent/historical earthquakes from the USGS FDSN Event Web Service by time window, magnitude range and geographic area (circle or bounding box), proxied by the OnDemand Spatial serverless route `api/[...route].js` -> `server/serverless/earthquakes-route.js` -> `server/sources/usgs-earthquakes.js`. … (first sentence shown; full 430-char `info.description` is intact in the JSON below) |
...
```json
{
  ...
  "servers": [
    {
      "url": "https://ondemand-eand-spatial.vercel.app",
      "description": "OnDemand Spatial production (Vercel project ondemand-eand-spatial) — PLACEHOLDER: 404 DEPLOYMENT_NOT_FOUND at 2026-09-20T03:26:50Z until the production deploy runs"
    },
    {
      "url": "https://ondemand-eand-spatial-j8s1kk2zd-schoolhack-web-team.vercel.app",
      "description": "Durable READY Vercel deployment of 0677f16 — verified HTTP 200 on /api/sources/earthquakes at 2026-09-20T03:26:51Z; use for Test and validate until production answers"
    }
  ],
  "paths": {
...
Validation: json.loads OK — openapi 3.0.3, 1 path(s), 1 operation(s), 14 query parameters, servers[0].url = https://ondemand-eand-spatial.vercel.app

Notes: (1) the only difference from the source file is `servers` (the source carried a single host placeholder; it is replaced by the two entries described above — remove `servers[1]` once production answers 200). (2) The `x-ondemand-spatial` vendor extension is copied unchanged; it still carries the 2026-09-18 status text that mentions the historical host placeholder and the planned project ids — OpenAPI importers ignore `x-` extensions, and none of it is a credential. (3) The path is exactly `/api/sources/earthquakes`; all 14 query parameters are optional.

### 1.1 Dashboard click path (plugin)

**My Plugins → Create Plugins → OpenAPI schema → configure → test**

1. **My Plugins** → **Create Plugins**.
2. Choose **"OpenAPI schema"** (define the API structure from an OpenAPI document).
3. Paste the JSON block above (§1) into the schema editor.
4. Configure: name `earthquake_search`; authentication **none** (the route is public and rate-limited server-side); leave all 14 query parameters optional (none is required).
5. Test with `latitude=30.25146&longitude=-97.7533&maxradiuskm=500` → expect HTTP 200 and a `count` field in the response body (the same query with `&limit=1` returned `count` 1, first event `tx2026smwdrk`, against `servers[1]` at 2026-09-20T03:26:51Z). While `servers[0]` still answers 404, run the test against `servers[1]`.
6. Save and note the plugin id (paste target: §3 ID paste table → `capabilities[id="earthquake.search"].ondemand_tool_id` in `src/registry/capabilities.json`).

### 1.2 Dashboard click path (REST agent)

**MyAgents → Create Agent → Rest Api Agent**

Pack §5(A), verbatim — name `earthquake_search`, description = `info.description` of the JSON in §1, paste the same OpenAPI JSON, auth none, test, save → note the agent id:
...
Cross-references for the steps above: pack §3(b) = the JSON in §1 of this file (host placeholder already replaced by the two `servers` entries); pack §3(a) `description` = `info.description` of that JSON (first sentence in the §1 table, full text inside the JSON); the two test queries that answered 200 on 2026-09-20T03:26:51Z are in the base-URL section.

Pack §5(B) summary — **MyAgents → Create Agent** → create the persona/chat agent **OnDemand Spatial Intelligence Agent** with the pack §2 field values and system prompt (both reproduced verbatim below), attach the `earthquake_search` REST agent from (A) and the nine skills from §2 of this file, then set `ONDEMAND_SPATIAL_AGENT_ID` in Vercel (pack §6, row 2). The pack steps, verbatim:
...
Agent dashboard field values (pack §2 "Dashboard field values", verbatim):
...
System prompt (pack §2 "System behaviour / prompt", verbatim — the pack's own introduction first, then the whole fenced block to paste into the agent's system-prompt / instructions field):
...
After saving: copy the returned agent id into `src/registry/capabilities.json` → `ondemand.agent.pluginId` and set the Vercel project environment variable `ONDEMAND_SPATIAL_AGENT_ID` to the same value (§3 below), then redeploy; `/api/ondemand/health` then reports `config.spatialAgentId.source` = `env` (pack §6.1).

## 2. The nine skills — one copyable block each

For each skill (pack §4 order): dashboard fields (Skill Name, Description, Category, Sample Prompts — values from the pack §4 table) then the **Instructions** block = the full `SKILL.md` body to paste into the skill editor (or zip the source file as `SKILL.md` and upload it). Pack §5(C) step 2 enters the slug as the dashboard **Skill Name** (lowercase-with-dashes, unique platform-wide); the display name from §4 is given for reference. Each `SKILL.md` also opens with its own "Dashboard fields" line (a slightly longer description and three sample prompts) — either set is acceptable; the §4 table is the pack's canonical one. Source files are listed with their sizes so you can upload them unchanged instead of pasting; the fenced bodies below are byte-identical to those files.
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-spatial-context-reader` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-spatial-context-reader.md` (10,351 chars, 10,387 bytes, sha256 `ab18d9db24d981acab8d0df0d111c881a0b9fa950be1f541d384d4816ce7463f`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-intent-classifier` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-intent-classifier.md` (9,141 chars, 9,175 bytes, sha256 `495bdbd36c088f311b9de03d0a8459f3af3a4b44713c22ab62fc1b856ee439c5`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-capability-resolver` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-capability-resolver.md` (11,288 chars, 11,339 bytes, sha256 `dd297c9bff6db247ad980a07bd0f231c52fe677c14699c32b73ac188c240fa6b`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-map-action-planner` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-map-action-planner.md` (10,157 chars, 10,183 bytes, sha256 `c5f8e56484755e9f839595e3ee136533ac07093edce2cbdf677fd6b24ec701f6`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-seismic-analyst` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-seismic-analyst.md` (10,218 chars, 10,263 bytes, sha256 `8e02ca9cea47ba23d4bb1d7da6ac5d149c0f327001e3d9e4fe4d3a9c192669ab`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-aviation-analyst` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-aviation-analyst.md` (10,941 chars, 11,000 bytes, sha256 `e00e71737c2a4874f7e0ccd8ab0b494db04d568ecfa6c1d1833346926d224975`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-maritime-analyst` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-maritime-analyst.md` (8,605 chars, 8,663 bytes, sha256 `215b79425d4852d20b8e0e4a28ca4ca2a2da2939af74ec1b07974a73f9d70894`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-evidence-verifier` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-evidence-verifier.md` (7,810 chars, 7,846 bytes, sha256 `3a395a09fdf5e53952d503e4966c38a191fa8bd7055dc19a1a439ed952269381`) |
...
| Slug (dashboard Skill Name per pack §5(C) step 2) | `ondemand-spatial-structured-response-writer` |
...
| Source file | `docs/ondemand-skills/ondemand-spatial-structured-response-writer.md` (9,837 chars, 9,884 bytes, sha256 `3e9c4658e76a7cfa677f9f17aa984d897d7e95612809212b2a4d8bd9a9745b1c`) |
...
### 2.10 Dashboard click path (skills)

**Skills → Create Skill → (fields + SKILL.md) → Validate → safety scan → private → Playground test** — pack §5(C), verbatim:
...
## 3. Workflow trigger + ID paste table

Copied verbatim from the pack (§5(D), §5(E), §6 and §6.1); only the section headings were renumbered to fit this file. The "PENDING-HUMAN → paste block" column of the 2026-09-19 results table refers to sections of the 2026-09-19 predecessor `docs/registration/READY_TO_PASTE.md`; the equivalents in this file are §1 (OpenAPI JSON) + §1.1/§1.2 (click paths), §1.2 (agent name, description, starters, system prompt), §2.1–§2.9 (skills) and §2.10 (skills click path).

### 3.1 Pack §5(D) — Read the workflow's webhook trigger URL and invoke it
...
### 3.2 Pack §5(E) — Optional — attach earthquake_search to the workflow's LLM nodes
...
#### 3.3.1 Pack §6.1 — Registration results — platform attempt 2026-09-19 (appended 2026-09-19T12:09:46Z)

<!-- prettier-ignore-start -->

Filled from the platform run of 2026-09-19 (`docs/registration/CREATION_LOG_2026-09-19.md`, `docs/registration/API_RECHECK_2026-09-19.md`; raw evidence `docs/registration/creation-evidence-2026-09-19/`). Only values actually returned by the platform are recorded; every dashboard-only item is `PENDING-HUMAN` with the exact paste block to use.

| Item | Returned id | Created (UTC) | Status | PENDING-HUMAN → paste block |
|---|---|---|---|---|
| earthquake_search REST agent (`tool ID`) | `null` | — (6 create attempts 2026-09-19T11:36:14Z → 11:39:50Z via MCP `plugin_v1_plugin_create` and `POST /plugin/v1` all answered `isError:true` / HTTP 500 with an empty body; `GET /plugin/v1/list` total 0 afterwards) | **`PENDING-HUMAN`** | `docs/registration/READY_TO_PASTE.md` **§1** (exact OpenAPI JSON, base URL filled) + §4.1 / §4.2 click path |
| OnDemand Spatial Intelligence Agent (`agent ID`) | `null` | — (no API/MCP field accepts a system prompt or an agent→tool attachment) | **`PENDING-HUMAN`** | `docs/registration/READY_TO_PASTE.md` **§3** (name, description, starters, system prompt) + §4.4 click path |
| Skill ids ×9 (`ondemand-spatial-spatial-context-reader`, `-intent-classifier`, `-capability-resolver`, `-map-action-planner`, `-seismic-analyst`, `-aviation-analyst`, `-maritime-analyst`, `-evidence-verifier`, `-structured-response-writer`) | `null` ×9 | — (no skills API/MCP surface; probes `POST /skills`, `/skills/v1/skills`, `/plugin/v1/skills`, `/automation/api/skills` → 404) | **`PENDING-HUMAN`** | `docs/registration/READY_TO_PASTE.md` **§2.1–§2.9** (one block per skill) + §4.3 click path |
| Workflow id | `6aace534859f7b0abb53d99a` (pre-existing; created via API 2026-09-18T07:16:04.335Z) | verified live `GET /automation/api/workflow/6aace534859f7b0abb53d99a` → 200, `isActive: true`, 9 `llm` nodes, no plugins attached, 2026-09-19T11:40:28Z | **unchanged** — not modified (nothing to attach until the tool id exists) | — (attach later per §5(E) / READY_TO_PASTE §4.2 step 4) |
| Tool base URL (`servers[0].url`) | `https://sb-1np4tjtbq20v.vercel.run` (rule: production URL only if 200 → production aliases 404 `DEPLOYMENT_NOT_FOUND` at 11:29:57Z, `vercel` CLI blocked exit 126) | probed 200 at 2026-09-19T11:29:59Z; **sandbox expired 2026-09-19T11:47:35Z** (HTTP 410 at 11:52:14Z) | must be **re-pointed** after the CLI redeploy of `1b9e9a8`; interim substitute: durable READY preview `https://ondemand-eand-spatial-j8s1kk2zd-schoolhack-web-team.vercel.app` (200 at 11:29:58Z and 11:40:27Z, 17 events ≥ M4.5 / 24 h) | READY_TO_PASTE.md "Base URL used in §1" |
| Creation timestamps | none (nothing was created) | — | — | — |

Runtime observability of the paste-back (added the same day): `/api/ondemand/health` → `config.spatialAgentId.source` and `config.spatialToolId.source` ∈ `env` (`ONDEMAND_SPATIAL_AGENT_ID` / `ONDEMAND_SPATIAL_TOOL_ID` set) · `registration-pack` (pasted into `src/registry/capabilities.json` — `ondemand.agent.pluginId` / `capabilities[id="earthquake.search"].ondemand_tool_id` — with no env var) · `unset`; presence and source only, the id is never echoed.
...
4. After the production deploy (`vercel --prod --token $VERCEL_TOKEN` from tip `0310089`) answers 200 on `https://ondemand-eand-spatial.vercel.app/api/sources/earthquakes`, re-run the plugin's **Test and validate** against `servers[0]` and remove `servers[1]` (the `0677f16` deployment) from the schema.

Generated 2026-09-20T03:36:45Z from tip 0310089 by scripts in the agent run; no credential values appear in this file.
