...
Package imports use `ondemand-spatial-intelligence` (the package name); `package.json` is the authoritative
...
`ondemand-spatial-intelligence/build/vite` is a separate Node-only export. `build/vite.js`
...
```js
import { createBrowserViteConfig } from 'ondemand-spatial-intelligence/build/vite';
...
The portable `ondemand-spatial-intelligence/sources/adsb-lol` export normalizes existing aircraft
...
`ondemand-spatial-intelligence/server/providers/places` exports the Google nearby-place and
...
`ondemand-spatial-intelligence/sources/places` exports portable response projections for Google
...
`ondemand-spatial-intelligence/server/providers/space` is a Node-only entry for the CelesTrak
...
`ondemand-spatial-intelligence/sources/space` owns only fixed upstream URL construction: the
...
`ondemand-spatial-intelligence/server/providers/terrain`, `/traffic`, `/firms` and `/gbfs`
...
`ondemand-spatial-intelligence/sources/terrain` exports existing point-key, retry and cache
reconstruction mechanics with injectable acquisition dependencies.
`ondemand-spatial-intelligence/sources/traffic` exports tile math and budget calculations.
`ondemand-spatial-intelligence/sources/gbfs` exports host/path acceptance and cache-header rules.
...
`ondemand-spatial-intelligence/sources/firms-csv` exports the existing CSV parser, header
...
`ondemand-spatial-intelligence/search` exports an explicit geocoding service and Google/Photon
...
`ondemand-spatial-intelligence/ui/panels` owns collapse-button binding, nearest-panel Escape
...
`ondemand-spatial-intelligence/ui/surfaces` exports `createSurfaceKeyboard` from
...
`ondemand-spatial-intelligence/ui/layout` exports synchronous `layoutLeftPanelRail` and
...
`ondemand-spatial-intelligence/ui/input` exports `bindApplicationShortcuts` and
...
`ondemand-spatial-intelligence/scenes` exports `SceneDirector`. Its `subscribe(listener)` supplies
...
`ondemand-spatial-intelligence/application/catalog` captures caller-supplied layer instances and
...
`ondemand-spatial-intelligence/sources/reference` constructs fresh earthquake and bundled cable
