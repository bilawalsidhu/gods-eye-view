...
`ondemand-spatial/infrastructure` exports `createInfrastructureLayers(services)`.
...
`ondemand-spatial/infrastructure/geojson` exports the lower-level
`createLocalGeoJsonLayer(options, services)` and existing infrastructure overlay
helpers. `ondemand-spatial/infrastructure/lod` exports the existing LOD policy helpers.
