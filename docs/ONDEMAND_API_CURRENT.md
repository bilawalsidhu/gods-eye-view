...
## §18 LIVE RE-VALIDATION 2026-09-18

Second live pass over the public documentation, run 2026-09-18T16:18–16:21Z with `curl -sS -m 30 -o <file> -w '%{http_code}'` (one UTC stamp per URL, §18.4). Nothing below is from memory: every statement was re-read from a page fetched in this pass, and the four live API calls in §18.2 are read-only `GET`s made with the account key (masked as `<redacted>`; never written to disk). Scope: the surfaces this repository depends on (chat, agent tools, reasoning modes / endpoint ids, Agents Flow Builder, speech services, media) plus an explicit hunt for any realtime / advanced-voice transport. The §0–§17 sections above are left intact; this section records what changed since 2026-09-17 (answer: nothing on the documentation side) and answers three open questions.

**Corpus check.** `https://docs.on-demand.io/llms.txt` (200, 9,493 bytes — same length as 2026-09-17) still indexes exactly the same **85** guide/reference pages (set difference against §0.1: none added, none removed). Every one of the 40 `.md` pages re-fetched in §18.4 came back with the **same byte length as in §0.1 and an `updatedAt` frontmatter no later than 2026-09-13** (`reference/updatelivesessionsettings.md`; every other page ≤ 2026-08-28), i.e. no page consulted here has been edited since the 2026-09-17 audit. The in-app docs API (`gateway.on-demand.io/config/v1/public/docs/…`) now answers **401 `{"message":"No API key header","errorCode":"unauthenticated"}` without a key**; with the `apikey` header it returns the same 40-operation categories index and the same specs as before (byte-identical lengths: categories 2,918; submitquery 6,196; createchatsession 3,623; post_workflow-id-execute 804; get_execution-executionid-logs 1,728; get_execution-executionid 1,990; post_workflow 5,314; converttexttoaudio 2,326; convertaudiototext 1,974; get_execution-executionid-transcript 1,904).

### 18.1 CONFIRMED / CHANGED / RETIRED — one row per surface

| Surface | Verdict | What the live page says today (2026-09-18) | Source (fetched this pass) |
|---|---|---|---|
| Chat session create | **CONFIRMED** | `POST https://api.on-demand.io/chat/v1/sessions`; schema `required: ["externalUserId"]`, `pluginIds` string[] `maxItems: 20`; security scheme `apikey` (header). Guide table still marks `externalUserId` "Required: No" (§2 contradiction unchanged). | `reference/createchatsession.md` (200, 16:18:07Z, updatedAt 2026-08-28); `docs/chat-api.md` (200, 16:18:07Z) |
| Submit query — sync | **CONFIRMED** | `POST /chat/v1/sessions/{sessionId}/query`; `required: [query, endpointId, responseMode]`; `responseMode` enum `sync \| stream \| webhook`; `pluginIds` (max 20, replaces the session list); `fulfillmentOnly` (default false); `modelConfigs{fulfillmentPrompt, stopSequences, temperature, topP, presencePenalty, frequencyPenalty}`; 200 "Sync Mode Response" `data.{sessionId, messageId, answer, status}`. `reasoningMode` is still **absent from the schema**; `agentIds` is still **schema-absent** (guide samples only); `endpointId` example still `predefined-openai-gpt4o` (inactive per §17.4). | `reference/submitquery.md` (200, 16:18:07Z, updatedAt 2026-08-28); `docs/chat-api.md` rows `pluginIds` / `responseMode` ("Defaults to `sync`") / `reasoningMode` ("Controls reasoning detail in `stream` mode (e.g., `low`, `high`)") |
| Submit query — stream / SSE | **CONFIRMED** | SSE over the `POST …/query` response; `event:` names `heartbeat`, `thinking`, `message`; `eventType` enum `statusLog \| metricsLog \| fulfillment`; terminal `data:[DONE]`; error `data:[ERROR]:<json>`; heartbeat payload `{sessionId, messageId, time}`. Guide samples send `agentIds` and `"reasoningMode": "low"` / `"grok-4-fast"`. No WebSocket transport (0 hits for `websocket`/`wss://` in page content). The live-only `fulfillment_thinking` eventType of §17.3 remains **undocumented**. | `docs/query-and-responses-modes.md` (200, 16:18:07Z, updatedAt 2026-08-17) |
| Agent tools & `pluginIds` | **CONFIRMED** | Only documented operation: `GET https://api.on-demand.io/plugin/v1/list?pluginIds=…&page=&limit=` (guide-only; not in the 40-spec reference set nor the keyed categories index). Attach points unchanged: `pluginIds` on session + query, `plugins` on media and on `llm` / `advancedVoiceMode` workflow nodes. | `docs/plugin-api.md` (200, 16:18:07Z); `docs/what-are-plugins.md`, `docs/plugins.md` (200, 16:18:07Z); keyed categories (200, 16:18:46Z) |
| Reasoning modes | **CONFIRMED (still guide-only, unenumerated)** | No dedicated page exists: `docs/reasoning-modes.md` → **404**. Values still appear only as examples (`low`, `high` in `docs/chat-api.md`; `low`, `grok-4-fast` in the stream samples) and as the free-string `reasoningMode` on the `advancedVoiceMode` node. The live `GET /config/v1/public/reasoning_modes` listing of §17.4 remains undocumented. | `docs/reasoning-modes.md` (404, 16:18:07Z); `docs/chat-api.md`; `docs/query-and-responses-modes.md`; `reference/post_workflow.md` |
| Predefined endpoint ids | **CONFIRMED (page unchanged; list still stale vs. live)** | `docs/fulfillment-models.md` (updatedAt 2026-08-24) still lists the same 18 `predefined-*` ids as §12 with the same "may be updated over time — refer to the playground" caveat; the §17.4 finding that 7 of those 18 are inactive live (and that 8 active ids are missing from the page) stands. Probes `docs/endpoints.md` and `docs/models.md` → **404** (no separate endpoint-id page). | `docs/fulfillment-models.md` (200, 16:18:07Z); `docs/endpoints.md`, `docs/models.md` (404, 16:18:07Z) |
| Workflow activate / deactivate / execute | **CONFIRMED** | `POST /workflow/{id}/activate` → 200 "Workflow activated successfully" / 500; `POST /workflow/{id}/deactivate` → 200 "Workflow deactivated successfully" / 500; `POST /workflow/{id}/execute` → 200 `{ "executionID": "<string>" }` ("Workflow execution started"), 400 "Invalid request or workflow inactive", 404 "Workflow not found", 500. All three: exactly ONE parameter (`id`, path, required), **no `requestBody`**, no query parameters; server `https://api.on-demand.io/automation/api`; header `apikey`. The guide's execute sample is the bare `curl --location --request POST '…/workflow/<id>/execute' --header 'apikey: …'` with no `--data`. | `reference/post_workflow-id-execute.md` (200, 16:18:08Z, updatedAt 2026-08-26); `…-activate.md`, `…-deactivate.md` (200, 16:18:08Z); `docs/workflow-api.md` (200, 16:18:07Z) |
| Execution status | **CONFIRMED (one detail sharpened)** | `GET /execution/{executionID}` → 200 `{message, data: Execution}`; `Execution{id, workflowId, trigger{id, type enum manual \| scheduled \| api, actorId (nullable)}, status (free string), startedAtInMilliseconds, endedAtInMilliseconds, timeTakenInMilliseconds}`. The spec spells the id fields `workflowId` / `actorId` while the guide samples (and the live responses of §18.2) use `workflowID` / `actorID` — treat the upper-case-`ID` form as the wire format. | `reference/get_execution-executionid.md` (200, 16:18:08Z); `docs/execution-api.md` (200, 16:18:07Z) |
| Execution logs (+ streaming?) | **CONFIRMED — polling only; NO log streaming** | `GET /execution/{executionID}/logs` → 200 `{message, data: ExecutionLog[]}`; `ExecutionLog{executionId, requestId, task, timestamp (int64 ms), workflowId, nodeKey, message, fields}` (guide sample: `executionID`/`requestID`/`workflowID`, `"message": "execution logs fetched successfully"`). Zero occurrences of `stream`, `sse`, `event-stream` or `websocket` in the logs spec, the execution guide or the workflow guide; the keyed categories index lists no streaming operation. | `reference/get_execution-executionid-logs.md` (200, 16:18:08Z); `docs/execution-api.md` |
| Workflow name PATCH | **CONFIRMED** | `PATCH /workflow/{id}/name`, body `{ "name": string }` (`required: ["name"]`) → 200 "Workflow name updated successfully" / 400 / 500. | `reference/patch_workflow-id-name.md` (200, 16:18:08Z) |
| Agent / REST-plugin CREATE | **CONFIRMED dashboard-only (no public REST create)** | `docs/rest-based-plugins.md` step 1 is still "Navigate to My Agents (`https://app.on-demand.io/rag-agents/my-agents`) and click on Create Agents"; `docs/open-api-schema.md` contains only example OpenAPI documents to paste into that form; `docs/plugin-api.md` documents `GET /plugin/v1/list` and nothing else; the keyed categories index (40 operations) has no plugin/agent create, update or publish operation. Skills likewise: Dashboard → Skills → Create Skill (`SKILL.md`, zip ≤ 50 MB, single file ≤ 25 MB, `SKILL.md` ≤ 2 MB); no REST endpoint. | `docs/rest-based-plugins.md`, `docs/open-api-schema.md`, `docs/plugin-api.md` (200, 16:18:07Z); `docs/agent-skills.md` (200, 16:18:07Z); keyed categories (200, 16:18:46Z) |
| Skills | **CONFIRMED (UI only)** | as above — create / attach / invoke via REST: still NOT FOUND IN LIVE DOCS. | `docs/agent-skills.md` (200, 16:18:07Z, updatedAt 2026-08-24) |
| Audio → text | **CONFIRMED** | `POST https://api.on-demand.io/services/v1/public/service/execute/speech_to_text`; body `required: ["audioUrl"]` (one field); header param `apikey` required; 200 `data.text`; 400 "Not subscribed to the service"; 401 "Incorrect API key". No language / format / streaming field. | `reference/convertaudiototext.md` (200, 16:18:08Z, updatedAt 2026-08-17); `docs/cloud-services-api.md` (200, 16:18:08Z) |
| Text → audio (voices, models) | **CONFIRMED (one nuance)** | `POST …/execute/text_to_speech`; `model` enum **`tts-1` \| `tts-1-hd`** (default `tts-1`), `voice` enum **`alloy` \| `echo` \| `fable` \| `onyx` \| `nova` \| `shimmer`** (default `alloy`), `input` string; 200 `data.audioUrl`. Nuance: the OpenAPI schema lists **all three** (`model`, `input`, `voice`) under `required` even though `model`/`voice` carry defaults — the proxy's health probe and tts route already send all three (§6.2 body). Still no streaming / format / sample-rate field. | `reference/converttexttoaudio.md` (200, 16:18:08Z, updatedAt 2026-08-17); `docs/cloud-services-api.md` ("Currently, two APIs are available") |
| Media create / fetch | **CONFIRMED** | `POST /media/v1/public/file` `required: [url, plugins, responseMode]`, `responseMode` enum `sync \| webhook`, optional `sessionId`, `externalUserId`, `name`, `sizeBytes`, `pluginInputs`; `GET /media/v1/public/file?sort&page&limit&plugins&externalUserId&source`. | `reference/createmediaurl.md`, `reference/fetchmedia.md` (200, 16:18:08Z, updatedAt 2026-08-17); `docs/media-api.md` (200, 16:18:08Z) |
| Realtime / advanced voice | **CONFIRMED: NO client-facing realtime API** (see 18.2 c) | Voice exists only as (a) the `advancedVoiceMode` workflow node — an outbound **phone call**, (b) the `phone` delivery channel, (c) `GET /execution/{executionID}/transcript` after the call, and (d) an **undocumented** `/automation/api/voice-agent/` resource found live (18.2 c). No WebSocket / WebRTC / audio-stream endpoint anywhere in the public docs. | `docs/workflow-nodes.md` (200, 16:18:07Z); `reference/post_workflow.md`; `reference/get_execution-executionid-transcript.md`; the seven 404 probes in §18.4 |

**RETIRED:** none — no page, field or endpoint used by this repository has disappeared or been renamed since 2026-09-17. **CHANGED:** none on the documentation side; the only behavioural change observed is that the in-app docs API now requires the `apikey` header (401 unkeyed) — irrelevant to the proxy, which never calls it.

### 18.2 Explicit answers

**(a) Executing the spatial workflow — exact path and parameters.**

- Path, verbatim from the spec: `POST https://api.on-demand.io/automation/api/workflow/{id}/execute` with header `apikey: <key>`. `id` is the **only** parameter (path, required, string). There is **no request body, no query parameter and no payload field** in the spec or the guide (`curl --location --request POST '…/workflow/66e4448d2cac15eadc6d9ffb/execute' --header 'apikey: token-here'`). Response 200 `{ "executionID": "…" }`; then poll `GET …/execution/{executionID}` and `GET …/execution/{executionID}/logs` (polling only — no streaming, 18.1). The workflow must be active (400 "Invalid request or workflow inactive").
- For this repository's workflow: `POST https://api.on-demand.io/automation/api/workflow/6aace534859f7b0abb53d99a/execute`. **Id discrepancy, recorded:** the operator prompt spelled the id `6aace534859f9f7b0abb53d99a` (26 hex characters — not a valid 24-hex ObjectId). Live on 2026-09-18: `GET …/workflow/6aace534859f7b0abb53d99a` → **200** (16:20:55.552Z; `data.name = "OnDemand Spatial Advanced Workflow"`, `isActive: true`, `trigger.type: webhook`, 9 `llm` nodes `session_context → … → structured_response`, `enableMemory: false`), whereas `GET …/workflow/6aace534859f9f7b0abb53d99a` → **404** `{"message":"Resource not found","errorCode":"not_found"}` (16:20:55.925Z). `GET …/execution/list?workflowID=6aace534859f7b0abb53d99a` → 200, 5 executions, all `status: "success"`, `trigger.type: "api"` (16:20:56.289Z). The real id is therefore **`6aace534859f7b0abb53d99a`**, exactly what `FLOW_DEFAULTS.spatialFlowId` carries.
- How the repo resolves the id and the version label (server/ondemand/config.js): the workflow id is reconciled **canonical-first** — `ONDEMAND_SPATIAL_WORKFLOW_ID` (canonical, added 2026-09-18) → `ONDEMAND_SPATIAL_FLOW_ID` (accepted alias) → `FLOW_DEFAULTS.spatialFlowId = '6aace534859f7b0abb53d99a'` (`WORKFLOW_ID_ENV.order`; `sources.spatialFlowId` names the winner; `/api/ondemand/health` `config.spatialFlowId = { configured, source, resolvedVia: 'canonical' | 'alias' | 'default', canonical, alias }`, names only). The version label is the one **alias-first** row — `GODS_EYE_FLOW_VERSION` (alias, already provisioned on the Vercel project) is checked **before** the canonical `ONDEMAND_SPATIAL_FLOW_VERSION`, then `FLOW_DEFAULTS.flowVersion = '1'` (`FLOW_VERSION_ENV.order`) — and is informational only: no version field exists on the workflow object (`reference/post_workflow.md`, `get_workflow-id.md`; §7.3 unchanged), so it is never sent upstream.

**(b) Does agent / tool / plugin CREATE exist in the public API?** **No.** Re-checked `docs/rest-based-plugins.md` (creation = dashboard "My Agents → Create Agents", OpenAPI schema pasted or imported from URL, configured, tested, optionally published), `docs/open-api-schema.md` (schema-authoring examples only), `docs/plugin-api.md` (only `GET /plugin/v1/list`), `docs/agent-skills.md` (Dashboard → Skills → Create Skill), `llms.txt` (85 pages, none about creating agents over REST) and the keyed categories index (40 operations: media 3, services 3, mqtt 2, projects 6, approval gate 2, execution 7, workflow 10, chat 7 — no plugin/agent/skill operation). Unchanged since §8/§9: **dashboard-only**. (An OnDemand-side MCP tool inventory available to this workspace lists `plugin_v1_plugin_create` / `plugin_v1_plugin_configuration_create` / `public_v1_plugin_ai_generated_tool_create`, which would imply a private `POST /plugin/v1/…` surface — it is **not** in any public page and was not probed, so it stays NOT FOUND IN LIVE DOCS.)

**(c) Does ANY realtime / advanced-voice WebSocket surface exist in the LIVE public docs?** **No — stated plainly.** What exists, precisely:

1. **`advancedVoiceMode` workflow node** (`reference/post_workflow.md` → `AdvancedVoiceModeMeta{ instructionPrompt (required), conversationStarter (required), plugins[{id}], reasoningMode (string), model (string) }`; `Node.type` enum `llm | inputText | advancedVoiceMode | approvalGate`). `docs/workflow-nodes.md` §"Advanced Voice Mode Node": "Initiates a real-time, interactive voice conversation **via an outbound phone call**"; one per workflow; cannot share a path with LLM nodes; recipients come from a **Phone Call delivery node** (`delivery[].channel: "phone"`, `config.phone.toNumbers[{countryCode, number}]`). The guide's create sample is literally "Marketing reach out - voice" with `"countryCode":"+971"`.
2. **`GET /automation/api/execution/{executionID}/transcript`** — "Get voice mode transcripts for an execution"; `Transcript{executionId, workflowId, content, responseId, eventId, itemId, author (user | ond | voice-server), createdAtInMilliseconds}`; 401 "Unauthorized access or non-voice mode workflow". Read-after-the-call, not a live stream.
3. **Undocumented, found live this pass:** `GET https://api.on-demand.io/automation/api/voice-agent` → **301** to `/automation/api/voice-agent/`; `GET https://api.on-demand.io/automation/api/voice-agent/` → **200** `{"message":"Voice agents retrieved successfully","data":null}` (16:21:11.216Z; this account has none). `…/voice-agent/list` → 404, `…/voice_agent` and `…/voiceagent` → 404 (`Not Found`). The platform's MCP tool inventory describes this resource as a *voice agent* with a **phone number** and a config of `callerGreeting, persona, voice, pluginIDs, reasoningMode, instructionPrompt`, plus activate/deactivate and a per-execution *voice call record* — i.e. **telephony**, the same product as the node above. It appears on no public page (`llms.txt`, categories, `docs/workflow-api.md`, `docs/execution-api.md`, `docs/workflow-nodes.md` all have zero hits for "voice agent" / "voice-agent").
4. **Not present anywhere:** a browser/client realtime **WebSocket or WebRTC** API (audio in → streaming transcript → tool calls → audio out). Grep of every page fetched this pass for `websocket`, `webrtc`, `wss://`, `socket.io`, `realtime`: the only `wss://` strings are ReadMe's own infrastructure inside the 404 shells (`wss://m.readme.io`, `wss://…collab.tiptap.cloud`), never OnDemand API content; the only "real-time" phrases are the phone-call node, the MQTT/IoT agent guide and "live (real-time) settings" of `PUT /chat/v1/sessions/{sessionId}/live-settings` (a messaging feature: `enabled`, `mode: proactive | onevents`, email/Slack destinations — not audio). Probed and **404**: `docs/advanced-voice-mode.md`, `docs/voice-agents.md`, `docs/voice-agent.md`, `docs/realtime.md`, `docs/realtime-api.md`, `docs/websocket.md` (all ReadMe "Page Not Found" shells, 116,6xx bytes each). STT still takes only a hosted `audioUrl` and returns a complete `text`; TTS still returns a hosted `audioUrl` (18.1). §14's verdict ("N — no native realtime voice API") therefore stands unchanged; the supported path remains STT → `POST …/query` (`responseMode: stream`, SSE) → TTS.

### 18.3 Consequences for this repository (no code change required by the docs)

- `api/ondemand/workflow.js` — execute/activate/deactivate with a bare `POST` and no body, logs by polling `GET …/logs`: still exactly what is documented. No log-streaming route can be added (nothing to proxy).
- `api/ondemand/health.js` — the TTS probe body `{ input, model: 'tts-1', voice: 'alloy' }` satisfies the spec's three-field `required` list (18.1, nuance).
- `server/ondemand/config.js` — `FLOW_DEFAULTS.spatialFlowId` is the id that answers 200 live; the 26-character spelling must never be used (404). The new canonical name `ONDEMAND_SPATIAL_WORKFLOW_ID` and the retained alias `ONDEMAND_SPATIAL_FLOW_ID` are documented in `.env.example` and docs/ONDEMAND_PROXY_DESIGN.md §10.1.
- Voice: OnDemand offers no client realtime socket, so OnDemand voice in this product stays STT → chat (SSE) → TTS over the documented Services API; the telephony `advancedVoiceMode` node / `voice-agent` resource is out of scope for a browser console.

### 18.4 Every URL fetched in this pass (HTTP status, UTC, bytes, page `updatedAt`, vs. 2026-09-17)

Public documentation (no credentials):

| URL fetched | HTTP | Retrieved (UTC) | Bytes | Page `updatedAt` | vs. §0.1 |
|---|---|---|---|---|---|
| https://docs.on-demand.io/docs/advanced-voice-mode.md | 404 | 2026-09-18T16:18:08Z | 116695 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/agent-skills.md | 200 | 2026-09-18T16:18:07Z | 13394 | 2026-08-24T13:20:16.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/agents-flow-builder.md | 200 | 2026-09-18T16:18:08Z | 962 | 2026-08-17T12:27:36.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/chat-api.md | 200 | 2026-09-18T16:18:07Z | 16084 | 2026-08-20T14:57:50.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/cloud-services-api.md | 200 | 2026-09-18T16:18:08Z | 7741 | 2026-08-17T12:27:48.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/creating-a-workflow.md | 200 | 2026-09-18T16:18:08Z | 5256 | 2026-08-20T14:00:25.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/endpoints.md | 404 | 2026-09-18T16:18:07Z | 116645 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/execution-api.md | 200 | 2026-09-18T16:18:07Z | 8888 | 2026-08-17T12:27:58.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/fulfillment-models.md | 200 | 2026-09-18T16:18:07Z | 4454 | 2026-08-24T13:17:28.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/knowledge-plugin.md | 200 | 2026-09-18T16:18:09Z | 13678 | 2026-08-24T12:50:22.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/media-api.md | 200 | 2026-09-18T16:18:08Z | 11742 | 2026-08-17T12:27:45.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/models.md | 404 | 2026-09-18T16:18:07Z | 116630 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/mqttiot-plugins.md | 200 | 2026-09-18T16:18:09Z | 11722 | 2026-08-24T13:08:12.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/open-api-schema.md | 200 | 2026-09-18T16:18:07Z | 16875 | 2026-08-17T12:26:42.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/plugin-api.md | 200 | 2026-09-18T16:18:07Z | 5837 | 2026-08-17T12:27:50.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/plugins.md | 200 | 2026-09-18T16:18:07Z | 4347 | 2026-08-17T12:28:01.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/query-and-responses-modes.md | 200 | 2026-09-18T16:18:07Z | 32036 | 2026-08-17T12:27:15.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/realtime-api.md | 404 | 2026-09-18T16:18:09Z | 116660 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/realtime.md | 404 | 2026-09-18T16:18:09Z | 116640 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/reasoning-modes.md | 404 | 2026-09-18T16:18:07Z | 116675 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/rest-based-plugins.md | 200 | 2026-09-18T16:18:07Z | 8993 | 2026-08-17T12:26:48.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/terminal-agent.md | 200 | 2026-09-18T16:18:09Z | 23976 | 2026-08-19T12:14:36.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/voice-agent.md | 404 | 2026-09-18T16:18:09Z | 116655 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/voice-agents.md | 404 | 2026-09-18T16:18:08Z | 116660 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/websocket.md | 404 | 2026-09-18T16:18:09Z | 116645 | — | NEW URL (not in 09-17 table) |
| https://docs.on-demand.io/docs/what-are-connectors.md | 200 | 2026-09-18T16:18:09Z | 3112 | 2026-08-21T08:12:30.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/what-are-plugins.md | 200 | 2026-09-18T16:18:07Z | 5652 | 2026-08-17T12:26:39.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/workflow-api.md | 200 | 2026-09-18T16:18:07Z | 12033 | 2026-08-17T12:27:55.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/docs/workflow-nodes.md | 200 | 2026-09-18T16:18:07Z | 7072 | 2026-08-24T11:56:07.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/llms.txt | 200 | 2026-09-18T16:18:07Z | 9493 | — | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/convertaudiototext.md | 200 | 2026-09-18T16:18:08Z | 4416 | 2026-08-17T12:28:39.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/converttexttoaudio.md | 200 | 2026-09-18T16:18:08Z | 5286 | 2026-08-17T12:28:40.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/createchatsession.md | 200 | 2026-09-18T16:18:07Z | 6326 | 2026-08-28T06:17:08.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/createmediaurl.md | 200 | 2026-09-18T16:18:08Z | 14297 | 2026-08-17T12:28:25.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/createmqttuser.md | 200 | 2026-09-18T16:18:09Z | 3161 | 2026-08-17T12:28:43.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/fetchmedia.md | 200 | 2026-09-18T16:18:08Z | 9760 | 2026-08-17T12:28:25.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/get_execution-executionid-logs.md | 200 | 2026-09-18T16:18:08Z | 3805 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/get_execution-executionid-transcript.md | 200 | 2026-09-18T16:18:08Z | 4142 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/get_execution-executionid.md | 200 | 2026-09-18T16:18:08Z | 4215 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/get_execution-list.md | 200 | 2026-09-18T16:18:08Z | 4414 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/getchatsessions.md | 200 | 2026-09-18T16:18:07Z | 7578 | 2026-08-28T06:17:08.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/patch_workflow-id-name.md | 200 | 2026-09-18T16:18:08Z | 1833 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/post_workflow-id-activate.md | 200 | 2026-09-18T16:18:08Z | 1318 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/post_workflow-id-deactivate.md | 200 | 2026-09-18T16:18:08Z | 1326 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/post_workflow-id-execute.md | 200 | 2026-09-18T16:18:08Z | 1814 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/post_workflow.md | 200 | 2026-09-18T16:18:08Z | 11222 | 2026-08-26T18:30:23.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/submitquery.md | 200 | 2026-09-18T16:18:07Z | 10816 | 2026-08-28T06:17:08.000Z | same bytes as 2026-09-17 |
| https://docs.on-demand.io/reference/updatelivesessionsettings.md | 200 | 2026-09-18T16:18:07Z | 5804 | 2026-09-13T14:46:56.000Z | same bytes as 2026-09-17 |

In-app docs API (`gateway.on-demand.io`) — unkeyed, then keyed (`apikey` header; value never recorded):

| URL fetched | HTTP (no key) | Retrieved (UTC) | HTTP (keyed) | Retrieved (UTC) | Bytes keyed | vs. §0.1 |
|---|---|---|---|---|---|---|
| https://gateway.on-demand.io/config/v1/public/docs/categories | 401 | 2026-09-18T16:18:09Z | 200 | 2026-09-18T16:18:46Z | 2918 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/submitquery | 401 | 2026-09-18T16:18:09Z | 200 | 2026-09-18T16:18:47Z | 6196 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/createchatsession | 401 | 2026-09-18T16:18:09Z | 200 | 2026-09-18T16:18:48Z | 3623 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/post_workflow-id-execute | 401 | 2026-09-18T16:18:09Z | 200 | 2026-09-18T16:18:48Z | 804 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/get_execution-executionid-logs | 401 | 2026-09-18T16:18:09Z | 200 | 2026-09-18T16:18:49Z | 1728 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/get_execution-executionid | — | — | 200 | 2026-09-18T16:18:49Z | 1990 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/post_workflow | — | — | 200 | 2026-09-18T16:18:49Z | 5314 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/converttexttoaudio | — | — | 200 | 2026-09-18T16:18:50Z | 2326 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/convertaudiototext | — | — | 200 | 2026-09-18T16:18:50Z | 1974 | same bytes |
| https://gateway.on-demand.io/config/v1/public/docs/reference/api/get_execution-executionid-transcript | — | — | 200 | 2026-09-18T16:18:51Z | 1904 | same bytes |

Live API (read-only `GET`, `apikey` header, account key masked):

| Call | HTTP | UTC | Result |
|---|---|---|---|
| `GET https://api.on-demand.io/automation/api/workflow/6aace534859f7b0abb53d99a` | 200 | 2026-09-18T16:20:55.552Z | `data.{id: 6aace534859f7b0abb53d99a, name: "OnDemand Spatial Advanced Workflow", isActive: true, enableMemory: false, trigger.type: webhook, nodes: 9 × llm, delivery: []}`; keys `companyID, contextMetadata, createdAtInMilliseconds, createdByUserID, delivery, enableMemory, id, isActive, lastModifiedAtInMilliseconds, name, nodes, trigger` |
| `GET https://api.on-demand.io/automation/api/workflow/6aace534859f9f7b0abb53d99a` (26-char spelling from the prompt) | 404 | 2026-09-18T16:20:55.925Z | `{"message":"Resource not found","errorCode":"not_found"}` |
| `GET https://api.on-demand.io/automation/api/execution/list?workflowID=6aace534859f7b0abb53d99a` | 200 | 2026-09-18T16:20:56.289Z | `message: "execution records retrieved successfully"`, 5 executions, all `status: "success"`, `trigger.type: "api"` |
| `GET https://api.on-demand.io/automation/api/voice-agent` | 301 | 2026-09-18T16:20:56.479Z | `Moved Permanently` → `/automation/api/voice-agent/` |
| `GET https://api.on-demand.io/automation/api/voice-agent/list` | 404 | 2026-09-18T16:20:56.781Z | `{"message":"Resource not found","errorCode":"not_found"}` |
| `GET https://api.on-demand.io/automation/api/voice_agent` | 404 | 2026-09-18T16:20:56.976Z | `Not Found` |
| `GET https://api.on-demand.io/automation/api/voiceagent` | 404 | 2026-09-18T16:20:57.146Z | `Not Found` |
| `GET https://api.on-demand.io/automation/api/voice-agent/` | 200 | 2026-09-18T16:21:11.216Z | `{"message":"Voice agents retrieved successfully","data":null}` — undocumented telephony resource (18.2 c) |
| `GET https://api.on-demand.io/automation/api/voice-agent/?limit=1` | 200 | 2026-09-18T16:21:11.509Z | same body |

No workflow was executed, nothing was created, and no key value appears in this document or in any file written during this pass.

## §19 Creation surface — verification 2026-09-19

Appended 2026-09-19T12:09:46Z from the live re-check of 2026-09-19 (11:29Z–11:42Z UTC; full evidence: `docs/registration/API_RECHECK_2026-09-19.md`, `docs/registration/CREATION_LOG_2026-09-19.md`, raw logs under `docs/registration/creation-evidence-2026-09-19/`). Scope: the creation endpoints only — tools/plugins, agents, skills, workflows (Agents Flow Builder) and the MCP server — checked against the 2026-09-17 baseline above (re-validated §18). Sources: the keyed docs API (`GET https://gateway.on-demand.io/config/v1/public/docs/categories` + the 40 per-operation references, 41/41 HTTP 200 at 11:31:53Z–11:32:10Z), 63 public-site URLs (all 40 comparable pages byte-identical to §0.1/§18.4), the OnDemand MCP server (`https://mcp-server-prd.on-demand.io/mcp`, streamable-HTTP, connected with the session apikey — value masked), and labelled probes of undocumented paths. Verdict = documentation status vs the baseline (**CONFIRMED / CHANGED / NEW / RETIRED**) plus creatability as observed (**API_CREATABLE / MCP_CREATABLE / DASHBOARD_ONLY**). No id was returned by any creation call: tool ID = null, agent ID = null, skill IDs = null ×9; the only pre-existing identifier is workflow `6aace534859f7b0abb53d99a` (§17/§18; `GET /automation/api/workflow/6aace534859f7b0abb53d99a` → 200, `isActive: true`, 9 `llm` nodes, no plugins attached, 2026-09-19T11:40:28Z).

| # | Surface | Endpoint (or MCP tool) | Method | Status (verdict · HTTP observed) | UTC fetch timestamp | Doc URL |
|---|---|---|---|---|---|---|
| 1 | Tools/plugins — REST tool from an OpenAPI schema (documented) | dashboard `https://app.on-demand.io/rag-agents/my-agents` → Create Agents → paste/import OpenAPI schema; no REST/MCP create operation documented | — | **CONFIRMED · DASHBOARD_ONLY** · doc page 200 (byte-identical to 2026-09-17) | 2026-09-19T11:34:28Z | https://docs.on-demand.io/docs/rest-based-plugins.md · https://docs.on-demand.io/docs/open-api-schema.md |
| 2 | Tools/plugins — undocumented create surface observed live | MCP tool `plugin_v1_plugin_create` (7 required fields: name, identifier, description, logoUrl, type, source, category; `action.schema` must be a JSON **string**) → REST `POST https://api.on-demand.io/plugin/v1` | MCP `tools/call` · POST | **NEW · not usable** (MCP_CREATABLE in schema only) · MCP HTTP 200 with `isError:true`, empty message, on all 6 payload variants (attempt 1 with a schema object: `json: cannot unmarshal object into Go struct field PluginAction.action.schema of type string`); REST 400 validation on `{}`, **500 empty body** on a complete body; `GET /plugin/v1/list` total 0 before and after | 2026-09-19T11:36:14Z → 11:39:50Z (MCP) · 11:37:45Z (REST 500) · 11:38:31Z (REST 400) | not documented (source: `tools/list`) |
| 3 | Agents — REST API Agent creation (docs' *Agents* = API's *plugins*) | only `GET https://api.on-demand.io/plugin/v1/list` (read-only; `data.total = 0` for this account) | GET | **CONFIRMED · DASHBOARD_ONLY** · 200 (list) · doc page 200 | 2026-09-19T11:36:07Z (list) · 11:34:22Z (doc) | https://docs.on-demand.io/docs/plugin-api.md · https://docs.on-demand.io/docs/rest-based-plugins.md |
| 4 | Agents — agent with a system prompt + attached tool (OnDemand Spatial Intelligence Agent) | no create field anywhere; attach-at-runtime only via `pluginIds`/`agentIds` on `POST https://api.on-demand.io/chat/v1/sessions` and `POST …/chat/v1/sessions/{sessionId}/query` | POST | **CONFIRMED · DASHBOARD_ONLY** · session 201 · query 200 (`status: completed`; no tool attached, the model said so) | 2026-09-19T11:40:52Z · 11:41:54Z | https://docs.on-demand.io/reference/createchatsession.md · https://docs.on-demand.io/reference/submitquery.md |
| 5 | Skills — creation | dashboard Skills → Create Skill (SKILL.md / zip); no operation in the 40 keyed specs, no MCP tool; probes `POST /skills`, `/skills/v1/skills`, `/plugin/v1/skills`, `/automation/api/skills`, `GET /skills` → 404 (`GET /plugin/v1/skills` → 400 `Agent plugin not found`, a plugin-id route) | — (probes POST/GET) | **CONFIRMED · DASHBOARD_ONLY** · doc page 200 (byte-identical) · probes 404/400 | 2026-09-19T11:32:24Z (doc) · 11:37:47Z–11:37:49Z (probes) | https://docs.on-demand.io/docs/agent-skills.md |
| 6 | Skills — attachment to an agent | dashboard agent editor / Playground ("Add the skill to an agent or a Playground session"); no endpoint or MCP tool | — | **CONFIRMED · DASHBOARD_ONLY** · doc page 200 | 2026-09-19T11:32:24Z | https://docs.on-demand.io/docs/agent-skills.md |
| 7 | Workflows / Agents Flow Builder — create | `POST https://api.on-demand.io/automation/api/workflow/` (also MCP `automation_api_workflow_create`) | POST | **CONFIRMED · API_CREATABLE** · keyed spec 200 · site page 200 (not exercised; existing workflow reused) | 2026-09-19T11:32:06Z · 11:32:30Z | https://docs.on-demand.io/reference/post_workflow.md · https://docs.on-demand.io/docs/workflow-api.md |
| 8 | Workflows — attach a tool/plugin to a node (`nodes[].llm.plugins[{id}]`) | `PATCH https://api.on-demand.io/automation/api/workflow/{id}` (also MCP `automation_api_workflow_update`); read via `GET …/workflow/{id}` | PATCH / GET | **CONFIRMED · API_CREATABLE** · live GET 200 (`6aace534859f7b0abb53d99a`, active, 9 nodes, no plugins) · keyed spec 200 · site page 200 — **not modified** (no pluginId exists to attach) | 2026-09-19T11:40:28Z (GET) · 11:32:08Z · 11:34:44Z | https://docs.on-demand.io/reference/patch_workflow-id.md · https://docs.on-demand.io/docs/workflow-nodes.md |
| 9 | Workflows — activate / deactivate / execute / logs | `POST …/workflow/{id}/activate`, `POST …/workflow/{id}/deactivate`, `POST …/workflow/{id}/execute`, `GET …/execution/{executionID}/logs` (plain JSON array; no streaming transport documented — unchanged since §7.3) | POST / GET | **CONFIRMED · API** · keyed specs 200 (not exercised; workflow already active) | 2026-09-19T11:32:06Z–11:32:09Z | https://docs.on-demand.io/reference/post_workflow-id-activate.md · https://docs.on-demand.io/reference/post_workflow-id-execute.md · https://docs.on-demand.io/reference/get_execution-executionid-logs.md |
| 10 | MCP server | `https://mcp-server-prd.on-demand.io/mcp?apikey=<MASKED>` — JSON-RPC `initialize` → `chat-proxy-mcp` 2.0.0 (protocol 2025-03-26, capabilities logging+tools); `tools/list` → **57 tools** (workflow CRUD/activate/execute/logs, chat sessions/queries, `plugin_v1_plugin_create`, `plugin_v1_plugin_configuration_create`, `public_v1_plugin_ai_generated_tool_create` (→ HTTP 520 on a draft probe), `public_v1_suggest_plugins`, serverless apps; **no skill tool, no agent-prompt tool**) | JSON-RPC POST | **NEW · undocumented** (no mention in §0.1/§18.4, `llms.txt`, or the keyed index; `/docs/mcp.md` and `/docs/mcp-server.md` → 404) · initialize 200 · tools/list 200 | 2026-09-19T11:33:28Z (initialize/tools/list) · 11:35:30Z (404 probes) | none |

**RETIRED:** nothing — every baseline operation for these surfaces is still present. **CHANGED:** no documentation change (40/40 comparable pages byte-identical); the only behavioural novelty is row 2 (an undocumented create surface that fails server-side). Consequence for this repository: the earthquake_search tool, the OnDemand Spatial Intelligence Agent and the nine skills remain **PENDING-HUMAN** (paste blocks: `docs/registration/READY_TO_PASTE.md` §1, §3, §2.1–§2.9; click paths §4); `/api/ondemand/health` now reports `config.spatialAgentId.source` and `config.spatialToolId.source` ∈ `env` | `registration-pack` | `unset` so the paste-back is observable without exposing any id (`server/ondemand/config.js` REGISTRATION_ID_ENV, env names `ONDEMAND_SPATIAL_AGENT_ID` / `ONDEMAND_SPATIAL_TOOL_ID`).
