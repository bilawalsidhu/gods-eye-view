...
Sixteen layers and map sources. **Thirteen have a keyless path.** Some offer additional capabilities with a provider key, and one needs one to run at all. (🟢 no key · 🟡 free key · 🔴 metered.)
...
| 🌧️ **Weather**              | Thirty selectable layers on one globe — observed radar, lightning, severe-weather warnings, cyclone tracks, storm cells, wind, temperature, air quality and sea state. A side panel picks what draws, refreshes on demand, and shows what it has spent. Needs a key like Live Vessels and Active Fires, and is metered past its free allowance                                                                                 | Vaisala Xweather                        | 🔴 metered (free tier, then per tile)                                                                |
...
| 🔴  | **Vaisala Xweather** | 🌧️ The Weather layer, which does not run without it — every other key here only unlocks extra capability. Two halves, a client ID and a secret, both server-side only: the vendor puts them in the tile URL, so the browser never sees either | [xweather.com/account](https://www.xweather.com/account) — 15,000 accesses/month free, then per tile                                                                 |
