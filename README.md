...
7. **Talk to it** *(OpenAI key or [local voice](docs/local-voice.md))*: *"Take me to LAX and select the nearest airborne aircraft."*
...
> Voice can use an **OpenAI key** or a fully local LocalAI pipeline on Apple
...
*🎙️ = voice missions — use an OpenAI key or the [local voice profile](docs/local-voice.md).*
...
- **No framework.** Vanilla JavaScript, **CesiumJS**, and **Vite** — plus **Google Photorealistic 3D Tiles** for the planet and a cloud or local Realtime voice provider. Fast to read, fast to hack on.
...
├── voice/                  # Cloud/local Realtime session + 28 voice tools
