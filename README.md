...
| ⛰ **Activations** | POTA, SOTA, WWFF and WWBOTA activators on the air right now, coloured by program. *"Show me POTA activations near Denver."* | POTA / SOTA / WWFF / WWBOTA (via HamRig) | 🟢 POTA / WWFF / WWBOTA · SOTA is off by default — set `HAMRIG_SOTA_ENABLED=1` only after obtaining SOTA API approval (see DATA_SOURCES.md) |
