...
test('a malformed or out-of-range extent yields no bounds rather than a bad box', () => {
  ...
  assert.equal(
    photonExtentToBounds([0, 95, 10, -95]),
    null,
    'latitude past the pole',
  );
  assert.equal(
    photonExtentToBounds([-200, 10, 10, 0]),
    null,
...
test('area features keep an area type so the camera frames them, not a rooftop', () => {
  ...
  const cases = [
    [
      { type: 'other', osm_key: 'water', osm_value: 'lake' },
      'natural_feature',
      'area-overview',
    ],
    [
      { type: 'other', osm_key: 'leisure', osm_value: 'park' },
      'park',
      'area-overview',
    ],
    [
      { type: 'other', osm_key: 'place', osm_value: 'region' },
      'natural_feature',
      'area-overview',
    ],
    [
      { type: 'other', osm_key: 'natural', osm_value: 'peak' },
      'natural_feature',
      'area-overview',
    ],
    [
      { type: 'other', osm_key: 'waterway', osm_value: 'river' },
      'natural_feature',
      'area-overview',
    ],
    [
      { type: 'house', osm_key: 'aeroway', osm_value: 'aerodrome' },
      'airport',
      'area-overview',
    ],
    [
      { type: 'house', osm_key: 'amenity', osm_value: 'university' },
      'university',
      'area-overview',
    ],
    [
      { type: 'street', osm_key: 'highway', osm_value: 'motorway' },
      'route',
      'street-corridor',
    ],
    [
      { type: 'country', osm_key: 'place', osm_value: 'country' },
      'country',
      'region-overview',
    ],
    [
      { type: 'state', osm_key: 'place', osm_value: 'state' },
      'administrative_area_level_1',
      'region-overview',
    ],
    [
      { type: 'district', osm_key: 'place', osm_value: 'suburb' },
      'sublocality',
      'neighborhood-close',
    ],
  ];
...
test('a tag rule wins over the coarse class — a town square is not a city', () => {
  ...
  assert.equal(
    geocodeNavigationMode(photonResultTypes(square)),
    'precise-place',
  );
});
...
test('the Google bias becomes a SOFT proximity bias, never a hard bbox', () => {
  ...
  assert.equal(
    ...
    '30.25',
    'centre of the biased rectangle',
  );
  ...
  assert.equal(
    ...
    null,
    'a bbox would filter out every off-screen place',
  );
});
...
test('an absent or unparsable bias drops the bias instead of sending a broken one', () => {
  for (const bias of [undefined, 'nonsense', 'a,b|c,d']) {
    const url = new URL(
      photonSearchUrl('Hanoi', bias === undefined ? {} : { bias }),
    );
...
test('a network failure resolves to null — callers read null as not-found', async () => {
  ...
  assert.equal(
    await geocodeKeyless('somewhere else', { fetchImpl: notOk }),
    null,
  );
  ...
  assert.equal(
    await geocodeKeyless('third distinct query', { fetchImpl: empty }),
    null,
  );
});
...
test('repeat queries are served from the memo, misses included', async () => {
  ...
  const counting = () => {
    ...
    return Promise.resolve({
      ok: true,
      json: () => Promise.resolve({ features: [HANOI] }),
    });
  };
  ...
  assert.equal(
    calls,
    2,
    'a miss must be memoized too, or a typo costs one request per keystroke',
  );
});
...
test('a feature without usable coordinates is rejected', () => {
  ...
  assert.equal(
    normalizePhotonFeature({ properties: { name: 'Nowhere' } }),
    null,
  );
  assert.equal(
    normalizePhotonFeature({ geometry: { coordinates: [200, 10] } }),
    null,
  );
  assert.equal(
    normalizePhotonFeature({ geometry: { coordinates: [10, 91] } }),
    null,
  );
});
...
function photonPair(nearby, anywhere) {
  ...
  const impl = (url) => {
    ...
    return Promise.resolve({
      ok: true,
      json: () => Promise.resolve({ features: biased ? nearby : anywhere }),
    });
  };
  impl.urls = urls;
  return impl;
...
test('a biased near-miss is refused, and the unbiased answer is taken instead', async () => {
  ...
  assert.equal(
    fetchImpl.urls.length,
    2,
    'the refused bias costs exactly one extra request',
  );
});
...
test('a name is matched at its head, so a street that merely contains it loses', () => {
  ...
  assert.equal(
    selectPhotonFeature([road], 'hue', { allowContains: true }),
    road,
  );
...
test('a "place, region" query is matched on the place, not the region', async () => {
  ...
  const street = {
    ...
    properties: { name: 'Hoan Kiem District Road' },
  };
  ...
  assert.equal(
    result.label.startsWith('Hoàn Kiếm Lake'),
    true,
    `got ${result.label}`,
  );
});
...
test('accents and punctuation do not change what a name compares as', () => {
  ...
  assert.equal(
    normalizeToponym("  St. John's-Ravenscourt  "),
...
test('forward geocoding is composed once and consumers do not call a source directly', () => {
  for (const file of [
    'locations.js',
    'annotations/annotationResolver.js',
    'voice/gevActions.js',
  ]) {
  ...
  const standalone = fs.readFileSync(
    path.join(ROOT, 'src/standalone/placeSearch.js'),
    'utf8',
  );
  assert.match(
    standalone,
  ...
  const setup = fs.readFileSync(
    path.join(ROOT, 'src/search/defaults.js'),
    'utf8',
  );
...
test('the normalized result carries the canonical name its callers need', () => {
  ...
  assert.equal(
    result.label.includes(result.name),
    true,
    'the label is built from the name, not beside it',
  );
});
...
test("Photon's own country name stays inside the label — it is not a field", () => {
  ...
  assert.equal(
    'country' in result,
    false,
    'a localized country name must not become a matchable field',
  );
...
test('an unanswered request is never memoized, so the next attempt really retries', async () => {
  ...
  assert.equal(
    await geocodeKeyless('blip probe', { fetchImpl: offline }),
    null,
  );
  ...
  assert.equal(
    attempts,
    2,
    'the retry must reach the network, not a memoized blip',
  );
...
function readGeocodingConsumer(file) {
  const files =
    file === 'annotations/annotationResolver.js'
