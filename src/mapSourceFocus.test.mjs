...
const locationMarkup = markup.slice(
  ...
  markup.indexOf('<div id="left-panel-stack"'),
);
const locationToggleMarkup = locationMarkup.match(
  /<button\b([^>]*\bid="location-bar-toggle"[^>]*)>([\s\S]*?)<\/button>/,
);
const locationToggleAttributes = Object.fromEntries(
  [...(locationToggleMarkup?.[1] || '').matchAll(/([\w-]+)="([^"]*)"/g)].map(
...
function harness({ hidden = false, selected = true, noChips = false } = {}) {
  ...
  const makeNode = (id, attributes = {}) => {
    ...
    return {
      ...
      removeEventListener(type, callback) {
        ...
        listeners.set(
          type,
          values.filter((item) => item !== callback),
        );
      },
      matches(selector) {
  ...
  const locationDisclosure = locationToggleMarkup
    ? makeNode('location-bar-toggle', { ...locationToggleAttributes })
    : null;
  ...
  location.contains = (node) =>
    Boolean(node) &&
    [location, locationDisclosure, locationSearch].includes(node);
  panel.querySelector = (selector) => {
    if (selector.startsWith('[data-dock-toggle-target')) return disclosure;
    if (selector.split(',').some((part) => part.trim() === '.panel-title'))
      return { textContent: 'VISUAL STYLES' };
  ...
  location.querySelector = (selector) => {
    if (selector === '[data-dock-toggle-target="location-bar"]') {
      return locationDisclosure?.getAttribute('data-dock-toggle-target') ===
        'location-bar'
        ? locationDisclosure
        : null;
    }
    if (
      selector
        .split(',')
        .some((part) => part.trim() === '.location-toolbar-label')
    )
  ...
  const focus = (node) => {
    ...
    for (const owner of [panel, location]) {
      if (owner.contains(prior))
        emit(owner, 'focusout', { target: prior, relatedTarget: node });
      if (owner.contains(node))
        emit(owner, 'focusin', { target: node, relatedTarget: prior });
    }
  };
  disclosure.focus = () => focus(disclosure);
  if (locationDisclosure)
    locationDisclosure.focus = () => focus(locationDisclosure);
  ...
  return {
    ...
    key(key = 'Enter', repeat = false) {
      return emit(disclosure, 'keydown', { key, repeat });
    },
    keyUp(key = ' ', repeat = false) {
      ...
      if (
        ...
        emit(disclosure, 'click', { detail: 0 });
      return event;
    },
    escape() {
      emit(panel, 'keydown', { key: 'Escape' });
    },
    locationKey(key = 'Enter', repeat = false) {
      return emit(locationDisclosure, 'keydown', { key, repeat });
    },
    locationKeyUp(key = ' ', repeat = false) {
      ...
      if (
        key === ' ' &&
        !repeat &&
        document.activeElement === locationDisclosure &&
        !event.defaultPrevented
      ) {
...
test('selection is read again when a delayed attempt can focus', () => {
  const h = harness({ hidden: true, selected: false });
  h.key();
  h.tick();
  h.select(true);
  h.show();
...
test('close then pointer or programmatic reopen cannot inherit a keyboard request', () => {
  for (const reopen of [
    (h) => h.click(1),
    (h) => h.manager.setPanelCollapsed('control-panel', false),
  ]) {
    const h = harness();
    h.key();
...
test('Location markup provides one named native disclosure linked to its popover', () => {
  assert.ok(
    locationToggleMarkup,
    'Location must expose a native button in the real markup',
  );
  assert.equal(locationToggleAttributes.type, 'button');
  assert.equal(
    locationToggleAttributes['data-dock-toggle-target'],
    'location-bar',
  );
  assert.equal(
    locationToggleAttributes['aria-controls'],
    'location-bar-popover',
  );
  ...
  assert.match(
    locationToggleAttributes.class,
    /(?:^|\s)dock-tray-toggle(?:\s|$)/,
  );
...
test('Location Enter stays immediate while Space waits for native key release', () => {
  for (const key of ['Enter', ' ']) {
    ...
    assert.equal(
      h.locationDisclosure.getAttribute('aria-label'),
      'Collapse LOCATION',
    );
    ...
    const release =
      key === ' '
        ? h.locationKeyUp(key, true)
        : h.emit(h.locationDisclosure, 'keyup', { key });
    assert.equal(
      release.defaultPrevented,
      false,
      'global voice release must remain reachable',
    );
...
test('Location restoration syncs its real name without claiming focus, collapsing its sibling or persisting', () => {
  ...
  assert.equal(
    h.locationDisclosure.getAttribute('aria-label'),
    'Collapse LOCATION',
    'same-state synchronization keeps the Location name',
  );
  assert.equal(h.timers.size, 0);
});
...
test('replacing hover controls removes the previous listeners before rebinding', () => {
  ...
  h.key();
  assert.equal(
    h.panel.classes.has('collapsed'),
    false,
    'one activation toggles only once',
  );
  h.tick();
  assert.equal(h.calls(), 1);
});
