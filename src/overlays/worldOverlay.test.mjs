...
test('scene presentation samples once per frame and recovers after a callback failure', () => {
  ...
  Object.assign(entry, {
    ...
    edgeFade: 'none',
    altitudeScale: null,
    altitudeFadeEnd: Number.POSITIVE_INFINITY,
  });
  let scale = 0.5;
  let alpha = 0.8;
  let leader = 0.5;
  ...
  const publish = () =>
    setOverlayEntries('scene-presentation', [entry], {
      cohortLimit: 1,
      collisionCapacity: 0,
    });
  const frame = () => {
  ...
  try {
    ...
    assert.equal(
      env.ctx.calls.some(([name]) => name === 'drawImage'),
      false,
    );
    ...
    frame();
    assert.equal(
      env.ctx.calls.some(([name]) => name === 'scale'),
      false,
    );
