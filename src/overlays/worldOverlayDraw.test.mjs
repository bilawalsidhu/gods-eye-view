...
test('selected elbow leader reveals from the glyph before its card fades in', () => {
  ...
  assert.ok(
    lines.length >= 2,
    'leader draws horizontally from the glyph, then vertically to the card',
  );
  assert.ok(
    ctx.calls.some(
      ([name, value]) =>
        name === 'strokeStyle' && value === 'rgba(255, 100, 116, 0.95)',
    ),
  );
});
