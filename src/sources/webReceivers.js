/**
 * Web receiver rules — pure and portable.
 *
 * Shared by the directory proxy (band parsing, coverage flags), the Web
 * Receivers layer (filtering, ranking, tune URLs) and the voice tools. Nothing
 * here touches the DOM, Cesium or Node APIs.
 *
...
export const RECEIVER_MODES = Object.freeze([
  'usb',
  'lsb',
  'am',
  'cw',
...
export const BAND_TABLE = Object.freeze([
  ...
  { keys: ['70cm'], lowHz: 430_000_000, highHz: 440_000_000, label: '70 cm' },
  {
    keys: ['23cm'],
    lowHz: 1_240_000_000,
    highHz: 1_300_000_000,
    label: '23 cm',
  },
  {
    keys: ['lw', 'longwave', 'long wave'],
    lowHz: 30_000,
    highHz: 300_000,
    label: 'LW',
  },
  {
    keys: ['mw', 'medium wave', 'mediumwave', 'am broadcast'],
    lowHz: 520_000,
    highHz: 1_710_000,
    label: 'MW',
  },
  {
    keys: ['hf', 'shortwave', 'short wave', 'sw', 'kw', 'kurzwelle'],
    lowHz: 1_600_000,
    highHz: 30_000_000,
    label: 'HF',
  },
  ...
  {
    keys: ['fm broadcast', 'fm radio', 'ukw', 'broadcast fm', 'fm band'],
    lowHz: 87_500_000,
    highHz: 108_000_000,
    label: 'FM broadcast',
  },
  {
    keys: ['airband', 'air band', 'aviation', 'aircraft'],
    lowHz: 118_000_000,
    highHz: 137_000_000,
    label: 'Airband',
  },
...
export const BAND_FILTERS = Object.freeze([
  {
    ...
    label: 'All bands',
    lowHz: 0,
    highHz: Number.POSITIVE_INFINITY,
  },
...
const BAND_FILTER_BY_ID = new Map(
  BAND_FILTERS.map((entry) => [entry.id, entry]),
);

const NUMBER_RE = /^-?\d+(?:[.,]\d+)?$/;

/** Collapse control characters and whitespace; bound the length. */
export function cleanReceiverText(value, maxLength = 200) {
  return String(value ?? '')
    .replace(/[\u0000-\u001f\u007f]/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
...
function finiteOrNull(value) {
  // null, undefined and '' are "not given", never zero: a missing origin must
  // not rank receivers by their distance from Null Island.
...
export function normalizeReceiverMode(value) {
  const raw = cleanReceiverText(value, 40).toLowerCase();
  if (!raw) return null;
...
export function parseFrequencyHz(value, unitHint = '') {
  if (value === null || value === undefined || value === '') return null;
  let text = cleanReceiverText(value, 40).toLowerCase().replace(/\s+/g, '');
  let unit = cleanReceiverText(unitHint, 8).toLowerCase();
...
export function formatFrequencyHz(hz) {
  ...
  return `${(value / MHZ).toLocaleString('en-US', {
...
function rangeLabel(lowHz, highHz) {
  return formatFrequencyRange(Math.max(lowHz, 1_000), highHz);
}
...
export function parseBandsFromText(text) {
  const source = cleanReceiverText(text, 400).toLowerCase();
  if (!source) return [];
  const bands = [];
  const seen = new Set();
  const push = (band) => {
    ...
    unit === 'ghz'
      ...
      : unit === 'khz'
        ...
        : unit === 'hz'
          ...
          : MHZ;
  const rangeRe =
    /(\d+(?:[.,]\d+)?)\s*(ghz|mhz|khz|hz)?\s*(?:-|–|—|to|bis|\.\.)\s*(\d+(?:[.,]\d+)?)\s*(ghz|mhz|khz|hz)\b/g;
  let match;
...
export function receiverCoversRangeHz(receiver, lowHz, highHz) {
  ...
  return receiver.bands.some(
    (band) => band.lowHz <= low && band.highHz >= high,
  );
}
...
export function describeReceiverBands(receiver) {
  ...
  return bands
    .slice(0, 6)
    .map((band) => band.label || rangeLabel(band.lowHz, band.highHz))
    .join(' · ');
}

function receiverBase(receiver) {
  if (!receiver?.url || !RECEIVER_TYPES.includes(receiver.type)) return null;
  let base;
  try {
...
function kHzText(hz) {
  const kHz = Math.round(hz) / KHZ;
  return Number.isInteger(kHz)
    ? String(kHz)
    : kHz.toFixed(3).replace(/0+$/, '').replace(/\.$/, '');
}
...
export function buildTuneUrl(receiver, { hz, mode = null } = {}) {
  const value = finiteOrNull(hz);
  const base = receiverBase(receiver);
  if (!base || value === null || value <= 0) return null;
  ...
  if (receiver.type === 'kiwisdr') {
    ...
    return `${base.href}?f=${kHzText(value)}${native}z${zoom}`;
  }
  if (receiver.type === 'websdr') {
    return `${base.href}?tune=${kHzText(value)}${native}`;
  }
...
export function buildSpectrumUrl(receiver, { lowHz, highHz } = {}) {
  ...
  const high = finiteOrNull(highHz);
  const base = receiverBase(receiver);
  if (!base || low === null || high === null || high <= low || low < 0)
    return null;
  const centerHz = Math.round((low + high) / 2);
  const spanHz = high - low;
  const label = formatFrequencyRange(low, high);
  const centerText = kHzText(centerHz);
  if (receiver.type === 'kiwisdr') {
    const kiwiBand =
      (receiver.bands || []).find(
        (band) => band.lowHz <= low && band.highHz >= high,
      ) || receiver.bands?.[0];
    const fullSpanHz = Math.max(
      30 * MHZ,
      kiwiBand ? kiwiBand.highHz - kiwiBand.lowHz : 0,
    );
    const zoom = Math.max(
      0,
      Math.min(KIWI_MAX_ZOOM, Math.floor(Math.log2(fullSpanHz / spanHz))),
    );
    ...
    return {
      ...
      shownSpanHz,
      rangeLabel: label,
      note: `waterfall spans about ${formatFrequencyRange(
        Math.max(0, centerHz - shownSpanHz / 2),
        centerHz + shownSpanHz / 2,
      )}, audio muted`,
    };
  }
  if (receiver.type === 'websdr') {
...
export function receiverMatchesFilter(
  receiver,
  { type = 'all', band = 'all' } = {},
) {
  ...
  const bandFilter =
    BAND_FILTER_BY_ID.get(band || 'all') || BAND_FILTER_BY_ID.get('all');
  if (bandFilter.id === 'all') return true;
  const bands = Array.isArray(receiver.bands) ? receiver.bands : [];
  if (!bands.length) return false;
  return bands.some(
    (entry) =>
      entry.lowHz < bandFilter.highHz && entry.highHz > bandFilter.lowHz,
  );
}

/**
 * Rank receivers for a request: online first, then receivers that publish
 * coverage of the requested frequency (or the whole range), then a preferred
 * family, then free user slots, then distance.
 */
export function rankWebReceivers(
  receivers,
  {
    lat = null,
    lon = null,
  ...
  const range =
    Array.isArray(rangeHz) &&
    finiteOrNull(rangeHz[0]) !== null &&
    finiteOrNull(rangeHz[1]) !== null
  ...
  for (const receiver of Array.isArray(receivers) ? receivers : []) {
    ...
    const covers = range
      ...
      : frequency === null
        ? null
        : receiverCoversHz(receiver, frequency);
    if (requireCoverage && (frequency !== null || range) && covers === false)
      continue;
    const distanceKm = origin
      ? haversineKm(lat, lon, receiver.lat, receiver.lon)
      : null;
    const full =
      receiver.users !== null &&
      receiver.usersMax !== null &&
      receiver.users !== undefined &&
      receiver.usersMax !== undefined &&
