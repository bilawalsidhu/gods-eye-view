...
const indexHtml = expandApplicationHtml(
...
test('first run opens with a subtle scope feather, at every surface that decides it', () => {
  ...
  assert.match(
    indexHtml,
    /id="scope-feather-slider"[^>]*\svalue="11"/,
    'index.html: the feather slider ships at 11',
  );
  assert.match(
    indexHtml,
    /id="scope-feather-value"[^>]*>11%</,
...
test('an explicit feather still wins over the subtle default', () => {
  ...
  assert.equal(
    managerForHash('#lat=10&lon=20&scf=35').parseInitialHash().scopeFeatherPct,
    35,
  );
  assert.equal(
    managerForHash('#lat=10&lon=20&scf=64').parseInitialHash().scopeFeatherPct,
    64,
  );
  assert.equal(
    managerForHash('#lat=10&lon=20&scf=0').parseInitialHash().scopeFeatherPct,
    0,
  ...
  assert.equal(
    managerForHash('#lat=10&lon=20&style=normal').parseInitialHash()
      .scopeFeatherPct,
    35,
...
test('the subtle default did not weaken the feather control, and 0 is still reachable', () => {
  ...
  assert.match(
    indexHtml,
    /id="scope-feather-slider"[^>]*\smin="0"[^>]*\smax="100"/,
  ...
  try {
    for (const ratio of [0.35, 0.7, 1]) {
      ...
      assert.ok(
        Math.abs(geo.outerR - geo.innerR - keyholeR * ratio) < 1e-9,
    ...
    assert.ok(
      Math.abs(
        soft.outerR - soft.innerR - keyholeR * SCOPE_FEATHER_RATIO_DEFAULT,
      ) < 1e-9,
...
test('first run opens at 7% detection fade, at every surface that decides it', () => {
  ...
  assert.match(
    indexHtml,
    /id="detection-fade-slider"[^>]*\svalue="7"/,
    'index.html: the fade slider ships at 7',
  );
  assert.match(
    indexHtml,
    /id="detection-fade-value"[^>]*>7%</,
...
test('first run opens at 1% OUTSIDE opacity, at every surface that decides it', () => {
  ...
  assert.match(
    indexHtml,
    /id="detection-opacity-slider"[^>]*\svalue="1"/,
    'index.html: the OUTSIDE slider ships at 1',
  );
  assert.match(
    indexHtml,
    /id="detection-opacity-value"[^>]*>1%</,
    'index.html: and its readout agrees with the handle',
  );
  assert.match(
    uiSource,
  ...
  assert.match(
    indexHtml,
    /id="detection-opacity-slider"[^>]*\sstep="1"/,
...
test('an explicit OUTSIDE opacity still wins over the new default', () => {
  assert.equal(
    managerForHash('#lat=10&lon=20&ko=5').parseInitialHash()
      .detectionOutsideOpacityPct,
    5,
  );
  assert.equal(
    managerForHash('#lat=10&lon=20&ko=40').parseInitialHash()
      .detectionOutsideOpacityPct,
    40,
  );
  assert.equal(
    managerForHash('#lat=10&lon=20&ko=0').parseInitialHash()
      .detectionOutsideOpacityPct,
    0,
    'an explicit 0 is a choice too, not an absent field',
  );
...
test('detection-on-by-default is a default, not an operator override', () => {
  ...
  const displayActions = uiSource.slice(
    uiSource.indexOf('this._displayControls ='),
  );
  const detectionButton = displayActions.slice(
  ...
  assert.match(
    detectionButton,
    /cycleDetectionMode\(\)/,
    'the button still invokes the detection action',
  );
  assert.match(
    detectionButton,
    /this\.claimDetection\(\);/,
  ...
  assert.match(
    uiSource,
    /claimDetection: \(\) => \{\s*this\._visualSettings\._detectionUserOverridden = true;/,
  );
...
test('a share link that carries detection OFF still restores OFF', () => {
  ...
  assert.equal(
    off.detectionMode,
    'OFF',
    'an explicit OFF survives the default flip',
  );

  const sparse = managerForHash(
    '#lat=10&lon=20&dm=SPARSE&dd=25',
  ).parseInitialHash();
  assert.equal(sparse.detectionMode, 'SPARSE');
  assert.equal(
    sparse.detectionDensity,
    25,
    'and a quieter explicit profile is not promoted to the new default',
  );

  const dense = managerForHash(
    '#lat=10&lon=20&dm=DENSE&dd=75',
  ).parseInitialHash();
