...
async function launch(overrides = {}, dotenv = '', omitCctv = false) {
  ...
  try {
    ...
    await fs.copyFile(
      new URL('./data/cctv.js', import.meta.url),
      path.join(root, 'src', 'data', 'cctv.js'),
    );
    for (const name of [
      'dev-cctv.sh',
      'dev-fresh.sh',
      'read-dotenv-value.mjs',
    ]) {
      await fs.copyFile(
    ...
    const catalog = await fs.readFile(
      new URL('./standalone/catalog.js', import.meta.url),
      'utf8',
    );
    await fs.writeFile(
      path.join(root, 'src', 'standalone', 'catalog.js'),
      catalog,
    );
    ...
    await fs.writeFile(
      path.join(root, 'src', 'app', 'constructCatalog.js'),
      omitCctv
        ? assembly.replace(/^\s*createApplicationCctv\(.*$/m, '')
        : assembly,
    );
    await fs.mkdir(path.join(root, 'node_modules'));
    await fs.symlink(
      fileURLToPath(new URL('.', import.meta.resolve('vite/package.json'))),
...
`,
    ...
    const result = await run(
      'bash',
      [path.join(root, 'scripts', 'dev-cctv.sh')],
      {
        cwd: os.tmpdir(),
        env: {
...
bashTest(
  'CCTV preset starts keyless on localhost through the normal launcher',
  async () => {
...
bashTest(
  'CCTV preset preserves explicit LAN and source overrides with a warning',
  async () => {
    const result = await launch({
      HOST: '0.0.0.0',
      PORT: '4999',
...
bashTest(
  'CCTV preset shares dotenv precedence and names-only credential provenance',
  async () => {
    const result = await launch(
    ...
    assert.doesNotMatch(
      result.output,
      /fixture-shell-maps|fixture-file-maps|fixture-file-voice/,
    );
  },
);

bashTest(
  'CCTV preset refuses a catalog that no longer registers its layer',
  async () => {
    await assert.rejects(launch({}, '', true), (error) => {
      assert.match(
        error.stdout,
        /CCTV layer not wired in src\/standalone\/catalog.js/,
      );
      return true;
    });
  },
