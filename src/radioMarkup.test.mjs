...
test('the edited existing tools changed exactly as intended', () => {
  ...
  assert.deepEqual(
    ...
    ['data-panel', 'location-bar', 'control-panel', 'cctv-panel', 'radio-panel', 'web-receivers-panel', 'scene-panel', 'pp-toggles', 'global-context-panel'],
  );
...
test('no unchanged Realtime tool definition drifts silently', () => {
  ...
  const TOUCHED = new Set([
    ...
    // The Web Receivers layer joined the two layer enums and added three
    // tools of its own (find_web_receivers, tune_web_receiver,
    // show_rf_spectrum); all four are additions, not rewordings.
  ...
  const digest = createHash('sha256')
    ...
    .digest('hex')
    .slice(0, 16);
  // Analyst additions, the ISS wording correction, the Cyber HUD choice and the
  // Web Receivers enum additions are explicitly excluded above; all other tool
  // definitions retain their pin.
