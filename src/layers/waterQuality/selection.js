...
export function createSelection({
  ...
  function selectRecord(id) {
    ...
    if (layerState.selectedId !== id) return false;
    // Selection is the only thing that spends a result query.
    void parts.ingestion.loadMeasurements(id);
    return true;
  }
  ...
  function installInteraction(viewer) {
    ...
    layerState.clickHandler.setInputAction((click) => {
      ...
      if (id && layerState.recordById.has(id) && id !== layerState.selectedId) {
        ...
        // sibling click handler's newly selected contact stays intact.
