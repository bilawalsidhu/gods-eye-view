...
import { GeminiLiveSession } from './geminiLiveAdapter.js';
...
export class GevRealtimeController extends RealtimeFacade {
  ...
  async start({ pushToTalk = false } = {}) {
    ...
    // ── Branch for Gemini 3.8 Live ──
    if (this.voiceTier === 'gemini') {
      return this.startGeminiLive(epoch);
    }
  ...
  async startGeminiLive(epoch) {
    this.setStatus('connecting', 'Connecting to Gemini 3.8 Live');
    let localStream = null;
    try {
      localStream = await navigator.mediaDevices.getUserMedia({
        audio: {
      ...
      if (this.abandonStart(epoch, { localStream })) return;
      this.stream = localStream;
      this.setMicrophoneEnabled(!this.pushToTalkMode || this.pushToTalkKeyHeld);
      this.startVoiceVisualizer(localStream);
      ...
      this.geminiSession = new GeminiLiveSession({
        onOpen: () => {
          const detail = this.pushToTalkMode
            ? (this.pushToTalkKeyHeld ? 'Release Space to send' : 'Hold Space to talk')
            : 'Ask or command (Gemini 3.8)';
          this.setStatus('listening', detail);
        },
        onMessage: (msg) => this.handleRealtimeEvent({ data: JSON.stringify(msg) }),
        onError: (err) => this.reportError('Gemini 3.8 Live', err),
        onClose: () => {
      ...
      await this.geminiSession.connect(localStream);
    } catch (error) {
      if (epoch !== this.startEpoch) {
      ...
      this.stop({ preserveStatus: true });
      this.reportError('Gemini Live connection', error);
    }
  }
  ...
  async handleRealtimeEvent(event) {
    ...
    for (const call of calls) {
      ...
      try {
        ...
        const isChannelOpen = this.geminiSession
          ? Boolean(this.geminiSession.ws?.readyState === 1)
          : (this.dc === resultChannel && resultChannel?.readyState === 'open');
    ...
    if (sentOutput && (this.dc?.readyState === 'open' || this.geminiSession?.ws?.readyState === 1)) {
  ...
  sendToolOutput(callId, result) {
    ...
    if (this.geminiSession) {
      this.geminiSession.sendToolResponse(callId, result);
      return true;
    }
    if (!this.dc || this.dc.readyState !== 'open') return false;
  ...
  syncCostUi() {
    ...
    const tierLabel = pendingTier === 'gemini' ? 'GEM 3.8' : pendingTier === 'mini' ? 'MINI' : 'STD';
    if (this.ui?.tierButton) {
      this.ui.tierButton.textContent = tierLabel;
      this.ui.tierButton.setAttribute('aria-pressed', pendingTier !== 'standard' ? 'true' : 'false');
      ...
      this.ui.tierButton.title = this.isActive() && state.modelId !== pendingId
        ...
        : `Voice model: ${pendingId} — click to cycle model; applies next session`;
    }
  ...
  /**
   * Cycles STANDARD (GPT-4o) ➔ MINI (GPT-4o Mini) ➔ GEMINI (Gemini Live).
   * Takes effect on the NEXT session.
   */
  toggleVoiceTier() {
    const current = resolveVoiceModel(this.voiceTier).tier;
    const nextTier = current === 'standard' ? 'mini' : current === 'mini' ? 'gemini' : 'standard';
    return this.setVoiceTier(nextTier);
  }
