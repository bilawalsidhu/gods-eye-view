...
test('aircraft identity narration acknowledges missing enrichment', () => {
  ...
  assert.match(
    text,
    /route, routeOrigin, and routeDestination as the only authoritative route fields/,
  );
  assert.match(
    text,
    /Every aircraft identity answer MUST explicitly cover operator, type, and route/,
  );
  ...
  const followupStart = realtime.indexOf(
    "if (result?.action === 'get_entity_context')",
  );
  const followupEnd = realtime.indexOf(
    "if (result?.action === 'get_current_view_state')",
    followupStart,
  );
  assert.ok(
    followupStart >= 0 && followupEnd > followupStart,
    'entity-context follow-up instruction is missing',
  );
  ...
  assert.match(
    followup,
    /explicitly cover operator, aircraft type, and route before finishing/,
  );
  assert.match(followup, /selectedProperties\.operator/);
  assert.match(followup, /selectedProperties\.type/);
  assert.match(
    followup,
    /selectedProperties\.route \|\|\s*selectedProperties\.routeOrigin \|\|\s*selectedProperties\.routeDestination/,
  );
