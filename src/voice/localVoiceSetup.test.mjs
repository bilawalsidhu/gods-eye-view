...
test('LocalAI compatibility patch honors thinking=false', () => {
  ...
  assert.equal(
    patchLocalAiBackendSource(patched),
    patched,
...
test('weights are detected through the Hugging Face cache layout', () => {
  ...
  try {
    assert.equal(
      weightsPresent(root, repoId),
      false,
      'an empty models directory has no weights',
    );

    const snapshot = path.join(
      root,
      hfCacheDirName(repoId),
      'snapshots',
      'abc123',
    );
    ...
    assert.equal(
      weightsPresent(root, repoId),
      false,
      'config without weights is an unfinished download',
    );
...
test('the installer derives backends, gallery models and weights from the profile', () => {
  ...
  assert.deepEqual(Object.keys(profile.stages).sort(), [
    'llm',
    'transcription',
    'tts',
    'vad',
  ...
  for (const backend of ['opus', 'mlx', 'whisper', 'parakeet-cpp', 'kokoro']) {
    assert.ok(
      profile.backends.includes(backend),
      `${backend} is installed for this profile`,
    );
  }
  ...
  assert.deepEqual(profile.weights, [
    { name: 'minicpm5-2b-mlx', repoId: 'openbmb/MiniCPM5-2B-MLX' },
  ]);
  assert.deepEqual(profile.compatibility, ['mlx-thinking', 'minicpm5-parser']);
});
...
test('Hub repos are told apart from gallery weight files', () => {
  ...
  assert.equal(
    isHuggingFaceRepo('parakeet-cpp/realtime_eou_120m-v1-f16.gguf'),
    false,
  );
...
test('a profile with no mlx stage installs off Apple Silicon', () => {
  ...
  try {
    ...
    assert.deepEqual(
      profile.backends,
      ['opus', 'llama-cpp'],
      'only the backends the stages name',
    );
    assert.deepEqual(profile.gallery, [
      'silero-vad-ggml',
      'whisper-large',
    ...
    assert.deepEqual(
      profile.weights,
      [],
      'a local weight file is not a Hub download',
    );
    assert.deepEqual(profile.compatibility, []);
    ...
    for (const name of fs.readdirSync(models)) {
      fs.copyFileSync(
        ...
        path.join(installedModels, name),
      );
    }
    assert.equal(
      setupLocalVoice({
    ...
    fs.appendFileSync(
      path.join(installedModels, 'gpt-realtime.yaml'),
      '# stale\n',
    );
    assert.throws(
      () =>
