...
export function createGevActionRunner({ viewer, styleManager, dataManager, sceneDirector = null, annotations = null }) {
  ...
  return async function runGevAction(name, rawArgs = {}, runOptions = {}) {
    ...
    if (name === 'find_web_receivers') {
      return findWebReceivers(viewer, dataManager, args, runOptions);
    }

    if (name === 'tune_web_receiver') {
      return tuneWebReceiver(viewer, dataManager, args, runOptions);
    }
  ...
  try {
    ...
    if (canvas && camera?.pickEllipsoid) {
      ...
      if (center) {
        ...
        return { lat: Cesium.Math.toDegrees(carto.latitude), lon: Cesium.Math.toDegrees(carto.longitude), label: 'the current view', country: '' };
      }
    }
    const position = camera?.positionCartographic;
    if (position) {
      return { lat: Cesium.Math.toDegrees(position.latitude), lon: Cesium.Math.toDegrees(position.longitude), label: 'the current view', country: '' };
    }
  } catch {
    /* fall through */
  }
...
export async function findWebReceivers(viewer, dataManager, args = {}, options = {}) {
  ...
  const hz = args.frequencyKhz !== undefined && args.frequencyKhz !== null ? parseFrequencyHz(args.frequencyKhz, 'khz') : null;
  const band = ['all', 'lf-mw', 'hf', 'vhf', 'uhf'].includes(args.band) ? args.band : 'all';
  const receiverType = ['all', 'kiwisdr', 'websdr', 'openwebrx'].includes(args.receiverType) ? args.receiverType : 'all';
  ...
  if (results.length && args.frameResults !== false) module.frame(results.map((row) => row.id));
  return {
    ok: results.length > 0,
...
export async function tuneWebReceiver(viewer, dataManager, args = {}, options = {}) {
  const module = await ensureWebReceiversReady(dataManager, options);
  const hz = parseFrequencyHz(args.frequencyKhz, 'khz');
  if (hz === null) {
    return { ok: false, action: 'tune_web_receiver', error: 'A frequency in kHz is required' };
  }
  ...
  if (!receiver && args.receiverQuery) {
    ...
    resolvedBy = 'query';
    if (!receiver) return { ok: false, action: 'tune_web_receiver', error: `No web receiver matched "${args.receiverQuery}"` };
  }
  ...
  if (!tuned.ok) return { ok: false, action: 'tune_web_receiver', error: tuned.error };
  ...
  if (openIn === 'tab' && typeof window !== 'undefined') {
    try { window.open(tuned.url, '_blank', 'noopener'); } catch { /* popup blocked: the dock link remains */ }
  }
  if (typeof document !== 'undefined') {
    document.dispatchEvent(new CustomEvent('gev:web-receiver-tune', {
      detail: { receiverId: receiver.id, url: tuned.url, openIn, origin: 'voice' },
    }));
  }
  return {
    ok: true,
...
export function spectrumRangeHz(args = {}) {
  ...
  const span = args.spanKhz !== undefined && args.spanKhz !== null ? Number(args.spanKhz) * 1000 : null;
  if (center !== null && span !== null && Number.isFinite(span) && span > 0) {
    return [Math.max(0, Math.round(center - span / 2)), Math.round(center + span / 2)];
  }
  return null;
}

/** Voice: silent spectrum view of a range on the best receiver. */
export async function showRfSpectrum(viewer, dataManager, args = {}, options = {}) {
  const module = await ensureWebReceiversReady(dataManager, options);
  const range = spectrumRangeHz(args);
  if (!range) {
    return { ok: false, action: 'show_rf_spectrum', error: 'A frequency range is required: startKhz and stopKhz, or centerKhz and spanKhz' };
  }
  const [lowHz, highHz] = range;
  if (highHz - lowHz > 40_000_000) {
    return { ok: false, action: 'show_rf_spectrum', error: 'Spans wider than 40 MHz are not useful on a web receiver; ask for a narrower range' };
  }
  let receiver = null;
  let resolvedBy = '';
  if (args.receiverId) {
  ...
  if (!receiver && args.receiverQuery) {
    receiver = module.resolveReceiver(args.receiverQuery);
    resolvedBy = 'query';
    if (!receiver) return { ok: false, action: 'show_rf_spectrum', error: `No web receiver matched "${args.receiverQuery}"` };
  }
  const target = String(args.target || '').toLowerCase();
  if (!receiver && target !== 'nearest' && !args.locationQuery && !args.locationId && !radioCoordinatePair(args).valid) {
    receiver = module.getUIState().selected;
    resolvedBy = 'selected';
  }
  let location = null;
  ...
  if (!view.ok) return { ok: false, action: 'show_rf_spectrum', error: view.error };
  module.selectReceiver(receiver.id, { flyTo: true, origin: 'voice' });
  const openIn = args.openIn === 'tab' ? 'tab' : 'dock';
  if (openIn === 'tab' && typeof window !== 'undefined') {
    try { window.open(view.url, '_blank', 'noopener'); } catch { /* popup blocked: the dock link remains */ }
  }
  if (typeof document !== 'undefined') {
    document.dispatchEvent(new CustomEvent('gev:web-receiver-tune', {
      detail: { receiverId: receiver.id, url: view.url, openIn, origin: 'voice', kind: 'spectrum' },
    }));
  }
  return {
    ok: true,
