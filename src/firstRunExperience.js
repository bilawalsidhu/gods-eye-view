...
export function environmentalLabel(choice = ENVIRONMENTAL_LABEL_CHOICE) {
  const key = ENVIRONMENTAL_LABEL_KEYS[choice] || ENVIRONMENTAL_LABEL_KEYS.ENVIRONMENTAL;
  return { title: t(key) };
}
...
export function initFirstRunExperience({
  ...
  if (environmentalSub) environmentalSub.textContent = t('setup.firstRun.choice.environmentalSub');
  ...
  const setBusy = (next, choice = '') => {
    ...
    if (!status) return;
    if (next) status.textContent = t(FIRST_RUN_MISSIONS[choice]?.busyKey || 'setup.firstRun.busy.working');
