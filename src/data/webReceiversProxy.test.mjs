...
test('publicWebReceiverUrl keeps public http(s) receivers and drops private/local targets', () => {
  assert.equal(publicWebReceiverUrl('http://sa4bna.hopto.org:8073'), 'http://sa4bna.hopto.org:8073/');
  assert.equal(publicWebReceiverUrl('HTTPS://Paris.Example.org/rx?x=1#y'), 'https://paris.example.org/rx/');
  ...
  assert.equal(webReceiverId('http://sa4bna.hopto.org:8073/'), webReceiverId('http://sa4bna.hopto.org:8073'));
  assert.notEqual(webReceiverId('http://sa4bna.hopto.org:8073/'), webReceiverId('http://sa4bna.hopto.org:8074/'));
});
...
test('normalizeReceiverbookSites parses the embedded array and infers coverage from labels', () => {
  ...
  assert.deepEqual(rows[0].bands.map((band) => band.label), ['Airband']);
  assert.ok(rows[2].bands.some((band) => band.lowHz === 0 && band.highHz === 29_000_000));
  assert.deepEqual(rows[4].bands.map((band) => band.label), ['2 m']);
  assert.equal(rows[4].site, 'Odd "quoted" [site]');
  assert.deepEqual(rows[0].sources, ['receiverbook']);
  assert.throws(() => normalizeReceiverbookSites('<html>no data</html>'), /not found/);
});
...
test('normalizeKiwiSdrRows reads gps, bands, slots and online state', () => {
  ...
  assert.deepEqual(rows[0].bands, [{ lowHz: 0, highHz: 30_000_000, label: '0–30 MHz' }]);
...
test('mergeWebReceivers enriches Receiverbook rows with KiwiSDR data and adds unknown Kiwis', () => {
  ...
  const arvika = merged.find((row) => row.url === 'http://sa4bna.hopto.org:8073/');
  assert.equal(arvika.users, 3);
  assert.equal(arvika.online, true);
  assert.deepEqual(arvika.sources, ['receiverbook', 'kiwisdr']);
  assert.equal(arvika.bands[0].highHz, 30_000_000, 'published Kiwi coverage replaces the label guess');
  assert.equal(arvika.coverage.hf, true);
  assert.equal(arvika.coverage.vhf, false);
  const sleepy = merged.find((row) => row.url === 'http://sleepy.example.net:8073/');
  assert.deepEqual(sleepy.sources, ['kiwisdr']);
  assert.equal(sleepy.online, false);
  assert.equal(merged.length, 6);
  assert.deepEqual([...merged].sort((a, b) => a.name.localeCompare(b.name)).map((row) => row.name), merged.map((row) => row.name), 'sorted by name');
});

function bigReceiverbook(count) {
  const sites = Array.from({ length: count }, (_, index) => ({
    label: `Site ${index}`,
    location: { coordinates: [10 + index * 0.01, 50 + index * 0.01], type: 'Point' },
    receivers: [{ label: `RX ${index} 0-30 MHz`, url: `http://rx${index}.example.org:8073/`, type: 'OpenWebRX' }],
  }));
...
test('the middleware serves a merged catalog, caches it, and degrades when one directory fails', async () => {
  ...
  const middleware = createWebReceiversProxyMiddleware({ fetchImpl, lookupImpl: publicLookup, now: () => now });
  ...
  assert.equal(body.degraded, true, 'one failed directory marks the catalog degraded');
...
test('the middleware refuses redirects, serves stale after an outage, and 503s cold', async () => {
  ...
  const { fetchImpl } = createFetch({
    'https://www.receiverbook.de/map': () => (mode === 'ok'
      ? fakeResponse(bigReceiverbook(70))
      : new Response('', { status: 302, headers: { Location: 'https://evil.example/' } })),
    'http://rx.linkfanel.net/kiwisdr_com.js': () => (mode === 'ok' ? fakeResponse(KIWI_JS) : fakeResponse('', 500)),
  });
  const middleware = createWebReceiversProxyMiddleware({ fetchImpl, lookupImpl: publicLookup, now: () => now });
  ...
  const cold = createWebReceiversProxyMiddleware({ fetchImpl, lookupImpl: publicLookup, now: () => now });
  ...
  assert.match(failed.json().sources.receiverbook.error, /redirects are refused/);
});
...
test('the middleware rejects forbidden resolved addresses and unknown routes', async () => {
  ...
  const middleware = createWebReceiversProxyMiddleware({ fetchImpl, lookupImpl: privateLookup });
  const res = fakeRes();
  await middleware(fakeReq(), res);
  assert.equal(res.status, 503);
  assert.equal(calls.length, 0, 'no request leaves when DNS resolves to a private address');
