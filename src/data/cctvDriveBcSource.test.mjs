...
test('DriveBC loader keeps published cameras and builds frame URLs from the camera id', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (url) => {
    ...
    return Response.json([
      ...
      driveBcRow(682, 49.0579, -123.0569, { orientation: 'se', elevation: null }),
  ...
  assert.deepEqual(cameras.map((camera) => camera.id), ['drivebc-13', 'drivebc-682']);
  ...
  assert.equal(deltaport.headingDeg, 135, 'orientation codes are case-insensitive');
  assert.equal(deltaport.groundElevationM, 0, 'a missing elevation falls back to sea level');
});
...
test('DriveBC loader keeps the cameras nearest Vancouver and Victoria under its cap', async (t) => {
  ...
  const rows = [
    ...
    ...[58.8, 58.81, 58.82, 58.83].map((lat, i) => driveBcRow(100 + i, lat, -122.7)),
    ...[49.28, 49.27, 49.26, 49.25].map((lat, i) => driveBcRow(200 + i, lat, -123.12)),
    ...[48.43, 48.44, 48.45, 48.46].map((lat, i) => driveBcRow(300 + i, lat, -123.37)),
  ];
  ...
  assert.deepEqual(
    cameras.map((camera) => camera.id).filter((id) => /^drivebc-10\d$/.test(id)),
    [],
...
test('CCTV catalog merges DriveBC cameras and CCTV_DRIVEBC_ENABLED=0 skips the request', async (t) => {
  quiet(t);
  const sourceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'gev-cctv-catalog-'));
  ...
  assert.deepEqual(sources.map((source) => source.id), ['drivebc-13']);
  ...
  assert.equal(requested.includes(DRIVEBC_WEBCAMS_URL), false, 'a disabled pack makes no request');
});
...
test('default per-pack camera caps fit inside the default catalog cap', () => {
  ...
  assert.ok(
    ...
    `default packs (${packs}) would be truncated by the catalog cap (${DEFAULT_CCTV_MAX_SOURCES})`,
  );
  assert.ok(DEFAULT_CCTV_MAX_SOURCES <= 1200, 'the catalog clamps CCTV_MAX_SOURCES to 1200');
});
