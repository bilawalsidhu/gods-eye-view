...
import { createInfrastructureLayers } from 'ondemand-spatial-intelligence/infrastructure';
import { createLocalGeoJsonLayer } from 'ondemand-spatial-intelligence/infrastructure/geojson';
...
test('package exports import without an application, DOM, fetch, or timers', () => {
  const result = spawnSync(
    ...
    await import('ondemand-spatial-intelligence/infrastructure');
    await import('ondemand-spatial-intelligence/infrastructure/geojson');
    await import('ondemand-spatial-intelligence/infrastructure/lod');
  `,
    ],
...
test('consumer build includes only infrastructure code and resolves assets under a non-root base', async () => {
  ...
  const result = await build({
    ...
    build: {
      ...
      rollupOptions: {
        input: fileURLToPath(
          import.meta.resolve('ondemand-spatial-intelligence/infrastructure'),
        ),
        external: ['cesium'],
