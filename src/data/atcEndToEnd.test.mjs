...
test('ATC Radio End-to-End: full flight workflow approaching KAUS', async () => {
  ...
  // 1. Aircraft inbound to Austin (KAUS: 30.1945° N, 97.6699° W) at ~12 NM out, descending
  ...
  assert.equal(atcRadio.state.frequency, '118.800');

  // 2. Start audio playback with configured custom stream
  atcRadio.setCustomStreamUrl('https://streams.test/kaus.mp3');
  await atcRadio.play();
  assert.equal(atcRadio.state.playing, true);
  assert.equal(atcRadio.audio.audioElement.src, 'https://streams.test/kaus.mp3');
  ...
  assert.equal(atcRadio.state.frequency, '121.700');
  assert.equal(atcRadio.state.playing, false);
...
test('ATC Radio End-to-End: Card UI reactivity and manual controls', async () => {
  ...
  try {
    ...
    assert.ok(freqDisplay.textContent === '118.500');
    ...
    assert.equal(atcRadio.state.frequency, '113.750');

    // Configure custom stream URL and toggle audio via live button
    atcRadio.setCustomStreamUrl('https://streams.test/egll.mp3');
