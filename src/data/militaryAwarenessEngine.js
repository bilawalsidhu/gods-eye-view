...
export function summarizeAwarenessCohort(items, { available = true, stale = false, limit = 3 } = {}) {
  if (!available) return { relationship: AWARENESS_RELATIONSHIP.UNKNOWN, count: null, nearest: [], reason: t('layers.awareness.reason.feedUnavailable') };
  if (stale) return { relationship: AWARENESS_RELATIONSHIP.UNKNOWN, count: null, nearest: [], reason: t('layers.awareness.reason.feedStale') };
  ...
  return {
    ...
    reason: normalized.length ? t('layers.awareness.reason.nearby') : t('layers.awareness.reason.none'),
  };
}
