...
test('a follow-up frame is owed only while work spans frames', () => {
  ...
  assert.equal(
    detectionNeedsFollowUpFrame({
      ...base,
      nowMs: 1_000 + DETECTION_ENABLE_FADE_MS - 1,
    }),
    true,
  );
  assert.equal(
    detectionNeedsFollowUpFrame({
      ...base,
      nowMs: 1_000 + DETECTION_ENABLE_FADE_MS,
    }),
    false,
    'the chain must terminate exactly when the fade lands',
  );
  assert.equal(
    detectionNeedsFollowUpFrame({ ...base, nowMs: 9_999_999 }),
    false,
  );
...
test('a broken or backward clock terminates demand instead of extending it', () => {
  ...
  for (const nowMs of [undefined, null, NaN, Infinity]) {
    assert.equal(
      detectionNeedsFollowUpFrame({ active: true, nowMs, enabledAtMs: 0 }),
      false,
    );
  }
  for (const enabledAtMs of [undefined, null, NaN, Infinity]) {
    assert.equal(
      detectionNeedsFollowUpFrame({ active: true, nowMs: 1_000, enabledAtMs }),
      false,
    );
  }
  assert.equal(
    detectionNeedsFollowUpFrame(),
    false,
    'no argument at all is still not an animation',
  );
...
test('a label fading IN is animating, not just one fading out', () => {
  ...
  assert.equal(
    renderEntryIsAnimating({ selected: true, temporalAlpha: 0.01 }),
    true,
    'a label that has just been selected is still climbing',
  );
  assert.equal(
    renderEntryIsAnimating({ selected: true, temporalAlpha: 0.5 }),
    true,
  );
  assert.equal(
    renderEntryIsAnimating({ selected: true, temporalAlpha: 1 }),
    false,
  ...
  assert.equal(
    renderEntryIsAnimating({ selected: false, temporalAlpha: 1 }),
    true,
  );
  assert.equal(
    renderEntryIsAnimating({ selected: false, temporalAlpha: 0.2 }),
    true,
  );
...
test('a skipped paint always hands its frame forward — skip and re-request are one decision', () => {
  ...
  const skipped = detectionPaintSkipDecision({
    layoutChanged: false,
    lastPaintMs: heavy,
    frameCount: 1,
  });
  assert.deepEqual(
    skipped,
    { skip: true, requestFollowUp: true },
  ...
  for (const input of [
    ...
    {
      layoutChanged: false,
      lastPaintMs: DETECTION_PAINT_SKIP_THRESHOLD_MS,
      frameCount: 1,
    }, // at the threshold, not past it
  ]) {
    assert.deepEqual(
      detectionPaintSkipDecision(input),
      { skip: false, requestFollowUp: false },
  ...
  for (const layoutChanged of [true, false]) {
    for (const lastPaintMs of [0, 1, 22, 23, 35, 500, NaN]) {
      for (const frameCount of [0, 1, 2, 3, 41, 100]) {
        const d = detectionPaintSkipDecision({
          layoutChanged,
          lastPaintMs,
          frameCount,
        });
  ...
  const first = detectionPaintSkipDecision({
    layoutChanged: false,
    lastPaintMs: heavy,
    frameCount: 7,
  });
  const second = detectionPaintSkipDecision({
    layoutChanged: false,
    lastPaintMs: heavy,
    frameCount: 8,
  });
  ...
  assert.equal(
    second.skip,
    false,
    'the very next frame paints — a skip costs one frame, not the work',
  );
...
test('the scanline scrolls off the clock, at an exact 60 fps cadence', () => {
  ...
  const walk = Array.from({ length: 12 }, (_, i) =>
    scanlineOffsetPx(SCANLINE_STEP_MS * i + 0.001),
  );
  for (let i = 1; i < walk.length; i++) {
    assert.notEqual(
      walk[i],
      walk[i - 1],
      `frame ${i} repeated the previous offset`,
    );
  }
  ...
  for (const t of [1, 1_000, 1_700_000_000_000, 1_700_000_000_016]) {
    ...
    assert.ok(
      offset >= 0 && offset < SCANLINE_PERIOD_PX,
      `offset ${offset} outside the period at t=${t}`,
    );
  }
  ...
  assert.equal(
    scanlineOffsetPx(stepStart),
    scanlineOffsetPx(stepStart + SCANLINE_STEP_MS - 1),
  );
  assert.equal(
    scanlineOffsetPx(Number.NaN),
    0,
    'a broken clock paints a stable pattern, not a wandering one',
  );
});
  ...
  for (const age of [
    ...
    if (alpha < 1) {
      assert.equal(
        owed,
        true,
        `age ${age}: painted alpha ${alpha} but no successor frame was owed`,
      );
    } else {
      assert.equal(
        owed,
        false,
        `age ${age}: alpha has settled, demand must end on this frame`,
      );
    }
  }
});
...
>>>>>>> 4c1dbe653b2589e5068a1c10e052d5d24249be77
  ...
  const emptyExit =
    /\/\/ Drop the replay buffer with it[\s\S]*?return \{\s*didSolve: false[\s\S]*?\};/.exec(
      source,
    )?.[0];
  ...
  assert.match(
    drawOverlay,
    /const now =\s*Number\.isFinite\(frame\.timestamp\) \? frame\.timestamp : _nowMs\(\);/,
  );
  assert.match(
    source,
    /_enableTime = _nowMs\(\);/,
  ...
  assert.match(
    shouldPaint,
    /if \(decision\.requestFollowUp\)\s*governorRequestRender\('detection-paint-skipped'\)/,
...
test('a layer whose detectable set changed dirties the solve', async () => {
  ...
  assert.match(
    detection,
    /export function markDetectionSourcesChanged\(/,
  ...
  assert.match(
    ...
    'and it really dirties the solve',
  );
  assert.doesNotMatch(
    hook,
  ...
  assert.deepEqual(reactions, [
    ['render', 'layer-tick:fixture'],
    ['detection', 'layer-tick:fixture'],
  ]);
  reactions.length = 0;
  result = false;
  await manager.refreshLayer('fixture');
  assert.deepEqual(
    ...
    'partial/rejected updates still invalidate detection',
  );
  reactions.length = 0;
  ...
  assert.deepEqual(
    reactions,
    [],
    'throwing updates do not publish changed data',
  );
  result = true;
...
test('the render-governor gate covers the parked case, with teeth on the painter', async () => {
  ...
  const gate = await readFile(
    new URL('../../scripts/qa-perf.mjs', import.meta.url),
    'utf8',
  );
  ...
  assert.match(
    gate,
    /detection ON still repaints promptly on camera motion/,
    'quiet must not be allowed to mean stale',
  );
  assert.match(
    gate,
    /detection is still PAINTING, not merely quiet/,
  ...
  assert.match(
    gate,
    /QUIET_RUN_WINDOWS = Math\.ceil\(HUD_SUMMARY_INTERVAL_MS \/ 1_000\) \+ 1/,
