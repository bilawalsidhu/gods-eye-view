...
test('transit brackets paint a 1.25 px mode stroke over 3.25 px dark backing', async () => {
  ...
  const ctx = {
    stroke(path) {
      strokes.push([path, this.lineWidth, this.strokeStyle, this.globalAlpha]);
    },
  };
  const path = {};
  paintTransitBracket(ctx, path, '#FF4538', 0.75);
  assert.deepEqual(strokes, [
    [path, 3.25, '#05080C', 0.75],
    [path, 1.25, '#FF4538', 0.75],
  ]);
});
...
test('transit bracket backing and colour use source-over under every inherited sensor theme', async () => {
  ...
  for (const theme of ['normal', 'thermal', 'surveillance', 'noir', 'retro']) {
    ...
    const ctx = {
      ...
      save() {
        this.saved = [this.globalCompositeOperation, this.globalAlpha];
      },
      restore() {
        [this.globalCompositeOperation, this.globalAlpha] = this.saved;
      },
    };
    ...
    assert.deepEqual(
      strokes,
      [
        ['source-over', 3.25, '#05080C'],
        ['source-over', 1.25, '#FF4538'],
      ],
      theme,
    );
