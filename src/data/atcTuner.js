...
import { getLiveAtcUrl, getCustomStreamUrl } from './atcCustomStreams.js';
...
export function evaluateFlightPhase({
  ...
  const effectiveSpeedKts =
    groundSpeedKts > 0
      ? groundSpeedKts
      : Number.isFinite(velocityMps)
        ? velocityMps * 1.943844
        : 0;
...
export function resolveAtcTune({
  ...
  if (
    ...
    if (manualAirportIcao) {
      ...
      if (manualAirport) {
        ...
        const frequencyMHz =
          manualAirport.frequencies?.[freqType] ||
          manualAirport.frequencies?.tower ||
          null;
        const liveAtcUrl = getLiveAtcUrl(manualAirport.icao);
        const resolvedStreamUrl =
          customStreamUrl ||
          getCustomStreamUrl(manualAirport.icao, storage) ||
          null;
        return {
          tuned: true,
          status: 'monitoring',
          ...
          hasCustomStream: Boolean(resolvedStreamUrl),
          inRange: true,
          callsign: null,
        };
      }
  ...
  if (manualAirportIcao) {
    ...
    if (targetAirport) {
      const match = findNearestAirport(
        aircraft.lat,
        aircraft.lon,
  ...
  const frequencyMHz =
    targetAirport.frequencies?.[selectedFreqType] ||
    targetAirport.frequencies?.tower ||
    null;
  const liveAtcUrl = getLiveAtcUrl(targetAirport.icao);
  const resolvedStreamUrl =
    customStreamUrl || getCustomStreamUrl(targetAirport.icao, storage) || null;

  return {
    tuned: inRange || Boolean(manualAirportIcao),
    status: inRange
      ? phase === 'approach' || phase === 'surface'
        ? 'active'
        : 'monitoring'
      : 'squelch',
    ...
    frequencyMHz,
    liveAtcUrl,
    streamUrl: inRange || Boolean(manualAirportIcao) ? resolvedStreamUrl : null,
    hasCustomStream: Boolean(resolvedStreamUrl),
    inRange,
    callsign: aircraft.callsign || null,
  };
