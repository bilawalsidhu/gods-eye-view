...
test('atcTuner: resolveAtcTune honors manual airport and frequency overrides', () => {
  ...
  assert.equal(manual.frequencyMHz, manual.airport.frequencies.tower);
  assert.equal(manual.freqType, 'tower');
  assert.equal(manual.liveAtcUrl, 'https://www.liveatc.net/search/?icao=klax');
  assert.equal(manual.hasCustomStream, false);
});

test('atcTuner: resolveAtcTune resolves custom stream URL when configured', () => {
  const aircraft = {
    lat: 30.10,
    lon: -97.65,
    altitudeM: 900,
  ...
  const mockStorage = {
    getItem(k) {
      if (k === 'gev_atc_custom_stream_KAUS') return 'https://streams.example.com/kaus.mp3';
      return null;
    },
  };

  const tune = resolveAtcTune({
    aircraft,
    storage: mockStorage,
  });

  assert.equal(tune.airport.icao, 'KAUS');
  assert.equal(tune.hasCustomStream, true);
  assert.equal(tune.streamUrl, 'https://streams.example.com/kaus.mp3');
  assert.equal(tune.liveAtcUrl, 'https://www.liveatc.net/search/?icao=kaus');
});
