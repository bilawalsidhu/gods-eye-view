...
test('CCTV upstream frame fetch supplies a bounded abort signal', async () => {
  ...
  assert.ok(
    ...
    'test timeout should settle promptly',
  );
  assert.ok(
    CCTV_FRAME_FETCH_TIMEOUT_MS < 10_000,
    'production timeout must beat the active refresh cadence',
  );
});
...
test('CCTV upstream frame fetch rejects a declared oversize body without draining it', async () => {
  ...
  assert.equal(
    result?.body?.length ?? null,
    null,
    'a declared oversize snapshot is a miss, not a buffered body',
  );
  assert.equal(
    cancelled,
    true,
    'the declared cap is enforced by cancelling, not reading',
  );
  // Only the stream's own one-chunk prefetch may have run; the proxy pulls none.
  assert.ok(
    pulled <= 1,
    `declared cap short-circuits the read, pulled ${pulled} chunks`,
  );
});
...
test('CCTV upstream frame fetch aborts an undeclared body once it crosses the cap', async () => {
  ...
  const result = await fetchCctvImageFromUpstream(
    'https://example.com/frame.jpg',
    {
      ...
      fetchImpl: async () =>
        chunkedImageResponse(chunkBytes, 64, {
          onChunk: () => {
            pulled += 1;
          },
          onCancel: () => {
  ...
  assert.equal(
    result?.body?.length ?? null,
    null,
    'a chunked body over the cap is a miss',
  );
  ...
  assert.equal(
    cancelled,
    true,
    'the upstream stream is cancelled, not drained',
  );
});
...
test('CCTV upstream frame fetch still returns a chunked body under the cap', async () => {
  ...
  assert.equal(
    result?.body?.length,
    chunkBytes * 3,
    'every chunk is reassembled in order',
  );
  assert.ok(
    CCTV_FRAME_MAX_BODY_BYTES > 0 &&
      CCTV_FRAME_MAX_BODY_BYTES <= CCTV_MEDIA_MAX_BODY_BYTES,
...
test('CCTV media upstream fetch aborts when response headers never arrive', async () => {
  ...
  assert.equal(
    ...
    true,
    'the header deadline aborts the attempt',
  );
  assert.ok(
    Date.now() - startedAt < 2_000,
    'the deadline is the injected one, not the production one',
  );
  assert.ok(
    CCTV_MEDIA_FETCH_TIMEOUT_MS >= 10_000,
    'production keeps a generous header deadline for slow cameras',
  );
});

test('CCTV media upstream fetch never cuts a stream whose headers arrived in time', async () => {
  let observedSignal = null;
  const upstream = await fetchCctvMediaUpstream(
  ...
  assert.equal(
    observedSignal?.aborted,
    false,
    'a slow body after timely headers is never aborted here',
  );
});
...
test('CCTV upstream frame fetch keeps a body that lands exactly on the cap', async () => {
  ...
  assert.equal(
    result?.ok,
    true,
    'exactly-at-cap is under the ceiling, not over it',
  );
...
test('CCTV upstream frame fetch ignores a garbage Content-Length and still caps the stream', async () => {
  ...
  const result = await fetchCctvImageFromUpstream(
    'https://example.com/frame.jpg',
    {
      ...
      fetchImpl: async () => {
        const response = chunkedImageResponse(chunkBytes, 8, {
          onCancel: () => {
            cancelled = true;
          },
        });
  ...
  assert.equal(
    result?.body?.length ?? null,
    null,
    'an unparseable length falls back to counting bytes',
  );
...
test("CCTV upstream frame fetch retains owned bytes, not the chunk's backing allocation", async () => {
  ...
  assert.ok(
    result.body.buffer.byteLength < backing.byteLength,
  ...
  const result = await fetchCctvImageFromUpstream(
    'https://example.com/frame.jpg',
    {
      ...
      fetchImpl: async () => ({
        ...
        arrayBuffer: async () => {
          throw new Error(
            'arrayBuffer must never be used as an uncapped fallback',
          );
        },
      }),
    },
