...
test('overview and evidence paths start at the lake reach, never the earlier trigger reach', () => {
  ...
  assert.equal(
    BHOTE_KOSHI_FLOOD_PATH.some(
      ([lon, lat]) => lon === 85.5306 && lat === 28.2916,
    ),
    false,
  );
  ...
  assert.deepEqual(
    BHOTE_KOSHI_FLOOD_PATH.slice(0, paths.get('debris-dammed-lake').length),
...
test('ten shot paths use only the existing centreline and join at shared endpoints', () => {
  ...
  const vertices = new Set(
    BHOTE_KOSHI_FLOOD_PATH.map(([lon, lat]) => `${lon},${lat}`),
  );
  ...
  for (const ids of [
    ...
    for (let index = 1; index < ids.length; index++) {
      assert.deepEqual(
        coordinate(paths.get(ids[index - 1]).at(-1)),
        coordinate(paths.get(ids[index])[0]),
      );
    }
  }
  assert.deepEqual(
    coordinate(paths.get('bidur-trishuli-bridge').at(-1)),
    BHOTE_KOSHI_FLOOD_PATH.at(-1),
  );
  for (const id of [
    ...
    assert.equal(
      paths.has(id),
      false,
      'out-of-coverage media must not gain a fabricated river route',
    );
  }
});
...
test('Dhunche starts at the exact comparison endpoint without stepping upstream first', () => {
  ...
  assert.ok(
    path[1].lat < boundary.lat,
    'the first new vertex is downstream of the comparison endpoint',
  );
});
