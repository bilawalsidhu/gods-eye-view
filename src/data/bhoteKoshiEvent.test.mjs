...
function eventViewer(splitPosition = 0.37) {
  ...
  return {
    ...
    dataSources: {
      ...
      remove(source) {
        removedDataSources.push(source);
      },
    },
    _test: {
      imagery,
    ...
    for (const initialMap of ['photoreal', 'esri-imagery']) {
      for (const origin of ['scene', 'local-restore', 'share-restore']) {
        ...
        viewer.camera.flyToBoundingSphere = () =>
          viewer._test.cameraLooks.push('corridor');
        ...
        const layer = createBhoteKoshiEventLayer({
          ...
          terrainSampler: async (_viewer, observations) =>
            observations.map(() => ({ height: 800 })),
          mediaLoader: async () => {
            const error = new Error('No bundled clip');
        ...
        manager.finalizeRegistrations([
          { id: layer.id, disposition: 'enabled-only' },
        ]);
        try {
          ...
          assert.deepEqual(
            mapCalls,
            [],
            `${initialMap}/${origin}: preserve the shot map`,
          );
          ...
          for (const id of [
            ...
            assert.equal(
              line.clampToGround.getValue(),
              true,
              'dynamic trail follows the rendered surface',
            );
            assert.equal(
              line.classificationType.getValue(),
              initialMap === 'photoreal'
                ? Cesium.ClassificationType.BOTH
                : Cesium.ClassificationType.TERRAIN,
            );
            assert.equal(
              line.depthFailMaterial,
              undefined,
              'do not rely on unsupported dynamic depth-fail rendering',
            );
          }
          assert.equal(
            floodSource.entities
              .getById('bhote-koshi-surge-front')
              .point.heightReference.getValue(),
            Cesium.HeightReference.CLAMP_TO_GROUND,
          );
          assert.deepEqual(
            viewer._test.cameraLooks,
            [],
            'enable must not focus the corridor',
          );
          assert.deepEqual(
            timers,
            [],
            'enable must not schedule independent playback',
          );
          assert.equal(layer.getPlaybackState().playing, false);
          assert.equal(
            await manager.setEnabled(layer.id, false, { origin: 'user' }),
            true,
          );
          assert.deepEqual(
            mapCalls,
            [],
            'disable must not restore a map it never owned',
          );
        } finally {
...
test('trimmed scene media holds through provider startup, playback and fade, then releases the camera', async () => {
  ...
  try {
    ...
    fire(100);
    assert.equal(
      layer.getSceneShotMediaHold('mailung-bazzar').pending,
      true,
      'startup is not treated as playback',
    );
    phase = 'playing';
    fire(100);
    assert.equal(layer.getSceneShotMediaHold('mailung-bazzar').pending, true);
...
test('Pinokio source-card fallback selects authored Director dwell, not an impossible player hold', async () => {
  ...
  const layer = createBhoteKoshiEventLayer({
    ...
    terrainSampler: async (_viewer, observations) =>
      observations.map(() => ({ height: 800 })),
    mediaLoader: async () => {
      throw Object.assign(new Error('No bundled clip'), { name: 'AbortError' });
    },
    overlayHost: { setEntries() {}, setVisible() {}, clearSource() {} },
    renderHost: { request() {}, hold() {}, release() {} },
    embeddedMediaFactory: (options) =>
      createBhoteKoshiEmbeddedMedia({
        ...options,
        globalRef: { navigator: { userAgent: 'Chrome Pinokio/8.0.40' } },
        documentRef: {
          createElement() {
            assert.fail('unsupported host must allocate no provider DOM');
          },
        },
      }),
  });
  ...
  try {
    ...
    assert.deepEqual(
      waits,
      [7650],
      'use the authored dwell instead of completing instantly or polling 20 seconds',
    );
    event.evidenceSpine.find(
      (item) => item.id === 'mailung-bazzar',
    ).media.videoPath = 'fixture-approved.mp4';
    assert.equal(
      layer.getSceneShotMediaHold('mailung-bazzar').pending,
      true,
...
test('saved autoplay controls cannot authorize media; live shot ownership is cancellable and never serialized', async () => {
  ...
  try {
    ...
    layer.setSceneMediaPlayback({
      sceneId: 'scene',
      shotId: 'shot',
      token: { cancelled: false, signal: abort.signal },
    });
    layer.setParams(params, { origin: 'scene' });
    ...
    assert.deepEqual(
      layer.getParams(),
      saved,
      'live playback authority must not enter saved params',
    );
    ...
    assert.equal(
      calls.at(-1),
      false,
      'late callbacks and restored flags cannot restart revoked playback',
    );
    layer.setSceneMediaPlayback({
      sceneId: 'scene',
    ...
    assert.equal(
      calls.at(-1),
      false,
      'a different shot cannot borrow this playback owner',
    );
  } finally {
    await layer.destroy();
    Object.assign(globalThis, previous);
...
test('Bhote Koshi flood corridor grows continuously to the animated surge head', () => {
  ...
  assert.equal(
    updateCorridorPositionCache(0.201, terrainPositions, cache),
    true,
  );
  ...
  assert.equal(
    updateCorridorPositionCache(0.299, terrainPositions, cache),
    true,
  );
  assert.notEqual(
    cache.positions,
    firstPositions,
    'the trail advances within the segment',
  );
  ...
  assert.equal(
    cache.positions,
    repeatedPositions,
    'an unchanged frame reuses its positions',
  );

  assert.equal(
    updateCorridorPositionCache(0.301, terrainPositions, cache),
    true,
  );
...
test('Bhote Koshi card summaries wrap on words instead of clipping mid-sentence', () => {
  ...
  assert.deepEqual(evidenceSummaryLines('one two three four five', 9, 1), [
    'one two…',
  ]);
});
...
test('Bhote Koshi evidence timeline is ordered by phase and corridor chainage with one active beat', async () => {
  ...
  for (let index = 0; index < timeline.length; index += 1) {
    assert.equal(
      activeEvidenceIndex(timeline[index].activationProgress, timeline),
      index,
    );
    const midpoint =
      (timeline[index].activationProgress +
        Math.min(1, timeline[index].deactivationProgress)) /
      2;
    ...
    assert.ok(
      dwellSeconds >= 2,
      `${timeline[index].observation.id} dwell is too short`,
    );
  }

  const anchors = timeline.map((_, index) => ({
    x: index,
    y: index + 1,
    z: index + 2,
  }));
  ...
  assert.deepEqual(
    cameraFrames.slice(0, 16).map((frame) => frame.id),
    timeline.map((beat) => beat.observation.id),
  );
  assert.deepEqual(
    cameraFrames.slice(0, 16).map((frame) => frame.progress),
    timeline.map((beat) => beat.activationProgress),
  );
...
test('Bhote Koshi scene beats resolve stable ids inside their reveal windows', async () => {
  ...
  for (const beat of timeline) {
    ...
    assert.ok(
      progress > beat.activationProgress ||
        beat.deactivationProgress <= beat.activationProgress,
    );
    assert.ok(progress < Math.min(1.000001, beat.deactivationProgress));
    assert.equal(
      activeEvidenceIndex(progress, timeline),
      timeline.indexOf(beat),
    );
  }
...
test('Bhote Koshi evidence presentation reveals dot, leader, and card then tears down in reverse', () => {
  ...
  assert.equal(
    receding.alpha,
    1,
    'the terrain dot remains after card recession starts',
  );
...
test('Bhote Koshi beat navigation is deterministic in both directions and clamps at its ends', async () => {
  ...
  assert.equal(
    adjacentEvidenceIndex(timeline[3].activationProgress, timeline, -1),
    2,
  );
  assert.equal(
    adjacentEvidenceIndex(timeline[3].activationProgress, timeline, 1),
...
test('Bhote Koshi evidence fallback stays media-first and source-linked without card copy overload', async () => {
  const event = JSON.parse(await readFile(eventUrl, 'utf8'));
  const timure = event.evidenceSpine.find(
    (observation) => observation.id === 'timure-cluster',
  );
  ...
  globalThis.document = {
    createElement() {
      ...
      const context = {
        ...
        drawImage() {
          throw new Error('fixture poster decode failure');
        },
      };
  ...
  try {
    ...
    const failedPosterFrame = createEvidenceFrame(timure, {
      width: 640,
      height: 400,
    });
    ...
    for (const context of contexts) {
      ...
      assert.doesNotMatch(
        context.text.join(' '),
        /CAPTURE TIME|GEOLOCATED|public viewpoints/,
      );
    }
...
test('Bhote Koshi evidence compositor contains portrait video from its decoded dimensions', async () => {
  ...
  assert.equal(
    updateEvidenceFrame(canvas, observation, video, { videoActive: true }),
    true,
  );
  ...
  assert.ok(
    destX > 0,
    'portrait video keeps narrow side gutters instead of being cropped',
  );
  ...
  assert.equal(
    context.textAlign,
    'left',
    'repeat paints cannot inherit the outside badge alignment',
  );
  assert.equal(
    updateEvidenceFrame(canvas, observation, video, { videoActive: true }),
    true,
...
test('Bhote Koshi failed direct enable restores the prior map and shared split', async () => {
  ...
  const layer = createBhoteKoshiEventLayer({
    eventLoader: async () => event,
    imageryProviderFactory: async () => {
      throw new Error('fixture image failure');
    },
    overlayHost: {
      setEntries() {},
      setVisible(id, visible) {
        overlayCalls.push(['visible', id, visible]);
      },
      clearSource(id) {
        overlayCalls.push(['clear', id]);
      },
    },
    renderHost: { request() {}, hold() {}, release() {} },
  });
...
test('Bhote Koshi disable stops active playback and releases its global render hold', async () => {
  ...
  try {
    ...
    assert.equal(
      panel.querySelector('[data-action="play-scene"]').hidden,
      true,
    );
    assert.equal(
      await panel.querySelector('[data-action="play-scene"]').dispatch('click'),
      false,
    );
...
test('Bhote Koshi passive restores do not start off-screen playback', async () => {
  ...
  try {
    assert.equal(
      await manager.setEnabled(layer.id, true, { origin: 'local-restore' }),
      true,
    );
  ...
  const layer = createBhoteKoshiEventLayer({
    ...
    terrainSampler: async (_viewer, observations) =>
      observations.map((observation) => ({
        height: observation.fallbackElevationM,
      })),
    mediaLoader: async () => {
...
test('Bhote Koshi scene beats preserve photoreal and never take camera ownership', async () => {
  ...
  try {
    ...
    assert.deepEqual(timers, []);
    assert.equal(
      viewer._test.imagery.length,
      2,
      'the event surface keeps its cached imagery pair',
    );
    assert.ok(
      viewer._test.imagery.every((imageryLayer) => imageryLayer.show === false),
    );
    const panel = body.children.find((element) => element.tagName === 'aside');
    assert.ok(panel);
    const splitLine = body.children.find(
      (element) => element.id === 'bhote-koshi-split-line',
    );
    assert.ok(splitLine);
    assert.equal(splitLine.hidden, true);
    assert.equal(
      panel.querySelector('[data-role="imagery-comparison"]').hidden,
      true,
    );
    assert.match(
      panel.querySelector('[data-role="caveat"]').textContent,
      /SCHEMATIC CORRIDOR/i,
    );
    assert.doesNotMatch(
      panel.querySelector('[data-role="caveat"]').textContent,
      /historical/i,
    ...
    assert.equal(
      panel.querySelector('[data-role="imagery-cloud-note"]').hidden,
      true,
    );
    ...
    assert.equal(
      sceneClock.getAttribute('aria-label'),
      'Seek Nepal scene clock',
    );
    assert.equal(
      panel.querySelector('[data-role="timeline-label"]').textContent,
      'NEPAL SCENE CLOCK',
    );
    assert.equal(
      panel.querySelector('[data-role="time"]').textContent,
      '~0:00 / 2:00',
    );
    publishSceneClock({
      sceneId: 'nepal-scene',
    ...
    assert.equal(
      panel.querySelector('[data-role="beat-index"]').textContent,
      '01 / 14',
    );
    assert.equal(
      panel.querySelector('[data-role="beat-title"]').textContent,
      'Global Incident Context',
    );
    assert.equal(
      panel.querySelector('[data-action="play"]').textContent,
      '▶ PLAY SHOT',
    );
    ...
    assert.equal(
      playScene.getAttribute('aria-label'),
      'Continue Nepal Flood Incident from next shot',
    );
    assert.equal(panel.querySelector('[data-action="cinematic"]').hidden, true);
    assert.equal(
      panel.querySelector('[data-action="cinematic"]').disabled,
      true,
    );
    assert.equal(panel.querySelector('[data-action="corridor"]').hidden, false);
    assert.equal(
      panel.querySelector('[data-action="corridor"]').disabled,
      false,
    );
    ...
    assert.equal(
      fullStory.hidden,
      true,
      'PLAY SCENE owns authored scene playback',
    );
    assert.equal(fullStory.disabled, true);
    assert.equal(
      panel.querySelector('[data-action="open-source"]').hidden,
      true,
    );
    ...
    assert.ok(
      viewer._test.imagery.every((imageryLayer) => imageryLayer.show === true),
    );
    ...
    for (const id of [
      ...
      assert.equal(
        viewer._test.dataSources[0].entities.getById(id).show,
        false,
      );
    }
    panel.querySelector('[data-action="play"]').dispatch('click');
    ...
    assert.equal(
      sceneCalls.length,
      3,
      'duplicate clicks cannot start another scene run',
    );
    ...
    assert.deepEqual(await playScene.dispatch('click'), {
      started: false,
      reason: 'already-running',
    });
    assert.equal(
      playScene.disabled,
      false,
      'a refused start releases the button',
    );
    layer.attachSceneController({
      continueScene: async () => {
        throw new Error('test scene failure');
      },
    });
    ...
    try {
      ...
      assert.equal(
        playScene.disabled,
        false,
        'a rejected start releases the button',
      );
    ...
    assert.equal(sceneCalls.length, 4);
    assert.equal(layer.getPlaybackState().playing, false);
    for (const reportButton of panel.querySelector('.bhote-event-report-list')
      .children) {
    ...
    assert.deepEqual(mapCalls, []);
    assert.equal(
      body.children.filter((element) => element.tagName === 'aside').length,
      1,
    );
    assert.equal(viewer.scene.splitPosition, 0.62);
    for (const beatId of [
      ...
      assert.equal(
        viewer._test.dataSources.filter(
          (source) => !viewer._test.removedDataSources.includes(source),
        ).length,
        1,
        'surface transitions leave exactly one active event surface',
      );
      assert.equal(
        viewer._test.cameraLooks.length,
        0,
        'consecutive beats never write the camera',
      );
    }
    panel.querySelector('[data-action="open-source"]').dispatch('click');
    assert.equal(
      openedUrls.at(-1),
      event.evidenceSpine.find(({ id }) => id === 'bidur-trishuli-bridge').media
        .sourceUrl,
    );
    layer.setParams(
      {
    ...
    assert.deepEqual(mapCalls, []);
    assert.equal(
      viewer._test.removedDataSources.length,
      viewer._test.dataSources.length,
    );
  } finally {
    globalThis.document = originalDocument;
    globalThis.window = originalWindow;
...
test('Nepal hybrid comparison preserves native split and terrain-clamped flood trail', async () => {
  ...
  layer.attachMapStackController({
    ...
    async setTerrainComparison(rectangle) {
      comparisonCalls.push(rectangle);
      return true;
    },
    clearTerrainComparison() {
      comparisonCalls.push(null);
    },
  });
  ...
  try {
    ...
    assert.deepEqual(
      mapCalls,
      [],
      'comparison does not switch the authored Google base',
    );
    ...
    assert.equal(
      viewer._test.imagery[0].splitDirection,
      Cesium.SplitDirection.LEFT,
    );
    assert.equal(
      viewer._test.imagery[1].splitDirection,
      Cesium.SplitDirection.RIGHT,
    );
    layer.setParams({ split: 0.25 });
    assert.equal(viewer.scene.splitPosition, 0.25);
    const flood = viewer._test.dataSources[0].entities.getById(
      'bhote-koshi-flood-core',
    );
    const surge = viewer._test.dataSources[0].entities.getById(
      'bhote-koshi-surge-front',
    );
    ...
    const tileFlood = viewer._test.dataSources
      .at(-1)
      .entities.getById('bhote-koshi-flood-core');
    assert.equal(tileFlood.polyline.clampToGround.getValue(), true);
    assert.equal(
      tileFlood.polyline.classificationType.getValue(),
      Cesium.ClassificationType.BOTH,
    );
...
test('Bhote Koshi event pack keeps observations separate from reconstruction', async () => {
  ...
  assert.equal(
    event.fieldReports.filter((report) =>
      report.status.includes('WORLD-PINNED'),
    ).length,
    1,
  );
  assert.ok(
    event.fieldReports.some((report) => report.status.includes('UNVERIFIED')),
  );
  assert.equal(event.evidenceSpine.length, 16);
  assert.equal(new Set(event.evidenceSpine.map(({ id }) => id)).size, 16);
  assert.deepEqual(
    event.evidenceSpine.map(({ sequence }) => sequence),
    Array.from({ length: 16 }, (_, index) => index + 1),
  );
  assert.equal(
    floodProgressForStoryProgress(0.195, event.reconstruction.storyFloodWindow),
    0,
  );
  assert.equal(
    floodProgressForStoryProgress(0.39, event.reconstruction.storyFloodWindow),
    1,
  );
  assert.equal(
    floodProgressForStoryProgress(0.9, event.reconstruction.storyFloodWindow),
    1,
  );
  ...
  assert.ok(
    event.evidenceSpine.every(
      (observation) => observation.timing.capturedAt === null,
    ),
  );
  assert.ok(
    event.evidenceSpine.every(
      (observation) => observation.timing.note === 'capture time unverified',
    ),
  );
  assert.ok(
    event.evidenceSpine.every((observation) =>
      /^https:\/\//.test(observation.media.sourceUrl),
    ),
  );
  assert.ok(
    event.evidenceSpine
      .filter((observation) => observation.media.posterUrl)
      .every((observation) =>
        observation.media.posterUrl.startsWith('https://i.ytimg.com/'),
      ),
  );
  assert.equal(
    event.evidenceSpine.find(({ id }) => id === 'gyirong-border-gate').media
      .clipInSec,
    26,
  );
  assert.ok(
    event.evidenceSpine.every(
      ({ media }) => !media.videoPath && !media.posterPath,
    ),
    'Public event records must not reference privately cached social clips',
  );
  assert.equal(
    event.evidenceSpine.filter(
      (observation) => observation.imageryCoverage === 'outside',
    ).length,
    13,
  );
  const timure = event.evidenceSpine.find(
  ...
  assert.equal(
    evidenceCorroborationLabel(timure),
    '5 GEOLOCATED SOURCE-MAP PLACEMENTS',
  );
  assert.match(
    event.reconstruction.caveat,
    /2021 historical reference versus 2026 post-event/i,
  );
  ...
  assert.equal(
    SCENE_RECIPES.some((recipe) => recipe.id === 'bhote-koshi-flood'),
    false,
  );
  assert.ok(
    SCENE_RECIPES.some((recipe) => recipe.id === 'bhote-koshi-nepal-scene'),
  );
});
...
test('Bhote Koshi publishes the evidence spine once and scrubs through cards and breadcrumbs', async () => {
  ...
  const layer = createBhoteKoshiEventLayer({
    ...
    overlayHost: {
      setEntries(id, entries, options) {
        publications.push({ id, entries, options });
      },
      setVisible() {},
      clearSource() {},
    },
  ...
  try {
    ...
    const cards = publications[0].entries.filter(
      (entry) => entry.variant === 'thumbnail',
    );
    const breadcrumbs = publications[0].entries.filter(
      (entry) => entry.variant === 'label',
    );
    assert.equal(cards.filter((entry) => entry.position()).length, 1);
    assert.match(
      cards.find((entry) => entry.position()).id,
      /immediate-collapse/,
    );
    assert.equal(breadcrumbs.filter((entry) => entry.position()).length, 0);
    assert.ok(
      cards.every((entry) => entry.requireImage && entry.image.stamp === 1),
    );
...
test('Bhote Koshi keeps one local fallback video active and unloads it on beat change and disable', async () => {
  ...
  event.evidenceSpine.find(
    ({ id }) => id === 'immediate-collapse-viewpoint',
  ).media.videoPath = 'evidence-01.mp4';
  event.evidenceSpine.find(
    ({ id }) => id === 'timure-cluster',
  ).media.videoPath = 'evidence-04.mp4';
  event.evidenceSpine.find(
    ({ id }) => id === 'immediate-collapse-viewpoint',
  ).media.sourceUrl = 'https://example.com/immediate-collapse';
  event.evidenceSpine.find(
    ({ id }) => id === 'timure-cluster',
  ).media.sourceUrl = 'https://example.com/timure';
  const originalDocument = globalThis.document;
  const originalWindow = globalThis.window;
  const { body } = installEventDom();
  globalThis.window = { setTimeout: () => 71, clearTimeout() {} };
  ...
  try {
    ...
    assert.equal(
      videos.length,
      2,
      'the new beat replaces rather than stacks a decoder',
    );
...
test('Upper Valley Collapse autoplays its muted local fallback without starting the event clock', async () => {
  ...
  try {
    ...
    video.drawFrame();
    assert.ok(
      frameUpdates > updatesBeforeFrame,
      'decoded frames repaint the anchored callout',
    );

    layer.setParams(
      { presentation: 'scene-beat', progress: 0.2 },
      { origin: 'scene' },
    );
    ...
    assert.equal(layer.getCinematicState().mediaEvidenceId, null);
    assert.equal(
      body.children.find((element) => element.tagName === 'aside')?.removed,
      false,
    );
    await layer.disable();
  } finally {
  ...
  const layer = createBhoteKoshiEventLayer({
    ...
    overlayHost: {
      setEntries(_id, entries) {
        activeCard = entries.find(
          (entry) => entry.id === 'evidence-card-debris-dammed-lake',
        );
      },
      setVisible() {},
      clearSource() {},
  ...
  try {
    ...
    try {
      ...
      assert.equal(
        opened.length,
        0,
        'drawing owns the pointer over evidence cards',
      );
    } finally {
    ...
    assert.deepEqual(opened, [
      [event.evidenceSpine[1].media.sourceUrl, '_blank', 'noopener,noreferrer'],
    ]);
    ...
    assert.equal(
      opened.length,
      1,
      'disabled layer releases card click ownership',
    );
  } finally {
    globalThis.document = originalDocument;
    globalThis.window = originalWindow;
...
test('Nepal evidence shots resume the standalone callout and flood sequence from Border Gate', async () => {
  ...
  try {
    ...
    const card = entries.find(
      ({ id }) => id === 'evidence-card-gyirong-border-gate',
    );
    const flood = viewer._test.dataSources[0].entities.getById(
      'bhote-koshi-flood-core',
    );
    ...
    assert.equal(
      card.position(),
      null,
      'the evidence card is absent during the camera flight',
    );
    assert.equal(flood.show, false);
    assert.equal(surge.show, false);
    assert.ok(
      embeddedCalls.some(
        ([action, id]) => action === 'warm' && id === 'gyirong-border-gate',
      ),
    ...
    assert.ok(
      embeddedCalls.some(
        ([action, id]) => action === 'show' && id === 'gyirong-border-gate',
      ),
    ...
    assert.equal(
      card.sourceAlpha(),
      0,
      'the card starts from a hidden reveal state',
    );
    ...
    assert.equal(
      flood.show,
      true,
      'the amber corridor grows with the callout reveal',
    );
    assert.equal(
      surge.show,
      true,
      'the amber surge circle advances with the corridor',
    );
    ...
    assert.equal(
      ...
      timeline.find(({ observation }) => observation.id === 'timure-cluster')
        .activationProgress,
    ...
    assert.equal(
      card.sourceAlpha(),
      0,
      'the Border Gate card recedes at the next beat',
    );
    assert.ok(releases.includes('bhote-koshi-scene-beat-reveal'));

    const timureBeat = timeline.find(
      ({ observation }) => observation.id === 'timure-cluster',
    );
    layer.setParams(
      {
    ...
    const timureCard = entries.find(
      ({ id }) => id === 'evidence-card-timure-cluster',
    );
    assert.equal(
      layer.getPlaybackState().progress,
      timureBeat.activationProgress,
    );
    assert.equal(
      timureCard.sourceAlpha(),
      0,
      'the next callout begins below its activation',
    );
    ...
    timureReveal(now);
    assert.ok(
      timureCard.sourceAlpha() > 0,
      'the Timure callout enters during its authored shot',
    );
    ...
    assert.equal(
      layer.getPlaybackState().progress,
      timureBeat.deactivationProgress,
    );
...
test('Bhote Koshi beat controls keep cards, flood, camera, and panel on the one clock', async () => {
  ...
  try {
    ...
    assert.match(
      panel.innerHTML,
      /2021 HISTORICAL REFERENCE \/ 2026 POST-EVENT/,
    );
    ...
    assert.equal(
      publications.length,
      1,
      'beat navigation never republishes overlay entries',
    );
    ...
    progress.dispatch('input');
    assert.equal(
      layer.getCinematicState().active,
      false,
      'manual release survives slider changes',
    );
    ...
    assert.equal(
      layer.getCinematicState().active,
      true,
      'explicit beat navigation reclaims camera',
    );
    ...
    assert.equal(layer.getCinematicState().active, true);
    assert.equal(
      layer.getCinematicState().evidenceId,
      'immediate-collapse-viewpoint',
    );
    assert.equal(
      panel.querySelector('[data-role="beat-index"]').textContent,
...
test('Bhote Koshi repeated event frames perform no panel lookup, DOM write, or camera churn', async () => {
  ...
  try {
    ...
    const writeCount = [...panel._selectors.values()].reduce(
      (sum, element) => sum + element._writeCount,
      0,
    );
    const cameraLookCount = viewer._test.cameraLooks.length;
    ...
    assert.equal(
      panel._queryCount,
      queryCount,
      'hot path uses cached panel references',
    );
    assert.equal(
      [...panel._selectors.values()].reduce(
        (sum, element) => sum + element._writeCount,
        0,
      ),
      writeCount,
...
test('Bhote Koshi frame path keeps evidence lookup scalar and wrapper-free', async () => {
  ...
  assert.doesNotMatch(
    source,
    /#ffd166/i,
    'event UI no longer publishes the prototype yellow',
  );
  assert.doesNotMatch(
    source,
...
test('every sourced shot grows its tail with its head, keeps its own card, and clears on exit', async () => {
  ...
  const layer = createBhoteKoshiEventLayer({
    ...
    mediaLoader: async () => {
      throw Object.assign(new Error('fixture'), { name: 'AbortError' });
    },
    overlayHost: {
      setEntries(_id, value) {
        entries = value;
      },
      setVisible() {},
      clearSource() {},
  ...
  const params = (
    ...
    sceneControls: {
      cameraSettled,
      evidencePath,
      evidencePathElevation,
      evidencePathDurationSec: 4,
      deferEvidenceUntilCameraSettled: true,
  ...
  try {
    ...
    for (const id of SOURCE_PATH_BEAT_IDS) {
      ...
      assert.equal(
        flood.show,
        false,
        `${id}: hide the previous route during flight`,
      );
      assert.equal(surge.show, false);
      ...
      for (const elapsed of [400, 1800, 4000]) {
        ...
        assert.ok(
          Cesium.Cartesian3.equals(points.at(-1), surge.position.getValue()),
          `${id}: tail must reach head in every frame`,
        );
      ...
      assert.equal(
        heightSamples,
        sampleCount,
        'do not resample geometry during motion',
      );
      const visibleCards = entries.filter(
        (entry) => entry.id.startsWith('evidence-card-') && entry.position(),
      );
      assert.deepEqual(
        visibleCards.map(({ id: cardId }) => cardId),
        [`evidence-card-${id}`],
      );
      assert.ok(
        visibleCards[0].sourceAlpha() > 0,
        'keep the source readable after the path settles',
      );
      now += 5000;
    }
    ...
    assert.equal(
      flood.show,
      false,
      'new shot clears its tail before the first animation frame',
    );
    assert.equal(surge.show, false);
    advance(1800);
    ...
    assert.equal(
      nextFrame,
      frameBeforeSync,
      'repeated settled notification does not restart the reveal',
    );
    assert.ok(
      Cesium.Cartesian3.equals(headBeforeSync, surge.position.getValue()),
    );
    advance(2200);
    assert.ok(
      !Cesium.Cartesian3.equals(headBeforeSync, surge.position.getValue()),
    ...
    for (const elapsed of [40, 400, 1800, 4000]) {
      ...
      for (const position of points) {
        ...
        assert.ok(
          height >= 1307 && height <= 1332,
          `source profile stays near river elevation, got ${height}`,
        );
      }
      assert.ok(
        Cesium.Cartesian3.equals(points.at(-1), surge.position.getValue()),
    ...
    assert.equal(
      heightSamples,
      samplesBeforeProfile,
      'source elevation does not sample the changing mesh',
    );
...
test('media-led upper-valley shots retain only their upstream prefix during camera travel', async () => {
  ...
  try {
    ...
    finishReveal();
    const lakeTail = flood.polyline.positions
      .getValue()
      .map((point) => Cesium.Cartesian3.clone(point));
    assert.ok(lakeTail.length > 1);
    const origin = Cesium.Cartographic.fromCartesian(lakeTail[0]);
    assert.ok(
      Math.abs(Cesium.Math.toDegrees(origin.longitude) - 85.48476) < 1e-6,
    );
    assert.ok(
      Math.abs(Cesium.Math.toDegrees(origin.latitude) - 28.33255) < 1e-6,
    ...
    assert.equal(
      flood.show,
      true,
      'lake trail survives departure before the next camera settles',
    );
    assert.deepEqual(
      flood.polyline.positions.getValue(),
      lakeTail,
      'retain the prefix without revealing the next segment',
    );
    assert.ok(
      Cesium.Cartesian3.equals(surge.position.getValue(), lakeTail.at(-1)),
    );
    layer.setParams(params('second-landslide', true));
    assert.deepEqual(
      flood.polyline.positions.getValue(),
      lakeTail,
      'arrival does not clear the prefix',
    );
    finishReveal();
    ...
    assert.equal(
      flood.show,
      false,
      'backward replay trims future segments instead of retaining stale history',
    );
    layer.setParams(params('second-landslide', false));
    assert.deepEqual(
      flood.polyline.positions.getValue(),
      lakeTail,
      'direct load reconstructs the same prefix',
    );
    layer.setParams(
      params('second-landslide', false, { evidencePathPersistent: false }),
    );
    assert.equal(
      flood.show,
      false,
      'nonpersistent shots retain their arrival gate',
    );
    for (const beatId of [
      'devighat-taadi-khola-bridge',
      'charaudi',
      ...
      const historyControls = {
        ...
        evidencePathHistoryBeatId: SOURCE_PATH_BEAT_IDS.at(-1),
        evidencePathDuringMedia: false,
        evidenceMediaAutoplay: beatId !== 'final-view',
      };
      layer.setParams(params(beatId, false, historyControls));
      const completedTail = [...flood.polyline.positions.getValue()];
      assert.ok(
        completedTail.length > 100,
        `${beatId}: direct load reconstructs full sourced history`,
      );
...
test('camera-led Nepal flood motion preserves its prefix and stays ahead of shot travel', async () => {
  ...
  try {
    layer.setParams(
      params('timure-cluster', 'comparison', travel(1), {
        evidenceSequence: true,
      }),
    );
    await layer.enable(viewer, { origin: 'scene' });
    ...
    assert.ok(
      timureStart > 1,
      'the completed Border Gate prefix is already visible during travel',
    );
    assert.equal(surge.show, true);
    advance(420);
    const earlyTimureLength = flood.polyline.positions.getValue().length;
    assert.ok(
      earlyTimureLength > timureStart,
    ...
    assert.equal(
      flood.show,
      true,
      'the completed tail remains visible at the next point',
    );
    assert.ok(
      Cesium.Cartesian3.equals(
        flood.polyline.positions.getValue().at(-1),
        surge.position.getValue(),
      ),
    );

    layer.setParams(
      params('syabru-besi', 'comparison', travel(2), {
        evidenceSequence: true,
      }),
    );
    assert.ok(
      flood.polyline.positions.getValue().length >= timureEnd,
    ...
    assert.equal(
      surge.show,
      true,
      'the amber head remains visible at comparison completion',
    );
    assert.ok(
      Cesium.Cartesian3.equals(
        flood.polyline.positions.getValue().at(-1),
    ...
    layer.setParams(
      params('dhunche', 'source', travel(3), {
        evidencePathHistory: true,
    ...
    assert.ok(
      dhuncheStart >= event.reconstruction.corridor.length,
    ...
    layer.setParams(
      params('mailung-upper-trishuli', 'source', travel(4), {
        evidencePathHistory: true,
        evidencePathElevation: 'source',
        evidenceHoldCard: true,
      }),
    ...
    assert.ok(
      flood.polyline.positions.getValue().length >= dhuncheEnd,
    ...
    assert.equal(
      frames.size,
      0,
      'cancelling camera travel revokes the path animation frame',
    );
    ...
    for (const beatId of SOURCE_PATH_BEAT_IDS.slice(3)) {
      layer.setParams(
        params(beatId, 'source', travel(travelId++), {
          evidencePathHistory: true,
          evidencePathElevation: 'source',
          evidenceHoldCard: true,
        }),
      ...
      assert.ok(
        startLength >= previousCompleteLength,
      ...
      assert.ok(
        completeLength > startLength,
        `${beatId} grows its own segment during travel`,
      );
      assert.equal(flood.show, true);
      assert.equal(surge.show, true);
      assert.ok(
    ...
    layer.setParams({
      presentation: 'scene-beat',
      sceneSurface: 'evidence-beat',
      beatId: 'devighat-taadi-khola-bridge',
      sceneControls: {
        evidencePath: 'none',
        deferEvidenceUntilCameraSettled: true,
        cameraSettled: false,
    ...
    assert.equal(
      flood.show,
      false,
      'Devighat does not leak the supported route',
    );
    assert.equal(surge.show, false);
    layer.setParams(
      params('bidur-trishuli-bridge', 'source', travel(travelId++), {
        evidencePathHistory: true,
        evidencePathElevation: 'source',
        evidenceHoldCard: true,
      }),
    ...
    advance(3300);
    assert.equal(
      flood.polyline.positions.getValue().length,
      bidurCompleteLength,
    );

    layer.setParams(
      params('dhunche', 'source', travel(travelId++), {
        evidencePathHistory: true,
        evidencePathElevation: 'source',
        evidenceHoldCard: true,
      }),
    ...
    assert.ok(
      flood.polyline.positions.getValue().length < bidurCompleteLength,
    ...
    layer.setParams(
      params('mailung-upper-trishuli', 'source', travel(travelId++), {
        evidencePathHistory: true,
        evidencePathElevation: 'source',
        evidenceHoldCard: true,
      }),
    );
    const deadlineEntry = [...timers.entries()].find(
      ([, entry]) => entry.delay >= 2400,
    );
    assert.ok(
      deadlineEntry,
      'camera-led travel registers a pre-arrival completion deadline',
    );
    ...
    assert.equal(
      frames.size,
      0,
      'the deadline cancels a stalled animation frame',
    );
    ...
    assert.equal(
      flood.show,
      false,
      'unsupported downstream shots do not invent or leak a route',
    );
    assert.equal(surge.show, false);
    await layer.disable();
  } finally {
