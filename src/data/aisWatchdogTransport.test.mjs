...
test('ws.terminate() hard-aborts a black-holed socket and frees the connection slot', async () => {
  ...
  assert.equal(
    ...
    'function',
    'the adapter depends on terminate()',
  );
  ...
  assert.equal(
    await closed,
    true,
    'terminate() must complete against a silent peer',
  );
  ...
  assert.equal(
    blackHole.liveConnections,
    0,
    'the socket slot must actually be released',
  );
});
...
test('the built-in WebSocket close() never completes on a black-holed socket', async () => {
  ...
  assert.equal(
    typeof new WebSocket(url).terminate,
    'undefined',
  ...
  assert.equal(
    await closed,
    false,
    'close() hangs in CLOSING — measured well past 30s by hand',
  );
  assert.equal(
    socket.readyState,
    WebSocket.CLOSING,
    'the socket slot is never handed back',
  );
});
