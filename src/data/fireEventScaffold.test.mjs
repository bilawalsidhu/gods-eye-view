...
test('service and sources follow the year', () => {
  ...
  assert.deepEqual(sourcesForYear(2024), [
    'VIIRS_SNPP_SP',
    'VIIRS_NOAA20_SP',
    'VIIRS_NOAA21_SP',
    'MODIS_SP',
...
test('candidateQueryUrl builds escaped, year-bounded queries per service', () => {
  ...
  assert.equal(
    new URL(hist.url).searchParams.get('where'),
    "UPPER(INCIDENT)='CAMP' AND FIRE_YEAR='2018'",
  );
  assert.throws(
    () => candidateQueryUrl({ name: 'x', year: 2020, service: 'nope' }),
    /Unknown perimeter service/,
  );
  assert.match(
    extentQueryUrl('wfigs', 30039.7),
    /where=OBJECTID%3D30039&returnExtentOnly=true&outSR=4326/,
  );
});
      ...
      features: [
        {
          attributes: {
            OBJECTID: 1,
            attr_IncidentName: 'Park',
            attr_POOState: 'US-NM',
            poly_GISAcres: 0.1,
            attr_FireDiscoveryDateTime: 1751495794000,
          },
        },
        {
          attributes: {
            ...
            attr_IncidentName: 'PARK',
            attr_POOState: 'US-CA',
            attr_POOCounty: 'Butte',
            poly_GISAcres: 429602.85,
            attr_FireDiscoveryDateTime: 1721857963000,
            attr_ContainmentDateTime: null,
            poly_DateCurrent: 1724525800000,
          },
        },
...
test('deriveWindow prefers published dates and flags estimates', () => {
  ...
  const full = deriveWindow({
    discoveryMs: disc,
    containmentMs: Date.UTC(2024, 8, 26),
  });
  ...
  const current = deriveWindow({
    discoveryMs: disc,
    currentMs: Date.UTC(2024, 7, 24),
  });
  ...
  const overridden = deriveWindow(
    { discoveryMs: disc },
    { start: '2024-07-20', end: '2024-07-10' },
  );
...
test('padBbox pads proportionally with a floor and rounds', () => {
  assert.deepEqual(
    padBbox({ xmin: -121.7778, ymin: 39.5986, xmax: -121.3526, ymax: 39.8978 }),
    [-121.829, 39.548, -121.302, 39.949],
  );
  assert.deepEqual(
    padBbox({ xmin: 0, ymin: 0, xmax: 0.01, ymax: 0.01 }),
    [-0.02, -0.02, 0.03, 0.03],
  );
  assert.throws(
    () => padBbox({ xmin: 1, ymin: 1, xmax: 0, ymax: 2 }),
    /--bbox/,
  );
});
...
test('buildEventEntry yields a validated config entry with honest warnings', () => {
  const candidate = {
    ...
    county: 'Butte',
    acres: 429602.85,
    discoveryMs: Date.UTC(2024, 6, 24),
  };
  ...
  assert.deepEqual(entry.perimeter, {
    service: 'wfigs',
    incident: 'PARK',
    state: 'US-CA',
    discoveredAfter: '2024-07-17',
  });
  ...
  const hist = buildEventEntry({
    candidate: {
      ...
      unitId: 'CABTU',
      acres: 153335.6,
      currentMs: Date.UTC(2018, 10, 26),
    },
    service: 'nifc-history',
    year: 2018,
    bbox: [-121.83, 39.55, -121.3, 39.95],
    window: deriveWindow(
      { currentMs: Date.UTC(2018, 10, 26) },
      { start: '2018-11-08' },
    ),
    ...
    summary: 'x',
    references: [{ label: 'a', url: 'https://a.example' }],
  });
  assert.deepEqual(hist.entry.perimeter, {
    service: 'nifc-history',
    incident: 'CAMP',
    fireYear: '2018',
    unitId: 'CABTU',
  ...
  assert.match(
    formatCandidateTable([candidate]),
    /\[0\]  PARK  US-CA  Butte  429,603 ac  disc 2024-07-24/,
  );
});
