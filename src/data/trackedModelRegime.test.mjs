...
test('the tracked 3D takeover sits at the owner-playtested swap distance', () => {
  ...
  assert.equal(
    trackedModelZoomActive(600_000, false),
    false,
    'the owner explicitly called 2D correct at ~600 km — it must not model there',
  );
  assert.equal(
    trackedModelZoomActive(140_000, false),
    true,
...
test('the tracked contact swaps NEARER than the fleet — a recorded inversion, not a bug', () => {
  ...
  assert.ok(
    TRACKED_MODEL_ENTER_ALT_M < FLEET_MODEL_ALT_CEIL_M,
    'the tracked contact deliberately enters 3D closer in than the fleet does',
  );
  assert.equal(
    trackedModelZoomActive(FLEET_MODEL_ALT_CEIL_M - 1, false),
    false,
...
test('enter and exit thresholds are ASYMMETRIC — the anti-flap band', () => {
  ...
  assert.equal(
    TRACKED_MODEL_EXIT_ALT_M,
    TRACKED_MODEL_ENTER_ALT_M * TRACKED_MODEL_EXIT_RATIO,
    'the exit ceiling stays derived from enter, so retuning enter carries the band with it',
  );
  assert.ok(
    TRACKED_MODEL_EXIT_ALT_M > TRACKED_MODEL_ENTER_ALT_M,
...
test('the regime enters at the enter ceiling and leaves only past the exit ceiling', () => {
  ...
  assert.equal(
    trackedModelZoomActive(TRACKED_MODEL_EXIT_ALT_M + 1, false),
    false,
  );
  assert.equal(
    trackedModelZoomActive(TRACKED_MODEL_ENTER_ALT_M + 1, false),
    false,
  );
  assert.equal(
    trackedModelZoomActive(TRACKED_MODEL_ENTER_ALT_M, false),
    false,
  ...
  assert.equal(
    trackedModelZoomActive(TRACKED_MODEL_ENTER_ALT_M + 1, true),
    true,
    'crossing back over the ENTER ceiling must not hand back — that is the flap',
  );
  assert.equal(
    trackedModelZoomActive(TRACKED_MODEL_EXIT_ALT_M - 1, true),
    true,
  );
...
test('an orbit sitting AT the boundary never flaps', () => {
  ...
  let active = trackedModelZoomActive(
    TRACKED_MODEL_ENTER_ALT_M - BAND_WIDTH_M / 4,
    false,
  );
  ...
  for (let i = 0; i < 40; i++) {
    ...
    const next = trackedModelZoomActive(
      TRACKED_MODEL_ENTER_ALT_M + wobble,
      active,
    );
  ...
  assert.equal(
    transitions,
    0,
    'the hysteresis band absorbs boundary wobble entirely',
  );
...
test('a missing camera height reads as infinitely far out, never as "zoomed in"', () => {
  for (const height of [undefined, null, NaN, Infinity]) {
    ...
    assert.equal(
      trackedModelZoomActive(height, true),
      false,
...
function viewerAtHeight(
  height,
  { tilesLoaded = true, sampleHeight = () => undefined } = {},
) {
...
for (const fixture of LAYERS) {
  test(`${fixture.name}: production fleet loader admits a HIDDEN primitive and fails back to 2D`, async () => {
    ...
    try {
      ...
      assert.equal(
        admitted,
        loadingModel,
        'the production fleet loader admits the resolved primitive',
      );
      assert.equal(
        loadedCollection.added.length,
        1,
        'the production loader adds it once',
      );
      ...
      assert.equal(
        loadingModel.show,
        false,
        'admission hides the primitive until it is placed',
      );
      assert.equal(
        loadingModel.scale,
        1,
        'and never touches its scale to do it',
      );
      assert.equal(
        billboard.show,
        true,
        'the 2D billboard owns the cold loading state',
      );

      const nearby = fixture.layer
        .getNearby(billboard.position, 1_000, 50)
        .find((object) => object.icao24 === loadingIcao);
      assert.ok(
        nearby,
        'fleet proximity retains the billboard-owned loading contact',
      );
      assert.ok(
      ...
      assert.equal(
        failedBillboard.show,
        true,
        'fleet load rejection leaves the 2D owner visible',
      );
      assert.ok(
        fixture.layer
          .getNearby(failedBillboard.position, 1_000, 50)
  ...
  test(`${fixture.name}: grounded fleet handoff waits through terrain load and keeps one visual owner`, () => {
    ...
    try {
      ...
      let ownsVisual = fixture.driveFleetModel({
        icao24,
        position,
        course: 92,
      });
      assert.equal(
        ownsVisual,
        false,
        'cold terrain cannot transfer ownership to 3D',
      );
      assert.equal(
        model.show,
        false,
        'the ready primitive is hidden until ground resolves',
      );
      assert.equal(
        billboard.show,
        true,
        'the 2D contact returns in the same handoff turn',
      );
      assert.ok(
        model.show || billboard.show,
        'the contact never has both visuals hidden',
      );
      ...
      assert.equal(
        ownsVisual,
        false,
        'the miss backoff remains in force before two seconds',
      );
      assert.equal(
        sampleCalls,
        0,
        'backoff prevents an early synchronous sample',
      );
      assert.ok(
        model.show || billboard.show,
        'backoff still leaves a visible owner',
      );
      ...
      assert.equal(
        ownsVisual,
        true,
        'the model takes over after a valid terrain sample',
      );
      assert.equal(
        sampleCalls,
        1,
        'the settled terrain is sampled exactly once',
      );
      assert.equal(model.show, true);
      assert.equal(billboard.show, false);
      assert.ok(
        model.show || billboard.show,
        'successful handoff keeps a visible owner',
      );
      const placed = Cesium.Matrix4.getTranslation(
        model.modelMatrix,
        new Cesium.Cartesian3(),
      );
      const placedHeight = Cesium.Cartographic.fromCartesian(placed).height;
      assert.ok(
        Math.abs(placedHeight - (sampledHeight + fixture.bellyM)) < 0.001,
      ...
      assert.equal(
        sampleCalls,
        1,
        'the height-normalized cache does not resample vertically',
      );
      assert.ok(
        model.show || billboard.show,
        'datum transition keeps a visible owner',
      );
      ...
      assert.equal(
        model.show,
        true,
        'no 3D→2D pop while the resample is backing off',
      );
      assert.equal(billboard.show, false);
      assert.equal(
        sampleCalls,
        1,
        'and the hold costs no extra synchronous sample',
      );
      const held = Cesium.Matrix4.getTranslation(
        model.modelMatrix,
        new Cesium.Cartesian3(),
      );
      assert.ok(
        Math.abs(
          Cesium.Cartographic.fromCartesian(held).height -
            (sampledHeight + fixture.bellyM),
        ) < 0.001,
      ...
      assert.ok(
        model.show || billboard.show,
        'release returns atomically to 2D',
      );
      ...
      assert.equal(
        ownsVisual,
        false,
        'a missing/failed model never claims ownership',
      );
      assert.equal(
        failedBillboard.show,
        true,
        'load failure keeps the billboard visible',
      );
    } finally {
      Date.now = realNow;
      fixture.layer.stopTracking();
  ...
  test(`${fixture.name}: a held snap is dropped when measured ground contradicts it`, () => {
    ...
    try {
      ...
      assert.equal(
        ownsVisual,
        true,
        'a measured snap at A hands the visual to the model',
      );
      assert.equal(sampleCalls, 1);
      assert.ok(
        Math.abs(placedHeightM() - (sampled + fixture.bellyM)) < 0.001,
      ...
      assert.equal(
        billboard.show,
        true,
        'the floored 2D billboard is the honest fallback',
      );
      assert.ok(
        model.show || billboard.show,
        'the release is atomic — never both hidden',
      );
      ...
      for (let i = 0; i < 5; i++) {
        now += 60_000;
        assert.equal(
          fixture.driveFleetModel({ icao24, position: atB, course: 92 }),
          false,
      ...
      assert.ok(
        Math.abs(placedHeightM() - (B_GROUND_M + fixture.bellyM)) < 0.001,
      ...
      assert.equal(
        model.show,
        true,
        'the contact keeps its model through a downhill taxi',
      );
      assert.ok(
        Math.abs(placedHeightM() - (B_GROUND_M + fixture.bellyM)) < 0.001,
  ...
  test(`${fixture.name}: grounded tracked model waits for a valid terrain snap with no visual gap`, async () => {
    ...
    try {
      ...
      assert.equal(
        ...
        1,
        'the loaded primitive enters the scene once',
      );
      assert.equal(
        model.ready,
        false,
        'the production loader is still awaiting GPU readiness',
      );
      assert.equal(
        model.show,
        false,
        'the loading primitive is admitted hidden, not zero-scaled',
      );
      assert.equal(model.scale, 1, 'its scale is left to the pixel-cap driver');
      assert.equal(
        fixture.trackedBillboardColor().alpha,
        1,
      ...
      assert.equal(
        sampleCalls,
        0,
        'cold tiles do not force a synchronous sample',
      );
      const loadingDetection = fixture.layer
        .getDetectableObjects({ maxCount: 50 })
        .find(
          (object) =>
            object.sourceId ===
            (fixture.name === 'flights' ? 'a0f001' : 'b0f001'),
        );
      assert.ok(
        loadingDetection,
        'the tracked billboard remains detectable while its model loads',
      );
      assert.ok(
        Cesium.Cartesian3.equalsEpsilon(
      ...
      const loadingNearby = fixture.layer
        .getNearby(billboard.position, 1_000, 50)
        .find(
          (object) =>
            object.icao24 ===
      ...
      assert.ok(
        loadingNearby,
        'the tracked contact remains in proximity while its model loads',
      );
      assert.ok(
        Cesium.Cartesian3.equalsEpsilon(
      ...
      assert.equal(
        sampleCalls,
        0,
        'the tracked path honors the same retry backoff before sampling',
      );
      assert.equal(
        model.show,
        false,
        'retry backoff keeps the unplaced primitive hidden',
      );
      assert.equal(
        fixture.trackedBillboardColor().alpha,
        1,
      ...
      assert.equal(
        model.show,
        false,
        'terrain alone does not reveal an unready model',
      );
      assert.equal(
        fixture.trackedBillboardColor().alpha,
        1,
      ...
      assert.equal(
        model.show,
        true,
        'the model appears only after its snapped matrix is written',
      );
      assert.equal(
        fixture.trackedBillboardColor().alpha,
        0,
      ...
      assert.equal(
        sampleCalls,
        1,
        'the hold costs no extra synchronous sample',
      );
  ...
  test(`${fixture.name}: the tracked contact goes 3D with the fleet 3D toggle OFF`, () => {
    ...
    fixture.layer.setParams({ models3d: false });
    assert.equal(
      fixture.layer.getParams().models3d,
      false,
      'the fleet toggle really is off',
    );
  ...
  test(`${fixture.name}: the tracked regime is hysteretic across a live zoom in/out`, () => {
    ...
    assert.equal(
      fixture.regimeActive(),
      false,
      'past the exit ceiling, 2D returns',
    );
  ...
  test(`${fixture.name}: deselect + re-track the SAME contact re-enters at the ENTER ceiling`, () => {
    ...
    assert.equal(
      fixture.regimeActive(),
      true,
      'the ORIGINAL selection still holds in the band',
    );
  ...
  test(`${fixture.name}: a converted TR-3B never takes the model, at any zoom`, () => {
    ...
    assert.equal(
      fixture.regimeActive(),
      true,
      'control: an unconverted contact does model',
    );
    ...
    assert.equal(
      fixture.regimeActive(),
      true,
      'un-converting restores the handoff',
    );
    clearTr3bRegistry();
  });
  ...
  test(`${fixture.name}: a rejecting GLB loader is bounded, not a per-frame retry loop`, async () => {
    ...
    try {
      ...
      assert.equal(
        fixture.regimeActive(),
        true,
        'the regime is open, so the driver will try to load',
      );
      ...
      assert.equal(
        collection.added.length,
        0,
        'a failed load never puts a model up',
      );
      assert.equal(
        fixture.trackedBillboardColor().alpha,
        1,
      ...
      assert.equal(
        warnings.length,
        1,
        'the give-up is announced once, not per frame',
      );
      assert.ok(
        warnings[0].includes(requested[0]),
        `the warning names the asset: ${warnings[0]}`,
      );
      ...
      assert.ok(
        attempts > settled,
        'selecting another contact restores the retry budget',
      );
    } finally {
      Cesium.Model.fromGltfAsync = realFrom;
      Date.now = realNow;
...
test('both layers gate the tracked-model load and record its failures', async () => {
  ...
  for (const name of ['flights.js', 'militaryFlights.js']) {
    ...
    assert.match(
      source,
      /!(?:flightState\.)?_trackedModel\s*&&\s*!(?:flightState\.)?_trackedModelLoading\s*&&\s*(?:parts\.\w+\.)?_trackedModelLoadAllowed\(\s*,?\s*\)/,
      `${name}: the driver asks permission before starting another tracked-model load`,
    );
    assert.match(
      source,
      /(?:parts\.\w+\.)?_noteTrackedModelLoadFailure\(\s*trackedSpec\.url,\s*err,?\s*\)/,
...
test('the tracked regime never consults the fleet models3d toggle', async () => {
  ...
  for (const name of ['flights.js', 'militaryFlights.js']) {
    const source = readLayerSource(new URL(`./${name}`, import.meta.url));
    const regime =
      /^([ \t]*)function _trackedModelRegimeActive\b[\s\S]*?\n\1\}/m.exec(
        source,
      )?.[0];
    assert.ok(regime, `${name}: _trackedModelRegimeActive is defined`);
    assert.doesNotMatch(
      regime,
      /(?:flightState\.)?_models3dEnabled|(?:parts\.\w+\.)?_modelRegimeActive\(\s*,?\s*\)/,
    ...
    assert.match(
      fleet,
      /if\s*\(\s*!(?:flightState\.)?_models3dEnabled,?\s*\)\s*return\s*false;/,
