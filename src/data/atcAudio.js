...
export class AtcAudioController {
  constructor({
    ...
    this._audioLease = null;
    this._unsubscribeOwnership = subscribeAudioOwnership((change) => {
      if (change.owner !== 'atc' && this._state === 'playing') {
        this.stop({ squelch: true });
      }
    });
  }
  ...
  async playStream(url, { squelch = true } = {}) {
    ...
    this._audioLease = claimAudio('atc', { preempt: true });
    ...
    try {
      ...
      if (this._audio === audio) {
        this._state = 'error';
        this._lastError =
          err?.message || 'Playback blocked or stream unreachable';
        this._notify();
      }
      return false;
    }
