...
test('atcAudio: yields playback when another owner claims audio', async () => {
  const { claimAudio, audioOwner } = await import('./audioOwnership.js');
  const ctrl = new AtcAudioController({ AudioConstructor: MockAudio });
  await ctrl.playStream('https://stream.test/atc.mp3', { squelch: false });

  assert.equal(audioOwner(), 'atc');

  // Another owner claims audio (e.g. World Radio)
  claimAudio('radio', { preempt: true });

  // ATC audio automatically stops
  assert.equal(ctrl.state, 'squelch');
  ctrl.destroy();
});
