...
test('installation failure reasons disclose no raw upstream error and do not guess overload', () => {
  assert.equal(
    militaryInstallationFailureReason(new Error('private network details')),
    'unavailable',
  );
  assert.equal(
...
test('exact-viewport answers are keyed apart from snapped ones', () => {
  ...
  const snapped = militaryInstallationCacheKey(
  ...
  assert.notEqual(
    snapped,
    exact,
    'a saturated retry must never overwrite the shared tile',
  );
  assert.notEqual(
    militaryInstallationDiskPath(snapped),
    militaryInstallationDiskPath(exact),
  );
});
...
test('exact-viewport keys carry the same precision as the query bounds', () => {
  ...
  const a = { south: 30.2612, west: -97.7431, north: 30.2884, east: -97.7099 };
  const b = { south: 30.2614, west: -97.7429, north: 30.2886, east: -97.7097 };
  assert.notEqual(
  ...
  assert.equal(
    militaryInstallationCacheKey(a, 5),
    '30.26120,-97.74310,30.28840,-97.70990',
  );
...
test('a pre-fix cache entry derives its missing saturation flag instead of lying', async () => {
  ...
  assert.equal(
    migrateMilitaryInstallationEntry(underCap).payload.saturated,
    false,
  );
  // An entry that already states it is authoritative.
  const explicit = {
    payload: { elements: new Array(700).fill({}), saturated: false },
    cachedAt: now,
  };
  assert.equal(
    migrateMilitaryInstallationEntry(explicit).payload.saturated,
    false,
  );
  ...
  try {
    ...
    await fsp.writeFile(
      militaryInstallationDiskPath('legacy', dir),
      JSON.stringify(legacyOnDisk),
    );
    const read = await readMilitaryInstallationDisk('legacy', TTL_MS, dir);
    assert.equal(
      read.payload.saturated,
      true,
      'the client must be told to re-ask for its exact viewport',
    );
  } finally {
...
test('persisted installations stay fresh for 30 days and no longer', () => {
  const now = 1_700_000_000_000;
  assert.equal(
    militaryInstallationDiskFresh(entry(now - DAY_MS), undefined, now),
    true,
  );
  assert.equal(
    militaryInstallationDiskFresh(entry(now - 29 * DAY_MS), undefined, now),
    true,
  );
  assert.equal(
    militaryInstallationDiskFresh(entry(now - 31 * DAY_MS), undefined, now),
    false,
  );
  // The serve-stale path accepts any age when Overpass is down.
  assert.equal(
    militaryInstallationDiskFresh(entry(now - 400 * DAY_MS), Infinity, now),
    true,
  );
});
...
test('a malformed or truncated disk entry is never served', () => {
  ...
  assert.equal(
    militaryInstallationDiskFresh({ cachedAt: now }, undefined, now),
    false,
  );
  assert.equal(
    militaryInstallationDiskFresh(
  ...
  assert.equal(
    militaryInstallationDiskFresh(
      { payload: { elements: 'nope' }, cachedAt: now },
      undefined,
      now,
    ),
    false,
...
test('a real miss, write, and restart round trip serves from disk', async () => {
  ...
  let memory = new Map();
  const readDisk = (maxAgeMs = TTL_MS) =>
    readMilitaryInstallationDisk(key, maxAgeMs, dir);
  ...
  try {
    ...
    assert.deepEqual(
      afterRestart.entry.payload.elements,
      fresh.payload.elements,
    );
...
test('an in-flight request is joined instead of paying for a disk read', async () => {
  ...
  try {
    ...
    assert.equal(
      result.source,
      'UPSTREAM',
      'the caller coalesces onto the live request',
    );
...
test('an expired disk entry stops answering, but still serves stale when upstream is down', async () => {
  ...
  try {
    await writeMilitaryInstallationDisk(
      key,
      entry(Date.now() - 31 * DAY_MS),
      dir,
    );
    assert.equal(
      await readMilitaryInstallationDisk(key, TTL_MS, dir),
      null,
      'past its TTL for a normal read',
    );
...
test('an unwritable cache directory degrades to a miss instead of throwing', async () => {
  ...
  try {
    ...
    assert.equal(
      await writeMilitaryInstallationDisk('key', entry(Date.now()), blocked),
      false,
    );
    assert.equal(
      await readMilitaryInstallationDisk('key', TTL_MS, blocked),
      null,
    );
  } finally {
    await fsp.rm(dir, { recursive: true, force: true });
