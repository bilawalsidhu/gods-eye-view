...
test('both flight layers keep a converted contact 2D and visible (render invariants)', async () => {
  for (const name of ['flights.js', 'militaryFlights.js']) {
    ...
    assert.match(
      source,
      /if\s*\(\s*!(?:flightState\.)?_trackedIcao\s*\|\|\s*(?:flightState\.)?_cockpitContactMode\s*\|\|\s*isTr3b\(\s*(?:flightState\.)?_trackedIcao,?\s*\),?\s*\)\s*\{/,
    ...
    assert.match(
      source,
      /if\s*\(\s*bb\s*&&\s*id\s*!==\s*(?:flightState\.)?_trackedIcao,?\s*\)\s*bb\.show\s*=\s*true;|if\s*\(\s*modelled\s*&&\s*id\s*!==\s*(?:flightState\.)?_trackedIcao,?\s*\)\s*modelled\.show\s*=\s*true;/,
    ...
    for (const call of callSites) {
      assert.match(
        call,
        /aircraftIcon\(\s*\s*(?:(?:parts\.)?rendering\.)?_iconKind\(\s*/,
...
test('a converted contact never consumes a 3D model CAP SLOT', async () => {
  ...
  for (const name of ['flights.js', 'militaryFlights.js']) {
    ...
    const loop =
      /const\s*cand\s*=\s*\[\];\s*\n\s*for\s*\(\s*const\s*\[icao,\s*bb\]\s*of\s*(?:flightState\.)?_billboards,?\s*\)[\s\S]*?cand\.push\(\s*/.exec(
        source,
      )?.[0];
