...
test('validatePlacesCoordinates refuses missing, blank, non-numeric, and out-of-range values', () => {
  const check = (query) =>
    validatePlacesCoordinates(new URLSearchParams(query));
  for (const bad of [
    '',
    'lat=30.27',
    'lon=-97.74',
  ...
  assert.deepEqual(check('lat=90&lon=-180'), {
    ok: true,
    latitude: 90,
    longitude: -180,
  });
  ...
  assert.deepEqual(check('lat=30.27&lon=-97.74'), {
    ok: true,
    latitude: 30.27,
    longitude: -97.74,
  });
  ...
  try {
    ...
    for (const query of badQueries) {
      ...
      assert.equal(
        ...
        400,
        `nearby-places must refuse ${query || '(empty)'}`,
      );
      assert.deepEqual(nearbyResponse.body.places, []);
      const textResponse = await invokeRoute(textSearch, {
        url: `/?q=capitol&${query}`,
      });
      assert.equal(
        textResponse.statusCode,
        400,
        `text-search must refuse ${query || '(empty)'}`,
      );
      assert.deepEqual(textResponse.body.places, []);
    }
    const missingQuery = await invokeRoute(textSearch, {
    ...
    assert.notEqual(
      valid.statusCode,
      429,
      'malformed requests must not consume limiter capacity',
    );
    assert.equal(
      upstreamCalls,
      1,
      'a valid request reaches Google exactly once',
    );
  } finally {
...
test('preview validates both Places routes and keeps the keyless response', async () => {
  for (const key of ['', 'test-key']) {
    ...
    for (const name of ['nearby-places', 'text-search']) {
      const result = await invokeRoute(routes.get(`/api/google/${name}`), {
        url: '/?q=capitol&lat=&lon=',
