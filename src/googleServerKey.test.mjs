...
test('the Street View tool resolves per-variable overrides before preferring the server key', () => {
  ...
  try {
    writeFileSync(
      envPath,
      'GOOGLE_MAPS_API_KEY=file-browser\nGOOGLE_MAPS_SERVER_API_KEY="file-server" # separate key\n',
    );
    assert.equal(
      loadApiKey(null, {
        envPath,
        environment: { GOOGLE_MAPS_API_KEY: 'shell-browser' },
      }),
      'file-server',
    );
    assert.equal(
      loadApiKey(null, {
        envPath,
        environment: { GOOGLE_MAPS_SERVER_API_KEY: 'shell-server' },
      }),
      'shell-server',
    );
    assert.equal(
    ...
    assert.equal(
      loadApiKey('explicit-key', { envPath, environment: {} }),
      'explicit-key',
    );
    assert.equal(
      loadApiKey(null, {
    ...
    assert.equal(
      loadApiKey(null, { envPath, environment: {} }),
      'file-browser',
    );
...
test('both Places routes select the intended key and keep it out of responses', async (t) => {
  const original = {
    ...
    limit: process.env.GEV_RATELIMIT_GOOGLE_PER_MIN,
  };
  const calls = [];
  ...
  try {
    ...
    for (const [name, value] of Object.entries({
      GOOGLE_MAPS_SERVER_API_KEY: original.server,
      GOOGLE_MAPS_API_KEY: original.browser,
      GEV_RATELIMIT_GOOGLE_PER_MIN: original.limit,
    })) {
      if (value === undefined) delete process.env[name];
      else process.env[name] = value;
    }
...
test('browser defines contain the browser key and exclude the server key', () => {
  withKeys({ server: 'server-secret', browser: 'browser-public' }, () => {
    ...
    assert.equal(
      defines['import.meta.env.GOOGLE_MAPS_API_KEY'],
      '"browser-public"',
    );
    assert.ok(!JSON.stringify(defines).includes('server-secret'));
    assert.ok(
      !Object.keys(defines).some((key) => key.includes('SERVER_API_KEY')),
    );
  });
});
