 ...
 * restores the prior state on exit — see contactsDetectionPolicy.js).
...
export class StyleManager {
  ...
  constructor(viewer, { mapStackController = null } = {}) {
    ...
    this.shareLinkManager = new ShareLinkManager(viewer, {
      onRestore: async (state) => {
        ...
        if (style) this.setStyle('normal', { restore: true });
    ...
    initDetection(viewer, [trafficLayer, flightsLayer, militaryFlightsLayer, satellitesLayer, cctvLayer, aisLiveVesselsLayer], (modeLabel) => {
  ...
  _isDroneViewActive() {
    return document.body?.classList?.contains('drone-view') === true;
  }

  _requestDroneViewExit() {
    if (!this._isDroneViewActive()) return false;
    window.dispatchEvent(new CustomEvent('gev:drone-view-exit-request'));
    return true;
  }
  ...
  _reassertNavigationHandoff(generation) {
    return reassertNavigationHandoff({
      ...
      // was refused by the immersive view leaves the old readout standing.
      release: () => {
   ...
   * third-person tracking inside Contacts").
   *
   * The scope is the CONTACTS SESSION. Third-person tracking moves within that
   * session and deliberately does not
   * touch detection — an earlier build hooked this to context navigation.
   *
   ...
   * entries. Provider failures are surfaced by the controller and resolve to
   * the always-available OpenStreetMap fallback.
   * @returns {void}
   */
  ...
  /**
   * Keeps both responsive panel lanes on the same measured layout commit.
   * HUD visibility transitions can outlive the first
   ...
   * survive a style switch); Context entry does not because it owns detection.
  ...
  _syncShareState() {
    ...
    this.shareLinkManager.onToggleChange(this.bloomEnabled, this.sharpenEnabled, {
      ...
      mapStack: this.mapStackController?.getActiveId?.() || 'azure-satellite',
    });
  }
  ...
  _initGlobalContextPanel() {
    this._globalContextFlightsBtn?.addEventListener('click', () => {
      ...
      void this._runUserFacingContextAction(
        (notificationToken) => this._selectContextMode(nextMode, { notificationToken }),
  ...
  async _restoreContextSession({
    ...
    // A direct command may finish after restore has already copied its target
    // and queued the opposite state. Replay that newer intent only
  ...
  async _selectContextMode(mode, { notificationToken = null, signal = null } = {}) {
    if (!this._dataManager || this._clearSelectedLayersPromise) return false;
    if (mode !== null && mode !== 'flights') return false;
    ...
    try {
      if (!mode) {
        ...
        return generation === this._contextModeGeneration;
      }
      this._captureContextSessionSnapshot();
      this._contextMode = 'flights';
      this._syncContextModeButtons();
      try {
        await this._clearLayersOutsideContextMode('flights', { notificationToken, signal });
        if (generation !== this._contextModeGeneration || signal?.aborted) return false;
        this._dataManager.setLayerParams('military-awareness', { passive: false });
        const activated = await this._dataManager.setEnabled('military-awareness', true, {
          notificationToken,
          ...(signal ? { signal } : {}),
        });
        if (activated === false || !this._dataManager.isEnabled('military-awareness')) {
          throw new Error('Context activation failed for: military-awareness');
        }
        return true;
      } catch (error) {
        let transitionError = error instanceof Error ? error : new Error(String(error));
        this._contextMode = null;
        this._syncContextModeButtons();
        try {
  ...
  _handleContextLayerChange(change) {
    if (!change || change.type === 'visibility-transition' || change.type === 'visibility-will-change') return;
    if (change.type === 'visibility-blocked' || change.type === 'visibility-failed') {
      if (!this._userFacingContextNotificationTokens.has(change.notificationToken)) {
        this._showToast(change.reason || `${change.layerId} could not ${change.enabled ? 'start' : 'stop'} cleanly`);
      }
      return;
    }
    recordContextSessionUserChange({
      ...
      change,
      effectiveContextMode: this._contextModeEntering || this._contextMode,
    });
    recordContextRestoreExplicitChange({ restoreState: this._contextRestoreState, change });
    if (shouldExitContextForLayerChange({
      ...
      void this._trackContextLayerReaction(this._runUserFacingContextAction(
        (notificationToken) => this._deactivateContextForLayerChange({ notificationToken }),
      ));
      return;
    }
    this._syncContextModeButtons();
  ...
  _syncContextModeButtons() {
    ...
    panel?.setAttribute('data-context-mode', flightsActive ? 'flights' : 'none');
    ...
    if (this._globalContextFlightsBtn) this._globalContextFlightsBtn.tabIndex = 0;
    if (this._contextModeStandby) this._contextModeStandby.hidden = flightsActive;
  ...
  async _toggleCctvEnabled(forceState) {
    ...
    await runCctvLayerEnableTransition({
      ...
      readOwnership: () => ({
        ...
        droneViewActive: this._isDroneViewActive(),
      }),
  ...
  _syncPanelCollapseButton(panelEl) {
    ...
    panelEl.querySelectorAll('.panel-collapse-btn[data-collapse-target]').forEach((btn) => {
      const owner = btn.closest('[data-panel-id]');
  ...
  setPanelCollapsed(panelId, collapsed, {
    ...
    const rightOwnerPanel = this._rightPanelStack?.contains(panelEl) ? panelEl : null;
  ...
  async setMapStack(stackId) {
    ...
    if (!target.available) {
      return { ok: false, error: `${target.label} is unavailable`, activeStack: this.mapStackController.getActiveId() };
    }
  ...
  setCelestialRingEnabled(enabled, { syncShare = true, focus = false } = {}) {
    ...
    if (this._celestialBtn) {
      this._celestialBtn.disabled = false;
      this._celestialBtn.setAttribute('aria-disabled', 'false');
      this._celestialBtn.title = 'Celestial ring — reveal the full globe';
    }
  ...
  /**
   * Reads Contacts context mode state for voice/state-sync consumers.
   */
  getContextModeState() {
    return {
  ...
  /**
   * Sets Contacts context mode for voice.
   * @param {'contacts'|'off'|null} mode - Requested context target.
   * @param {object} [options]
   ...
   *   lane. Context choreography calls this facade INTERNALLY for its own
  ...
  async setContextMode(mode, {
    ...
    try {
      if (!mode || mode === 'off') {
        ...
        // Context choreography opts out: it is not an operator Context request.
  ...
  async applyVisualState(state = {}, { isCurrent = null } = {}) {
    ...
    if (state.style) this.setStyle('normal', { restore: true });
  ...
  // Normal is the only retained visual style. Unknown and historical style
  // requests intentionally collapse to Normal.
  setStyle(_styleName, { restore = false } = {}) {
    if (!restore) this.shareLinkManager?.claimRestoreLane?.('visual');
    this.activeStyle = 'normal';
    document.documentElement.dataset.gevStyle = 'normal';
    if (this._styleIndicator) this._styleIndicator.textContent = 'NORMAL';
    this._updateStyleMiniStatus('normal');
    this.hud.onStyleChange('normal');
    setDetectionStyle('normal');
    this.shareLinkManager.onStyleChange('normal');
    this._syncShareState();
  }
