...
const wrongWinners = () => [
  squareWay(ANCHOR, -660, -90, 3093, {
    building: 'yes',
    tourism: 'hotel',
    name: 'Thompson Austin',
  }),
  squareWay(ANCHOR, -780, 120, 3772, {
    building: 'yes',
    name: 'Austin Police HQ',
  }),
  squareWay(ANCHOR, -380, -60, 4499, {
    leisure: 'park',
    name: 'Black Austin Matters',
  }),
  squareWay(ANCHOR, -240, -700, 1500, {
    building: 'yes',
    amenity: 'library',
    name: 'Austin Public Library - Austin History Center',
  }),
...
test('point mode: an (almost) exactly-named, monument-scale polygon still outlines', () => {
  ...
  assert.ok(
    Math.abs(lat0 - ANCHOR.lat) < 0.001,
    `ring landed at ${lat0}, expected ~${ANCHOR.lat}`,
  );
});
...
test('point mode: an exactly-named but park-sized polygon is rejected (size cap)', () => {
  ...
  const park = squareWay(ANCHOR, 0, 0, 4_100_000, {
    leisure: 'park',
  ...
  const fp = selectFootprint(
    ...
    ANCHOR.lat,
    ANCHOR.lon,
    'Pioneer Monument, Golden Gate Park',
    'point',
  );
...
test('loose mode: a named compound still beats an unnamed containing building (Presidio case)', () => {
  const els = [
    ...
    squareWay(ANCHOR, 400, 400, 6_000_000, {
      landuse: 'military',
...
test('strict mode: unchanged — named district-sized areas only', () => {
  const els = [
    ...
    squareWay(ANCHOR, 0, 0, 12_000, {
      landuse: 'retail',
      name: 'Mission Market',
    }), // tiny parcel → rejected
    squareWay(ANCHOR, 200, 200, 500_000, {
      landuse: 'residential',
...
test('loose mode: a named water body beats a shore feature named after it (field test 9)', () => {
  ...
  const els = [
    ...
    squareWay(ANCHOR_ON_WATER, 0, 0, 3_500_000, {
      natural: 'water',
      water: 'reservoir',
      name: 'Lady Bird Lake',
    }),
    // Shore park named after the lake (partial name coverage, does not contain anchor).
    squareWay(ANCHOR_ON_WATER, 450, -300, 90_000, {
      leisure: 'park',
      name: 'Auditorium Shores at Lady Bird Lake Metropolitan Park',
    }),
  ];
  const fp = selectFootprint(
    els,
  ...
  const park = selectFootprint(
    els,
    ANCHOR_ON_WATER.lat + 0.004,
    ANCHOR_ON_WATER.lon - 0.003,
    'Auditorium Shores',
    'loose',
  );
  assert.ok(park);
  assert.ok(
    Math.abs(park.ring[0][1] - (ANCHOR_ON_WATER.lat + 450 / 111320)) < 0.01,
    'land ask keeps the park',
  );
});
...
test('isGroundsLikeAsk: label wording and entityKind both count (field test 8)', () => {
  ...
  assert.equal(
    isGroundsLikeAsk('Texas State Capitol, Austin', 'Capitol grounds', null),
    true,
  );
  // Target wording still works as before.
  assert.equal(
    isGroundsLikeAsk('Texas State Capitol grounds, Austin', null, null),
    true,
  );
  // An explicit non-compound entityKind vetoes grounds wording (trust the model's fact).
  assert.equal(
    isGroundsLikeAsk('Capitol complex', 'the complex', 'building'),
    false,
  );
...
test('ask-side admin bypass: "the Texas Capitol" recovers near-view despite a far state-typed geocode', async (t) => {
  ...
  assert.equal(
    calls.length,
    2,
    'admin result types must not suppress near-view recovery',
  );
});
...
test('ask-side admin bypass: explicit "state of Texas" skips recovery and proximity gating', async (t) => {
  ...
  assert.ok(
    resolved,
    'the explicit state ask keeps its legitimate far centroid',
  );
  ...
  assert.equal(
    calls.length,
    1,
    'explicit state scope bypasses near-view recovery',
  );
});
...
for (const fixture of [
  ...
  test(`ask-side admin bypass: trailing name "${fixture.target}" remains guarded`, async (t) => {
    ...
    assert.equal(
      resolved,
      null,
      'a recovery miss continues through the proximity gate',
    );
    assert.equal(
      calls.length,
      2,
      'a proper name ending in State must try near-view recovery',
    );
  });
}
...
test('ask-side admin bypass: bare "Texas" remains on the guarded recovery path', async (t) => {
  ...
  assert.equal(
    calls.length,
    2,
    'bare state names are not an explicit admin-scoped ask',
  );
});
...
test('ask-side admin bypass: admin level 2/3 result types never grant a township bypass', async (t) => {
  const fixtures = new Map([
    [
      'FB-3 township level 2 fixture',
      ['administrative_area_level_2', 'locality', 'political'],
    ],
    [
      'FB-3 township level 3 fixture',
      ['administrative_area_level_3', 'locality', 'political'],
    ],
  ]);
  const calls = [];
  installGoogleMocks(t, async (url) => {
          ...
          results: [
            {
              ...
              geometry: {
                location: { lat: CAPITOL_ANCHOR.lat, lng: CAPITOL_ANCHOR.lon },
              },
            },
          ],
        }),
...
for (const [target, deferFootprint] of [
  ...
  test(`address-only geocode retains landmark identity: ${target}`, async (t) => {
    const capitol = squareWay(CAPITOL_ANCHOR, 0, 0, 20000, {
      building: 'yes',
      name: 'United States Capitol',
    });
    const hotel = squareWay(CAPITOL_ANCHOR, 620, -140, 3000, {
      building: 'yes',
      tourism: 'hotel',
    ...
    assert.deepEqual(
      outline.ring,
      capitol.geometry.map((p) => [p.lon, p.lat]),
    );
...
test('address-only landmark without a matching outline keeps its geocoded point', async (t) => {
  ...
  const result = await resolveAnnotationTarget({
    viewer: capitolViewer(),
    target: 'US Capitol building, Washington',
    entityKind: 'building',
    footprint: true,
    deferFootprint: true,
  });
  ...
  assert.deepEqual(
    ...
    [CAPITOL_ANCHOR.lat, CAPITOL_ANCHOR.lon],
  );
...
test('missing address components do not turn a formatted city address into the landmark name', async (t) => {
  ...
  const result = await resolveAnnotationTarget({
    viewer: capitolViewer(),
    target: 'United States Capitol building',
    footprint: true,
  });
  assert.deepEqual(
    result.ring,
...
test('a genuine geocoded feature name still canonicalizes an alternate user name', async (t) => {
  ...
  const decoy = squareWay(CAPITOL_ANCHOR, 620, -140, 3000, {
    building: 'yes',
    name: 'Congress meeting building',
  });
  installCapitolMocks(
    t,
    [decoy, capitol],
    [{ long_name: 'United States Capitol', types: ['landmark'] }],
  );
  const result = await resolveAnnotationTarget({
    viewer: capitolViewer(),
    target: 'Congress meeting building',
    footprint: true,
  });
  assert.deepEqual(
    result.ring,
...
test('keyless: a key that geocodes to nothing still anchors the annotation', async (t) => {
  ...
  assert.equal(
    requests.filter((url) => url.includes('maps.googleapis.com')).length,
    1,
  );
  assert.equal(
    requests.filter((url) => url.includes('photon.komoot.io')).length,
    1,
  );
});
...
test("keyless: OSM matching uses the feature's canonical name, not the user's words", async (t) => {
  ...
  const decoy = squareWay(ANCHOR, -250, -250, 3000, {
    building: 'yes',
    tourism: 'hotel',
    name: 'Texas Capitol Austin Visitor Center',
  });
  const monument = squareWay(ANCHOR, 5, 5, 300, {
    tourism: 'artwork',
    name: 'Tejano Monument',
  });
  ...
  installGoogleMocks(t, async (url, init) => {
    ...
    if (href.startsWith('https://maps.googleapis.com/'))
      return GOOGLE_FOUND_NOTHING;
    ...
    return {
      ...
      status: 200,
      headers: { get: () => null },
      json: async () => ({ elements: [decoy, monument] }),
    };
  });
  ...
  assert.ok(
    resolved.ring,
    'the monument polygon is monument-scale and exactly named — it must outline',
  );
...
test('keyless: a Photon outage is not remembered as "no such place"', async (t) => {
  ...
  assert.equal(
    await resolveAnnotationTarget({ viewer: closeViewportViewer(), target }),
    null,
  );
  const duringOutage = requests.length;

  photonReachable = true;
  const retried = await resolveAnnotationTarget({
    viewer: closeViewportViewer(),
    target,
  });
...
test('a not-found is remembered only when every source consulted gave a verdict', async () => {
  ...
  for (const [sources, expected] of cases) {
    const providers = [
      sources.googleConsulted ? Boolean(sources.googleSaysNoSuchPlace) : true,
      sources.photonAnswered,
    ].map((answered) => ({ geocode: async () => ({ place: null, answered }) }));
    assert.equal(
      (await createPlaceSearch({ providers }).geocode('miss')).answered,
      expected,
