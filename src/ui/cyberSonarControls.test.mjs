...
test('real runner and facade use visual settings, preserve visibility and return actual values', async () => {
  ...
  try {
    ...
    const result = await runner('set_cyber_sonar', {
      enabled: false,
      intensityPct: 73,
      sectorDeg: 9,
    });
    ...
    const invalid = await runner('set_cyber_sonar', {
      rings: 6,
      opacityPct: 0,
    });
    ...
    const unavailable = createGevActionRunner({
      styleManager: {},
      viewer,
      dataManager: {},
    });
    assert.equal(
      (await unavailable('set_cyber_sonar', { enabled: true })).ok,
      false,
    );
  } finally {
...
test('maps all settings to the slider owner and reads actual state, including zero', () => {
  const f = fixture();
  const result = setCyberSonarControls(
    f.owner,
    {
      enabled: false,
      rings: 6,
...
test('requires Cyber and preserves off state and every omitted setting', () => {
  for (const theme of ['tactical', 'operator', 'minimal']) {
    ...
    assert.equal(
      setCyberSonarControls(f.owner, { enabled: true }, f.doc).ok,
      false,
    );
    assert.deepEqual(f.calls, []);
  }
