...
function fixture(theme = 'cyber') {
  ...
  const display = node('pp-toggles'),
    context = node('global-context-panel'),
    cctv = node('cctv-panel'),
    radio = node('radio-panel');
  ...
  const owner = {
    ...
    shareLinkManager: {
      claimRestoreLane: (...args) => claimed.push(args),
      onPanelStateChange() {},
    },
  };
  ...
  return {
    ...
    claimed,
    doc: {
      documentElement: { dataset: { uiTheme: theme } },
...
test('explicit Cyber opening collapses peers and claims their restoration lanes', () => {
  ...
  try {
    ...
    assert.ok(
      f.saved.some(
        ([id, collapsed]) => id === 'global-context-panel' && collapsed,
      ),
    );
