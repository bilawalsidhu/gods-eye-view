...
test('the voice TOOL SCHEMA matches the pinned release — the mission mapping is instructions only', () => {
  ...
  // Re-derived for Local ADS-B (one enum value), the Fire Perimeters layer and
  // Web Receivers (three additive tools, three enum additions); the missions
  // still ride existing tools.
  assert.equal(block.length, 32700, 'serialized tool schema length drifted');
  assert.equal(
    crypto.createHash('sha256').update(block).digest('hex'),
    'a853bbce864fbf695a105803330269d020daf3fd35877b8ee9554680e12a58e7',
