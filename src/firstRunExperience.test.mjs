...
test('welcome params work in both directions and outrank both suppressions', () => {
  ...
  assert.equal(
    shouldShowFirstRun({ ...fresh(), location: { search: '?welcome=0' } }),
    false,
  );
...
test('privacy-restricted storage fails open and every write stays best-effort', () => {
  ...
  assert.equal(
    shouldShowFirstRun({ storage: blocked, sessionStorageRef: blocked }),
    true,
  );
...
test('a THROWING storage getter still fails open — Safari private mode', () => {
  ...
  const savedSession = Object.getOwnPropertyDescriptor(
    globalThis,
    'sessionStorage',
  );
  const hostile = {
  ...
  try {
    ...
    if (savedSession)
      Object.defineProperty(globalThis, 'sessionStorage', savedSession);
...
test('no storage is touched from a default parameter position', () => {
  const module = fs.readFileSync(
    new URL('./firstRunExperience.js', import.meta.url),
    'utf8',
  );
  ...
  assert.equal(
    globalReads.length,
    2,
    'exactly two global storage reads, both in resolveStore',
  );
  const resolver = code.slice(
    code.indexOf('function resolveStore'),
    code.indexOf('function readStored'),
  );
  assert.equal(
  ...
  assert.match(
    resolver,
    /try \{[\s\S]*globalThis\.sessionStorage[\s\S]*\} catch/,
  );
...
test('the JS and CSS lists of screen-claiming surfaces stay in step', () => {
  ...
  const inCss = [
    ...hideRule.matchAll(/body\.([a-z-]+) #first-run-launcher/g),
  ].map((m) => m[1]);
  assert.deepEqual(
    inCss.sort(),
...
test('exclusiveSurfaceActive reads the live body classes', () => {
  ...
  for (const name of EXCLUSIVE_SURFACE_CLASSES) {
    assert.equal(
      exclusiveSurfaceActive(make([name])),
      true,
      `${name} must count as exclusive`,
    );
  }
...
test('the key handler refuses to act for a card that is not really on screen', () => {
  ...
  assert.match(
    module,
    /&&\s*root\.getClientRects\(\)\.length > 0 &&\s*!coveredByOverlay\(\);/,
  );
  const keyboard = fs.readFileSync(
    new URL('./ui/surfaceKeyboard.js', import.meta.url),
    'utf8',
  );
  const handler = keyboard.slice(
    keyboard.indexOf('const onKeyDown = (event) => {'),
  );
  assert.match(
...
test('a refused write takes the tick back instead of promising "never again"', () => {
  ...
  assert.match(
    handler,
    /box\.checked = !wanted;/,
    'a refused write must revert the tick',
  );
});
...
test('a surface class that never clears is an ACCEPTED no-show, not a timer', () => {
  ...
  assert.ok(
    accepted.length > 0,
    'the acceptance must be written where the next editor will read it',
  );
  assert.match(
    accepted,
  ...
  assert.match(
    state,
    /a surface class that never clears means no launcher for that page/i,
  );
});
...
test('the scroll fade only appears when the list really overflows', () => {
  ...
  assert.match(
    css,
    /\.first-run-choices\[data-scrollable='true'\] \{[\s\S]*?mask-image/,
  );
  const bareFade =
    css.match(/^\s*\.first-run-choices \{[\s\S]*?\n\}/m)?.[0] || '';
  assert.doesNotMatch(
    bareFade,
    /mask-image/,
    'the fade must never apply unconditionally',
  );
...
test('the launcher yields on engage and waits when a surface is already up', () => {
  ...
  assert.doesNotMatch(
    module,
    /setInterval|requestAnimationFrame\(function poll/,
  );
});
  ...
  return {
    ...
    deps: {
      setContextMode: async (mode) => {
        ...
        return contextOk
          ? { ok: true, mode }
          : { ok: false, failedLayerIds: ['rocket-launches'] };
      },
...
test('the menu is the four owner-ordered missions', () => {
  ...
  assert.equal(
    FIRST_RUN_MISSIONS.infrastructure,
    undefined,
...
test('the tile is the FULLY CONFIGURED experience: quakes and fires together', () => {
  ...
  const table = module.slice(
    module.indexOf('  environmental: Object.freeze({'),
    module.indexOf('  explore:'),
  );
...
test('a refused layer fails the mission by name, and a stalled flight never does', async () => {
  ...
  const flightDown = missionSpy({
    globe: () => {
      throw new Error('cancelled');
    },
  });
  assert.equal(
    (await runFirstRunChoice('environmental', flightDown.deps)).ok,
    true,
  );
});
...
test('the fires/quakes tile name is switchable from one constant', () => {
  ...
  assert.equal(
    ...
    environmentalLabel(ENVIRONMENTAL_LABEL_CHOICE).title,
  );
});
...
test('no mission writes a preference the visitor did not choose by picking it', () => {
  ...
  const code = module.slice(
    module.indexOf('export function shouldShowFirstRun'),
  );
  ...
  for (const forbidden of [
    ...
    assert.doesNotMatch(
      ...
      `a mission must never touch ${forbidden}`,
    );
  }
  ...
  assert.equal(
    panelWrites.length,
    1,
    'exactly one panel reveal, on the Context path',
  );
  const contextPath = code.slice(
    code.indexOf('setContextMode: async (mode)'),
    code.indexOf('setLayerEnabled:'),
  );
  assert.match(
    contextPath,
    /result\?\.ok[\s\S]*?setPanelCollapsed\?\.\('global-context-panel', false, \{\s*explicit: true,?\s*\}\)/,
  );
});
...
test('the decision table is written down where the next editor will read it', () => {
  ...
  for (const row of ['TOUCHED, DURABLE', 'TOUCHED, SESSION', 'NOT TOUCHED']) {
    assert.ok(
      module.includes(row),
      `decision table is missing its "${row}" rows`,
    );
  }
...
test('markup, startup ordering and accessibility remain pinned', () => {
  ...
  assert.match(
    html,
    /data-first-run-status[^>]*role="status"[^>]*aria-live="polite"/,
  );
  ...
  assert.match(
    visible,
    /fires?/i,
    'the tile must promise the fires it enables',
  );
  ...
  const order = [...html.matchAll(/data-first-run-choice="([a-z-]+)"/g)].map(
    (match) => match[1],
  );
  ...
  assert.match(
    startup,
    /initializeWelcome\?\.\(\{ styleManager, dataManager \}\)/,
  );
  ...
  assert.ok(
    reducedMotion.some(
      (block) =>
        block.includes('#first-run-launcher') &&
        block.includes('.first-run-choices'),
    ),
  ...
  const base = css.slice(
    css.indexOf('#first-run-launcher {'),
    css.indexOf('#first-run-launcher.visible'),
  );
  assert.match(base, /pointer-events: none/);
  assert.match(
    css,
    /#first-run-launcher\.visible \{[\s\S]*?pointer-events: auto/,
  );
...
test('the DISPLAY rail starts collapsed on a first run, and a stored choice wins', () => {
  const ui = fs.readFileSync(
    new URL('./ui/panelPositionControls.js', import.meta.url),
    'utf8',
  );
...
test('the voice TOOL SCHEMA matches the pinned release — the mission mapping is instructions only', () => {
  ...
  const mapping = instructions.slice(
    instructions.indexOf('NAMED VIEWS are shorthand'),
  );
  ...
  for (const layerId of [
    ...
    assert.ok(
      paragraph.includes(layerId),
      `mapping must name the existing ${layerId} enum value`,
    );
  }
...
test('every layer a mission drives is already in the shipped set_layer_visibility enum', () => {
  const tool = GEV_REALTIME_TOOLS.find(
    (tool) => tool.name === 'set_layer_visibility',
  );
  const allowedLayers = tool.parameters.properties.layerId.enum;
  const missionLayerIds = Object.values(FIRST_RUN_MISSIONS).flatMap(
    (mission) => mission.layerIds || [],
  );
  assert.ok(missionLayerIds.length > 0);
  for (const layerId of missionLayerIds) {
    assert.ok(
      allowedLayers.includes(layerId),
      `${layerId} must already be an allowed enum value`,
    );
  }
});
