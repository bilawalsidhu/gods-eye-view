...
function fixture() {
  ...
  class Node {
    ...
    get nextElementSibling() {
      return (
        this.parentNode?.children[this.parentNode.children.indexOf(this) + 1] ||
        null
      );
    }
    get textContent() {
    ...
    insertBefore(node, anchor) {
      assert.ok(
        anchor === null || anchor.parentNode === this,
        'insert anchor belongs to its parent',
      );
    ...
    matches(selector) {
      if (selector === 'button[data-signal-layer][data-signal-id]') {
        return (
          this.tagName === 'BUTTON' &&
          this.dataset.signalLayer != null &&
          this.dataset.signalId != null
        );
      }
    ...
    focus() {
      assert.ok(
        this.isConnected,
        'a focus destination must still be connected',
      );
      focusCalls += 1;
  ...
  const selected = [];
  controller.services.militaryAwarenessLayer = {
...
const item = (id, overrides = {}) => ({
  ...
  tone: 'nearby',
  title: `Flight ${id}`,
  detail: 'COMMERCIAL FLIGHT · 2 KM',
  target: { layerId: 'flights', id },
  timestamp: 1_700_000_000_000,
  ...overrides,
});
const render = (f, items) => {
  f.controller.signalItems = items;
  f.controller.renderCockpitSignals();
};
  ...
  cohorts: [
    {
      id: 'flights',
      count: 1,
      nearest: [{ id: 'nearby', label: 'NEARBY', distanceM }],
    },
  ],
});
...
test('actual Context refresh retains the focused signal button across unchanged and changed details', () => {
  ...
  const target = f.buttons()[1];
  target.focus();
  const calls = f.focusCalls();
  for (let tick = 0; tick < 8; tick += 1)
    f.controller.updateCockpitSignals(snapshot(2500), 0);
  ...
  assert.equal(
    f.focusCalls(),
    calls,
    'refresh must preserve focus without repeatedly restoring it',
  );
...
test('signal content and accessible name update without replacing the active target', () => {
  ...
  assert.equal(
    f.controller.signalList.children[0].className,
    'track actionable',
  );
  assert.equal(f.document.activeElement, target);
  assert.equal(f.losses(), 0);
...
test('signal ranking changes move surrounding rows without disconnecting the focused row', () => {
  ...
  for (const order of [
    ...
    render(
      f,
      order.map((id) => item(id)),
    );
    assert.deepEqual(
      f.buttons().map((button) => button.dataset.signalId),
      order,
    );
    assert.equal(f.document.activeElement, target);
    assert.equal(f.losses(), 0);
...
test('a new pushed signal preserves existing focus and delegated selection reads the current target', () => {
  const f = fixture();
  render(f, [item('a'), item('b')]);
  const target = f.buttons()[1];
  target.focus();
  f.controller.pushCockpitSignal(
    'status',
    'warning',
    'INPUT UNKNOWN',
    'SOURCE UNAVAILABLE',
...
test('removing the focused identity uses the existing briefing tab once and permits forward traversal', () => {
  ...
  assert.equal(
    ...
    f.display,
    'later refresh must not pull focus back',
  );
  f.tab();
...
test('a changed target cannot reuse a focused action for another flight', () => {
  const f = fixture();
  render(f, [item('a', { key: 'shared-status-key' })]);
  const removed = f.buttons()[0];
  removed.focus();
  render(f, [
    item('b', {
      key: 'shared-status-key',
      target: { layerId: 'military', id: 'b' },
    }),
  ]);
  const target = f.buttons()[0];
