...
test('the row renders exactly the five accepted sources', () => {
  ...
  assert.deepEqual(container.children.map((chip) => chip.dataset.stackId), [
    'photoreal', 'bing-aerial', 'bing-labels', 'osm', 'gibs-nrt',
  ]);
  assert.deepEqual(container.children.map(chipText), [
    'Google 3D', 'Bing Aerial', 'Bing Labels', 'OSM', 'NASA Imagery',
  ]);
  assert.deepEqual(PRESENTED_MAP_STACK_IDS, ['photoreal', 'bing-aerial', 'bing-labels', 'osm', 'gibs-nrt']);
