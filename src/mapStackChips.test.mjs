...
// it projects the accepted six-source allowlist from the controller's
...
const CONTROLLER_STACKS = [
  ...
  { id: 'gibs-nrt', label: 'NASA Imagery', requiresIon: false, available: true, unavailableReason: null },
];
...
test('the row renders exactly the six accepted sources', () => {
  ...
  assert.deepEqual(container.children.map(chipText), [
    'Google 3D', 'Bing Aerial', 'Bing Labels', 'Esri Satellite', 'OSM', 'NASA Imagery',
  ]);
  assert.deepEqual(PRESENTED_MAP_STACK_IDS, ['photoreal', 'bing-aerial', 'bing-labels', 'esri-imagery', 'osm', 'gibs-nrt']);
