...
| **OpenStreetMap (Nominatim)** | Reverse-geocoded place label in the cockpit Local Info page, and last-resort forward geocoding via `/api/geocode` when Google and Photon do not answer | ODbL 1.0 + Nominatim usage policy (1 req/s) | "© OpenStreetMap contributors" |
...
- **Place search.** Named-place fly-to uses Google Geocoding when a Maps key is configured, then Photon's public instance, then `/api/geocode` (Nominatim search) when those do not answer. `/api/geocode` shares the cockpit Nominatim 1 req/s queue, caches short-lived results, and is rate-limited per client.
