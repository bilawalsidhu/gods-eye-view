...
| **Kreis Warendorf webcams** | Webcam frames, Kfz-Zulassungsstellen Warendorf and Beckum (`config/cctv_sources.warendorf.json`) | Public municipal webcam data | "Kreis Warendorf — kreis-warendorf.de" (courtesy); positions derived from OSM building geometry, "© OpenStreetMap contributors" (ODbL) |
...
- **Warendorf webcams.** Three municipal webcams published by Stadt Warendorf and Kreis Warendorf, registered from the curated `config/cctv_sources.warendorf.json`; only the two municipal image hosts are proxied, frames are fetched at request time and never stored or redistributed. Disable with `CCTV_WARENDORF_ENABLED=0`.
