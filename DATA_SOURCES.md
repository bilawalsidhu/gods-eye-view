...
| **Transport Department, Hong Kong (data.gov.hk)** | CCTV camera catalog + frames, Hong Kong | [data.gov.hk Terms and Conditions of Use](https://data.gov.hk/en/terms-and-conditions) — attribution | "Transport Department, HKSAR — data.gov.hk" |
| **DriveBC (images.drivebc.ca)** | Highway camera catalog + frames + orientation, British Columbia | [Open Government Licence – British Columbia](https://www2.gov.bc.ca/gov/content/data/policy-standards/data-policies/open-data/open-government-licence-bc) | "DriveBC — BC Ministry of Transportation and Transit" |
| **Ontario 511 (511on.ca)** | Camera views + frames, Ontario; optional extra 511-platform feeds (NY, GA, LA, AB, …) via `CCTV_511_FEEDS` with operator-issued keys | [Open Government Licence – Ontario](https://www.ontario.ca/page/open-government-licence-ontario); other feeds under their operator's developer terms | "Ontario 511 — Ministry of Transportation" (others: the operator named in the feed config) |
| **Fintraffic Digitraffic** | Weather-camera stations + frames, Finland | [CC BY 4.0](https://www.digitraffic.fi/en/terms-of-service/) | "Source: Fintraffic / digitraffic.fi, license CC 4.0 BY" |
| **NZ Transport Agency Waka Kotahi (trafficnz.info)** | Traffic camera catalog + frames + travel direction, New Zealand | [NZTA open data terms](https://nzta.govt.nz/traffic-and-travel-information/use-our-data) — attribution | "NZ Transport Agency Waka Kotahi" |
| **Windy.com Webcams API** (optional, `WINDY_API_KEY`) | Global webcam directory: positions, titles, preview frames | [Windy Webcams API terms](https://api.windy.com/webcams/terms) — every image links to its Windy page, "Webcams provided by Windy.com" credit, images never upscaled, free-tier image tokens expire after 10 min | "Webcams provided by Windy.com" + per-camera link |
...
- **Open-data camera packs (Hong Kong TD, DriveBC, Ontario 511 and other 511-platform feeds, Fintraffic Digitraffic, NZTA, Windy).** Each pack is one catalog fetch through the same 15-minute source cache, normalized into the registry's camera shape, capped per pack (`CCTV_<PACK>_MAX_SOURCES`) and prioritized around a few anchor cities like Caltrans; `CCTV_<PACK>_ENABLED=0` switches one off. Image URLs are pinned to each provider's official host, so a poisoned catalog can never redirect the frame proxy. Published facing directions (DriveBC orientation, Ontario "Looking East" view descriptions, NZTA travel direction) become high-confidence heading priors; Hong Kong, Digitraffic and Windy cameras start from the low-confidence personality and the id-hashed heading. Extra 511-platform feeds are configured entirely by the operator in `CCTV_511_FEEDS` (URL with the operator's key, image origin, provider, license, anchors) — GEV ships no keys. Windy is off until `WINDY_API_KEY` is set; its free-tier preview URLs carry tokens that expire after 10 minutes, so the frame route refreshes a camera's preview through the single-webcam endpoint when its cached URL is older than 8 minutes, each Windy camera carries a link back to its Windy page (required by the terms), and frames are never upscaled. Frames from every pack are fetched per request and never stored.
