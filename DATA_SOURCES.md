...
| **GDELT Project DOC 2.0** | Fail-soft fallback for location-matched cockpit headlines, and the World News globe layer | [GDELT Terms of Use](https://www.gdeltproject.org/about.html#termsofuse): unrestricted academic/commercial/governmental dataset use, with citation and link required; linked articles retain publisher terms | "GDELT Project" plus each article's linked publisher/domain. World News pins are outlet country, not verified incident locations. |
...
- **World Desk historical catalog.** Bundled major recorded events from 3000 BCE to the present (`src/data/local_data/world_desk_events.json`). This is a teaching chronology, not a complete newscast. Religious lifetimes are flagged traditional. Coordinates are approximate.
- **World News layer.** `/api/world-news` queries GDELT DOC 2.0 for a 24-hour crisis/election/disaster theme slice (75 articles), caches a successful response for five minutes, archives one JSON file per UTC day under `.gev-cache/world-news/` (gitignored), and may serve that snapshot stale for up to 30 minutes. Pins are clustered by **outlet country**, not event location; empty countries and unknown source-country labels are dropped. Headlines are coverage mentions with publisher links — not verified incidents, risk rankings, or evidence a place is safe. GDELT asks for about one request every five seconds; the cache plus a 8/12 per-minute limiter keep the proxy inside that. The Data attribution popover cites GDELT; each article retains its publisher.
