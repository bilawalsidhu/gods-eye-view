...
| **NOAA National Weather Service API** | Active U.S. tornado, tsunami, severe thunderstorm, flood, winter weather, heat, high-wind, and fire-weather alerts | U.S. government open data; reasonable rate limits | "NOAA National Weather Service" |
| **NOAA Storm Prediction Center** | Preliminary daily tornado, hail, and wind reports | U.S. government open data; reports are preliminary | "NOAA Storm Prediction Center" |
...
- **NOAA hazards.** The alert layers use the NWS active-alert GeoJSON feed. The tornado layer also uses the SPC previous-day storm-report CSV; SPC reports are preliminary and are not warnings. Some NWS alerts have no inline geometry, so the server resolves their allowlisted `affectedZones` through NOAA forecast-zone GeoJSON and uses the validated zone-center average as a presentation point; it never geocodes alert text. The server proxy sends a descriptive `User-Agent`, uses fixed upstream URLs, applies response caps and timeouts, caches successful responses for one minute and forecast zones for six hours, and serves a last-good response for up to 15 minutes during transient outages. Missing or malformed feeds never replace the last rendered snapshot. These layers are informational and do not replace official emergency instructions.
