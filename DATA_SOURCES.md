...
| **MeshCore public node map** (map.meshcore.io) | Mesh Network layer — worldwide LoRa mesh node positions, type, and radio parameters | Community-run, keyless, no published ToS found; nodes are self-reported by their operators via the MeshCore app | "MeshCore (map.meshcore.io)" |
...
- **MeshCore public node map.** `/api/meshcore/nodes` fetches the full, keyless `map.meshcore.io/api/v1/nodes?short=1` feed (the same open data that powers community deployments like cascadiamesh.org/map), trims each record to the fields the globe renders (position, type, name, radio params, last-seen), and caches the result in memory + on disk for 10 minutes — a single shared upstream fetch regardless of how many clients ask. Node positions and radio parameters are self-reported by their operators through the MeshCore app and are not independently verified; a node's on-map freshness color reflects when it was last heard from, not its current operational status. No API key exists for this feed.
