...
| **Fintraffic Digitraffic (weather cameras)** | CCTV camera catalog + frames, Finland — keyless | [CC BY 4.0](https://www.digitraffic.fi/en/support/instructions/) — attribution REQUIRED; send a `Digitraffic-User` header and `Accept-Encoding: gzip` | "Contains data from Fintraffic Digitraffic, CC BY 4.0" |
| **Ontario 511** | CCTV camera catalog + frames, Ontario — keyless (the only 511 deployment that is) | Open Government Licence – Ontario | "Ontario 511 — 511on.ca" |
| **DriveBC** | CCTV camera catalog + frames, British Columbia — keyless; publishes real orientation and elevation | Province of British Columbia open data | "DriveBC.ca — Province of British Columbia" |
| **WSDOT (Washington State DOT)** | CCTV camera catalog + frames, Washington — **opt-in, needs a free `WSDOT_ACCESS_CODE`** | Public WSDOT traveler information data | "Washington State Department of Transportation — wsdot.wa.gov" |
| **Live Traffic NSW** | CCTV camera catalog + frames, New South Wales — **opt-in, needs a free `NSW_API_KEY`** | [Transport for NSW Open Data](https://opendata.transport.nsw.gov.au/) — CC BY 4.0, attribution REQUIRED | "Live Traffic NSW — Transport for NSW (CC BY 4.0)" |
| **LTA DataMall (Traffic Images)** | CCTV camera catalog + frames, Singapore — **opt-in, needs a free `LTA_ACCOUNT_KEY`** | [Singapore Open Data Licence v1.0](https://datamall.lta.gov.sg/content/datamall/en/SingaporeOpenDataLicence.html) — attribution REQUIRED; commercial use and redistribution permitted | "Contains information from LTA DataMall, made available under the terms of the Singapore Open Data Licence version 1.0" |
