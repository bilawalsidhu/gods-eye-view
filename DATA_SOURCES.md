...
- **Precomputed camera ground heights.** `config/cctv_ground_heights.json` holds, per camera, the WGS84 ellipsoidal ground height Google Photorealistic 3D Tiles report at the camera's position and under the nine support points of its monitor plane, sampled by `scripts/precompute-cctv-heights.mjs` from a running app. It is placement data derived from the rendered surface, not imagery, and is regenerated whenever a pack's poses change (each entry is keyed to the pose it was sampled for).
