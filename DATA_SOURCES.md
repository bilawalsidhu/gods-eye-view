...
| **Windy Webcams API v3** | Public webcam discovery and current stills across Geneva, Vaud, and Valais (optional, BYOK) | [Windy Webcams API terms](https://api.windy.com/webcams) | "Webcams provided by windy.com — add a webcam"; each preview links to its Windy source page |
...
- **Windy Webcams.** With `WINDY_WEBCAMS_API_KEY` configured, the server fetches active webcams in Geneva, Vaud, and Valais from API v3, validates Windy's catalog, detail, and image URLs, and exposes stills only through the existing registered-source CCTV proxy. Catalog refreshes renew expiring image URLs; keys remain server-side. Set `CCTV_WINDY_ENABLED=0` to disable the pack or `CCTV_WINDY_MAX_SOURCES` to change its 450-camera cap.
