...
| **adsb.lol** | Military flights (merged, see below) + aircraft traces | ODbL 1.0 | "adsb.lol" (ODbL) |
| **adsb.fi** | Military flights — merged into the military layer alongside adsb.lol | ODbL 1.0 | "adsb.fi contributors" (ODbL); `opendata.adsb.fi/api/v2/mil` |
| **airplanes.live** | Military flights — merged into the military layer alongside adsb.lol | ODbL 1.0; [airplanes.live is free for personal/non-commercial use, asks for a descriptive User-Agent and no aggressive polling](https://airplanes.live/) | "airplanes.live" (ODbL); `api.airplanes.live/v2/mil` |
...
| **NASA public affairs live video** | ISS live video in the SAT FEED panel (embedded YouTube channel stream) | U.S. public-domain broadcast; YouTube embed subject to YouTube ToS. Optional "24/7 Earth" source is a third-party rebroadcast (Space Videos) | "ISS live video: NASA" (+ rebroadcaster when the Earth-view source is selected) |
| **NOAA STAR / NESDIS** | GOES-19 (East) & GOES-18 (West) GeoColor full-disk stills in the SAT FEED panel (`cdn.star.nesdis.noaa.gov/.../GEOCOLOR/latest.jpg`, keyless, CORS-open, ~10 min old) | U.S. public domain | "GOES-19 / GOES-18 GeoColor: NOAA STAR / NESDIS" |
| **NICT** (from JMA Himawari-9) | Himawari-9 full-disk stills in the SAT FEED panel (`himawari8-dl.nict.go.jp`, keyless; frame timestamp computed client-side, ~30 min old) | NICT real-time web imagery, processed from JMA Himawari-9 data | "Himawari-9 full disk: NICT (JMA data)" |
...
| **NYC DOT (webcams.nyctmc.org)** | CCTV camera catalog + frames, New York City | [NYC Open Data Terms of Use](https://www.nyc.gov/html/data/terms.html) | "NYC DOT — webcams.nyctmc.org" |
| **foto-webcam.eu** | Weather / landscape webcam catalog + frames, Austria (`country=at`) | [CC BY-NC 3.0](https://creativecommons.org/licenses/by-nc/3.0/) — non-commercial, attribution REQUIRED | "© foto-webcam.eu" |
| **QLDTraffic (via queensland.opendatasoft.com)** | CCTV camera catalog + frames, Queensland, Australia | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) — attribution REQUIRED | "© State of Queensland (QLDTraffic)" |
| **Transport for NSW (Live Traffic Cameras API)** | CCTV camera catalog + frames, Sydney / NSW, Australia — **BYO key** (`NSW_TRANSPORT_API_KEY`, free from opendata.transport.nsw.gov.au) | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) — attribution REQUIRED | "© Transport for NSW" |
| **YouTube live "place cams"** | ~57 curated public YouTube live streams of notable places, folded into the CCTV layer as `feedType: 'youtube'` cameras (`src/data/placeCamSeeds.js`). Play in the CCTV panel via the YouTube IFrame API; **no** proxy, keyless. Video ids rotate — the panel probes candidates and falls back to the operator's channel `/live` page | Per-stream operator terms (explore.org, EarthCam, Skyline Webcams, individual operators); YouTube ToS for embeds. Ids are curated best-effort and drift over time | "Place webcams: public YouTube live streams (per-channel operators)" |
...
| **NASA EOSDIS GIBS** (Global Imagery Browse Services) | "NASA Imagery" basemap stack — near-real-time satellite imagery (VIIRS/SNPP Corrected Reflectance true colour, EPSG:4326 WMTS, keyless) | U.S. public-domain data; [GIBS citation guidance](https://nasa-gibs.github.io/gibs-api-docs/) — acknowledgement requested | "Imagery: NASA EOSDIS GIBS (VIIRS, Suomi NPP)" |
...
- **Merged military feed.** `/api/adsblol/mil` fans out to three `/v2/mil` endpoints — adsb.lol, adsb.fi, and airplanes.live — in parallel (8 s per-feed timeout), unions their rows by ICAO hex, and keeps the copy with the freshest position when an airframe is in more than one. Each network runs an independent set of volunteer receivers, so a contact one has lost — or never received — is often still live on another. Feeds that error or time out are skipped, not fatal; if all three fail a stale cached response is served. The response is cached 12 s and its `X-ADS-B-Sources` header names the feeds that contributed. Still ADS-B only: an aircraft transmitting nothing appears on no feed. All three publish ODbL data; airplanes.live additionally asks for a descriptive `User-Agent` and no aggressive polling, which the 12 s cache and 15 s layer poll satisfy.
...
- **NASA GIBS imagery.** Keyless (no API key), fetched client-side straight from `gibs.earthdata.nasa.gov` (CORS-enabled). The "NASA Imagery" basemap stack builds a `Cesium.WebMapTileServiceImageryProvider` against the EPSG:4326 "best available" WMTS endpoint for `VIIRS_SNPP_CorrectedReflectance_TrueColor`, with the `TIME` dimension pinned to `now − 3 h` (UTC) so the mosaic is fully populated and rolls back across midnight instead of requesting a not-yet-imaged day. The provider is rebuilt on every activation rather than cached, so re-selecting the chip refreshes to the latest pass. This is near-real-time (~1.5–3 h latency), one daytime pass per location per day — not live video, and not a completeness guarantee. Nothing is cached, stored, or redistributed by the app.
