...
| **Gemeente Amsterdam (DSO-API `verkeersinformatiesystemen`)** | CCTV camera **positions only**, Amsterdam — no frames | "openbaar, tenzij anders aangegeven" (public, except where noted); [dataset docs](https://api.data.amsterdam.nl/v1/docs/datasets/verkeersinformatiesystemen.html) | "Gemeente Amsterdam — Verkeer en Openbare Ruimte — data.amsterdam.nl" (courtesy) |
...
- **Amsterdam camera register.** The keyless `api.data.amsterdam.nl` DSO-API serves the city's traffic-information-systems asset register; the pack reads it filtered to `objectSoort=Camera` and asks for WGS84 with an `Accept-Crs` header (the register is natively EPSG:28992). **This pack carries camera positions, not imagery** — Amsterdam publishes where its ~390 public traffic and ANPR cameras are, but not what they see, so records register with no frame URL and the proxy's existing fallback chain owns the frame: a Street View still where a Google key is configured, otherwise the synthetic placeholder stamped `NO UPSTREAM CONFIGURED`. `/api/cctv/health` reports these as `status: degraded` with `sourceKind: streetview | synthetic`, so no camera in this pack ever claims a live feed. An optional `AMSTERDAM_DATA_API_KEY` is forwarded as `X-Api-Key` ahead of the platform's announced move to mandatory keys. **The publicly viewable live camera feeds around Amsterdam (Rijkswaterstaat's A10/A4/A9 cameras) are deliberately NOT used:** their images are served by a commercial host whose `robots.txt` is `Disallow: /` and which returns a 401 placeholder unless the request carries a forged `Referer`. Reading them would mean circumventing hotlink protection against the operator's stated wishes.
