...
| **OpenStreetMap (Nominatim)** | Keyless forward geocoding for location/voice search when Google is not configured; reverse-geocoded place label in the cockpit Local Info page | ODbL 1.0 + Nominatim usage policy | "© OpenStreetMap contributors" |
| **Foursquare Places API** | Optional POI/venue lookup when Google Places is not configured | Foursquare developer terms and the quota attached to your Service API Key | "Foursquare Places" |
