...
| **TxDOT ITS (its.txdot.gov)** | CCTV camera catalogs + frames, Texas districts | Public TxDOT highway camera data; frames carry a TxDOT watermark | "TxDOT — its.txdot.gov" (courtesy) |
...
- **TxDOT ITS cameras.** The catalog comes from the keyless
  `its.txdot.gov/its/DistrictIts/GetCctvStatusListByDistrict` endpoint, one call per
  district. Frames come from the per-camera `GetCctvSnapshotByIcdId` endpoint, which
  answers JSON carrying a base64 JPEG rather than an image body — the proxy decodes it
  and only ever does so for responses from the official TxDOT origin. Only cameras
  reporting `Device Online` are registered: offline devices still return a stale frame
  (some years old) that would otherwise look live. The district defaults to Austin
  (`CCTV_TXDOT_DISTRICTS`), this project's reference camera city; all 25 districts
  together would be ~4,370 cameras.
