...
- **Merged military feed.** `/api/adsblol/mil` fans out to two `/v2/mil` endpoints — adsb.lol and adsb.fi — in parallel (8 s per-feed timeout), unions their rows by ICAO hex, and keeps the copy with the freshest position when an airframe is in both. Each network runs an independent set of volunteer receivers, so a contact one has lost — or never received — is often still live on the other. A feed that errors or times out is skipped, not fatal; if both fail a stale cached response is served. The response is cached 12 s and its `X-ADS-B-Sources` header names the feeds that contributed. Still ADS-B only: an aircraft transmitting nothing appears on no feed. `mergeMilitaryFeeds` accepts any number of feeds; airplanes.live and adsb.one are not included because as of 2026-09 both gate their API behind manual email approval (keyless requests get HTTP 403).
