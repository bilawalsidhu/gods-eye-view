...
| **Open Calgary** (`data.calgary.ca`, Socrata) | CCTV camera catalog + frames, Calgary | [Open Government Licence – City of Calgary](https://data.calgary.ca) — attribution REQUIRED | "Contains information licensed under the Open Government Licence – City of Calgary" |
...
- **Open Calgary traffic cameras.** Keyless. The catalog is Socrata dataset `k7p9-kppz` (~214 cameras, found through the Socrata catalog API rather than a hardcoded id); frames come from the city's own `trafficcam.calgary.ca` host. The dataset publishes `http://` frame URLs, but the host serves HTTPS and 301-redirects to it, so URLs are upgraded to HTTPS and pinned to that exact origin before registration — the same official-origin pin the TfL pack uses. Attribution is required by the licence.

  **These cameras publish no facing direction, and the fields that look like one are not.** Every record carries a `quadrant` ("NE"/"NW"/"SE"/"SW") and a `camera_location` ending in the same token ("9 Avenue / 3 Street SE"). That is Calgary's address grid — the quarter of the city the intersection sits in — not a camera bearing. Parsing either would hand a confident heading to all ~214 cameras and every one would be wrong. Headings therefore come from the shared id-hash fallback at `headingConfidence: 'low'`, exactly as TfL JamCams do, and are corrected with the in-app calibration gizmo. (This is the same trap `src/data/directionText.js` documents for street names containing cardinal words; Calgary hits it on every row rather than a few.)

  Adding this pack raised the global `CCTV_MAX_SOURCES` default from 900 to 1100: the four live packs total 1014 at their default per-pack caps, and the global cap keeps the first N after merging, so at 900 the last pack merged was silently truncated (Calgary registered 100 of 214). Per-pack caps remain the intended tuning knob.
