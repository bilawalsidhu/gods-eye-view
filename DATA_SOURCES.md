...
| **OpenStreetMap (Overpass API)** | Webcam catalog for the OSM pack — position, `camera:direction`, `camera:mount` and the operator's own `contact:webcam` frame URL | ODbL 1.0 **for the catalog only**; each frame stays the publishing operator's under their own terms | "© OpenStreetMap contributors" plus the operator named in each camera record |
