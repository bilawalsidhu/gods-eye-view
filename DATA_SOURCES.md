...
| **Receiverbook + KiwiSDR map feed** | Web receiver directory (KiwiSDR / WebSDR / OpenWebRX): receiver name, type, position, URL; band coverage, user slots and online state for KiwiSDRs | Receiverbook (openwebrx.de, community directory, no reuse restriction stated) and the dyatlov map-maker KiwiSDR feed at rx.linkfanel.net (derived from the public kiwisdr.com list); directory metadata only — websdr.org's own list forbids reuse and is not used | "Receiverbook" and "KiwiSDR community map feed" plus a link to the selected receiver |
...
- **Web receivers.** `/api/web-receivers/catalog` fetches exactly two server-registered directories — Receiverbook's map page (all three receiver families, with coordinates and URLs) and the community KiwiSDR feed at rx.linkfanel.net (band coverage, user slots, antenna, online state) — merges them by receiver URL, caches the catalog for 30 minutes and may serve the last good catalog for up to seven days during an outage (`degraded`/`stale` are reported). Both requests reject redirects, validate every resolved address as globally routable and pin the connection to a validated address; receiver URLs that point at private or local hosts are dropped. Band coverage for WebSDR and OpenWebRX entries is inferred from their published labels and is marked as such (unknown coverage is never presented as a guarantee). GEV never proxies receiver audio or control: tuning builds the receiver's own URL (`?f=…`, `?tune=…`, `#freq=…`) and loads that page in the panel dock or a new tab, so the receiver operator sees the listener's browser directly and their own terms and user limits apply. websdr.org's list carries an explicit no-reuse notice and is not fetched.
