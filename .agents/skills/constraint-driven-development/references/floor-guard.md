...
- **Input:** the diff between the merge base and the working tree (added _and_ removed lines, plus untracked files). A guard that reads only `git diff` misses new files and staged-but-uncommitted work.
...
// 4. Unfinished work.
const STUBS =
  /throw new (Error|NotImplemented).*[Nn]ot implemented|catch\s*\(\w*\)\s*\{\s*\}|catch\s*\{\s*\}|\bTODO\b|\bpass\s*# *stub/;
...
for (const { file, text } of added) {
  ...
  if (/CONSTRAINTS\.md$/.test(file) && /^\| *(W|E)\d+ *\|/.test(text))
...
for (const { file, text } of removed) {
  if (
    /\.(test|spec)\.|_test\.|test_/.test(file) &&
    /\b(expect|assert|should)\b/.test(text)
  ) {
...
const removedConstraints = removed.filter((l) =>
  /CONSTRAINTS\.md$/.test(l.file),
);
const addedConstraints = added.filter((l) => /CONSTRAINTS\.md$/.test(l.file));
for (const r of removedConstraints) {
  const a = addedConstraints.find(
    (x) => x.text.split(/[|:]/)[0] === r.text.split(/[|:]/)[0],
  );
  if (
    a &&
    nums(a.text).some(
      (n, i) => nums(r.text)[i] !== undefined && n < nums(r.text)[i],
    )
  ) {
