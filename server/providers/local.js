...
import { nominatimToGeocodeResult, nominatimViewboxFromBounds } from '../../src/nominatimGeocode.js';
...
const _nominatimSearchRateLimiter = makeRateLimiter({ windowMs: 60_000, max: 30, globalMax: 90 });
...
async function fetchNominatimSearch(query, bounds) {
  ...
  const rows = await enqueueNominatim(() => fetchRegionalJson(
  ...
  function install(middlewares) {
    middlewares.use('/api/geocode', async (req, res) => {
      ...
      if (!query || query.length > NOMINATIM_SEARCH_MAX_QUERY) {
        res.writeHead(400, { 'Content-Type': 'application/json' });
        res.end(JSON.stringify({ error: 'A place query of 1–200 characters is required' }));
        return;
      }
      try {
        const payload = await fetchNominatimSearch(query, url.searchParams.get('bounds'));
        res.writeHead(200, {
          'Content-Type': 'application/json',
          'Cache-Control': payload.cached ? 'public, max-age=60' : 'no-store',
        });
        res.end(JSON.stringify({ status: payload.status, results: payload.results }));
      } catch {
        res.writeHead(503, { 'Content-Type': 'application/json', 'Cache-Control': 'no-store' });
        res.end(JSON.stringify({ error: 'Place search is temporarily unavailable' }));
      }
    });
