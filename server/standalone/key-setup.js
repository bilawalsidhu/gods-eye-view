...
function keySetupEndpoint({ sourceRoot = defaultSourceRoot } = {}) {
  ...
  return {
    ...
    configureServer(server) {
      // SECURITY: rate-limit the credential management surface. Legitimate
      // use is rare manual key entry; a tight cap stops brute-force and
      // automated manipulation (especially via DNS-rebinding vectors).
      const _keySetupRateLimiter = makeRateLimiter({
        windowMs: 60_000,
        max: 10,
        globalMax: 30,
      });
      server.middlewares.use('/api/setup/status', (req, res) => {
        ...
        if (!_keySetupRateLimiter(clientKey(req)))
