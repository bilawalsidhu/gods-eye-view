...
| `FOURSQUARE_SERVICE_KEY` | Server only | Server proxies Places search through `/api/foursquare/place-search`; the Service API Key never ships |
...
1. **Google Maps API key** — loads Photorealistic 3D Tiles directly and powers Google-backed geocoding/Places when configured. **Restrict it** (HTTP referrer + API restriction to the required Google APIs) in the Google Cloud Console. An unrestricted key in a public deployment can be abused and billed to you.
