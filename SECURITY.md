...
- **Bounded aircraft-track fetches.** Uncached track requests are limited to 30 per socket IP per minute, 60 globally per minute, and four concurrent upstream fetches (at most three OpenSky, reserving a slot for adsb.lol); OpenSky tracks also have a process-local 250-request rolling 24-hour limit (up to 1,000 track credits at four credits each). Same-key requests share a fetch. Track responses have a 5 MiB body cap and a 24 MiB aggregate cache limit; an oversized source gets a five-second per-key cooldown rather than an error cached as a valid track. OpenSky track authentication follows `OPENSKY_AUTH_MODE`, including anonymous and Basic modes, with a 12-second OAuth-token deadline and a Basic retry in `auto` mode after bearer rejection. Daily-quota denials advertise the remaining wait and do not consume minute admission.
- **Coalesced OpenSky state fetches.** Concurrent cache misses for the same auth configuration share one worldwide upstream request; it has a 12-second deadline and a 32 MiB response-body ceiling before a successful snapshot can enter the module cache. OAuth refresh is also coalesced; oversized or failed responses are not cached as new successes. On upstream failure or rate limit, a stale worldwide frame yields to fresh regional data when the view anchor is available; otherwise the last-good frame remains available. Missing budget headers select a conservative five-minute cache TTL; missing retry headers use a two-minute cooldown. Rate-limit responses are handled without reading their bodies.
...
- **Aircraft-track limits are enabled by default**, but are process-local admission guards, not authentication or a provider-side billing cap. They reset on restart; cached responses do not spend their quota. Other cost-bearing proxies still require a trusted network or authenticated reverse proxy when exposed.
