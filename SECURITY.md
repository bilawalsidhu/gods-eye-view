...
- **No verbatim client headers upstream.** The CCTV media proxy is the one route that relays a request header (`Range`, for video seeking). It is parsed and canonicalized before forwarding: one `bytes=` range only, with an explicit span bounded to the same 64 MB cap the relay enforces on the response. Multi-range, malformed, inverted and non-`bytes` values are ignored per RFC 7233 §3.1 and the request proceeds without a `Range`.
