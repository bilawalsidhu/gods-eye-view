...
- **Allowlisted upstream products.** The Weather layer names the Xweather raster layer it wants in the request path. That name is checked against the catalogue before the credential is read, so a request for a product this app does not draw is refused rather than forwarded and billed — forwarded on trust it would be an open proxy to every product on the account, including the ones charging ten times the base rate.
- **Credential scrubbed from Xweather logging.** That vendor authenticates by putting the client id and secret in the URL *path*, not a header or query parameter, so an upstream failure that quotes the request it attempted would write the secret to the server log. Every log line from that proxy is masked first. Client responses never carry upstream text at all.
