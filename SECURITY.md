...
| Codex CLI login (`~/.codex/auth.json`) | Server only, read-only | When `OPENAI_API_KEY` is unset, `/api/realtime/token` mints the same ephemeral secret with the operator's Codex OAuth token (ChatGPT subscription); the file is never refreshed or rewritten, and the token never ships |
