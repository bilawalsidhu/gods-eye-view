...
try {
  ...
  await page.evaluate(() => {
    ...
    for (const type of ['keydown', 'keyup'])
      document.addEventListener(type, recordSpace, true);
    ...
    probe.snapshot = () => ({
      ...
      holdObservation: {
        ...
        focusOwnerMatches:
          document.activeElement === voice.pushToTalkHoldFocusOwner,
      },
    });
    probe.reset = () => {
    ...
    probe.restore = () => {
      ...
      observer.disconnect();
      for (const type of ['keydown', 'keyup'])
        document.removeEventListener(type, recordSpace, true);
  ...
  if (forceKeyless) {
    await page.evaluate(async () => {
      ...
      for (const source of controller._sources.values()) {
        if (
          source.descriptor.requiresIon ||
          source.descriptor.kind === 'photoreal'
        ) {
  ...
  const acquiringFailureArbitration = await page.evaluate(async () => {
    ...
    const waitForQueuedNotice = async (label, timeoutMs = 1000) => {
      ...
      while (
        styleManager._feedback._globalStatusNotice?.label !== label &&
  ...
  await page.evaluate(() => {
    ...
    const probe = {
      transitions: [],
      restore: () => {
        chrome.setPanelCollapsed = original;
      },
    };
