...
async function main() {
  ...
  try {
    ...
    console.log(
      FIXTURES
        ? '(i) FIXTURE — loading synthetic fires through the source adapter...'
    ...
    const live = await bootAndEnable(page);
    {
      ...
      record(
        `${FIXTURES ? 'FIXTURE' : 'LIVE'}: >1000 detections, no error, LIVE label`,
        ok,
        `count=${live.count} error=${JSON.stringify(live.error)} label=${JSON.stringify(live.loadingLabel)}`,
      );
      if (!FIXTURES)
        record(
          'LIVE: /api/firms/status has key + quota statistics',
          status.hasKey === true &&
    ...
    if (!(
      ...
      if (!prepared) {
        ...
        const refreshOk =
          Boolean(refresh.before?.id) &&
          refresh.after?.id === refresh.before.id &&
          refresh.contextId === refresh.before.id &&
          refresh.selections === 0;
        record(
          'REFRESH: selected detection survives without another selection event',
          refreshOk,
