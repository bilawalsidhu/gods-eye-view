...
try {
  ...
  if (!control) {
    const labelLatency = await page.evaluate(
      async (layerId, isLegacy) => {
        const gev = window.__godsEyeView;
        ...
        const labelsUp = () => {
          if (isLegacy) {
            const stats =
              gev.dataManager.layers.get(layerId)?.module?.getStats?.() || {};
    ...
    check(
      'labels became visible after enable',
      labelLatency.timedOut === false,
      labelLatency,
    );
  ...
  const counts = await page.evaluate((layerId) => {
    ...
    for (let i = 0; i < v.dataSources.length; i++) {
      ...
      for (const e of list) {
        ...
        if (e.position && e.position.isConstant === false)
          out.dynamicPositionEntities++;
        if (
          e.polyline?.positions &&
          e.polyline.positions.isConstant === false
        ) {
          out.dynamicPositionEntities++;
        }
      }
  ...
  report(
    control ? 'counts (control, cables OFF)' : 'counts with cables ON',
    counts,
  );
  ...
  report(
    control
      ...
      : 'scene.render cost, cables ON, 10s orbit',
    frameCost,
  );
  ...
  report(
    control
      ...
      : 'parked idle with cables ON (postRender fires / 5s)',
    idle,
  );
  ...
  if (!legacy && !control) {
    ...
    check(
      'zero native LabelGraphics on cable entities',
      counts.nativeLabels === 0,
      counts.nativeLabels,
    );
    check(
      'zero per-frame CallbackProperty cable entities',
      counts.dynamicPositionEntities === 0,
      counts.dynamicPositionEntities,
    );
    check('host has cable entries', counts.hostEntries > 0, counts.hostEntries);
    check(
      'host painted cable labels',
      counts.hostPainted > 0,
      counts.hostPainted,
    );
    ...
    check(
      'disable clears host entries (no orphans)',
      cycle.offEntries === 0 && cycle.offPainted === 0,
      cycle,
    );
