...
const PHRASES = [
  ...
  {
    phrase: 'Show me the Alps from above',
    expect: { oneOf: ['fly_to_location', 'frame_overhead'] },
  },
  ...
  {
    phrase: 'Frame the aircraft near us from overhead',
    expect: 'frame_overhead',
  },

  // — the satellites trap: data layer, never basemap —
  {
    phrase: 'Show me the satellites',
    expect: { oneOf: ['set_layer_visibility', 'frame_overhead'] },
  },
  {
    phrase: 'Turn off the satellites',
    expect: 'set_layer_visibility',
    args: { layerId: 'satellites' },
  },
  ...
  // — layers —
  {
    phrase: 'Turn on the flights layer',
    expect: 'set_layer_visibility',
  ...
  {
    phrase: 'Turn on street traffic',
    expect: 'set_layer_visibility',
    args: { layerId: 'traffic' },
  },
  ...
  {
    phrase: 'Is there anything interesting in view?',
    expect: 'get_entity_context',
  },
  ...
  {
    phrase: 'Track that plane',
    expect: { oneOf: ['track_entity', 'get_entity_context'] },
  },
  {
    phrase: 'Follow the nearest aircraft',
    expect: { oneOf: ['track_entity', 'get_entity_context'] },
  },
  ...
  {
    phrase: 'Play a news radio station near Austin',
    expect: 'control_radio',
    args: { action: 'select', category: 'news', locationId: 'austin' },
  },
  {
    phrase: 'Turn on the radio',
    expect: 'control_radio',
    args: { action: 'play' },
  },
  {
    phrase: 'Set the radio volume to thirty percent',
    expect: 'control_radio',
    args: { action: 'volume', volumePct: 30 },
  },
  {
  ...
  // — annotations —
  {
    phrase: 'Annotate the Texas State Capitol and its grounds',
    expect: 'annotate_map',
  },
  { phrase: 'Outline the state of Texas', expect: 'annotate_map' },
  { phrase: 'Outline Lady Bird Lake', expect: 'annotate_map' },
  {
    phrase: 'Draw the walking route from the Capitol to Zilker Park',
    expect: 'annotate_map',
  },
  {
    phrase: 'How far is the Eiffel Tower from the Louvre?',
    expect: 'annotate_map',
  },
  ...
  {
    phrase: 'What is the biggest fire near Los Angeles?',
    expect: 'analyst_query',
  },
  {
    phrase: 'Is anything flying above forty thousand feet?',
    expect: 'analyst_query',
  },
  ...
  for (let i = 0; i < list.length; i += PHRASES_PER_SESSION) {
    ...
    for (const p of batch) {
      ...
      if (p.expectNone) {
        report(
          names.length === 0,
          `route: "${p.phrase}" → (no tool)`,
          names.length
            ? `unexpected calls: ${names.join(',')}`
            : 'clean conversational turn',
        );
        continue;
      }
      let ok;
      ...
      if (ok && p.args) {
        const call = calls.find((c) =>
          Array.isArray(p.expect) ? true : c.name === p.expect,
        );
        if (!matchArgs(p.args, call?.args)) {
          ok = false;
          detail += ` args mismatch: ${JSON.stringify(call?.args)}`;
        }
      }
      if (ok && p.argsByTool) {
      ...
      report(
        ok,
        `route: "${p.phrase}" → ${Array.isArray(p.expect) ? p.expect.join('+') : p.expect.oneOf ? p.expect.oneOf.join('|') : p.expect}`,
        detail,
      );
    }
    session.close();
...
<<<<<<< HEAD
    ...
    report(
      ...
      `count=${r?.count} scope=${r?.coverage?.scope} err=${r?.error || ''}`,
    );
    r = await run('analyst_query', {
      followUp: true,
...
>>>>>>> 4c1dbe653b2589e5068a1c10e052d5d24249be77
    ...
    report(
      ...
      `distanceM=${r?.distanceM} waypoints=${r?.waypoints} cam=(${camNow.lat.toFixed(3)},${camNow.lon.toFixed(3)}) nearRoute=${nearRoute} stopHadMotion=${stopRes?.stopped}`,
    );
