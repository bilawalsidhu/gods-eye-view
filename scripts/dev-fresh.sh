...
[[ -n "${MAPILLARY_ACCESS_TOKEN:-}" ]] && KEY_SETUP_EXTERNAL_KEYS+=(MAPILLARY_ACCESS_TOKEN)
...
MAPILLARY_ACCESS_TOKEN="${MAPILLARY_ACCESS_TOKEN:-$(read_dotenv_value "MAPILLARY_ACCESS_TOKEN")}"
...
MAPILLARY_ACCESS_TOKEN="${MAPILLARY_ACCESS_TOKEN:-$(read_keychain_secret "mapillary" "access-token")}"
...
[[ -n "${MAPILLARY_ACCESS_TOKEN}" ]] && echo "Mapillary token (street imagery): configured" || echo "Mapillary token (street imagery): not set — imagery layer requires a token"
...
put_env_if_set MAPILLARY_ACCESS_TOKEN "${MAPILLARY_ACCESS_TOKEN}"
