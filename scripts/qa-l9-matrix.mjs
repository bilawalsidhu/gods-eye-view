...
const passWithSkips = (detail, unrun) => ({
  status: PASS_SKIPS,
  detail,
  unrun,
});
...
function normalizeVerdict(res) {
  if (!res || typeof res !== 'object' || Array.isArray(res)) {
    return crash(
      `the runner produced a malformed verdict (${res === undefined ? 'undefined' : JSON.stringify(res)?.slice(0, 80)}) — this check verified nothing`,
    );
  }
  if (!OUTCOMES.has(res.status)) {
    return crash(
      `the runner produced an unknown outcome "${String(res.status)}" — not one of ${[...OUTCOMES].join(' / ')}; this check verified nothing`,
    );
  }
  if (res.status === SKIP && !SKIP_TAGS.has(res.tag)) {
    return crash(
      `a SKIPPED verdict carried an invalid tag "${String(res.tag)}" — a skip must name its reason (${[...SKIP_TAGS].join(' / ')}), or it is indistinguishable from a silent pass`,
    );
  }
  return res;
}
...
function keyGuard(name, state) {
  ...
  if (state === 'error') {
    return fail(
      `${name} key state UNKNOWN — its status endpoint errored or returned a malformed payload on a responsive server, so this check cannot say whether the key is merely absent`,
    );
  }
  return skip(
    `${name} key presence was not probed (--cheap) — this check cannot pick its keyed/keyless branch`,
    'CHEAP',
  );
}
...
const CREDIT_EXEMPT_LAYERS = {
  'military-awareness':
    'derived view over other layers; it ships no data of its own and its sources carry their own credits',
  detection:
    'a rendering treatment over already-credited layers, not a data source',
...
function requiredCreditFor(layerId) {
  if (CREDIT_EXPECTATIONS[layerId])
    return { regex: CREDIT_EXPECTATIONS[layerId] };
  if (CREDIT_EXEMPT_LAYERS[layerId])
    return { exempt: CREDIT_EXEMPT_LAYERS[layerId] };
  return null;
}
...
const CONNECTION_REFUSED_RE =
  /ECONNREFUSED|ECONNRESET|EHOSTUNREACH|ENOTFOUND|fetch failed|socket hang up|Connect Timeout/i;
/** Harness preflight messages that positively mean "no server", not "harness broke". */
const HARNESS_ENV_RE =
  /Dev server (?:not reachable|unavailable)|Dev server unavailable at|not reachable at http/i;

// ── small helpers ─────────────────────────────────────────────────────────
async function jget(
  path,
  { method = 'GET', timeoutMs = 25000, body = null } = {},
) {
...
function sh(
  cmd,
  args,
  { cwd = REPO_ROOT, env: extraEnv = {}, timeoutMs = 600000 } = {},
) {
...
function classifyNoScoreboard(kind, { code, out, err }) {
  const blob = `${err}\n${out}`;
  if (HARNESS_ENV_RE.test(blob))
    return skip(
      `harness preflight: target not reachable — ${tail(err) || tail(out)}`,
      'ENV',
    );
  return crash(
    `no ${kind} line (exit ${code}) — the harness did not complete: ${tail(err) || tail(out)}`,
  );
}
...
function readResultLine({ code, out, err, timedOut }) {
  ...
  if (Number(p) === 0)
    return classifyNoScoreboard('asserting RESULT', {
      code,
      out,
  ...
  if (code !== 0)
    return crash(
      `${detail}, but the harness exited ${code} after reporting: ${tail(err)}`,
    );
...
function readCockpit({ code, out, err, timedOut }) {
  ...
  if (failures > 0) {
    return crash(
      `contradictory harness output: "READY (${failures} failures)" — the verdict and the count disagree, so this check cannot be trusted either way`,
    );
  }
  if (code !== 0)
    return crash(
      `READY, but the harness exited ${code} after reporting: ${tail(err)}`,
    );
...
function applyKnownConditions(verdict, conditions, transcript) {
  if (!verdict || verdict.status === PASS || !Array.isArray(conditions))
    return verdict;
...
function readFloorVerdict({ code, out, err, timedOut }) {
  ...
  if (m[1] === 'FAIL')
    return fail(
      'VERDICT: FAIL — grounded contacts buried below the mesh floor',
    );
  if (m[1] === 'INCONCLUSIVE') {
    ...
    const counts = /low contacts with plausible mesh readings:\s*(\d+)/.exec(
      out,
    );
    return fail(
      `VERDICT: INCONCLUSIVE — the floor oracle had no candidates to measure (${counts ? `${counts[1]} plausible low contacts` : 'no count reported'}). Preconditions unmet: expected grounded/low traffic at the test airport`,
    );
  }
  if (code !== 0 && code !== null)
    return crash(
      `VERDICT: PASS, but the harness exited ${code} afterwards: ${tail(err)}`,
    );
...
function readOverlaySummary(layerId, jsonPath) {
  return ({ code, out, err, timedOut }) => {
    ...
    if (Number(errors) > 0)
      return fail(`${measured} measured, ${errors} errors`);
    if (Number(measured) === 0) {
      return fail(
        `0 measured, ${skipped} skipped — the ${layerId} scene produced nothing to measure, so the layer never activated`,
      );
    }
    let run = null;
    try {
      ...
      return crash(
        `harness reported ${measured} measured but wrote no readable --json (${String(e?.message || e).slice(0, 80)}); this check cannot prove the ${layerId} overlay was nonempty`,
      );
    }
    ...
    if (!act)
      return crash(
        `no activation record for ${layerId} in the harness JSON — the check cannot verify its claim`,
      );
    if (act.toggleResult?.error)
      return fail(
        `${layerId} failed to activate: ${String(act.toggleResult.error).slice(0, 120)}`,
      );
    if (act.toggleResult?.enabled !== true)
      return fail(
        `${layerId} did not report enabled after activation: ${JSON.stringify(act.toggleResult).slice(0, 120)}`,
      );
    const d = act.delta || {};
    const drawn =
      (d.entities || 0) +
      (d.primitiveLabels || 0) +
      (d.primitiveBillboards || 0) +
      (d.primitivePoints || 0);
    const layerCount = act.state?.stats?.count;
    if (!(drawn > 0)) {
      return fail(
        `${layerId} activated but drew nothing (entities=${d.entities ?? 'n/a'} primitiveLabels=${d.primitiveLabels ?? 'n/a'} stats.count=${layerCount ?? 'n/a'}) — an empty overlay is not a baseline`,
      );
    }
    if (code !== 0)
      return crash(
        `${measured} measured, but the harness exited ${code} after reporting: ${tail(err)}`,
      );
    return pass(
      `${measured} measured, 0 errors; ${layerId} activated and drew ${drawn} (entities=${d.entities} labels=${d.primitiveLabels} nonemptyLabelText=${d.nonemptyLabelText ?? 'n/a'} stats.count=${layerCount ?? 'n/a'})`,
    );
  };
}
...
function harness({
  ...
  return async () => {
    ...
    const r = await sh(
      process.execPath,
      [resolve(REPO_ROOT, 'scripts', script), ...args],
      {
        timeoutMs,
        env: { QA_BASE_URL: APP_URL, ...envExtra },
      },
    );
    const verdict = applyKnownConditions(
      parse(r),
      knownConditions,
      `${r.out}\n${r.err}`,
    );
    ...
    if (
      (verdict.status === FAIL || verdict.status === CRASH) &&
...
check({
  id: 'A1',
  group: 'A',
  desc: 'Node runtime satisfies package.json engines (allocation budgets are pinned to Node 24)',
  run: async () => {
    ...
    if (satisfied === null) {
      return crash(
        `could not evaluate engines range "${range}" against node ${process.versions.node} — this check cannot confirm the runtime is supported`,
      );
    }
    if (satisfied)
      return pass(`node ${process.versions.node} satisfies "${range}"`);
    ...
    return env.node24
      ? skip(
          `node ${process.versions.node} is OUTSIDE "${range}"; A3 runs the allocation gate under ${env.node24.label}, but prefer running the whole L9 pass on Node 24`,
          'ENV',
        )
      : fail(
          `node ${process.versions.node} is OUTSIDE "${range}" and no Node 24 runtime was found — the allocation gate cannot run at all`,
        );
  },
});
...
check({
  id: 'A2',
  group: 'A',
  desc: 'npm test — unit suite green',
  heavy: false,
  run: async () => {
    const r = await sh('npm', ['test'], { timeoutMs: 600000 });
    const m =
      /ℹ pass (\d+)[\s\S]*?ℹ fail (\d+)/.exec(r.out) ||
      /pass (\d+)[\s\S]*?fail (\d+)/.exec(r.out);
    if (!m)
      return fail(
        `could not parse test output (exit ${r.code}): ${tail(r.err) || tail(r.out)}`,
      );
    const [, p, f] = m;
...
check({
  id: 'A3',
  group: 'A',
  desc: 'Allocation microbenchmarks actually EXECUTE (they silently skip off Node 24)',
  run: async () => {
    ...
    const runGate = async (bin, label) => {
      ...
      if (!isCalibratedAllocationRuntime(version)) {
        ...
        return crash(
          `the runtime selected for the allocation gate (${label}) reports ${version || 'no parseable version'}, not Node 24 — the gate would refuse or silently skip, so this check verified nothing`,
        );
      }
      const r = await sh(
        bin,
        [resolve(REPO_ROOT, 'scripts/run-unit-tests.mjs')],
        {
          timeoutMs: 600000,
          env: { GEV_REQUIRE_ALLOCATION_GATE: '1' },
        },
      );
      return r.code === 0
        ? pass(`allocation gate ran under ${label} (${version}) and passed`)
        : fail(
            `allocation gate failed under ${label} (${version}): ${tail(r.out) || tail(r.err)}`,
          );
    };
    ...
    return skip(
      `no Node 24 runtime found (running ${process.versions.node}); the gate SKIPS silently — install Node 24 and re-run`,
      'OWNER-RUN',
    );
  },
});
...
check({
  id: 'A4',
  group: 'A',
  desc: 'npm run build — production build clean',
  run: async () => {
    const r = await sh('npm', ['run', 'build'], { timeoutMs: 900000 });
    return r.code === 0
      ? pass('build succeeded')
      : fail(`build failed: ${tail(r.err) || tail(r.out)}`);
  },
});
...
check({
  ...
  run: async () => {
    ...
    if (tracked.code !== 0)
      return crash(
        `git ls-files failed (exit ${tracked.code}): ${tail(tracked.err)}`,
      );
    const files = tracked.out.split('\n').filter(Boolean);
    if (files.length === 0)
      return crash(
        'git ls-files returned nothing — cannot claim the tree is clean',
      );
    const envFiles = files.filter((f) =>
      /(^|\/)\.env($|\.(?!example))/.test(f),
    );
    if (envFiles.length)
      return fail(`tracked env file(s): ${envFiles.join(', ')}`);
    ...
    const grep = await sh(
      'git',
      ['grep', '-nIE', `(${patterns.join('|')})`, '--', ':!*.md', ':!docs/**'],
      { timeoutMs: 120000 },
    );
    // git grep: 0 = matches found, 1 = no matches (the clean case), >1 = error.
    if (grep.code > 1)
      return crash(`git grep failed (exit ${grep.code}): ${tail(grep.err)}`);
    const hits = grep.out.split('\n').filter(Boolean);
    return hits.length === 0
      ? pass(
          `no tracked .env; ${patterns.length} credential prefixes scanned across ${files.length} tracked files, no hits`,
        )
      : fail(
          `${hits.length} credential literal(s): ${hits.slice(0, 2).join(' | ').slice(0, 200)}`,
        );
  },
});
...
check({
  id: 'A6',
  group: 'A',
  desc: 'Private-name scan over publicly shipped paths (release checklist)',
  run: async () => {
    ...
    return hits.length === 0
      ...
      : fail(
          `${hits.length} file(s) carry private terms: ${hits.slice(0, 4).join(', ')}`,
        );
  },
});
...
check({
  id: 'A7',
  group: 'A',
  desc: 'Public-facing files present (LICENSE, SECURITY, DATA_SOURCES, README, .env.example)',
  run: async () => {
    const want = [
      'LICENSE',
      'SECURITY.md',
...
check({
  id: 'A8',
  group: 'A',
  desc: 'Default launch binds localhost; LAN is an explicit opt-in that warns (release bar #3)',
  run: async () => {
    const sh_ = readFileSync(
      resolve(REPO_ROOT, 'scripts/dev-fresh.sh'),
      'utf8',
    );
    const localDefault = /HOST="\$\{HOST:-localhost\}"/.test(sh_);
    const warns =
      /WARNING: HOST=/.test(sh_) &&
      /brokers your configured API keys/i.test(sh_);
    const localBanner = /Local-only mode/.test(sh_);
    if (localDefault && warns && localBanner)
      return pass(
        'HOST defaults to localhost; LAN path prints the key-exposure warning',
      );
    return fail(
      `localhost-default=${localDefault} lan-warning=${warns} local-banner=${localBanner}`,
    );
  },
});
...
check({
  id: 'A9',
  group: 'A',
  desc: '.env.example documents LAN opt-in + the cost-control throttles',
  run: async () => {
    ...
    const bad = Object.entries(bits)
      .filter(([, v]) => !v)
      .map(([k]) => k);
    return bad.length === 0
      ? pass('LAN opt-in + both throttles + the not-a-billing-cap caveat')
...
check({
  id: 'B1',
  group: 'B',
  desc: 'Target serves the app shell',
  run: async () => {
    const r = await jget('/');
...
check({
  id: 'B2',
  group: 'B',
  desc: 'Flights proxy returns live contacts (/api/opensky)',
  run: async () => {
    const r = await jget(
      '/api/opensky?lamin=24&lomin=-125&lamax=50&lomax=-66',
      { timeoutMs: 45000 },
    );
    if (!r.ok) return fail(`HTTP ${r.status}: ${r.text.slice(0, 120)}`);
    const n = r.json?.states?.length || 0;
    return n > 0
      ? pass(`${n} states, cache=${r.headers.get('x-opensky-cache') || 'n/a'}`)
      : fail('0 states returned');
  },
});
...
check({
  id: 'B3',
  group: 'B',
  desc: 'OpenSky credentials are actually in use (not the anonymous/fallback path)',
  run: async () => {
    const r = await jget(
      '/api/opensky?lamin=24&lomin=-125&lamax=50&lomax=-66',
      { timeoutMs: 45000 },
    ...
    const used =
      r.headers.get('X-OpenSky-Auth-Mode-Used') ||
      r.headers.get('X-OpenSky-Auth') ||
      '';
    if (!r.ok)
      return fail(
        `HTTP ${r.status} from a responsive proxy (auth=${used || 'n/a'} reason=${reason || 'n/a'}): ${r.text.slice(0, 120)}`,
      );
    if (!used)
      return fail(
        'proxy answered 200 without the X-OpenSky-Auth headers — cannot verify which auth mode served this',
      );
    if (/invalid_credentials|rejected/.test(reason))
      return fail(
        `OpenSky rejected the configured credentials (reason=${reason})`,
      );
    if (/missing_.*creds|invalid_or_missing/.test(reason) || used === 'anon') {
      return skip(
        `OpenSky served ANONYMOUSLY (auth=${used}, reason=${reason || 'n/a'}) — the keyed claim needs configured credentials`,
        'OWNER-RUN',
      );
    }
    if (!/^(oauth|basic)$/.test(used)) {
      ...
      return fail(
        `served by mode "${used}" (reason=${reason || 'n/a'}) — not a live authenticated OpenSky fetch, so this check cannot confirm credentials are in use`,
      );
    }
    return pass(
      `authenticated: auth=${used}, reason=${reason || 'n/a'}, cache=${r.headers.get('X-OpenSky-Cache') || 'n/a'}`,
    );
  },
});
...
check({
  id: 'B4',
  group: 'B',
  desc: 'CelesTrak TLE proxy serves and caches (/api/celestrak/stations)',
  run: async () => {
    ...
    return lines > 0
      ? pass(`${lines} TLE records, x-tle-cache=${cache}`)
      : fail('no TLE lines in response');
  },
});
...
check({
  id: 'B5',
  group: 'B',
  desc: 'TLEs are FRESH (epoch under 14 days — a stale catalog silently mis-propagates)',
  run: async () => {
    const r = await jget('/api/celestrak/stations', { timeoutMs: 40000 });
    if (!r.ok) return fail(`HTTP ${r.status}`);
    ...
    return ageDays >= 0 && ageDays < 14
      ? pass(
          `newest epoch ${epoch.toISOString().slice(0, 10)} (${ageDays.toFixed(1)} d old)`,
        )
      : fail(
          `TLE epoch ${epoch.toISOString().slice(0, 10)} is ${ageDays.toFixed(1)} days old`,
        );
  },
});
...
check({
  id: 'B6',
  group: 'B',
  desc: 'FIRMS proxy returns live fires',
  needsKey: 'FIRMS',
  run: async () => {
    ...
    return n > 0
      ? pass(
          `${n} fires, stale=${r.json?.stale}, sources=${(r.json?.sources || []).length}`,
        )
...
check({
  id: 'B7',
  group: 'B',
  desc: 'FIRMS without a key fails HONESTLY (503 no_key, never a healthy-empty)',
  run: async () => {
    const guard = keyGuard('FIRMS', env.keys.FIRMS);
    if (guard) return guard;
    if (env.keys.FIRMS === true)
      return skip(
        'server HAS a FIRMS key — the keyless path needs an unkeyed server',
        'N/A',
      );
...
check({
  id: 'B8',
  group: 'B',
  desc: 'AIS vessel feed is live (/api/ais-live)',
  needsKey: 'AIS',
  run: async () => {
    ...
    if (healthy && rows > 0) {
      return pass(
        `${rows} vessels, status=${status}, newest=${r.json?.newestPositionAt || 'n/a'}, silentFor=${r.json?.silentForMs ?? 'n/a'}ms`,
      );
    }
    if (status === 'auth-failed') {
      // A rejected key is terminal and is the product's problem to report.
      return fail(
        `AISStream rejected the configured key (status=auth-failed, rows=${rows}) — retry is terminal (retryInSec 0)`,
      );
    }
    if (healthy && rows === 0) {
      return skip(
        `status=live but 0 rows — AISStream connects open-but-silent upstream (their #23/#15); recheck when it wakes`,
        'ENV',
      );
    }
    if (['stale', 'reconnecting', 'down', 'connecting'].includes(status)) {
      ...
      return skip(
        `feed is ${status}${rows > 0 ? ` while still serving ${rows} CACHED rows` : ''} (attempt=${r.json?.reconnectAttempt ?? 'n/a'}, nextAttemptAt=${r.json?.nextAttemptAt ?? 'n/a'}, silentFor=${r.json?.silentForMs ?? 'n/a'}ms) — surfaced honestly, but this is not a live feed`,
        'ENV',
      );
    }
    return fail(
      `unexpected AIS feed status "${status}" with ${rows} rows — not one of live/stale/reconnecting/down/auth-failed/connecting`,
    );
  },
});
...
check({
  id: 'B9',
  group: 'B',
  desc: 'AIS without a key fails HONESTLY (503 + missing-key status, no reconnect loop)',
  run: async () => {
    const guard = keyGuard('AIS', env.keys.AIS);
    if (guard) return guard;
    if (env.keys.AIS === true)
      return skip('server HAS an AISStream key', 'N/A');
    const r = await jget('/api/ais-live');
    return r.status === 503 &&
      r.json?.status === 'missing-key' &&
      Array.isArray(r.json?.rows)
      ? pass(
          `503 status=missing-key, rows=[] — "${String(r.json?.error).slice(0, 60)}"`,
        )
      : fail(
          `expected 503/missing-key, got ${r.status} ${r.text.slice(0, 120)}`,
        );
  },
});
...
check({
  id: 'B10',
  group: 'B',
  desc: 'TomTom traffic reports LIVE mode with budget accounting',
  needsKey: 'TOMTOM',
  run: async () => {
    ...
    return r.json?.hasKey
      ? pass(
          `hasKey=true, used ${r.json.dailyCount}/${r.json.budget} tiles today`,
        )
...
check({
  id: 'B11',
  group: 'B',
  desc: 'TomTom without a key identifies SIMULATION honestly (200 hasKey:false)',
  run: async () => {
    ...
    return r.ok &&
      r.json?.hasKey === false &&
      tile.status === 503 &&
      tile.json?.error === 'no_key'
      ? pass('status 200 hasKey:false; flow tile 503 no_key')
      : fail(
          `status=${r.status} hasKey=${r.json?.hasKey}; tile=${tile.status} ${tile.text.slice(0, 60)}`,
        );
  },
});
...
check({
  id: 'B12',
  group: 'B',
  desc: 'CCTV source packs registered (Austin + at least one more city)',
  run: async () => {
    ...
    return src.length > 0 && austin && cities.length >= 2
      ? pass(
          `${src.length} cameras across ${cities.length} packs: ${cities.slice(0, 6).join(', ')}`,
        )
      : fail(
          `${src.length} cameras, austin=${austin}, packs=${cities.join(',')}`,
        );
  },
});
...
check({
  id: 'B13',
  group: 'B',
  desc: 'CCTV frame proxy returns real image bytes',
  run: async () => {
    ...
    const res = await fetch(
      `${APP_URL}/api/cctv/frame/${encodeURIComponent(first.id)}`,
...
check({
  id: 'B14',
  group: 'B',
  desc: 'CCTV health route answers (per-camera upstream status)',
  run: async () => {
...
check({
  id: 'B15',
  group: 'B',
  desc: 'Radio directory proxy returns stations (or a labelled degraded state)',
  run: async () => {
    const r = await jget('/api/radio/stations?limit=20', { timeoutMs: 45000 });
    const rows = Array.isArray(r.json)
      ? r.json.length
      : r.json?.stations?.length || 0;
    if (r.ok && rows > 0) return pass(`${rows} stations`);
    if (r.status === 503 && r.json?.degraded)
      return skip(
        `upstream Radio Browser degraded: ${r.json.degradedReason}`,
        'ENV',
      );
...
check({
  id: 'B16',
  group: 'B',
  desc: 'Launch Library proxy returns upcoming missions',
  run: async () => {
    const r = await jget('/api/launches', { timeoutMs: 45000 });
    const n =
      r.json?.results?.length ??
      r.json?.launches?.length ??
...
check({
  id: 'B17',
  group: 'B',
  desc: 'Terrain height service answers (the height-datum backbone)',
  run: async () => {
    // Contract: points="lon,lat;lon,lat;…" (longitude first).
    const r = await jget('/api/terrain/heights?points=-97.7431,30.2672', {
      timeoutMs: 45000,
    });
    if (r.status === 502)
      return skip(
        `terrain upstream unavailable and no cache: ${r.text.slice(0, 90)}`,
        'ENV',
      );
    ...
    return Number.isFinite(first?.elevation) && Number.isFinite(first?.geoid)
      ? pass(
          `Austin: elevation ${first.elevation.toFixed(1)} m, geoid ${first.geoid.toFixed(1)} m, ellipsoid ${first.ellipsoid.toFixed(1)} m`,
        )
...
check({
  id: 'B18',
  group: 'B',
  desc: 'Overpass proxy answers a real query (roads, annotations, installations)',
  heavy: true,
  run: async () => {
    // Contract: a form-encoded body with exactly one bounded `data` query.
    const query =
      '[out:json][timeout:25];node(30.26,-97.75,30.27,-97.74)["amenity"="cafe"];out 5;';
    ...
    if (res.ok && Array.isArray(json?.elements))
      return pass(`${json.elements.length} elements`);
...
check({
  id: 'B19',
  group: 'B',
  desc: 'Realtime token endpoint mints an EPHEMERAL secret and never the raw key',
  needsKey: 'OPENAI',
  costly: true,
  run: async () => {
    const r = await jget('/api/realtime/token', {
      method: 'POST',
      timeoutMs: 30000,
    });
    if (!r.ok) return fail(`HTTP ${r.status}: ${r.text.slice(0, 120)}`);
    const blob = r.text;
    if (/\bsk-[A-Za-z0-9]{20,}/.test(blob))
      return fail('response contains a raw sk- key');
...
check({
  id: 'B20',
  group: 'B',
  desc: 'Voice without a key fails HONESTLY (503, app unaffected)',
  run: async () => {
    const guard = keyGuard('OPENAI', env.keys.OPENAI);
    if (guard) return guard;
    if (env.keys.OPENAI === true)
      return skip(
        'server HAS an OpenAI key — the keyless path needs an unkeyed server',
        'N/A',
      );
...
check({
  id: 'B21',
  group: 'B',
  desc: 'No proxy echoes credential material back to the client (P1-5 acceptance #4)',
  run: async () => {
    const paths = [
      '/api/cctv/sources',
      '/api/tomtom/status',
    ...
    for (const p of paths) {
      ...
      try {
        r = await jget(p, { timeoutMs: 30000 });
      } catch (e) {
      ...
      const documentedKeyless =
        r.status === 503 &&
        (r.json?.status === 'missing-key' ||
          r.json?.error === 'no_key' ||
          /OPENAI_API_KEY is not set/.test(r.text));
      ...
      if (
        ...
        leaked.push(`${p}: key-shaped assignment`);
    }
    if (leaked.length) return fail(leaked.join('; '));
    if (unscannable.length) {
      return fail(
        `could not read ${unscannable.length}/${paths.length} routes, so this check cannot claim they are clean: ${unscannable.join(', ')}`,
      );
    }
...
const BROWSER_CHECKS = [
  ...
  [
    'C10',
    'Traffic: LIVE mode when keyed, clearly-labelled SIMULATION when not',
  ],
  [
    'C11',
    'Bundled layers render: datacenters, dams, submarine cables, installations',
  ],
...
for (const [id, desc] of BROWSER_CHECKS)
  check({ id, group: 'C', desc, browser: true });

// ─── D · EXISTING HARNESS FLEET ───────────────────────────────────────────
check({
  id: 'D1',
  group: 'D',
  desc: 'track-regression — the tracking/height-datum invariant gate (P1-7)',
  heavy: true,
  run: harness({
    id: 'D1',
    script: 'track-regression.mjs',
    args: ['--url', APP_URL],
    timeoutMs: 1200000,
  }),
});
check({
  id: 'D2',
  group: 'D',
  desc: 'qa-heading-b3 — path-derived display heading',
  heavy: true,
  run: harness({
    id: 'D2',
    script: 'qa-heading-b3.mjs',
    args: ['--url', APP_URL],
    timeoutMs: 900000,
  }),
});
check({
  id: 'D3',
  group: 'D',
  desc: 'qa-sprites-b5 — per-class billboard silhouettes',
  heavy: true,
  run: harness({
    id: 'D3',
...
check({
  id: 'D4',
  group: 'D',
  desc: 'qa-cctv-v2 — camera geometry, projection, ambient cards',
  heavy: true,
  run: harness({
    id: 'D4',
    script: 'qa-cctv-v2.mjs',
    args: ['--url', APP_URL],
    timeoutMs: 1500000,
  }),
});
check({
  id: 'D5',
  group: 'D',
  desc: 'qa-failstate-b10 — fabricated upstream failures degrade honestly',
  run: harness({
    id: 'D5',
    script: 'qa-failstate-b10.mjs',
    args: ['--url', APP_URL],
    timeoutMs: 900000,
  }),
});
check({
  id: 'D6',
  group: 'D',
  desc: 'qa-attribution-b12 — per-layer credits + clean/recording credit line',
  run: harness({
    id: 'D6',
    script: 'qa-attribution-b12.mjs',
    args: ['--url', APP_URL],
    timeoutMs: 600000,
  }),
});
check({
  id: 'D7',
  group: 'D',
  desc: 'qa-cockpit-utility — cockpit display/radio layout readiness',
  heavy: true,
  parseNote: 'READY/NOT_READY',
  run: harness({
    id: 'D7',
...
check({
  id: 'D8',
  group: 'D',
  desc: 'qa-radio — worldwide radio browse/play surface',
  heavy: true,
  run: harness({
    id: 'D8',
    script: 'qa-radio.mjs',
...
check({
  id: 'D9',
  group: 'D',
  desc: 'qa-floor-verify — grounded contacts sit ON the rendered mesh floor',
  heavy: true,
  run: harness({
    id: 'D9',
...
check({
  id: 'D10',
  group: 'D',
  desc: 'qa-voice-routing (behavior layer) — tool behavior without model turns',
  heavy: true,
  run: harness({
    id: 'D10',
    script: 'qa-voice-routing.mjs',
    args: ['--layer', 'behavior', '--url', APP_URL],
    timeoutMs: 1500000,
  }),
});
check({
  id: 'D11',
  group: 'D',
  desc: 'qa-firms — live fire rendering and interaction',
  needsKey: 'FIRMS',
  heavy: true,
  run: harness({
    id: 'D11',
...
check({
  id: 'D12',
  group: 'D',
  desc: 'qa-overlay-baseline (submarine cables scene) — overlay/label baseline',
  heavy: true,
  run: harness({
    id: 'D12',
...
const MANUAL = [
  [
    'M1',
    'Voice mic round trip 1/3 — "when is the next ISS pass?" (next_iss_pass)',
  ],
  [
    'M2',
    'Voice mic round trip 2/3 — connect/disconnect twice in one tab + keyed set_context_mode and control_cockpit',
  ],
  [
    'M3',
    'Voice mic round trip 3/3 — adsbdb enrichment readout on a live tracked flight',
  ],
  [
    'M4',
    'LAN warning path — HOST=0.0.0.0 banner, LAN URL, and a throttled response',
  ],
  [
    'M5',
    'Live AIS vessel one-click camera transfer — requires status=live, not cached rows (never verified against a live feed)',
  ],
  [
    'M6',
    'CCTV dense-city interaction — cold fill, hover, select, card removal, monitor plane, coverage, auto-hop suspend',
  ],
  [
    'M7',
    'Grounded + airborne tracked aircraft from 2-3 headings (DISPLAY 3D ON, non-TR-3B subject)',
  ],
  [
    'M8',
    'Voice analyst_query: exact unrounded count + scopeLabel, contactsWindow verbatim, follow-up re-filter',
  ],
  ['M9', 'First-run: the arrival camera feels alive within 30 seconds'],
  [
    'M10',
    'Sensor styles + Contacts-owned detection (DENSE/75 on activation-from-OFF; Cockpit inert; restore on deactivation)',
  ],
  [
    'M11',
    'Cancelled cross-mode Context switch rests on Context OFF (contextOff + priorMode, no restoration)',
  ],
];
...
async function runBrowserGroup(record) {
  const ids = BROWSER_CHECKS.map(([id]) => id).filter((id) =>
    selected(CHECKS.find((c) => c.id === id)),
  );
  if (!ids.length) return;
  const emit = (id, res, ms) => {
    if (ids.includes(id))
      record(
  ...
  const evalBounded = (fn, arg, ms = 8000) =>
    Promise.race([
  ...
  const mustEval = async (fn, arg = null, ms = 30000) => {
    const outcome = await Promise.race([
      page.evaluate(fn, arg).then(
        ...
        (e) => ({
          ok: false,
          reason: `page threw: ${String(e?.message || e).slice(0, 140)}`,
        }),
      ),
      new Promise((r) =>
        setTimeout(
          () =>
            r({
              ok: false,
              reason: `page did not answer within ${ms} ms`,
  ...
  const step = async (id, fn) => {
    ...
    try {
      ...
      emit(
        id,
        crash(
          `probe threw, so this check verified nothing: ${String(e?.message || e).slice(0, 180)}`,
        ),
        Date.now() - t0,
      );
      return null;
  ...
  for (let attempt = 1; attempt <= 3 && !booted; attempt += 1) {
    try {
      ...
      const stale = consoleErrors.some((e) =>
    ...
    const paintR = await mustEval(
      async () => {
        ...
        return {
          ...
          w: canvas?.width ?? 0,
          h: canvas?.height ?? 0,
          frames: g.viewer.scene.frameState?.frameNumber ?? null,
        };
      },
      null,
      45000,
  ...
  emit(
    ...
    (() => {
      if (!booted)
        return fail(
          `app never booted in ${bootSec.toFixed(0)} s — ${bootNote}; if 504, clear node_modules/.vite and restart the dev server`,
        );
      if (paint?.probeError)
        return crash(
          `booted in ${bootSec.toFixed(1)} s but the first-paint probe failed: ${paint.probeError}`,
        );
      if (!paint?.painted)
        return fail(
          `${bootDetail} — the scene never rendered a frame, so "first paint" is unproven`,
        );
      if (!(paint.w > 0 && paint.h > 0))
        return fail(`${bootDetail} — the render canvas has no size`);
      if (bootSec > BOOT_BUDGET_S) {
        return fail(
          `${bootDetail} — over the ${BOOT_BUDGET_S} s budget this check states${bootNote ? ` (${bootNote})` : ''}`,
        );
      }
  ...
  const settle = async (layerId, maxSec = 45) => {
    const enabled = (await evalBounded(
      async (id) => {
        ...
        try {
          ...
          return { enableError: String(e?.message || e) };
        }
        return { ok: true };
      },
      layerId,
      30000,
    )) || { ok: true, enableUnconfirmed: true };
    ...
    for (let i = 0; i < maxSec; i += 1) {
      ...
      if (
        answered &&
        (!stats.loading || i >= Math.min(12, Math.floor(maxSec / 2)))
      )
        break;
    }
      ...
      async () => {
        ...
        for (const id of heavy) {
          ...
          try {
            // eslint-disable-next-line no-await-in-loop
            await Promise.race([
              dm.setEnabled(id, false, { origin: 'user' }),
  ...
  await step('C2', async () => {
    ...
    if (!infoR.ok)
      return crash(`could not read the scene graph: ${infoR.reason}`);
    const info = infoR.value;
    const photoreal = info.tilesets.find((t) =>
      /google|tile\.googleapis|photorealistic|3dtiles/i.test(t.url),
    );
    if (!info.tilesets.length) {
      return fail(
        `no Cesium3DTileset attached (imagery layers=${info.imagery}, mapStack=${info.mapStack}) — the photorealistic globe is absent`,
      );
    }
    if (!photoreal) {
      return fail(
        `a 3D tileset is attached but none resolves to Google Photorealistic tiles (urls: ${info.tilesets.map((t) => t.url.slice(0, 60) || '(no url)').join(' | ')}, mapStack=${info.mapStack})`,
      );
    }
    if (!photoreal.ready) {
      return fail(
        `the photorealistic tileset is attached but not ready (mapStack=${info.mapStack})`,
      );
    }
    return pass(
      `Photorealistic 3D attached and ready (mapStack=${info.mapStack}, ${info.tilesets.length} tileset(s), imagery layers=${info.imagery})`,
    );
  });
  ...
  await step('C3', async () =>
    pageErrors.length === 0
      ? pass(
          `0 uncaught page errors (${consoleErrors.length} console errors, keyless 503s expected)`,
        )
      : fail(
          `${pageErrors.length} uncaught: ${pageErrors.slice(0, 2).join(' | ')}`,
        ),
  );
  ...
  const voiceSnapshotR = await mustEval(
    () => {
      ...
      return {
        ...
        hasRunner: typeof vc.runner === 'function',
      };
    },
    null,
    30000,
  );
  const voiceSnapshot = voiceSnapshotR.ok
    ? voiceSnapshotR.value
    : { probeError: voiceSnapshotR.reason };
  ...
  await step('C4', async () => {
    ...
    if (!(s.count > 0))
      return fail(
        `0 contacts (status=${s.status || ''} error=${s.error || ''})`,
      );
    const src = String(s.source || '');
    return /adsb\.lol/i.test(src)
      ? skip(
          `${s.count} contacts but via the adsb.lol FALLBACK (source=${src}) — OpenSky credentials needed for the live claim`,
          'OWNER-RUN',
        )
      : pass(
          `${s.count} contacts, source=${src || 'OpenSky'}, stale=${!!s.stale}`,
        );
  });
  ...
  await step('C5', async () => {
    ...
    const s = r.stats || {};
    return s.count > 0
      ? pass(`${s.count} satellites, status=${s.status || 'nominal'}`)
      : fail(`0 satellites (status=${s.status} error=${s.error || ''})`);
  });
  ...
  await step('C7', async () => {
    ...
    for (let i = 0; i < 30; i += 1) {
      ...
      ui =
        (await evalBounded(() => {
          const st = window.__godsEyeView.dataManager.layers
            .get('cctv')
            ?.module?.getUIState?.();
          return st
            ? {
      ...
      if (
        ui &&
        ui.count > 0 &&
        (ui.ambient?.count || 0) > 0 &&
        (ui.ambient?.frameFetches || 0) > 0
      )
        break;
      // eslint-disable-next-line no-await-in-loop
    ...
    if (!(ui.count > 0))
      return fail(
        `0 cameras registered (error=${ui.error || s.error || 'none'})`,
      );
    const cards = ui.ambient?.count || 0;
    const fetches = ui.ambient?.frameFetches || 0;
    if (cards === 0)
      return fail(
        `${ui.count} cameras registered but 0 ambient cards were produced — nothing is rendering`,
      );
    if (fetches === 0) {
      return fail(
        `${ui.count} cameras and ${cards} ambient cards, but frameFetches=0 — the frame loop never ran, so "healthy" is unproven`,
      );
    }
    return pass(
      `${ui.count} cameras, ${cards} ambient cards, ${fetches} frame fetches (${ui.ambient?.fetchMode}, ${ui.ambient?.fetchesInFlight} in flight), loading=${ui.loading?.active}`,
    );
  });
  ...
  await step('C8', async () => {
    ...
    if (!chipR.ok)
      return crash(`could not read the vessels layer row: ${chipR.reason}`);
    const chip = chipR.value;
    if (!chip)
      return fail(
        'no [data-layer-id="ais-live-vessels"] row in the DOM — cannot read the surfaced feed state',
      );

    if (env.keys.AIS === false) {
      const surfaced =
        chip.feedState === 'unavailable' && /UNAVAILABLE/i.test(chip.text);
      if (!surfaced) {
        return fail(
          `keyless vessels did not SURFACE the unavailable state: chip="${chip.text}" feedState=${chip.feedState} (stats: status=${s.status} error=${s.error}) — a silently empty layer is the failure mode this check exists for`,
        );
      }
      if (s.count > 0)
        return fail(
          `chip says UNAVAILABLE but the layer reports ${s.count} vessels`,
        );
      return pass(
        `keyless and honest: chip="${chip.text}" feedState=unavailable, stats.status=${s.status}, error="${String(s.error || '').slice(0, 40)}", count=0`,
      );
    }
    ...
    if (live && s.count > 0) {
      ...
      return /^ON$/i.test(chip.text)
        ? pass(
            `${s.count} vessels live: chip="ON", transport=${transport}, lastMessage=${s.lastMessageAt || 'n/a'}`,
          )
        : fail(
            `transport=${transport} with ${s.count} vessels, but the chip reads "${chip.text}" — a live feed must present as ON`,
          );
    }
    if (transport === 'auth-failed') {
      const surfaced =
        /UNAVAILABLE/i.test(chip.text) &&
        /key rejected/i.test(String(s.error || ''));
      return surfaced
        ? fail(
            `AISStream rejected the key — surfaced correctly (chip="${chip.text}", error="${s.error}", retryInSec=${s.retryInSec}) but a rejected key is a product-blocking failure, not an environment condition`,
          )
        : fail(
            `AISStream rejected the key and the UI did not say so: chip="${chip.text}", error="${String(s.error || 'none')}"`,
          );
    }
    if (['stale', 'reconnecting', 'down'].includes(transport)) {
      ...
      const expected =
        s.count > 0 ? /STALE|DEGRADED/i : /UNAVAILABLE|DEGRADED/i;
      return expected.test(chip.text)
        ? skip(
            `feed is ${transport}${s.count > 0 ? ` with ${s.count} CACHED vessels` : ''} and the UI says so (chip="${chip.text}", error="${String(s.error || '').slice(0, 60)}", retryInSec=${s.retryInSec}) — honest degradation, not a live feed`,
            'ENV',
          )
        : fail(
            `feed is ${transport} with ${s.count} vessels but the chip reads "${chip.text}" — a degraded feed that presents as healthy is exactly the invisible outage this check exists for`,
          );
    }
    if (s.count > 0) {
      return fail(
        `${s.count} vessels with transport="${transport}" — neither live nor a recognised degraded state, so this cannot be called a live feed (chip="${chip.text}")`,
      );
    }
    // Keyed but empty: honest only if the UI says so.
    return /UNAVAILABLE|LOADING|DEGRADED|STALE/i.test(chip.text)
      ? skip(
          `keyed but 0 vessels and the UI says so (chip="${chip.text}", transport=${transport}, error="${String(s.error || '').slice(0, 60)}") — AISStream connects open-but-silent upstream`,
          'ENV',
        )
      : fail(
          `keyed, 0 vessels, and the chip claims "${chip.text}" — the layer is empty without surfacing it`,
        );
  });
  ...
  await step('C9', async () => {
    ...
    if (env.keys.FIRMS === false) {
      return s.error === 'KEY REQUIRED'
        ...
        : fail(
            `keyless but error=${s.error} count=${s.count} — expected "KEY REQUIRED"`,
          );
    }
    return s.count > 0
      ? pass(`${s.count} fires, cells=${s.cells}`)
      : fail(`keyed but 0 fires (error=${s.error || ''})`);
  });
  ...
  await step('C10', async () => {
    ...
    await evalBounded(async () => {
      ...
      g.styleManager.applyCameraState(
        { lat: 30.2672, lon: -97.7431, alt: 2500, heading: 0, pitch: -40 },
        1.2,
      );
    ...
    if (env.keys.TOMTOM === false) {
      ...
      return s.mode === 'sim' && /SIMULATED/i.test(label)
        ...
        : fail(
            `mode=${s.mode} label="${label.slice(0, 70)}" — expected sim + a SIMULATED label`,
          );
    }
    const colored =
      (s.flowBuckets?.free || 0) +
      (s.flowBuckets?.slow || 0) +
      (s.flowBuckets?.jam || 0);
    if (s.mode === 'live' && s.tilesFetched > 0 && colored > 0) {
      return pass(
        `live: ${s.flowCoveragePct}% coverage, ${s.tilesFetched} tiles, ${colored} colored dots`,
      );
    }
    ...
    if (trafficFlowInconclusive(s)) {
      return crash(
        `traffic's flow fetch had not landed when the 45 s budget expired: tiles=0, loading=${!!s.loading}, label="${String(s.loadingLabel || '').slice(0, 40)}", road dots=${s.count ?? 0} — this check could not determine whether live flow renders, so it verified nothing`,
      );
    }
    return fail(
      `mode=${s.mode} tiles=${s.tilesFetched} coverage=${s.flowCoveragePct} colored=${colored}`,
    );
  });
  ...
  await step('C11', async () => {
    ...
    await evalBounded(
      async () => {
        ...
        g.viewer.camera.cancelFlight();
        g.styleManager.applyCameraState(
          { lat: 20, lon: 0, alt: 14000000, heading: 0, pitch: -90 },
          1.5,
        );
        await new Promise((r) => setTimeout(r, 4000));
      },
    ...
    for (const id of bundled) {
      ...
      if (
        !r.missing &&
        !(s.count > 0) &&
        (s.loading || s.loadingLabel) &&
        !s.error
      )
        stillLoading.push(label);
    }
    if (stillLoading.length) {
      return crash(
        `still loading when the ${45}s budget expired: ${stillLoading.join(', ')} [all: ${out.join(', ')}] — this check could not determine whether they render, so it verified nothing`,
      );
    }
    ...
    if (zero.length) {
      ...
      for (const label of contested) {
        const id = bundled.find(
          (b2) => b2.replace(/^local-|^telegeography-/, '') === label,
        );
        if (!id) continue;
        // eslint-disable-next-line no-await-in-loop
      ...
      if (stillZero.length) {
        return fail(
          `empty bundled layer(s) even on a quiet stage: ${stillZero.join(', ')} [under load: ${out.join(', ')}]`,
        );
      }
    ...
    const box = {
      name: 'San Diego / Coronado',
      lat: 32.7,
      lon: -117.18,
      span: 0.6,
    };
    const api = await jget(
      `/api/military-installations?south=${(box.lat - box.span).toFixed(5)}&west=${(box.lon - box.span).toFixed(5)}&north=${(box.lat + box.span).toFixed(5)}&east=${(box.lon + box.span).toFixed(5)}`,
      { timeoutMs: 60000 },
    ).catch((e) => ({ status: 0, json: null, text: String(e?.message || e) }));
    const apiRows = Array.isArray(api.json?.features)
      ? api.json.features.length
      : Array.isArray(api.json?.elements)
        ? api.json.elements.length
        : Array.isArray(api.json)
          ? api.json.length
          : null;
    ...
    await evalBounded(
      async (b) => {
        ...
        g.viewer.camera.cancelFlight();
        g.styleManager.applyCameraState(
          { lat: b.lat, lon: b.lon, alt: 25000, heading: 0, pitch: -90 },
          1.2,
        );
        await new Promise((r) => setTimeout(r, 4000));
      },
    ...
    for (let i = 0; i < 40; i += 1) {
      ...
      const snap = await evalBounded(
        () => {
          ...
          try {
            dm.layers.get('military-installations')?.module?.refresh?.();
          } catch {
            /* optional */
          }
          return (
            dm.layers.get('military-installations')?.module?.getStats?.() ??
            null
          );
        },
        null,
    ...
    if (mi.missing)
      return fail(
        `military-installations layer is not registered [bundled: ${out.join(', ')}]`,
      );
    if (ms.count > 0)
      return pass(
        `${out.join(', ')}, military-installations=${ms.count} over ${box.name}${loadNote}`,
      );
    if (/zoom-in/.test(String(ms.status || ''))) {
      return fail(
        `military-installations refused the ${box.span * 2}° box over ${box.name} as too wide (status=${ms.status}, error="${ms.error}") — the probe camera and the layer's own ≤10° gate disagree`,
      );
    }
    if (api.status !== 200) {
      ...
      const honestOutage =
        api.status === 503 &&
        /temporarily unavailable/i.test(String(api.text || ''));
      if (honestOutage) {
        return skip(
          `bundled layers OK (${out.join(', ')}); military-installations could not be checked — its upstream is down and the proxy says so honestly (HTTP 503 "${String(api.json?.error || '').slice(0, 60)}")`,
          'ENV',
        );
      }
      return fail(
        `military-installations rendered 0 and its API returned HTTP ${api.status} for ${box.name} — a responsive app failing this route is a product failure: ${String(api.text || '').slice(0, 100)}`,
      );
    }
    if (apiRows === null)
      return crash(
        `could not read a row count from /api/military-installations to cross-check the empty layer: ${String(api.text || '').slice(0, 100)}`,
      );
    if (apiRows > 0) {
      return fail(
        `/api/military-installations returned ${apiRows} features over ${box.name} but the layer rendered 0 (status=${ms.status}, error=${ms.error || 'none'}) [bundled: ${out.join(', ')}]`,
      );
    }
    return skip(
      `bundled layers OK (${out.join(', ')}); military-installations rendered 0 AND its API returned 0 features over ${box.name} — positively an upstream-data condition, not a render failure`,
      'ENV',
    );
  });
  ...
  await step('C12', async () => {
    ...
    const armed =
      (await evalBounded(
        () => [
          ...(window.__godsEyeView.dataManager.getEnabledLayerIds?.() || []),
        ],
        null,
        20000,
      )) || [];
    ...
    if (armed.length === 0) {
      ...
      if (nowOn.length === 0) {
        return crash(
          'no layers are enabled and self-arming failed — this check has nothing to verify credits against',
        );
      }
    }
    const credR = await mustEval(
      async () => {
        ...
        const html = (viewer.creditDisplay._staticCredits || []).map(
          (c) => c.html,
        );
        ...
        const listCount =
          document.querySelector('.cesium-credit-lightbox > ul')?.children
            ?.length || 0;
    ...
    if (!credR.ok)
      return crash(`could not read the credit display: ${credR.reason}`);
    ...
    if (missing.length) {
      return fail(
        `enabled layer(s) with NO registered attribution: ${missing.join(', ')} — legal requirement (DATA_SOURCES.md, finding H11); lightbox items=${cred.listCount}`,
      );
    }
    if (unmapped.length) {
      // Fail closed: an unmapped layer is an unchecked layer.
      return fail(
        `enabled layer(s) this check has no attribution expectation for: ${unmapped.join(', ')} — add them to CREDIT_EXPECTATIONS, or to CREDIT_EXEMPT_LAYERS with a reason. Until then their attribution is unverified`,
      );
    }
    if (!(cred.listCount > 0))
      return fail(
        `the attribution lightbox listed 0 items despite ${cred.html.length} registered credits`,
      );
    return pass(
      `${cred.html.length} credits registered, lightbox lists ${cred.listCount}; all ${cred.enabled.length} enabled layers verified${exempted.length ? ` (${exempted.length} exempt: ${exempted.join(', ')})` : ''}`,
    );
  });

  await step('C13', async () => {
    const stR = await mustEval(
      async () => {
        const viewer = window.__godsEyeView.viewer;
        ...
        const out = {
          ...
          childBox: [...(el?.querySelectorAll('*') || [])]
            ...
            .reduce(
              (best, r) =>
                r.width * r.height > best.w * best.h
    ...
    if (!stR.ok)
      return crash(`could not measure the clean-UI credit line: ${stR.reason}`);
    const st = stR.value;
    ...
    return visible && labelled && st.clean
      ? pass(
          `clean-UI on, #cesium-credits painted ${box} ("${st.text.slice(0, 40)}")`,
        )
      : fail(
          `clean=${st.clean} present=${st.present} display=${st.display} opacity=${st.opacity} size=${st.w}x${st.h} text="${st.text.slice(0, 40)}"`,
        );
  });
  ...
  await step('C14', async () => {
    ...
    if (!inPageR.ok)
      return crash(
        `could not read browser state for the key-leak scan: ${inPageR.reason}`,
      );
    ...
    if (keyish.length)
      leaked.push(
        `${keyish.length} same-origin URL(s) carry a key query param: ${keyish[0].slice(0, 90)}`,
      );
    return leaked.length === 0
      ? pass(
          `${requestUrls.length} requests + localStorage scanned, no credential material`,
        )
      : fail(leaked.join('; '));
  });
  ...
  await step('C15', async () => {
      ...
      async (styles) => {
        ...
        for (const [style, label] of styles) {
          ...
          seen.push({
            style,
            label,
            observed: vs?.style ?? null,
            activeStyle: g.styleManager.activeStyle ?? null,
          });
        }
        const after = {
          x: cam.position.x,
    ...
    if (!rR.ok)
      return crash(`could not exercise the style presets: ${rR.reason}`);
    const r = rR.value;

    const threw = r.seen.filter((s) => s.error);
    if (threw.length)
      return fail(
        `style application threw for: ${threw.map((s) => `${s.style} (${s.error})`).join('; ')}`,
      );
    const noop = r.seen.filter((s) => s.observed !== s.style);
    if (noop.length) {
      return fail(
        `style did not take effect for: ${noop.map((s) => `${s.label}/${s.style} → getVisualState().style=${s.observed}`).join('; ')} — the call was a no-op`,
      );
    }
    if (r.seen.length !== wanted.length)
      return crash(
        `only ${r.seen.length}/${wanted.length} styles were exercised`,
      );
    return r.drift < 1
      ? pass(
          `${r.seen.map((s) => `${s.label}→${s.observed}`).join(', ')} each confirmed in getVisualState(); camera drift ${r.drift.toFixed(3)} m`,
        )
      : fail(
          `styles applied but the camera moved ${r.drift.toFixed(1)} m while switching`,
        );
  });
  ...
  await step('C16', async () => {
    ...
    if (!foundR.ok)
      return crash(
        `could not look for the reset-to-globe control: ${foundR.reason}`,
      );
    const found = foundR.value;
    if (!found)
      return fail('#reset-globe-view is missing from the DOM (L6 shipped it)');
    ...
    // control at all.
    const setupR = await mustEval(
      async () => {
        const g = window.__godsEyeView;
        g.viewer.camera.cancelFlight();
        g.styleManager.applyCameraState(
          { lat: 30.2672, lon: -97.7431, alt: 3000, heading: 0, pitch: -35 },
          1.5,
        );
        await new Promise((r) => setTimeout(r, 3000));
    ...
    if (!setupR.ok)
      return crash(
        `could not put the camera at city altitude to test the reset: ${setupR.reason}`,
      );
    const before = setupR.value?.altKm;
    if (!Number.isFinite(before))
      return crash(
        'the city-altitude setup returned no altitude — cannot establish a starting point',
      );
    if (before > 5000) {
      return crash(
        `the camera is still at ${Math.round(before)} km after the city-altitude setup, so "returns to the global band" cannot be tested from here`,
      );
    }
    const clickR = await mustEval(
      () => {
        ...
        if (!el)
          return {
            clicked: false,
            reason: 'control vanished before the click',
          };
        el.click();
        return { clicked: true };
      },
    ...
    if (!clickR.ok)
      return crash(
        `could not click the reset-to-globe control: ${clickR.reason}`,
      );
    if (!clickR.value?.clicked)
      return fail(
        `the reset control could not be clicked: ${clickR.value?.reason}`,
      );
    let altKm = before;
    ...
    return altKm > 5000
      ? pass(
          `"${found.label}" ${Math.round(before)} km → ${Math.round(altKm)} km (global band); cockpit twin present=${found.cockpitTwin}`,
        )
      : fail(
          `reset left the camera at ${Math.round(altKm)} km (from ${Math.round(before)} km)`,
        );
  });
  ...
  await step('C17', async () => {
    ...
    if (v.probeError)
      return crash(
        `voice snapshot failed, so nothing was verified: ${v.probeError}`,
      );
    if (!v.present)
      return fail(
        '__gevVoiceCommands never initialised — the voice surface did not load',
      );
    ...
    if (env.keys.OPENAI === true) {
      ...
      return skip(
        `voice surface present (status=${v.status}); the keyless-degradation claim needs an UNKEYED server, and a real mic round trip is owner-run — see runbook M1-M3`,
        'OWNER-RUN',
      );
    }
    if (!v.hasRunner)
      return fail('voice controller present but exposes no runner');
    ...
    const beforeR = await mustEval(
      () => document.querySelectorAll('#gev-voice-control').length,
      null,
      45000,
    );
    if (!beforeR.ok)
      return crash(`could not look for the voice control: ${beforeR.reason}`);
    if (!beforeR.value)
      return fail(
        '#gev-voice-control is absent — the voice surface never rendered',
      );
    const surfacedR = await mustEval(
      async () => {
        const vc = window.__gevVoiceCommands;
        try {
          await vc.start?.();
        ...
        for (let i = 0; i < 20; i += 1) {
          ...
          const status =
            document.getElementById('gev-voice-status')?.textContent?.trim() ||
            '';
        ...
        return {
          ...
          errorDetail:
            document
              .getElementById('gev-voice-error-detail')
              ?.textContent?.trim() || null,
          dataStatus:
            document.getElementById('gev-voice-control')?.dataset?.status ||
            null,
          recentError: diag?.recentErrors?.[0]
            ? {
                source: diag.recentErrors[0].source,
                message: diag.recentErrors[0].message,
              }
            : null,
    ...
    if (!surfacedR.ok)
      return crash(
        `could not drive the keyless voice path: ${surfacedR.reason}`,
      );
    const surfaced = surfacedR.value;
    const saysKey = /OPENAI_API_KEY is not set/i.test(
      `${surfaced.errorDetail || ''} ${surfaced.recentError?.message || ''}`,
    );
    const saysUnavailable =
      surfaced.dataStatus === 'error' ||
      /ERROR|UNAVAILABLE/i.test(
        `${surfaced.status || ''} ${surfaced.detail || ''}`,
      );
    if (!surfaced.appAlive)
      return fail(
        'the app died when voice was started without a key — a missing optional key must never take the globe down',
      );
    if (!saysUnavailable) {
      return fail(
        `voice start without a key left the UI at status="${surfaced.status}" detail="${surfaced.detail}" (data-status=${surfaced.dataStatus}) — the 503 never surfaced to the user`,
      );
    }
    if (!saysKey) {
      return fail(
        `voice surfaced an error state (${surfaced.status}) but never named the cause: errorDetail="${surfaced.errorDetail}" recentError=${JSON.stringify(surfaced.recentError)} — expected the OPENAI_API_KEY reason`,
      );
    }
    return pass(
      `keyless voice degrades honestly: status="${surfaced.status}", detail="${surfaced.detail}", reason "${surfaced.recentError?.message}"; globe still alive`,
    );
  });
...
async function preflight() {
  ...
  const statusKey = async (path) => {
    ...
    try {
      ...
      return CONNECTION_REFUSED_RE.test(String(e?.message || e))
        ? 'error'
        : 'error';
    }
  ...
  try {
    ...
    if (ais.status === 503 && ais.json?.status === 'missing-key')
      env.keys.AIS = false;
    else if (ais.status === 200 && ais.json && Array.isArray(ais.json.rows))
      env.keys.AIS = true;
  ...
  try {
    const os = await jget(
      '/api/opensky?lamin=29&lomin=-99&lamax=31&lomax=-97',
      { timeoutMs: 40000 },
    );
    const reason = os.headers.get('X-OpenSky-Auth-Reason') || '';
    const used =
      os.headers.get('X-OpenSky-Auth-Mode-Used') ||
      os.headers.get('X-OpenSky-Auth') ||
      '';
    if (!os.ok) env.keys.OPENSKY = 'error';
    else if (/missing_.*creds|invalid_or_missing/.test(reason))
      env.keys.OPENSKY = false;
    else if (/^(oauth|basic)$/.test(used)) env.keys.OPENSKY = true;
    else if (used === 'anon') env.keys.OPENSKY = false;
    else env.keys.OPENSKY = 'error';
  } catch {
    env.keys.OPENSKY = 'error';
  }
  if (CHEAP) {
    ...
    try {
      ...
      if (t.status === 503 && /OPENAI_API_KEY is not set/.test(t.text))
        env.keys.OPENAI = false;
      else if (t.ok) env.keys.OPENAI = true;
      else env.keys.OPENAI = 'error';
    } catch {
      env.keys.OPENAI = 'error';
    }
  }
  ...
  if (m24) {
    const where = await sh('mise', ['where', `node@${m24[1]}`], {
      timeoutMs: 20000,
    });
...
// ── main ──────────────────────────────────────────────────────────────────
function keyLabel(v) {
  return v === true ? 'present' : v === false ? 'absent' : 'unknown';
}
...
async function main() {
  ...
  console.log(
    `  target : ${APP_URL}${CHEAP ? C.y('   [--cheap: read-only subset]') : ''}`,
  );

  if (LIST_ONLY) {
    for (const c of CHECKS.filter(selected)) {
      console.log(
        `  ${c.id.padEnd(4)} ${c.group}  ${c.desc}${c.heavy ? C.d(' [heavy]') : ''}${c.needsKey ? C.d(` [needs ${c.needsKey}]`) : ''}`,
      );
    }
    console.log(
  ...
  if (env.shellStatus >= 400) {
    console.log(
      C.r(
        `  shell  : HTTP ${env.shellStatus} — the target is RESPONDING but erroring. Running the matrix anyway; this is a product failure, not an environment one.`,
      ),
    );
  }
  console.log(
    `  node   : ${process.versions.node}${env.node24 ? C.d(` (Node 24 available: ${env.node24.label})`) : ''}`,
  );
  console.log(
    `  keys   : OpenSky ${keyLabel(env.keys.OPENSKY)} · FIRMS ${keyLabel(env.keys.FIRMS)} · TomTom ${keyLabel(env.keys.TOMTOM)} · AISStream ${keyLabel(env.keys.AIS)} · OpenAI ${keyLabel(env.keys.OPENAI)}`,
  );
  console.log(
    C.d(
      "  (key presence is read from each proxy's own status report; no key value is ever read or logged)\n",
    ),
  );
  ...
  const record = (c, rawRes, ms) => {
    ...
    if (res.detail)
      console.log(`       ${C.d(String(res.detail).slice(0, 300))}`);
    if (res.note)
      console.log(
        `       ${C.m(`KNOWN CONDITION: ${String(res.note).slice(0, 260)}`)}`,
      );
  };
  ...
  const runSerial = async (c) => {
    if (c.manual) {
      record(
        c,
        skip(
          "owner-eyes step — see the maintainers' release runbook",
          'OWNER-RUN',
        ),
        0,
      );
    ...
    if (CHEAP && (c.heavy || c.costly)) {
      record(
        c,
        skip('heavy/cost-bearing check omitted by --cheap', 'CHEAP'),
        0,
      );
      return;
    }
    if (c.needsKey && env.keys[c.needsKey] !== true) {
      ...
      if (state === 'error') {
        ...
        record(
          c,
          fail(
            `${c.needsKey} key state UNKNOWN: its status endpoint returned an error or a malformed payload on a responsive server — cannot claim the key is merely absent`,
          ),
          0,
        );
        return;
      ...
      record(
        c,
        skip(
          `server reports no ${c.needsKey} key (${keyLabel(state)}) — run on the fully keyed server`,
          'OWNER-RUN',
        ),
        0,
      );
    ...
    try {
      ...
      record(
        c,
        crash(
          `the check itself threw, so it verified nothing: ${String(e?.message || e).slice(0, 200)}`,
        ),
        Date.now() - t0,
      );
    }
  ...
  const runGuarded = async (c) => {
    if (!targetAlive && c.group !== 'A' && c.group !== 'M') {
      record(
        c,
        skip(
          `target ${APP_URL} stopped responding mid-run — restart the dev server and re-run this group`,
          'ENV',
        ),
        0,
      );
    ...
    if (!(await recheckTarget())) {
      for (const c of runList.filter((x) => x.browser)) {
        record(
          c,
          skip(
            `target ${APP_URL} stopped responding before the browser group`,
            'ENV',
          ),
          0,
        );
      ...
      try {
        ...
        for (const c of runList.filter(
          (x) => x.browser && !emitted.has(x.id),
        )) {
          record(
            c,
            crash(`browser group aborted before this check could run: ${why}`),
            0,
          );
        }
      }
      for (const c of runList.filter((x) => x.browser && !emitted.has(x.id))) {
        record(
          c,
          crash('browser group finished without reporting this check'),
          0,
        );
      }
    }
  ...
  for (const g of ['A', 'B', 'C', 'D', 'M']) {
    ...
    console.log(
      `  ${g} ${name.padEnd(15)} ${C.g(`${n(PASS)} pass`)}  ${n(PASS_SKIPS) ? C.m(`${n(PASS_SKIPS)} pass*`) : '0 pass*'}  ${n(FAIL) ? C.r(`${n(FAIL)} fail`) : '0 fail'}  ${n(CRASH) ? C.m(`${n(CRASH)} crash`) : '0 crash'}  ${C.y(`${n(SKIP)} skipped`)}   (${rows.length})`,
    );
  }
  console.log(`  ${'─'.repeat(68)}`);
  console.log(
    `  ${C.b('TOTAL')}  ${C.g(`${p} PASS`)}  ${ps ? C.m(`${ps} PASS-WITH-SKIPS`) : '0 PASS-WITH-SKIPS'}  ${f ? C.r(`${f} FAIL`) : '0 FAIL'}  ${x ? C.m(`${x} HARNESS-CRASH`) : '0 HARNESS-CRASH'}  ${C.y(`${s} SKIPPED`)}  of ${runList.length} selected   ${C.d(`${mins} min wall clock`)}`,
  );
  ...
  console.log(
    `  ${
      green
        ? C.g(
            'GREEN — every selected check either verified its claim or honestly skipped.',
          )
        : C.r(
            'NOT GREEN — a fully green L9 run requires 0 FAIL, 0 HARNESS-CRASH and 0 PASS-WITH-SKIPS.',
          )
    }`,
  );
  ...
  if (ps) {
    ...
    for (const r of byStatus(PASS_SKIPS))
      console.log(
        `   ${r.id}  ${r.desc}\n        ${C.d(String(r.detail).slice(0, 300))}`,
      );
  }
