    ...
    () =>
      new Promise((resolve, reject) => {
        ...
        const timeout = setTimeout(() => {
          remove();
          reject(new Error('Radio rendered-frame deadline exceeded'));
        }, 10_000);
...
async function main() {
  ...
  try {
      ...
      HTMLMediaElement.prototype.play = function play() {
        ...
        if (window.__qaRadioDelayNextFailure) {
          ...
          return new Promise((resolve, reject) => {
            window.__qaRejectRadioPlay = () => {
              reject(
                new DOMException(
                  'QA delayed broadcaster failure',
                  'NotSupportedError',
                ),
              );
        ...
        if (window.__qaRadioFailNextPlay) {
          ...
          return Promise.reject(
            new DOMException('QA silent broadcaster', 'NotSupportedError'),
          );
        }
        if (this.src.endsWith('/330.mp3') && !window.__qaRadioFailedNearest) {
          window.__qaRadioFailedNearest = true;
          return Promise.reject(
            new DOMException('QA broadcaster unavailable', 'NotSupportedError'),
          );
        }
    ...
    page.on('request', (request) => {
      ...
      if (
        ...
        ['/api/celestrak/active', '/api/celestrak/starlink'].includes(
          url.pathname,
        )
      ) {
      ...
      if (url.origin === APP_ORIGIN && url.pathname === '/api/radio/stations') {
        ...
        if (delayMs > 0)
          setTimeout(() => void request.respond(response), delayMs);
        else void request.respond(response);
        return;
      }
      if (
        url.origin === APP_ORIGIN &&
        /^\/api\/radio\/click\/[^/]+$/.test(url.pathname)
      ) {
    ...
    page.on('requestfailed', (request) => {
      ...
      if (
        /^https:\/\/api\.cesium\.com\/v1\/assets\/1\/endpoint(?:\?|$)/.test(url)
      ) {
        externalCesiumEndpointFailures.push(
    ...
    await page.goto(RADIO_URL.href, {
    ...
    await page.waitForFunction(
      () =>
        window.__godsEyeView?.styleManager?._dataManager?.layers?.has('radio'),
      { timeout: 60_000 },
    );
    await page.waitForFunction(
      () =>
    ...
    const initialDisclosure = await page.evaluate(() => {
      ...
      return {
        enabledBefore,
        enabledAfterDisclosure:
          window.__godsEyeView.dataManager.isEnabled('radio'),
    ...
    check(
      ...
      !initialDisclosure.enabledBefore &&
        !initialDisclosure.enabledAfterDisclosure &&
        initialDisclosure.expanded === 'true' &&
        !initialDisclosure.miniHidden,
      JSON.stringify(initialDisclosure),
    );
    await page.$eval('#context-radio-mini-enable-btn', (button) =>
      button.click(),
    );
    await page.waitForFunction(() => {
    ...
    const initial = await page.evaluate(() => {
      ...
      return {
        ...
        filterPalette: Array.from(
          document.getElementById('radio-filter').options,
        ).map((entry) => ({
          id: entry.value,
          label: entry.textContent,
          color: entry.dataset.radioColor,
        })),
        enablePressed: document
          .getElementById('radio-enable-btn')
          .getAttribute('aria-pressed'),
        launcherExpanded: document
          .getElementById('context-radio-toggle-btn')
          .getAttribute('aria-expanded'),
        launcherActive: document
          .getElementById('context-radio-toggle-btn')
          .classList.contains('active'),
        compactEnablePressed: document
          .getElementById('context-radio-mini-enable-btn')
          .getAttribute('aria-pressed'),
        fullPlayEnabled: !document.getElementById('radio-play-btn').disabled,
        fullPlayLabel: document
          .getElementById('radio-play-btn')
          .getAttribute('aria-label'),
        tunerVisible: !document.getElementById('radio-tuner').hidden,
        tunerCount:
          window.__godsEyeView.styleManager._radioControls._radioTunerStations
            .length,
        tunerLabel: document.getElementById('radio-tuner-band-label')
          .textContent,
      };
    });
    check(
      '750-station catalog renders within the browser budget',
      initial.state.stationCount === 750 && enableElapsedMs < 5000,
      `${enableElapsedMs}ms`,
    );
    check(
      'enable does not autoplay',
      initial.plays === 0 && initial.state.audioState === 'stopped',
    );
    check(
      'compact and detailed Radio power controls reflect the layer manager independently of disclosure',
      initial.enablePressed === 'true' &&
        initial.compactEnablePressed === 'true' &&
        initial.launcherExpanded === 'true' &&
        initial.launcherActive,
    );
    check(
      'dynamic canonical and genre categories reach the UI',
      initial.filterOptions >= 10,
      `${initial.filterOptions} options`,
    );
    const musicOption = initial.filterPalette.find(
      (entry) => entry.id === 'music',
    );
    const newsOption = initial.filterPalette.find(
      (entry) => entry.id === 'news',
    );
    check(
    ...
    check(
      'All is the initial Radio filter',
      initial.state.filter === 'all' && initial.state.filteredCount === 750,
      `${initial.state.filteredCount} stations`,
    );
    check(
      'the default All filter exposes a draggable directory tuner band',
      initial.tunerVisible &&
        initial.tunerCount === 750 &&
        initial.tunerLabel === 'DIRECTORY BAND',
      JSON.stringify({
        visible: initial.tunerVisible,
        count: initial.tunerCount,
        label: initial.tunerLabel,
      }),
    );
    const uncertainPresentation = await page.evaluate(async () => {
      ...
      const originalPinned =
        style._radioControls._radioTunerBandPinnedForNavigation;
      ...
      const result = {
        ...
        markerSourceVisible:
          gev.viewer.dataSources.getByName('Radio stations')[0]?.show,
        ...
        disclosureLabel: document
          .getElementById('context-radio-toggle-btn')
          .getAttribute('aria-label'),
        compactPowerLabel: document
          .getElementById('context-radio-mini-enable-btn')
          .getAttribute('aria-label'),
        compactPowerText: document.getElementById(
        ...
        playbackMessage: document.getElementById('radio-playback-state')
          .textContent,
        miniStation: document.getElementById('context-radio-mini-station')
          .textContent,
        dataRailText: document.querySelector(
          '[data-layer-id="radio"] .data-toggle-btn',
        )?.textContent,
        dataRailState: document.querySelector(
          '[data-layer-id="radio"] .data-toggle-btn',
        )?.dataset.feedState,
        dataRailLabel: document
          .querySelector('[data-layer-id="radio"] .data-toggle-btn')
          ?.getAttribute('aria-label'),
        dataRailMeta: document.querySelector(
          '[data-layer-id="radio"] .data-toggle-meta',
        )?.textContent,
        directSelect,
        directPlay,
        directCycle,
    ...
    check(
      ...
      uncertainPresentation.presentationActive === false &&
        ...
        uncertainPresentation.filterAfter ===
          uncertainPresentation.filterBefore &&
        uncertainPresentation.rejectedUIFilter.moduleFilter ===
          uncertainPresentation.filterBefore &&
        uncertainPresentation.rejectedUIFilter.controlFilter ===
          uncertainPresentation.filterBefore &&
        ...
        uncertainPresentation.selectedAfter ===
          uncertainPresentation.selectedBefore &&
    ...
    await page.evaluate(() => {
      ...
      module.disable = async (...args) => {
        ...
        await new Promise((resolve) => {
          window.__qaReleaseRadioDisable = resolve;
        });
        module.disable = originalDisable;
        return originalDisable(...args);
      };
      window.__qaRadioDisablePromise = gev.dataManager.setEnabled(
        'radio',
        false,
        { origin: 'qa-setup' },
      );
    ...
    const disablingPresentation = await page.evaluate(() => ({
      ...
      presentationActive: window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState().presentationActive,
      markerSourceVisible:
        window.__godsEyeView.viewer.dataSources.getByName('Radio stations')[0]
          ?.show,
      full: document.getElementById('radio-enable-btn').textContent,
      disclosure: document
        .getElementById('context-radio-toggle-btn')
        .getAttribute('aria-label'),
      disclosureControls: document
        .getElementById('context-radio-toggle-btn')
        .getAttribute('aria-controls'),
      disclosureExpanded: document
        .getElementById('context-radio-toggle-btn')
        .getAttribute('aria-expanded'),
      compactPower: document.getElementById('context-radio-mini-enable-btn')
        .textContent,
      layerState: document.getElementById('radio-layer-state').textContent,
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'lifecycle-disabling.png'),
    });
    check(
    ...
    const explicitRevealBefore = await page.evaluate(async () => {
      ...
      const scroller = document.querySelector(
        '#global-context-panel .global-context-panel-inner',
      );
      const radio = document.getElementById('radio-panel');
      scroller.scrollTop = Math.max(
        0,
        radio.offsetTop - scroller.clientHeight + 70,
      );
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'explicit-enable-before.png'),
    });
    ...
    await page.evaluate(() => {
      ...
      window.__qaRestoreRadioEnable = () => {
        radio.enable = originalEnable;
      };
    });
    ...
    await page.waitForFunction(
      () =>
        window.__godsEyeView.dataManager.getLayerLifecycleState('radio')
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'lifecycle-enabling.png'),
    });
    check(
    ...
    await page.waitForFunction(
      () => {
        ...
        const directory = document
          .querySelector('#radio-panel .radio-directory-row')
          .getBoundingClientRect();
        const play = document
          .getElementById('radio-play-btn')
          .getBoundingClientRect();
        return (
          directory.top >= viewport.top &&
          directory.bottom <= viewport.bottom &&
          play.top >= viewport.top &&
          play.bottom <= viewport.bottom
        );
      },
      { timeout: 10_000 },
    );
    const explicitRevealAfter = await page.evaluate(() => {
      ...
      return {
        ...
        camera: { x: camera.x, y: camera.y, z: camera.z },
        directoryVisible:
          directory.top >= viewport.top && directory.bottom <= viewport.bottom,
        playVisible: play.top >= viewport.top && play.bottom <= viewport.bottom,
        focusId: document.activeElement?.id,
        plays: window.__qaRadioPlayCalls.length,
        audioState: gev.dataManager.layers.get('radio').module.getUIState()
          .audioState,
      };
    });
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'explicit-enable-after.png'),
    });
    ...
    check(
      ...
      explicitRevealAfter.scrollTop > explicitRevealBefore.scrollTop &&
        explicitRevealAfter.directoryVisible &&
        explicitRevealAfter.playVisible &&
      ...
      JSON.stringify({
        before: explicitRevealBefore,
        after: explicitRevealAfter,
        cameraDelta: revealCameraDelta,
      }),
    );
    await page.evaluate(() => {
      window.__godsEyeView.styleManager.setPanelCollapsed('radio-panel', true);
        ...
        const sample = await page.evaluate(async () => {
          ...
          return new Promise((resolve) => {
            ...
            const finish = () => {
              ...
              const paintedSingletonIds = overlay.singletonIds.filter(
                (entryId) => Boolean(getOverlayPaintRect('radio', entryId)),
              );
              const pickableSingletonIds = paintedSingletonIds.filter(
                (entryId) => {
                  ...
                  return (viewer.scene.drillPick(anchor, 16) || []).some(
                    (picked) =>
                      radioStationIdFromPick(picked) ===
                      entryId.slice('station:'.length),
                  );
                },
              );
              resolve({
                ...
                nativeLabelCount: dataSource.entities.values.filter((entity) =>
                  Boolean(entity.label),
                ).length,
              });
            };
    ...
    const userFacingFailures = await page.evaluate(async () => {
      ...
      const originalClear =
        styleManager._contextControls._clearLayersOutsideContextMode;
      ...
      try {
        ...
        const runContextFailure = async (mode, phase, outcome) => {
          ...
          dataManager.setEnabled = async (layerId, shouldEnable) => {
            const failsIsolation =
              phase === 'isolation' &&
              layerId === 'local-datacenters' &&
              !shouldEnable;
            const failsActivation =
              phase === 'activation' &&
            ...
            if (!failureUsed && (failsIsolation || failsActivation)) {
              failureUsed = true;
              if (outcome === 'reject')
                throw new Error(`QA ${phase} rejection`);
              return false;
            }
          ...
          return {
            ...
            flightsButtonDisabled: document.getElementById(
              'global-context-flights-btn',
            ).disabled,
            missionsButtonDisabled: document.getElementById(
        ...
        dataManager.restoreEnabledLayerIds = async (
          enabledLayerIds,
          { excludeLayerIds = [] } = {},
        ) => {
          ...
          for (const candidateId of layerIds) {
            ...
            const restored = await dataManager.setEnabled(
              candidateId,
              target.has(candidateId),
            );
            if (restored === false) {
              throw new Error(
                `Failed to restore layer "${candidateId}" visibility`,
              );
            }
          }
        };
          ...
          for (const phase of ['isolation', 'activation']) {
            for (const outcome of ['false', 'reject']) {
              contextFailures.push(
                await runContextFailure(mode, phase, outcome),
              );
            }
          }
        }
          ...
          for (const outcome of ['false', 'reject']) {
            ...
            dataManager.setEnabled = async (
              ...
              if (
                ...
                if (outcome === 'reject')
                  throw new Error('QA direct isolation rejection');
                return false;
              }
              shouldEnable
                ? enabled.add(candidateId)
                : enabled.delete(candidateId);
              return true;
            };
        ...
        for (const layerId of ['military-awareness', 'rocket-launches']) {
          ...
          dataManager.setEnabled = async (candidateId, shouldEnable) => {
            if (candidateId === 'local-datacenters' && shouldEnable)
              return false;
            shouldEnable
              ? enabled.add(candidateId)
              : enabled.delete(candidateId);
          ...
          directActivationRollbackFailures.push({
            ...
            toastMessages,
            retryRetained:
              styleManager._contextControls._contextSessionSnapshot?.enabledLayerIds?.has(
                'local-datacenters',
              ) === true,
          });
        }
        ...
        for (const outcome of ['false', 'reject']) {
          ...
          const qaLayer = {
            ...
            init: async () => true,
            enable: async () => {
              if (enableOutcome === 'reject')
                throw new Error(`QA Context exit ${outcome}`);
          ...
          try {
            ...
            const result = await styleManager._runUserFacingContextAction(
              (notificationToken) =>
                styleManager._contextControls._deactivateContextForLayerChange({
                  notificationToken,
                }),
              `QA Context exit ${outcome} surfaced once`,
            );
            const retainedForRetry =
              styleManager._contextControls._contextSessionSnapshot?.enabledLayerIds?.has(
                layerId,
              ) === true;
        ...
        while (!releasePendingAction)
          await new Promise((resolve) => setTimeout(resolve, 0));
        const unrelatedResult = await dataManager.setEnabled(
          unrelatedLayerId,
          true,
        );
        ...
        await new Promise((resolve) => setTimeout(resolve, 50));
        const installationFalseMessage =
          document.getElementById('toast').textContent;
        const installationFalseReleased = !document.getElementById(
        ...
        document.getElementById('installations-search-btn').click();
        await new Promise((resolve) => setTimeout(resolve, 50));
        const installationEnableRejectionMessage =
          document.getElementById('toast').textContent;
        const installationEnableRejectionReleased = !document.getElementById(
          'installations-search-btn',
        ).disabled;
        ...
        if (installations)
          installations.searchNearby = async () => {
            throw new Error('QA installation search rejection');
          };
        document.getElementById('installations-search-btn').click();
        await new Promise((resolve) => setTimeout(resolve, 50));
        const installationRejectionMessage =
          document.getElementById('toast').textContent;
        const installationRejectionReleased = !document.getElementById(
          'installations-search-btn',
        ).disabled;
        ...
        document.getElementById('installations-search-btn').click();
        await new Promise((resolve) => setTimeout(resolve, 50));
        const installationSearchFalseMessage =
          document.getElementById('toast').textContent;
        const installationSearchFalseReleased = !document.getElementById(
          'installations-search-btn',
        ).disabled;
        await new Promise((resolve) => setTimeout(resolve, 0));
        return {
    ...
    check(
      ...
      userFacingFailures.semanticFalseMessage ===
        'Radio could not stop cleanly' &&
        userFacingFailures.contextFailures.length === 8 &&
        userFacingFailures.contextFailures.every(
          (failure) =>
        ...
        userFacingFailures.directEntryFailures.every(
          (failure) =>
        ...
        userFacingFailures.directActivationFailures.every(
          (failure) =>
            failure.mode === null &&
            failure.snapshotCleared &&
            failure.priorLayerRestored &&
        ...
        userFacingFailures.directActivationRollbackFailures.every(
          (failure) =>
            failure.toastMessages.length === 1 &&
            failure.toastMessages[0] ===
              `${failure.layerId} could not start cleanly` &&
            failure.retryRetained,
        ) &&
        userFacingFailures.contextExitFailures.length === 2 &&
        userFacingFailures.contextExitFailures.every(
          (failure) =>
            failure.result === false &&
        ...
        userFacingFailures.unrelatedManagerFailure.toastMessages[0].includes(
          'qa-unrelated-manager-failure could not start cleanly',
        ) &&
        userFacingFailures.installationFalseMessage.includes(
          'could not be refreshed',
        ) &&
        userFacingFailures.installationFalseReleased &&
        userFacingFailures.installationEnableRejectionMessage.includes(
          'could not be refreshed',
        ) &&
        userFacingFailures.installationEnableRejectionReleased &&
        userFacingFailures.installationRejectionMessage.includes(
          'could not be refreshed',
        ) &&
        userFacingFailures.installationRejectionReleased &&
        userFacingFailures.installationSearchFalseMessage.includes(
          'could not be refreshed',
        ) &&
    ...
    const clusterBadge = await page.evaluate(async () => {
      ...
      const { getOverlayPaintRect, getWorldOverlayDiagnostics } =
        await import('/src/overlays/worldOverlay.js');
      const { distanceFade } =
        await import('/src/overlays/worldOverlayDraw.js');
      const { radioStationIdFromPick } = await import('/src/data/radio.js');
      ...
      return new Promise((resolve) => {
        ...
        const remove = source.clustering.clusterEvent.addEventListener(
          async (entities, cluster) => {
            clearTimeout(timeout);
            remove();
            const native = {
              ...
              pointMaxDistance:
                cluster.point.distanceDisplayCondition?.far ?? null,
            };
              ...
              painted = await new Promise((done) => {
                ...
                removePostRender = viewer.scene.postRender.addEventListener(
                  () => {
                    ...
                    let match = null;
                    for (const entryId of radio.getOverlayDiagnostics()
                      .clusterIds) {
                      const rect = getOverlayPaintRect('radio', entryId);
    ...
    check(
      ...
      clusterBadge?.native?.labelShow === false &&
        ...
        clusterBadge?.source?.clusterTexts?.some((text) =>
          /^\d+\s+[A-Z]/.test(text),
        ) &&
        clusterBadge?.source?.entryCount <= 65 &&
        clusterBadge?.host?.entriesBySource?.radio ===
          clusterBadge?.source?.entryCount &&
        clusterBadge?.host?.solveRevision >
          clusterBadge?.baselineSolveRevision &&
    ...
    const highGlobalClusterLabels = await page.evaluate(async () => {
      ...
      const points = Array.from(
        { length: viewer.dataSources.length },
        (_, index) => viewer.dataSources.get(index),
      ).find((item) => item.name === 'Radio stations')?.clustering
      ...
      for (let index = 0; index < (points?.length || 0); index += 1) {
        ...
        if (!point?.show || !Array.isArray(point.id) || !point.id.length)
          continue;
        const anchor = viewer.scene.cartesianToCanvasCoordinates(
          point.position,
        );
        if (!anchor) continue;
        if (
          anchor.x < 0 ||
          anchor.y < 0 ||
          anchor.x > viewer.canvas.clientWidth ||
          anchor.y > viewer.canvas.clientHeight
        )
          continue;
      ...
      visiblePoints.sort(
        (a, b) =>
          Math.hypot(
            a.anchor.x - viewer.canvas.clientWidth / 2,
            a.anchor.y - viewer.canvas.clientHeight / 2,
          ) -
          Math.hypot(
            b.anchor.x - viewer.canvas.clientWidth / 2,
            b.anchor.y - viewer.canvas.clientHeight / 2,
          ),
      );
      if (visiblePoints.length) {
        const { point, anchor } = visiblePoints[0];
        const exactPick = (viewer.scene.drillPick(anchor, 16) || []).find(
          (picked) => picked?.primitive === point && picked?.id === point.id,
        );
        clusterPoint = {
      ...
      return {
        ...
        horizonDistance,
        horizonDistanceAlpha: distanceFade(horizonDistance, {
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'high-global-all-labels.png'),
    });
    const clusterContinuity = await page.evaluate(async () => {
      ...
      const paintedClusters = () =>
        new Promise((resolve) => {
          ...
          removePostRender = viewer.scene.postRender.addEventListener(() => {
            ...
            const source = radio.getOverlayDiagnostics();
            const host = getWorldOverlayDiagnostics();
            const clusters = source.clusterIds
              .map((entryId, index) => {
                const rect = getOverlayPaintRect('radio', entryId);
                return rect
                  ? {
      ...
      for (let index = 1; index < settled.length; index += 1) {
        const previous = new Map(
          settled[index - 1].clusters.map((cluster) => [
            cluster.entryId,
            cluster.text,
          ]),
        );
        for (const cluster of settled[index].clusters) {
          ...
          if (previous.get(cluster.entryId) !== cluster.text)
            countChangingRetained += 1;
        }
      }
      return {
        sampleCount: samples.length,
        paintedSamples: samples.filter((sample) => sample.clusters.length > 0)
          .length,
        unpaintedProjectedSamples: samples.filter(
          (sample) =>
            sample.clusters.length === 0 &&
            (sample.host?.projectedCount || 0) > 0,
        ).length,
        emptySamples: samples
          .filter((sample) => sample.clusters.length === 0)
          .map((sample) => ({
        ...
        fadingSamples: samples
          .filter((sample) => sample.fadingCount !== 0)
          .map((sample) => ({
            longitude: sample.longitude,
            delay: sample.delay,
            fadingCount: sample.fadingCount,
          })),
        allHardOpacity: samples.every((sample) =>
          sample.clusters.every(
    ...
    const directoryRefreshContinuity = await page.evaluate(async () => {
      ...
      const beforeByMembership = new Map(
        before.map((entry) => [entry.membershipId, entry.entryId]),
      );
      const unchanged = after.filter((entry) =>
        beforeByMembership.has(entry.membershipId),
      );
      return {
        beforeCount: before.length,
        afterCount: after.length,
        unchangedCount: unchanged.length,
        retainedCount: unchanged.filter(
          (entry) =>
            beforeByMembership.get(entry.membershipId) === entry.entryId,
        ).length,
      };
    });
    check(
      ...
      directoryRefreshContinuity.beforeCount > 0 &&
        ...
        directoryRefreshContinuity.retainedCount ===
          directoryRefreshContinuity.unchangedCount,
      JSON.stringify(directoryRefreshContinuity),
    );
    await page.select('#radio-filter', 'news');
    check(
      'launcher enables Radio without forcing Context open',
      !initial.panelExpanded &&
        !initial.contextExpanded &&
        initial.nestedInContext,
    );
    check(
      'full Play is available before a station is selected',
      !initial.state.selected &&
        initial.fullPlayEnabled &&
        initial.fullPlayLabel === 'Play nearest radio station',
    );
    ...
    await sleep(100);
    const hoverOnlyVisible = await page.$eval(
      '#context-radio-mini',
      (mini) => !mini.hidden && getComputedStyle(mini).display !== 'none',
    );
    check(
      'hover alone does not disclose compact Radio controls',
      hoverOnlyDisclosure.expanded === 'false' &&
        hoverOnlyDisclosure.hidden &&
        !hoverOnlyVisible,
    ...
    await page.waitForFunction(() => {
      ...
      return (
        ...
        !mini.hidden &&
        getComputedStyle(mini).display !== 'none'
      );
    });
    const compact = await page.evaluate(() => {
      ...
      return {
        ...
        offsetParent:
          mini.offsetParent?.id || mini.offsetParent?.className || null,
        prevEnabled: !document.getElementById('context-radio-mini-prev-btn')
          .disabled,
        playEnabled: !document.getElementById('context-radio-mini-play-btn')
          .disabled,
        playLabel: document
          .getElementById('context-radio-mini-play-btn')
          .getAttribute('aria-label'),
        nextEnabled: !document.getElementById('context-radio-mini-next-btn')
          .disabled,
        volumeEnabled: !document.getElementById('context-radio-mini-volume')
          .disabled,
    ...
    check(
      ...
      compact.visible &&
        ...
        compact.playEnabled &&
        compact.nextEnabled &&
        compact.volumeEnabled,
      JSON.stringify(compact),
    );
    check(
      'compact Play is available before a station is selected',
      compact.playLabel === 'Play nearest radio station',
    );
    check(
      'compact Radio controls sit 20px below the launcher',
      compact.offsetBelowToggle === 20,
      `${compact.offsetBelowToggle}px`,
    );
    const compactCloseDisclosure = await page.evaluate(() => {
      ...
      const result = {
        ...
        radioCollapsed: document
          .getElementById('radio-panel')
    ...
    const multiExpandedPanelLayout = await page.evaluate(async () => {
      ...
      const prior = {
        ...
        stacks: Object.fromEntries(
          ['left-panel-stack', 'right-context-rail'].map((id) => {
      ...
      try {
        ...
        result = {
          independentLeft: ['data-panel', 'scene-panel'].every(
            (id) =>
              !document.getElementById(id).classList.contains('collapsed'),
          ),
          independentRight: ['pp-toggles', 'global-context-panel'].every(
            (id) =>
              !document.getElementById(id).classList.contains('collapsed'),
          ),
          leftCountsAllExpanded: left.dataset.expandedCount === '2',
          rightCountsAllExpanded: right.dataset.expandedCount === '2',
          leftInsideCorridor: leftExpanded.every(
            (rect) =>
              rect.top >= leftRect.top - 1 &&
              rect.bottom <= leftRect.bottom + 1,
          ),
          rightInsideCorridor: rightExpanded.every(
            (rect) =>
              rect.top >= rightRect.top - 1 &&
              rect.bottom <= rightRect.bottom + 1,
          ),
          internalScrollOwned:
            getComputedStyle(
              document.querySelector('#data-panel .data-toggle-list'),
            ).overflowY === 'auto' &&
            getComputedStyle(
              document.querySelector('#scene-panel .scene-panel-inner'),
            ).overflowY === 'auto' &&
            getComputedStyle(
        ...
        result.tacticalAutoCollapse = {
          ...
          dataHiddenWhileScenesOwnsLane:
            dataPanel.getBoundingClientRect().height === 0 &&
            dataPanel.getAttribute('aria-hidden') === 'true',
          dataAriaExpanded: dataPanel
            .querySelector('[data-collapse-target="data-panel"]')
            ?.getAttribute('aria-expanded'),
          displayExpanded: !document
            .getElementById('pp-toggles')
            .classList.contains('collapsed'),
          contextCollapsed: contextPanel.classList.contains(
            'layout-auto-collapsed',
          ),
          contextHiddenWhileDisplayOwnsLane:
            contextPanel.getBoundingClientRect().height === 0 &&
            contextPanel.getAttribute('aria-hidden') === 'true',
          contextAriaExpanded: contextPanel
        ...
        result.tacticalAutoRestore = {
          ...
          dataAutoMarkerCleared: !dataPanel.classList.contains(
            'layout-auto-collapsed',
          ),
          dataAriaExpanded: dataPanel
            .querySelector('[data-collapse-target="data-panel"]')
          ...
          contextAutoMarkerCleared: !contextPanel.classList.contains(
            'layout-auto-collapsed',
          ),
          contextAriaExpanded: contextPanel
            .querySelector('[data-collapse-target="global-context-panel"]')
        ...
        result.tacticalDisplayExclusive = {
          ...
          cctvHidden:
            cctvPanel.getBoundingClientRect().height === 0 &&
        ...
        for (const [id, state] of Object.entries(prior.stacks)) {
          ...
          for (const key of Object.keys(stack.dataset))
            delete stack.dataset[key];
          Object.assign(stack.dataset, state.dataset);
        }
        manager._panelLayout._leftStackPreferredPanelId =
          prior.preferredLeftPanelId;
        manager._panelLayout._rightStackPreferredPanelId =
          prior.preferredRightPanelId;
        localStorage.clear();
        for (const [key, value] of Object.entries(prior.storage))
        ...
        prior.focusId &&
          document
            .getElementById(prior.focusId)
            ?.focus({ preventScroll: true });
        const panelsRestored = Object.entries(prior.panels).every(
          ([id, state]) => {
            const panel = document.getElementById(id);
            return (
              panel.className === state.className &&
        ...
        const stacksRestored = Object.entries(prior.stacks).every(
          ([id, state]) => {
            const stack = document.getElementById(id);
            return (
              stack.className === state.className &&
              stack.getAttribute('style') === state.style &&
              JSON.stringify({ ...stack.dataset }) ===
        ...
        result.restoreDetails = {
          panelsRestored,
          stacksRestored,
          preferredPanelRestored:
            manager._panelLayout._leftStackPreferredPanelId ===
            prior.preferredLeftPanelId,
          preferredRightPanelRestored:
            manager._panelLayout._rightStackPreferredPanelId ===
            prior.preferredRightPanelId,
          ...
          storageRestored:
            ...
            JSON.stringify(prior.storage),
          focusRestored:
            !prior.focusId || document.activeElement?.id === prior.focusId,
        };
        result.restoredExactly = Object.values(result.restoreDetails).every(
          Boolean,
        );
      }
    ...
    check(
      ...
      multiExpandedPanelLayout.tacticalAutoCollapse.latestOpenedPanel ===
        'scene-panel' &&
        multiExpandedPanelLayout.tacticalAutoCollapse.scenesExpanded &&
        multiExpandedPanelLayout.tacticalAutoCollapse.dataCollapsed &&
        multiExpandedPanelLayout.tacticalAutoCollapse
          .dataHiddenWhileScenesOwnsLane &&
        multiExpandedPanelLayout.tacticalAutoCollapse.dataAriaExpanded ===
          'false' &&
        ...
        multiExpandedPanelLayout.tacticalAutoCollapse
          .contextHiddenWhileDisplayOwnsLane &&
        multiExpandedPanelLayout.tacticalAutoCollapse.contextAriaExpanded ===
          'false',
    ...
    check(
      ...
      multiExpandedPanelLayout.tacticalAutoRestore.dataExpanded &&
        ...
        multiExpandedPanelLayout.tacticalAutoRestore.dataAriaExpanded ===
          'true' &&
        ...
        multiExpandedPanelLayout.tacticalAutoRestore.contextAriaExpanded ===
          'true',
    ...
    check(
      ...
      Object.values(multiExpandedPanelLayout.tacticalDisplayExclusive).every(
        Boolean,
      ),
    ...
    const parameterizedDisplayScroll = await page.evaluate(async () => {
      ...
      const prior = {
        ...
        contextCollapsed: document
          .getElementById('global-context-panel')
          .classList.contains('collapsed'),
        cctvCollapsed: document
          .getElementById('cctv-panel')
          .classList.contains('collapsed'),
        hudMode: manager.hud.getMode(),
        hudVariant: manager.hud.getVariant(),
        focusId: document.activeElement?.id || null,
      ...
      for (const state of [
        ...
        await waitForLayout();
        manager._syncRightPanelAdaptiveLayout();
        display.scrollTop = Math.max(
          1,
          display.scrollHeight - display.clientHeight,
        );
        const before = display.scrollTop;
        manager._syncRightPanelAdaptiveLayout();
        const lastSlider = document.querySelector(
          '#param-sliders .param-slider-row:last-child input',
        );
        ...
        cases.push({
          ...
          finalRowReachable:
            sliderRect.top >= displayRect.top - 1 &&
            sliderRect.bottom <= displayRect.bottom + 1,
          displayOwnsScroll: getComputedStyle(display).overflowY === 'auto',
          parametersDoNotTrapScroll:
            getComputedStyle(parameters).overflowY === 'visible',
          parameterRows: document.querySelectorAll(
            '#param-sliders .param-slider-row',
          ).length,
        });
      }
    ...
    check(
      ...
      parameterizedDisplayScroll.every(
        (state) =>
    ...
    const cockpitCompactControls = await page.evaluate(async () => {
      ...
      const priorHudTransitionPriority =
        intelHud.style.getPropertyPriority('transition');
      const waitForLayout = async () => {
        await new Promise((resolve) => setTimeout(resolve, 320));
        await new Promise((resolve) =>
          requestAnimationFrame(() => requestAnimationFrame(resolve)),
      ...
      const prior = {
        ...
        preferredLeftPanelId: manager._panelLayout._leftStackPreferredPanelId,
        panels: Object.fromEntries(
          ['data-panel', 'global-context-panel'].map((id) => {
            const panel = document.getElementById(id);
            return [
              id,
              {
      ...
      try {
        ...
        const recordLayoutStep = () => {
          ...
          const utilityRect = document
            .getElementById('cockpit-utility-controls')
            .getBoundingClientRect();
          const displayRect = display
            .closest('.cockpit-utility-control')
            .getBoundingClientRect();
          const toplineRect = document
            .querySelector('#cockpit-hud .cockpit-topline')
            .getBoundingClientRect();
          const recRect = visibleRect('#intel-hud .hud-top-right');
          const creditsRect = visibleRect(
          ...
          const passableTops = [
            ...
            .filter(
              (rect) =>
                rect &&
                rect.right > dataRect.left &&
                rect.left < dataRect.right,
            )
            .map((rect) => rect.top);
          const stack = document.getElementById('left-panel-stack');
          layoutSteps.push({
            hud: manager.hud.visible
              ? manager.hud.getVariant().toUpperCase()
              : 'OFF',
            ...
            layoutBottom:
              (window.innerHeight *
                (Number(stack.dataset.safeBottomPct) || 0)) /
              100,
        ...
        const toggleRows = [
          ...toggleList.querySelectorAll('.data-toggle-row'),
        ].filter((row) => row.querySelector('.data-toggle-btn'));
        const toggleRow =
          toggleRows.find((row) =>
            String(row.dataset.layerId).startsWith('local-'),
          ) ||
          toggleRows.find(
            (row) =>
              !/flight|military|vessel|ais|radio/i.test(
                row.dataset.layerId || '',
              ),
          );
        ...
        const toggleBefore = toggleLayerId
          ? window.__godsEyeView.dataManager.isEnabled(toggleLayerId)
          : null;
        toggleButton?.click();
        await waitForLayout();
        ...
        for (let index = 0; index < 3; index += 1) {
          ...
          const militaryParams =
            manager._dataManager.getLayerParams('military');
          models3dSteps.push({
            label: manager._models3dEnabled
              ? manager._models3dMode.toUpperCase()
              : 'OFF',
        ...
        manager.cockpitView.syncSignalLayout();
        const displayOpened =
          display.getAttribute('aria-expanded') === 'true' &&
          !document.getElementById('cockpit-display-panel').hidden;
        const sharedDisplayControlsPortaled = [
          hudToggle,
          detectionToggle,
          models3dToggle,
        ].every((control) =>
          control
            .closest('.pp-toggle-group')
            ?.parentElement?.closest('#cockpit-display-panel'),
        );
        display.click();
        ...
        radio.click();
        const radioOpened =
          radio.getAttribute('aria-expanded') === 'true' &&
          !document.getElementById('cockpit-radio-panel').hidden;
        const mutuallyExclusive =
          display.getAttribute('aria-expanded') === 'false';
        radio.click();
        ...
        const photorealAvailable = manager.mapStackController
          ?.getStacks()
        ...
        result = {
          ...
          scenesHidden,
          mapProviderUtilityStable,
          hudInteractionsExact:
            layoutSteps.map((step) => step.hud).join('>') ===
          ...
          leftLaneClearAcrossHudCycle: layoutSteps.every(
            (step) =>
              step.dataTop >= step.toplineBottom &&
              (step.creditsTop === null ||
                step.dataBottom <= step.creditsTop) &&
              (step.passableTop === null ||
                step.dataBottom <= step.passableTop + 0.5),
          ),
          ...
          rightStripClearAcrossHudCycle: layoutSteps.every((step) => {
            ...
            return (
              step.displayTop >= step.toplineBottom &&
              step.utilityBottom <= step.signalTop - 7.9 &&
              Math.abs(step.utilityTop - expected) < 0.6
            );
          }),
          models3dInteractionsExact:
            models3dSteps.map((step) => step.label).join('>') ===
            'PROXIMITY>ALL>OFF',
          models3dLayersSynchronized: models3dSteps.every((step, index) => {
            ...
            return (
              step.flightsEnabled === expectedEnabled &&
        ...
        if (prior.mapStack)
          await manager._setMapStack(prior.mapStack, { syncShare: false });
        ...
        manager._setHudVariant(prior.hudVariant);
        manager.hud.setMode(prior.hudMode);
        if (priorHudTransition)
          intelHud.style.setProperty(
            'transition',
            priorHudTransition,
            priorHudTransitionPriority,
          );
        ...
        manager.setPanelCollapsed(
          'global-context-panel',
          prior.contextCollapsed,
        );
        if (
          display.getAttribute('aria-expanded') !==
          String(prior.displayExpanded)
        )
          display.click();
        if (radio.getAttribute('aria-expanded') !== String(prior.radioExpanded))
          radio.click();
        ...
        result.restoredExactly =
          ...
          document
            .getElementById('global-context-panel')
            .classList.contains('collapsed') === prior.contextCollapsed &&
          display.getAttribute('aria-expanded') ===
            String(prior.displayExpanded) &&
          ...
          document
            .getElementById('context-radio-toggle-btn')
            ?.getAttribute('aria-expanded') ===
            String(prior.contextRadioExpanded) &&
          hud.hidden === prior.hudHidden &&
          signal.hidden === prior.signalHidden &&
          intelHud.style.getPropertyValue('transition') ===
            priorHudTransition &&
          intelHud.style.getPropertyPriority('transition') ===
            priorHudTransitionPriority &&
    ...
    const firstPlayPrecondition = await page.evaluate(
      () =>
        window.__godsEyeView.dataManager.layers.get('radio').module.getUIState()
          .selected?.id || null,
    );
    check(
      'first Play scenario starts without a leaked Radio selection',
      firstPlayPrecondition === null,
    ...
    await page.$eval('#context-radio-mini-play-btn', (button) =>
      button.click(),
    );
    await page.waitForFunction(() => {
      const state = window.__godsEyeView.dataManager.layers
    ...
    check(
      'micro-drag release leaves the selected station playable',
      postMicroDragPlayback.audioState === 'playing',
      JSON.stringify(postMicroDragPlayback),
    );
    ...
    check(
      ...
      Boolean(firstPlay.state.selected?.id) &&
        firstPlay.state.filter === 'all' &&
        firstPlay.state.audioState === 'playing' &&
        firstPlay.calls.length >= 1,
      JSON.stringify({
        selected: firstPlay.state.selected?.id,
        calls: firstPlay.calls,
      }),
    );
    check(
      'first Play never moves the camera',
      firstPlay.flyToCalls === 0,
      `${firstPlay.flyToCalls} flyTo calls`,
    );
    ...
    const newsClusterTarget = await page.evaluate(() => {
      const gev = window.__godsEyeView;
      const source = Array.from(
        { length: gev.viewer.dataSources.length },
        (_, index) => gev.viewer.dataSources.get(index),
      ).find((item) => item.name === 'Radio stations');
      ...
      for (let index = 0; index < (points?.length || 0); index += 1) {
        ...
        if (!point.show || !Array.isArray(point.id) || !point.id.length)
          continue;
        const canvasPoint = gev.viewer.scene.cartesianToCanvasCoordinates(
          point.position,
        );
      ...
      return {
        ...
        points: Array.from({ length: points?.length || 0 }, (_, index) => ({
          ...
          pointIds: Array.isArray(points.get(index)?.id)
            ? points.get(index).id.length
            : 0,
        })),
      };
    });
      ...
      await page.waitForFunction(
        (stationId) => {
          const state = window.__godsEyeView.dataManager.layers
            ...
            .module.getUIState();
          return (
            state.selected?.id === stationId && state.audioState === 'playing'
          );
        },
        {},
    ...
    check(
      ...
      weatherCluster.texts.length > 0 &&
        weatherCluster.texts.every((text) => /^\d+ WEATHER$/.test(text)),
    ...
    const compactVolume = await page.evaluate(() => ({
      volume: window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState().volume,
      full: document.getElementById('radio-volume').value,
      mini: document.getElementById('context-radio-mini-volume').value,
    }));
    check(
      'compact volume shares the full Radio player state',
      Math.abs(compactVolume.volume - 0.35) < 0.001 &&
        compactVolume.full === '35' &&
        compactVolume.mini === '35',
    );
    ...
    check(
      ...
      compactDisclosureContinuity.closed.expanded === 'false' &&
        ...
        compactDisclosureContinuity.after.selected?.id ===
          compactDisclosureContinuity.before.selected?.id &&
        compactDisclosureContinuity.after.filter ===
          compactDisclosureContinuity.before.filter &&
        compactDisclosureContinuity.after.volume ===
          compactDisclosureContinuity.before.volume,
      JSON.stringify({
    ...
    const voicePause = await page.evaluate(async () => {
      ...
      window.__godsEyeView.voiceCommands.setStatus(
        'connecting',
        'QA voice connect',
      );
      const state = window.__godsEyeView.dataManager.layers
    ...
    check(
      ...
      voicePause.audioState === 'paused' &&
        Math.abs(voicePause.volume - 0.35) < 0.001,
      JSON.stringify({
        audioState: voicePause.audioState,
        user: voicePause.volume,
      }),
    );
    check(
    ...
    const idleVoice = await page.evaluate(() => {
      ...
      return window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState();
    });
    check(
      ...
      await page.waitForFunction(
        () =>
          window.__godsEyeView.dataManager.layers
            .get('radio')
            .module.getUIState().audioState === 'playing' &&
          window.__godsEyeView.voiceCommands.status === 'idle',
      );
    }
    const manualTakeover = await page.evaluate(() => ({
      audioState: window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState().audioState,
      voiceStatus: window.__godsEyeView.voiceCommands.status,
      voiceDucked: window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState().voiceDucked,
    }));
    check(
    ...
    await page.evaluate(() =>
      window.__godsEyeView.voiceCommands.setStatus('listening', 'QA standby'),
    );
    const explicitResume = await page.evaluate(async () => {
      ...
      return {
        beforeSpace,
        afterSpace,
        afterAi: radio.getUIState().audioState,
      };
    });
    check(
      'holding Space and assistant speech each pause an explicitly resumed station',
      explicitResume.beforeSpace === 'playing' &&
        explicitResume.afterSpace === 'paused' &&
        explicitResume.afterAi === 'paused',
      JSON.stringify(explicitResume),
    );
    await page.evaluate(() =>
      window.__godsEyeView.voiceCommands.setStatus('idle', 'QA complete'),
    );
    await page.evaluate(async () => {
      const manager = window.__godsEyeView.styleManager;
    ...
    check(
      'far-side markers are horizon culled',
      horizon.front > 0 &&
        horizon.front < 750 &&
        horizon.back > 0 &&
        horizon.back < 750,
      `${horizon.front}/${horizon.back} visible`,
    );
    ...
    const absoluteTuner = await page.evaluate(async () => {
      ...
      const slider = document.getElementById('radio-tuner-slider');
      const tunerRect = document
        .getElementById('radio-tuner')
        .getBoundingClientRect();
      const dialRect = document
        .querySelector('.radio-tuner-dial')
        .getBoundingClientRect();
      const sliderRect = slider.getBoundingClientRect();
      const panelRect = document
        .querySelector('.radio-panel-inner')
        .getBoundingClientRect();
      const count = style._radioControls._radioTunerStations.length;
      const usable = sliderRect.width - 14;
      const xFor = (coordinate) =>
        sliderRect.left + 7 + (usable * coordinate) / Math.max(1, count - 1);
      const pointer = (type, x, pointerId = 71) =>
      ...
      const snapshot = () => {
        const markerId =
          gev.viewer.entities.values.find((entity) =>
            String(entity.id).startsWith('radio:selected:'),
          )?.id || null;
        return {
          ...
          ratio: Number(
            document
              .getElementById('radio-tuner')
              .style.getPropertyValue('--radio-tuner-ratio'),
          ),
          needleX: document
            .getElementById('radio-tuner-needle')
            .getBoundingClientRect().left,
          ticks: Array.from(document.querySelectorAll('.radio-tuner-tick')).map(
            (tick) => ({
      ...
      const shiftedByIndex = new Map(
        shifted.ticks.map((tick) => [tick.index, tick.x]),
      );
      const sharedTick = center.ticks.find((tick) =>
        shiftedByIndex.has(tick.index),
      );
      const tapeDelta = sharedTick
        ? shiftedByIndex.get(sharedTick.index) - sharedTick.x
        : null;
      ...
      return {
        ...
        bandFrozen:
          style._radioControls._radioTunerBandSignature === frozenSignature,
        left,
        center,
        shifted,
        right,
        ...
        centerExpectedId:
          style._radioControls._radioTunerStations[
            Math.floor(centerCoordinate + 0.5)
          ]?.id || null,
        rightExpectedId:
          style._radioControls._radioTunerStations[count - 1]?.id || null,
        ...
        contained:
          tunerRect.left >= panelRect.left &&
          tunerRect.right <= panelRect.right &&
          dialRect.left >= tunerRect.left &&
          dialRect.right <= tunerRect.right &&
          sliderRect.left >= dialRect.left &&
          sliderRect.right <= dialRect.right,
      };
    });
    check(
      'All maps the complete directory to real left, center, and right station snap points',
      absoluteTuner.visible &&
        absoluteTuner.stationCount === 750 &&
        absoluteTuner.sliderMax === 749 &&
        absoluteTuner.left.slot === 0 &&
        absoluteTuner.left.markerId ===
          `radio:selected:${absoluteTuner.leftExpectedId}` &&
        absoluteTuner.center.slot === absoluteTuner.centerExpected &&
        absoluteTuner.center.markerId ===
          `radio:selected:${absoluteTuner.centerExpectedId}` &&
        absoluteTuner.right.slot === absoluteTuner.stationCount - 1 &&
        absoluteTuner.right.markerId ===
          `radio:selected:${absoluteTuner.rightExpectedId}` &&
        absoluteTuner.bandFrozen &&
        absoluteTuner.noPreviewPlay &&
        !absoluteTuner.tuningActiveAfterCancel &&
        !absoluteTuner.tuningStaticAfterCancel &&
    ...
    check(
      ...
      absoluteTuner.maxTickCount > 0 &&
        absoluteTuner.maxTickCount < 100 &&
        absoluteTuner.needleDelta > 0 &&
        absoluteTuner.tapeDelta < 0 &&
    ...
    const tunerCancelRestore = await page.evaluate(() => {
      ...
      const xFor = (index) =>
        sliderRect.left +
        7 +
        ((sliderRect.width - 14) * index) /
          Math.max(1, beforeStations.length - 1);
      const pointer = (type, index, pointerId = 52) =>
        slider.dispatchEvent(
          new PointerEvent(type, {
            bubbles: true,
            pointerId,
      ...
      const changedFilterCleanup = {
        ...
        entityRebuilt: Boolean(
          changedFilterEntity && changedFilterEntity !== restoredEntity,
        ),
        entityId: changedFilterEntity?.id || null,
        snapshotPreserved:
          radio.getAcceptedCatalogSnapshot() === changedFilterSnapshot,
      ...
      const sameFilterCleanup = {
        ...
        entityRebuilt: Boolean(
          sameFilterEntity && sameFilterEntity !== sameFilterRestoredEntity,
        ),
        entityId: sameFilterEntity?.id || null,
        snapshotPreserved:
          radio.getAcceptedCatalogSnapshot() === sameFilterSnapshot,
      ...
      return {
        ...
        after,
        identityRestored:
          afterStations.length === beforeStations.length &&
          afterStations.every(
            (station, index) => station === beforeStations[index],
          ),
      };
    });
    check(
      ...
      tunerCancelRestore.shiftedSlot === tunerCancelRestore.targetIndex &&
        tunerCancelRestore.after.signature ===
          tunerCancelRestore.before.signature &&
        JSON.stringify(tunerCancelRestore.after.ids) ===
          JSON.stringify(tunerCancelRestore.before.ids) &&
        tunerCancelRestore.after.slot === tunerCancelRestore.before.slot &&
        tunerCancelRestore.after.selectedId ===
          tunerCancelRestore.before.selectedId &&
        tunerCancelRestore.after.plays === tunerCancelRestore.before.plays &&
        tunerCancelRestore.previewBeforeCancel !==
          `radio:selected:${tunerCancelRestore.before.selectedId}` &&
        tunerCancelRestore.flyToCallsBeforeCancel > 0 &&
        tunerCancelRestore.flyToCallsAfterCancel ===
          tunerCancelRestore.flyToCallsBeforeCancel &&
        tunerCancelRestore.after.previewId ===
          `radio:selected:${tunerCancelRestore.before.selectedId}` &&
    ...
    check(
      ...
      tunerCancelRestore.changedFilterCleanup.filter ===
        tunerCancelRestore.changedFilter &&
        tunerCancelRestore.changedFilterCleanup.restoredStationId === null &&
        tunerCancelRestore.changedFilterCleanup.selectedId ===
          tunerCancelRestore.before.selectedId &&
        tunerCancelRestore.changedFilterCleanup.entityRebuilt &&
        tunerCancelRestore.changedFilterCleanup.entityId ===
          `radio:selected:${tunerCancelRestore.before.selectedId}` &&
    ...
    check(
      ...
      tunerCancelRestore.sameFilterCleanup.began &&
        ...
        tunerCancelRestore.sameFilterCleanup.restoredBefore ===
          tunerCancelRestore.before.selectedId &&
        tunerCancelRestore.sameFilterCleanup.restoredAfter === null &&
        tunerCancelRestore.sameFilterCleanup.filter ===
          tunerCancelRestore.changedFilter &&
        tunerCancelRestore.sameFilterCleanup.selectedId ===
          tunerCancelRestore.before.selectedId &&
        tunerCancelRestore.sameFilterCleanup.entityRebuilt &&
        tunerCancelRestore.sameFilterCleanup.entityId ===
          `radio:selected:${tunerCancelRestore.before.selectedId}` &&
    ...
    const tunerRefreshTarget = await page.evaluate(() => {
      ...
      const clientX =
        rect.left +
        7 +
        ((rect.width - 14) * targetIndex) / Math.max(1, stations.length - 1);
      slider.dispatchEvent(
        new PointerEvent('pointerdown', {
          bubbles: true,
          pointerId: 51,
      ...
      return {
        ...
        clientX,
        clientY: rect.top + rect.height / 2,
        generation: gev.dataManager.layers.get('radio').module.getUIState()
          .tuningCatalogGeneration,
        plays: window.__qaRadioPlayCalls.length,
      };
    });
    catalogStations = stations.map((station) =>
      station.id === tunerRefreshTarget.id
        ? {
            ...station,
            name: `${station.name} B`,
            streamUrl: `${station.streamUrl}?generation=b`,
          }
        : station,
    );
    catalogGeneration = 2;
    ...
    const tunerRefreshRelease = await page.evaluate((target) => {
      const gev = window.__godsEyeView;
      const slider = document.getElementById('radio-tuner-slider');
      const displayedBeforeRelease = document.getElementById(
        'radio-tuner-station',
      ).textContent;
      const frozenGeneration = gev.dataManager.layers
        .get('radio')
        .module.getUIState().tuningCatalogGeneration;
      slider.dispatchEvent(
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'tuner-refresh-unavailable.png'),
    });
    check(
      ...
      tunerRefreshTarget.generation === 1 &&
        ...
        tunerRefreshRelease.displayedBeforeRelease ===
          tunerRefreshTarget.name &&
        ...
        /unavailable after directory refresh/i.test(
          tunerRefreshRelease.message,
        ),
    ...
    const tunerDirectRelease = await page.evaluate(async () => {
      ...
      const targetId =
        gev.styleManager._radioControls._radioTunerStations[targetIndex]?.id ||
        null;
      const rect = slider.getBoundingClientRect();
      const xFor = (index) =>
        rect.left + 7 + ((rect.width - 14) * index) / Math.max(1, max);
      const pointer = (type, index) =>
        slider.dispatchEvent(
          new PointerEvent(type, {
            bubbles: true,
            pointerId: 41,
    ...
    await page.$eval('#radio-tuner-slider', (slider) =>
    ...
    await page.waitForFunction(() => {
      ...
      const x =
        rect.left +
        7 +
        ((rect.width - 14) * Number(slider.value)) /
          Math.max(1, Number(slider.max));
      return (
        document.elementFromPoint(x, rect.top + rect.height / 2) === slider
      );
    });
    const tunerCommitTarget = await page.evaluate(() => {
      ...
      const station =
        gev.styleManager._radioControls._radioTunerStations[targetIndex];
      const rect = slider.getBoundingClientRect();
      const currentRatio =
        Number(slider.value) / Math.max(1, Number(slider.max));
      ...
      return {
        ...
        frozenSignature:
          gev.styleManager._radioControls._radioTunerBandSignature,
        startX: rect.left + 7 + (rect.width - 14) * currentRatio,
        targetX:
          rect.left +
          7 +
          ((rect.width - 14) * targetIndex) /
            Math.max(
              1,
              gev.styleManager._radioControls._radioTunerStations.length - 1,
            ),
    ...
    await page.mouse.move(tunerCommitTarget.targetX, tunerCommitTarget.y, {
      steps: 12,
    });
    await page.mouse.up();
    await sleep(650);
    const tunerCommit = await page.evaluate((target) => {
      ...
      return {
        ...
        selectedIndex:
          gev.styleManager._radioControls._radioTunerStations.findIndex(
        ...
        pinned:
          gev.styleManager._radioControls._radioTunerBandPinnedForNavigation,
      };
    }, tunerCommitTarget);
    check(
      'tuner release keeps channel 12 pinned while its broadcaster connects',
      tunerCommit.audioState === 'loading' &&
        tunerCommit.tuningStatic &&
        tunerCommit.awaiting === tunerCommit.id &&
        tunerCommit.signatureStable &&
        tunerCommit.selectedIndex === 11 &&
        tunerCommit.sliderSlot === 11 &&
        Math.abs(
          tunerCommit.ratio - 11 / Math.max(1, tunerCommitTarget.count - 1),
        ) < 0.0001 &&
        tunerCommit.pinned,
    ...
    const broadcastIndicator = await page.evaluate(() => ({
      active: document
        ...
        .classList.contains('radio-broadcasting'),
      innerOpacity: getComputedStyle(
        document.querySelector('#title-bar .title-logo'),
        '::before',
      ).opacity,
      outerOpacity: getComputedStyle(
        document.querySelector('#title-bar .title-logo'),
        '::after',
      ).opacity,
    }));
    check(
      'actual broadcaster playback fades out the tuner static',
      true,
      tunerCommit.id,
    );
    check(
      ...
      broadcastIndicator.active &&
        Number(broadcastIndicator.innerOpacity) > 0 &&
        Number(broadcastIndicator.innerOpacity) <= 0.28 &&
        Number(broadcastIndicator.outerOpacity) > 0 &&
        Number(broadcastIndicator.outerOpacity) <= 0.17,
    ...
    for (const delta of [1, 2, 10]) {
      ...
      await sleep(80);
      microDragSamples.push(
        await page.evaluate((pixelDelta) => {
          const style = window.__godsEyeView.styleManager;
          const slider = document.getElementById('radio-tuner-slider');
          return {
            pixelDelta,
    ...
    await page.mouse.up();
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'tuner-micro-drag.png'),
    });
    ...
    check(
      ...
      microDragPlayback.audioState === 'playing' &&
        Boolean(microDragPlayback.selectedId),
      JSON.stringify(microDragPlayback),
    );
    check(
      'native 1px, 2px, and 10px tuner moves stay monotonic on one frozen strip',
      microDragSamples.every(
        (sample) =>
          sample.signature === microDragStart.signature &&
          Math.abs(
            sample.ratio -
              (microDragStart.ratio +
                sample.pixelDelta / microDragStart.usableWidth),
          ) < 0.002,
      ) &&
        microDragSamples.every(
          (sample, index) =>
    ...
    const viewportTuner = await page.evaluate(async (selectedId) => {
      ...
      const before = {
        signature: style._radioControls._radioTunerBandSignature,
        ids: style._radioControls._radioTunerStations.map(
          (station) => station.id,
        ),
        selectedIndex: style._radioControls._radioTunerStations.findIndex(
      ...
      const oppositeLongitude =
        ((current.longitude + 2 * Math.PI) % (2 * Math.PI)) - Math.PI;
      gev.viewer.canvas.dispatchEvent(
    ...
    check(
      ...
      viewportTuner.count === 750 &&
        ...
        JSON.stringify(viewportTuner.ids) ===
          JSON.stringify(viewportTuner.before.ids) &&
    ...
    const nextNeedleBefore = await page.evaluate(() => {
      const style = window.__godsEyeView.styleManager;
      const selectedId = window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState().selected?.id;
      const selectedIndex = style._radioControls._radioTunerStations.findIndex(
        (station) => station.id === selectedId,
      );
      const selectedPoolIndex = style._radioControls._radioTunerPool.findIndex(
        (station) => station.id === selectedId,
      );
      const expectedPoolIndex =
        (selectedPoolIndex + 1) % style._radioControls._radioTunerPool.length;
      return {
        signature: style._radioControls._radioTunerBandSignature,
        selectedIndex,
        count: style._radioControls._radioTunerStations.length,
        expectedId:
          style._radioControls._radioTunerPool[expectedPoolIndex]?.id || null,
      };
    });
    ...
    check(
      ...
      nextNeedleAfter.selectedId === nextNeedleBefore.expectedId &&
        ...
        Math.abs(
          nextNeedleAfter.ratio -
            nextNeedleAfter.selectedIndex /
              Math.max(1, nextNeedleAfter.count - 1),
        ) < 0.0001,
    ...
    const tunerKeyboard = await page.evaluate(() => {
      ...
      const expectedIndex = Math.min(
        style._radioControls._radioTunerStations.length - 1,
        startIndex + 1,
      );
      const expectedId =
        style._radioControls._radioTunerStations[expectedIndex]?.id || null;
      const playsBefore = window.__qaRadioPlayCalls.length;
    ...
    check(
      ...
      tunerKeyboard.preview.slot === tunerKeyboard.expectedIndex &&
        tunerKeyboard.preview.markerId ===
          `radio:selected:${tunerKeyboard.expectedId}` &&
        tunerKeyboard.preview.tuningActive &&
        tunerKeyboard.preview.playDelta === 0 &&
        tunerKeyboard.committed.selectedId === tunerKeyboard.expectedId &&
        tunerKeyboard.committed.slot === tunerKeyboard.expectedIndex &&
        !tunerKeyboard.committed.tuningActive &&
        tunerKeyboard.committed.playDelta === 1 &&
        tunerKeyboard.cancelled.selectedId === tunerKeyboard.expectedId &&
        tunerKeyboard.cancelled.slot === tunerKeyboard.expectedIndex &&
        !tunerKeyboard.cancelled.tuningActive &&
        tunerKeyboard.cancelled.playDelta === 0 &&
    ...
    check(
      ...
      tunerKeyboard.navigation.every(
        (sample) =>
          sample.slot === sample.expectedSlot &&
          sample.restoredSlot === tunerKeyboard.expectedIndex &&
          sample.playDelta === 0 &&
          sample.cancelPlayDelta === 0,
      ),
    ...
    const filteredTunerCenter = await page.evaluate(() => {
      ...
      const preview = {
        ...
        expectedId:
          style._radioControls._radioTunerStations[centerIndex]?.id || null,
      };
      slider.dispatchEvent(
    ...
    check(
      ...
      filteredTunerCenter.filter === 'news' &&
        ...
        filteredTunerCenter.count > 1 &&
        filteredTunerCenter.count < 750 &&
        filteredTunerCenter.slot === filteredTunerCenter.centerIndex &&
        Math.abs(filteredTunerCenter.ratio - 0.5) < 0.0001 &&
        filteredTunerCenter.markerId ===
          `radio:selected:${filteredTunerCenter.expectedId}`,
    ...
    const fullPoolNavigation = await page.evaluate(() => {
      ...
      const selectWithoutPlayback = (index) =>
        radio.selectStation(ids[index], {
          autoplay: false,
          focus: false,
          origin: 'programmatic',
        });
    ...
    check(
      ...
      fullPoolNavigation.poolLength > 41 &&
        fullPoolNavigation.afterForty ===
          fullPoolNavigation.expectedAfterForty &&
        fullPoolNavigation.afterPreviousWrap ===
          fullPoolNavigation.expectedPreviousWrap &&
        fullPoolNavigation.afterNextWrap ===
          fullPoolNavigation.expectedNextWrap,
    ...
    const failedTuner = await page.evaluate(async () => {
      ...
      const xFor = (index) =>
        rect.left + 7 + ((rect.width - 14) * index) / Math.max(1, count - 1);
      const pointer = (type, index) =>
        slider.dispatchEvent(
          new PointerEvent(type, {
            bubbles: true,
    ...
    check(
      ...
      failedTuner.failed.audioState === 'error' &&
        failedTuner.failed.tuningStatic &&
        failedTuner.failed.awaiting &&
        !failedTuner.failed.broadcasting &&
        !failedTuner.stopped.tuningStatic &&
        !failedTuner.stopped.awaiting &&
        !failedTuner.stopped.broadcasting,
      ...
      return new Promise((resolve) => {
        ...
        const probe = () => {
          ...
          for (const entity of source.entities.values) {
            ...
            const canvas =
              position && viewer.scene.cartesianToCanvasCoordinates(position);
    ...
    check(
      'Radio exposes a visible marker for click-tolerance QA',
      markerTarget?.found === true,
    ...
    if (markerTarget?.found) {
      ...
      await page.waitForFunction(
        (id) =>
          window.__godsEyeView.dataManager.layers
            .get('radio')
            .module.getUIState().selected?.id === id,
        {},
        markerTarget.id,
      );
      const offsetSelected = await page.evaluate(
        () =>
          window.__godsEyeView.dataManager.layers
            .get('radio')
            .module.getUIState().selected?.id,
      ...
      check(
        'an offset click within 8px reliably selects the intended Radio dot',
        offsetSelected === markerTarget.id,
        offsetSelected,
      );
    }
    ...
    await page.evaluate(() => {
      ...
      radio.stopPlayback();
      Array.from({ length: gev.viewer.dataSources.length }, (_, index) =>
        gev.viewer.dataSources.get(index),
      ).find((item) => item.name === 'Radio stations').clustering.enabled =
        true;
      radio.selectStation('00000000-0000-4000-8000-000000000001', {
        autoplay: false,
        focus: false,
      });
      const pool = gev.styleManager._radioControls._radioTunerPool;
      const selectedIndex = pool.findIndex(
        (station) => station.id === radio.getUIState().selected?.id,
      );
      window.__qaRadioExpectedPrimary = pool[(selectedIndex + 1) % pool.length];
      window.__qaRadioExpectedFallback =
        pool[(selectedIndex + 2) % pool.length];
      window.__qaRadioPlayCalls = [];
      window.__qaRadioFailedNearest = false;
    ...
    if (!radioOnlyNextDispatched)
      throw new Error('Radio compact next button unavailable');
    await page.waitForFunction(
      () => typeof window.__qaRejectRadioPlay === 'function',
    );
    await page.evaluate(() => {
      const viewer = window.__godsEyeView.viewer;
      viewer.camera.setView({
    ...
    const playing = await page.evaluate(async () => {
      const radio = window.__godsEyeView.dataManager.layers.get('radio').module;
      const viewer = window.__godsEyeView.viewer;
      const selectedEntity = window.__godsEyeView.viewer.entities.values.find(
        (entity) => String(entity.id).startsWith('radio:selected:'),
      );
      const { getWorldOverlayDiagnostics } =
        await import('/src/overlays/worldOverlay.js');
      const flightTargets = window.__qaRadioFlyToCalls.map(
        ({ destination }) => {
          const position =
            viewer.scene.globe.ellipsoid.cartesianToCartographic(destination);
          return {
      ...
      return {
        ...
        compactPlayLabel: document
          .getElementById('context-radio-mini-play-btn')
          .getAttribute('aria-label'),
        regularMarkerSize: Array.from(
          { length: window.__godsEyeView.viewer.dataSources.length },
          (_, index) => window.__godsEyeView.viewer.dataSources.get(index),
        )
          .find((item) => item.name === 'Radio stations')
          ?.entities.values[0]?.point?.pixelSize?.getValue(),
    ...
    check(
      'compact next is explicit playback and updates play/pause state',
      playing.calls.length === 2 &&
        playing.compactPlayLabel.startsWith('Pause'),
    );
    check(
      'compact next rotates toward its station without changing zoom, including fallback',
      playing.flyToCalls === 2 &&
        Math.abs(playing.flightTargets[0]?.lat - playing.expectedPrimary?.lat) <
          0.001 &&
        Math.abs(playing.flightTargets[0]?.lon - playing.expectedPrimary?.lon) <
          0.001 &&
        Math.abs(
          playing.flightTargets[1]?.lat - playing.expectedFallback?.lat,
        ) < 0.001 &&
        Math.abs(
          playing.flightTargets[1]?.lon - playing.expectedFallback?.lon,
        ) < 0.001 &&
        playing.flightTargets.every(
          (target) => Math.abs(target.height - playing.cameraHeight) < 1,
        ),
      JSON.stringify({
        calls: playing.flyToCalls,
        targets: playing.flightTargets,
        cameraHeight: playing.cameraHeight,
      }),
    );
    ...
    check(
      ...
      trackedRadioControls.every(
        (sample) =>
          sample.afterStationId &&
    ...
    const delayedFallbackOwnership = await page.evaluate((beforeFlyToCalls) => {
      ...
      const result = {
        ...
        selectedId:
          window.__godsEyeView.dataManager.layers
            .get('radio')
            .module.getUIState().selected?.id || null,
      };
      viewer.trackedEntity = undefined;
      viewer.entities.remove(owner);
    ...
    check(
      'the current station carries a 40px four-corner selection bracket',
      playing.selectedBracketWidth === 40,
      `${playing.selectedBracketWidth}px`,
    );
    check(
      ...
      JSON.stringify({
        overlay: playing.overlay,
        host: playing.host.entriesBySource,
      }),
    );
    const closeSelectedText = await page.evaluate(async () => {
      ...
      const selectedEntity = viewer.entities.values.find(
        (entity) => String(entity.id) === `radio:selected:${selected.id}`,
      );
      const { getOverlayPaintRect, getWorldOverlayDiagnostics } =
        await import('/src/overlays/worldOverlay.js');
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'selected-label-desktop.png'),
    });
    const highGlobalSelectedText = await page.evaluate(async () => {
      ...
      for (
        let attempt = 0;
        entityAnchor && attempt < 8 && !selectedPointPick;
        attempt += 1
      ) {
        const picks = viewer.scene.drillPick(entityAnchor, 16) || [];
        selectedNativePick = picks.find(
          (picked) => picked?.id === selectedEntity,
        );
        selectedPointPick = picks.find(
          (picked) =>
            picked?.id === selectedEntity &&
            String(picked?.primitive?.constructor?.name || '').includes(
              'PointPrimitive',
            ),
        );
        selectedBillboardPick = picks.find(
          (picked) =>
            picked?.id === selectedEntity &&
            String(picked?.primitive?.constructor?.name || '').includes(
              'Billboard',
      ...
      const selectedPointDistance =
        selectedEntity?.point?.distanceDisplayCondition?.getValue?.();
      return {
        rect,
        ...
        entityAnchor: entityAnchor
          ? { x: entityAnchor.x, y: entityAnchor.y }
          : null,
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'selected-label-high-global.png'),
    });
    ...
    await page.screenshot({
      path: path.join(SHOTS_DIR, 'selected-label-mobile.png'),
    });
    await page.setViewport({ width: 1440, height: 900, deviceScaleFactor: 1 });
    await page.evaluate(() => {
      const viewer = window.__godsEyeView.viewer;
    ...
    const radioOnlyFocusCalls = await page.evaluate(() => {
      ...
      const heights = window.__qaRadioFlyToCalls.map(
        (options) =>
    ...
    check(
      ...
      radioOnlyFocusCalls.calls === 1 &&
        radioOnlyFocusCalls.heights.every(
          (height) => Math.abs(height - radioOnlyFocusCalls.cameraHeight) < 1,
        ),
    ...
    const stagedGlobeNavigation = await page.evaluate(async () => {
      ...
      if (stationIds.length < 3)
        throw new Error('Radio recenter QA requires three visible stations');
      const originalFlyTo = camera.flyTo;
      const calls = [];
      ...
      const directSelectionId = stationIds.find(
        (id) => id !== radio.getUIState().selected.id,
      );
      radio.selectStation(directSelectionId, {
        autoplay: false,
        focus: false,
        origin: 'user',
      });
      ...
      const voiceSelectionId = stationIds.find(
        (id) => id !== radio.getUIState().selected.id,
      );
    ...
    check(
      ...
      stagedGlobeNavigation.partialInitialCalls === 1 &&
        ...
        Math.abs(
          stagedGlobeNavigation.stationFocus.lat -
            stagedGlobeNavigation.selectedAfterRecenter.lat,
        ) < 0.001 &&
        Math.abs(
          stagedGlobeNavigation.stationFocus.lon -
            stagedGlobeNavigation.selectedAfterRecenter.lon,
        ) < 0.001 &&
    ...
    check(
      ...
      stagedGlobeNavigation.latestRecenterIndex === 1 &&
        ...
        Math.abs(
          stagedGlobeNavigation.latestFocus.lat -
            stagedGlobeNavigation.latestSelected.lat,
        ) < 0.001 &&
        Math.abs(
          stagedGlobeNavigation.latestFocus.lon -
            stagedGlobeNavigation.latestSelected.lon,
        ) < 0.001,
      JSON.stringify(stagedGlobeNavigation),
    );
    check(
      ...
      stagedGlobeNavigation.directSelectionCalls === 1 &&
        ...
        stagedGlobeNavigation.directSelectionSelectedId ===
          stagedGlobeNavigation.directSelectionId,
      JSON.stringify(stagedGlobeNavigation),
    );
    check(
      ...
      stagedGlobeNavigation.voiceSelectionCalls === 1 &&
        ...
        stagedGlobeNavigation.voiceSelectionMatchedId ===
          stagedGlobeNavigation.voiceSelectionId &&
        stagedGlobeNavigation.voiceSelectionSelectedId ===
          stagedGlobeNavigation.voiceSelectionId,
      JSON.stringify(stagedGlobeNavigation),
    );
    check(
      ...
      Math.abs(stagedGlobeNavigation.extremeRecenter.height - 13_000_000) < 1 &&
        ...
        Math.abs(stagedGlobeNavigation.extremeRecenter.pitch + Math.PI / 2) <
          1e-9 &&
    ...
    check(
      ...
      stagedGlobeNavigation.localDirectCalls === 1 &&
        ...
        Math.abs(
          stagedGlobeNavigation.localDirect.pitch + (35 * Math.PI) / 180,
        ) < 1e-9 &&
        Math.abs(stagedGlobeNavigation.centeredDirect.pitch + Math.PI / 2) <
          1e-9,
      JSON.stringify(stagedGlobeNavigation),
    );
    ...
    const companion = await page.evaluate(() => ({
      ...
      state: window.__godsEyeView.dataManager.layers
        .get('radio')
        .module.getUIState().audioState,
      radioCollapsed: document
        .getElementById('radio-panel')
      ...
      nested: document
        .getElementById('global-context-panel')
        .contains(document.getElementById('radio-panel')),
    }));
    check(
      'expanding Context preserves playback and the user-selected detailed Radio disclosure',
      companion.radioEnabled &&
        companion.state === 'playing' &&
        companion.radioCollapsed &&
        !companion.contextCollapsed &&
        companion.nested,
    );
    ...
    await page.waitForFunction(
      (priorScroll) =>
        document.activeElement?.getAttribute('data-collapse-target') ===
          'radio-panel' &&
        document.querySelector(
          '#global-context-panel .global-context-panel-inner',
        ).scrollTop > priorScroll,
      { timeout: 10_000 },
      expandedContextRadioBefore.scrollTop,
    );
    const expandedContextRadioAfter = await page.evaluate(() => {
      const gev = window.__godsEyeView;
      gev.styleManager._renderRadioState(
        gev.dataManager.layers.get('radio').module.getUIState(),
      );
      const launcher = document.getElementById('context-radio-toggle-btn');
      const radio = document.getElementById('radio-panel');
      const scroller = document.querySelector(
      ...
      return {
        ...
        compactHidden: document.getElementById('context-radio-mini').hidden,
        scrollTop: scroller.scrollTop,
        focusTarget: document.activeElement?.getAttribute(
          'data-collapse-target',
        ),
        pageY: window.scrollY,
        camera: { x: camera.x, y: camera.y, z: camera.z },
        plays: window.__qaRadioPlayCalls.length,
        state: gev.dataManager.layers.get('radio').module.getUIState()
          .audioState,
      };
    });
    ...
    check(
      ...
      expandedContextRadioAfter.ariaControls === 'radio-panel' &&
        ...
        expandedContextRadioAfter.scrollTop >
          expandedContextRadioBefore.scrollTop &&
    ...
    await page.waitForFunction(
      () => !document.getElementById('context-flights-view').hidden,
    );
    const companionStack = await page.evaluate(() => {
      ...
      return {
        contactsNaturalHeight:
          contextView.clientHeight === contextView.scrollHeight,
    ...
    check(
      ...
      companionStack.contactsNaturalHeight &&
        companionStack.radioNaturalHeight &&
        companionStack.separated &&
        companionStack.scrollable,
    ...
    check(
      'filter changes markers/navigation without stopping the selected stream',
      filtered.filteredCount === 75 &&
        filtered.audioState === 'playing' &&
        filtered.selectedIndex === -1,
      `${filtered.filteredCount} matches`,
    );

    const playsBeforeRestore = await page.evaluate(
      () => window.__qaRadioPlayCalls.length,
    );
    await page.evaluate(async () => {
      const gev = window.__godsEyeView;
    ...
    check(
      ...
      restored.state.enabled &&
        restored.state.audioState === 'stopped' &&
        restored.plays === playsBeforeRestore &&
        restored.radioCollapsed,
    ...
    const desktop = await page.$eval('#radio-panel', (element) => {
      ...
      const contextRect = element
        .closest('#global-context-panel')
        .getBoundingClientRect();
      const tunerRect = element
        .querySelector('.radio-tuner')
        .getBoundingClientRect();
      ...
      return {
        ...
        radioInside:
          rect.left >= contextRect.left && rect.right <= contextRect.right,
        tunerInside:
          tunerRect.left >= innerRect.left &&
          tunerRect.right <= innerRect.right,
      };
    });
    check(
      'desktop Context host keeps Radio and its tuner contained',
      desktop.left >= 0 &&
        desktop.right <= 1440 &&
        desktop.top >= 0 &&
        desktop.bottom <= 900 &&
    ...
    await sleep(500);
    await page.$eval('#radio-panel', (element) =>
    ...
    const mobile = await page.$eval('#radio-panel', (element) => {
      ...
      return {
        ...
        radioVisible:
          rect.bottom > contextRect.top && rect.top < contextRect.bottom,
        scrollable: scroller.scrollHeight >= scroller.clientHeight,
        broadcastVisible:
          Number(getComputedStyle(logo, '::before').opacity) > 0 &&
          Number(getComputedStyle(logo, '::after').opacity) > 0,
        broadcastInsideViewport:
          logoRect.left >= 0 &&
          logoRect.right <= innerWidth &&
          logoRect.top - 15 >= 0,
        tunerInside:
          tunerRect.left >= innerRect.left &&
          tunerRect.right <= innerRect.right &&
          tunerRect.left >= 0 &&
          tunerRect.right <= innerWidth,
      };
    });
    check(
      'mobile Context host is full-width and keeps the tuner contained',
      mobile.left >= 0 &&
        mobile.right <= 390 &&
        mobile.width >= 350 &&
        mobile.radioWidth < mobile.width &&
        mobile.radioVisible &&
        mobile.scrollable &&
        mobile.tunerInside,
    ...
    check(
      'mobile playing state keeps both broadcast waves visible and on-screen',
      mobile.broadcastVisible && mobile.broadcastInsideViewport,
      JSON.stringify(mobile),
    );
    ...
    if (externalFontFailures.length > 0) {
      console.log(
        `INFO external Google Fonts resource unavailable (${externalFontFailures.length}); Radio assertions continued with the local fallback font`,
      );
    }
    if (externalCesiumEndpointFailures.length > 0) {
      const requestEventIndex = actionableConsoleErrors.findIndex(
        (entry) =>
          entry.startsWith('[object RequestErrorEvent]') &&
          entry.includes('/node_modules/.vite/deps/cesium.js'),
      );
      if (requestEventIndex >= 0)
        actionableConsoleErrors.splice(requestEventIndex, 1);
      for (let i = actionableConsoleErrors.length - 1; i >= 0; i -= 1) {
        if (
          actionableConsoleErrors[i].includes(
            'api.cesium.com/v1/assets/1/endpoint',
          ) &&
      ...
      console.log(
        `INFO external Cesium ion asset endpoint unavailable (${externalCesiumEndpointFailures.length}); Radio assertions continued against the loaded app`,
      );
    }
    check(
  ...
  console.log(
    `RESULT: ${results.length - failures} passed, ${failures} failed, 0 skipped`,
  );
