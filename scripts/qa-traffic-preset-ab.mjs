...
const OUT_DIR = path.resolve(
  REPO_ROOT,
  getOpt('--out', 'qa-shots/preset-traffic'),
);
  ...
  try {
    ...
    for (const view of VIEWS) {
      ...
      if (
        ...
        console.error(
          '  [FAIL] traffic never settled live ' +
      ...
      for (const style of STYLES) {
        ...
        await page.evaluate((s) => {
          ...
          if (sm.stages.thermal)
            sm.stages.thermal.uniforms.palette = s.ironbow ? 1.0 : 0.0;
        }, style);
        ...
        for (const detect of ['off', 'on']) {
          await page.evaluate((d) => {
            window.__godsEyeView.styleManager._setDetectionMode(
              d === 'on' ? 'DENSE' : 'OFF',
            );
          }, detect);
