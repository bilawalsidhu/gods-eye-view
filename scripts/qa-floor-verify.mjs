...
await page.waitForFunction(
  () => window.__godsEyeView?.dataManager && window.__godsEyeView?.viewer,
  { timeout: 150000 },
);
await sleep(2000);
  ...
  page.evaluate(() => {
    ...
    v.camera.setView({
      destination: v.scene.globe.ellipsoid.cartographicToCartesian(
        C.fromDegrees(window.__QA_SITE.lon, window.__QA_SITE.lat, 3000),
      ),
    });
    v.camera.moveEnd.raiseEvent();
    const c = v.camera.positionCartographic;
    return {
      lat: +((c.latitude * 180) / Math.PI).toFixed(3),
      lon: +((c.longitude * 180) / Math.PI).toFixed(3),
      h: Math.round(c.height),
    };
  });
console.log('camera pinned:', JSON.stringify(await camPin()));
await page.evaluate(async () => {
  await window.__godsEyeView.dataManager.toggle('flights');
});
...
const ready = await page.evaluate(() => {
  ...
  for (let i = 0; i < prims.length; i++) {
    const p = prims.get(i);
    if (p && p.tilesLoaded !== undefined && p.show)
      return { tilesLoaded: !!p.tilesLoaded };
  }
...
const report = await page.evaluate(() => {
  ...
  const center = ell.cartographicToCartesian(
    C.fromDegrees(window.__QA_SITE.lon, window.__QA_SITE.lat, 200),
  );
  ...
  const walk = (coll) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (!pr) continue;
      if (typeof pr.length === 'number' && typeof pr.get === 'function') {
        walk(pr);
        continue;
      }
      if (pr.image !== undefined && pr.alignedAxis !== undefined) {
        excludes.push(pr);
        continue;
      }
      if (
        pr.activeAnimations !== undefined &&
        pr.minimumPixelSize !== undefined
      )
        excludes.push(pr);
    }
  };
  ...
  for (const p of nearby) {
    ...
    const latDeg = (c.latitude * 180) / Math.PI,
  ...
  const censusWalk = (coll) => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const la = (cc.latitude * 180) / Math.PI,
        lo = (cc.longitude * 180) / Math.PI;
      if (
        Math.abs(la - window.__QA_SITE.lat) > 0.08 ||
        Math.abs(lo - window.__QA_SITE.lon) > 0.1
      )
        continue;
      if (pr.show) shown++;
      else hidden++;
...
const lows = (report.contacts || []).filter(
  (c) =>
    c.renderAltM < SITE.floorMax + 450 &&
    c.aboveMeshM != null &&
    c.meshM > SITE.floorMin &&
    c.meshM < SITE.floorMax,
);
const buried = lows.filter((c) => c.aboveMeshM < -2);
console.log(
  `low contacts with plausible mesh readings: ${lows.length}; buried (< -2m): ${buried.length}`,
);
for (const b of buried) {
  console.log(
    `  BURIED ${b.id}: render ${b.renderAltM} m vs mesh ${b.meshM} m (${b.aboveMeshM} m)`,
  );
}
const verdict =
  lows.length === 0 ? 'INCONCLUSIVE' : buried.length === 0 ? 'PASS' : 'FAIL';
