...
function analyzeMembership(samples) {
  ...
  for (let index = 0; index < samples.length; index++) {
    ...
    if (index > 0) {
      ...
      const elapsedSec = Math.max(
        0.001,
        (sample.t - samples[index - 1].t) / 1000,
      );
  ...
  for (const times of transitions.values()) {
    for (let index = 1; index < times.length; index++) {
      const previousIndex = samples.findIndex(
        (sample) => sample.t === times[index - 1],
      );
      const nextIndex = samples.findIndex(
        (sample) => sample.t === times[index],
      );
      if (
        nextIndex - previousIndex > 1 &&
        times[index] - times[index - 1] <= 2000
      ) {
        showHideShow++;
        break;
      }
  ...
  const churnPctPerSec = exposure > 0 ? (replacements / exposure) * 100 : 0;
  return {
    churnPctPerSec,
    showHideShow,
    replacements,
...
async function waitForConclusiveSamples(
  page,
  {
    minimumElapsedMs,
    minimumSolves = 0,
  ...
  while (Date.now() < deadline) {
    ...
    if (
      state.elapsedMs >= minimumElapsedMs &&
      state.solveCount >= minimumSolves &&
      state.solveSpanMs >= minimumSolveSpanMs &&
      state.normalFrameCount >= minimumNormalFrames
    )
      return state;
...
async function main() {
  ...
  console.log(
    `  Sample window: >=${SYNTHETIC_SAMPLE_MS / 1000}s synthetic + >=${NORMAL_SAMPLE_MS / 1000}s normal\n`,
  );
  ...
  try {
        ...
        window.fetch = (input, init) => {
          ...
          if (
            ...
            return Promise.resolve(
              new Response(JSON.stringify({ summary: 'QA globe ready' }), {
                status: 200,
      ...
      ({ fieldCounts, normalCounts }) => {
        ...
        for (const layerId of layerIds) {
          ...
          for (let localIndex = 0; localIndex < count; localIndex++) {
            ...
            const angle =
              localIndex * goldenAngle + layerIds.indexOf(layerId) * 0.7;
            const lat = Math.max(
              -65,
              Math.min(75, 30 + radius * Math.sin(angle)),
            );
            const lon =
              -97 +
              (radius * Math.cos(angle)) /
                Math.max(0.4, Math.cos((lat * Math.PI) / 180));
            ...
            field[layerId].push({
              ...
              metric:
                layerId === 'satellites'
                  ? 'LEO'
                  : `${250 + (localIndex % 450)}KT`,
              position: Cartesian3.fromDegrees(
                lon,
                lat,
                layerId === 'satellites' ? 550000 : 9000,
              ),
        ...
        for (const layerId of layerIds) {
          ...
          entry.module.getDetectableObjects = () =>
            window.__LABEL_QA_FIELD[layerId];
        }
        ...
        const sample = () => {
          ...
          if (
            diagnostics &&
            frameNumber !== window.__LABEL_QA.lastFrameNumber
          ) {
          ...
          if (
            dataset &&
            revision > 0 &&
            revision !== window.__LABEL_QA.lastRevision
          ) {
            ...
            window.__LABEL_QA.solve.push({
              ...
              entitlementByLayer: JSON.parse(
                dataset.entitlementByLayer || '{}',
              ),
        ...
        return Object.fromEntries(
          layerIds.map((layerId) => [layerId, field[layerId].length]),
        );
      },
    ...
    record(
      ...
      Object.values(injected).reduce((sum, count) => sum + count, 0) ===
        OBSERVATION_COUNT,
    ...
    await page.evaluate(() => {
      ...
      const revision = Number(
        document.querySelector('#world-overlay-canvas')?.dataset
          ?.solveRevision || 0,
      );
      window.__LABEL_QA.solve = [];
      window.__LABEL_QA.frames = [];
      window.__LABEL_QA.lastRevision = revision;
      window.__LABEL_QA.lastFrameNumber =
        viewer.scene.frameState?.frameNumber ?? -1;
    ...
    const syntheticSamples = await page.evaluate(() => {
      ...
      for (const layerId of Object.keys(window.__LABEL_QA_FIELD)) {
        window.__LABEL_QA_FIELD[layerId] = window.__LABEL_QA_FIELD[
          layerId
        ].slice(0, counts[layerId]);
      }
      window.__LABEL_QA = {
        solve: [],
        frames: [],
    ...
    const steadySolveSamples = solveSamples.filter(
      (sample) =>
    ...
    const solveP95 = percentile(
      solveSamples.map((sample) => sample.solveMs),
      0.95,
    );
    const renderP95 = percentile(
      normalFrames.map((sample) => sample.frameTotalMs),
      0.95,
    );
    ...
    const paintP95 = percentile(
      (normalFrames.length ? normalFrames : normalFieldFrames).map(
        (sample) => sample.paintMs,
      ),
      0.95,
    );
    const throttles = normalSamples.frames
      .map((sample) => sample.throttleSkipCount)
      .filter(Number.isFinite);
    const throttleDelta = throttles.length
      ? Math.max(...throttles) - Math.min(...throttles)
      : 0;
    const normalPlacementOverflow = normalFieldFrames.filter(
      (sample) =>
        sample.placementBuildCount >
        sample.selectedCount +
          sample.fadingCount +
          (sample.didSolve ? sample.cohortCount : 0),
    );
    const solvePlacementOverflow = solveSamples.filter((sample) => {
      const cohort = Object.values(sample.cohortByLayer).reduce(
        (sum, count) => sum + count,
        0,
      );
      return (
        sample.placementBuildCount >
        cohort + sample.selectedCount + sample.fadingCount
      );
    });
    const cohortOverflow = solveSamples.filter((sample) =>
      Object.entries(sample.cohortByLayer).some(([layerId, count]) => {
    ...
    record(
      ...
      normalPlacementOverflow.length === 0 &&
        solvePlacementOverflow.length === 0,
    ...
    console.log(
      `  Screenshot: ${path.join(SHOT_DIR, 'normal-global-5200.png')}`,
    );
    console.log(
      `  Synthetic solves: ${solveSamples.length}; normal frames: ${normalFieldFrames.length} (${normalFrames.length} non-solve)`,
    );
  } finally {
    await browser.close();
  }
