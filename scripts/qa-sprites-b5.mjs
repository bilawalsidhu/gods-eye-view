...
const flightsWantScale = (klass) =>
  CLASS_MODEL_REAL[klass] ? 1 : FL_MODEL_SCALE * CLASS_SCALE_3D[klass];
const MIL_PLANE_CLASSES = new Set(['airliner', 'quadjet', 'glider']);
const militaryWantScale = (klass) =>
  CLASS_MODEL_REAL[klass]
    ? 1
...
const SPRITES = {
  ...
  flights: [
    ...
    // 3D cluster (150 m spacing): widebody vs airliner (tracked) vs fastjet
    {
      icao: 'cbb001',
      callsign: '3D-WID',
      lon0: CLUSTER_LON0 - mDegLon(150, 30.1),
      lat0: 30.1,
      courseDeg: 0,
      speedMps: 60,
      altM: 3000,
...
const expectedFlights = new Map(
  SPRITES.flights.map((p) => [
    p.icao,
    classifyAircraft({ category: p.category }),
  ]),
);
const expectedMilitary = new Map(
  SPRITES.military.map((p) => [p.hex, classifyAircraft({ typeCode: p.t })]),
);
...
async function main() {
  ...
  console.log(
    `  Expected classes — flights: ${[...expectedFlights.entries()].map(([k, v]) => `${k}:${v}`).join(' ')}`,
  );
  console.log(
    `  Expected classes — military: ${[...expectedMilitary.entries()].map(([k, v]) => `${k}:${v}`).join(' ')}\n`,
  );
  ...
  try {
      ...
      window.fetch = (input, init) => {
        ...
        if (url.includes('/api/openai/hud-summary')) {
          return Promise.resolve(
            jsonResponse({ summary: 'Aircraft scale QA' }),
          );
        }
        ...
        const walk = (coll) => {
          ...
          for (let i = 0; i < n; i++) {
            ...
            if (
              ...
              try {
                radius =
                  p.ready && p.boundingSphere ? p.boundingSphere.radius : null;
              } catch {
                /* not ready */
              }
              out.push({
      ...
      window.__planeCanvasCoords = function (layer, renderDelaySec, altOf) {
        ...
        return S[layer]
          .map((p, i) => {
            ...
            const pos = C3.fromDegrees(
              s.lon,
              s.lat,
              altOf === 'ft' ? p.altFt * 0.3048 : p.altM,
            );
    ...
    console.log(
      'Priming straight-flight history through the render delay (30 s / 15 s)...',
    );
    ...
    record(
      'B1 ingest: synthetic planes in both layers',
      primed.fl === SPRITES.flights.length &&
        primed.mil === SPRITES.military.length,
    ...
    for (const [id, klass] of expectedFlights) {
      ...
      if (!bb || bb.image !== wantImg)
        glyphBad.push(`${id}(${klass})${bb ? ':wrong-image' : ':missing'}`);
      if (bb && Math.abs(bb.scale - wantScale) > 1e-9)
        scaleBad.push(`${id}: ${bb.scale} != ${wantScale}`);
    }
    ...
    record(
      'B2a: every synthetic billboard has its class glyph (data-URI match)',
      glyphBad.length === 0,
      glyphBad.length
        ? glyphBad.join(' ')
        : `${expectedFlights.size + expectedMilitary.size} billboards matched`,
    );
    record(
      'B2b: every synthetic billboard has its class scale',
      scaleBad.length === 0,
      scaleBad.length ? scaleBad.join(' ') : 'all scales exact',
    );
    const distinctGlyphs = new Set(
      [...expectedFlights.values(), ...expectedMilitary.values()].map((k) =>
        aircraftIcon(k),
      ),
    );
    record(
      'B2c: all 8 classes exercised with 8 distinct glyphs',
      distinctGlyphs.size === 8,
      `${distinctGlyphs.size} distinct data URIs`,
    );
    ...
    const rowLat = async (layer, idx, delay) =>
      (
        await page.evaluate(
          ({ layer, idx, delay }) => window.__displayedState(layer, idx, delay),
          { layer, idx, delay },
        )
      ).lat;
    ...
    await topDown(
      (flRowLat + milRowLat) / 2,
      ROW_LON0 + 3 * ROW_STEP_DEG,
      16000,
    );
    ...
    console.log(
      '  saved b5-2d-spread-bothrows.png (flights row top, military row bottom)',
    );
    ...
    await topDown(
      await rowLat('military', 4, 15),
      ROW_LON0 + 3.5 * ROW_STEP_DEG,
      17000,
    );
    await page.screenshot({
    ...
    console.log(
      `    glyph positions: ${await page.evaluate(() => window.__planeCanvasCoords('military', 15, 'ft'))}`,
    );

    await topDown(
      await rowLat('flights', 2, 30),
      ROW_LON0 + 2.5 * ROW_STEP_DEG,
      12000,
    );
    await page.screenshot({
    ...
    console.log(
      `    glyph positions: ${await page.evaluate(() => window.__planeCanvasCoords('flights', 30, 'm'))}`,
    );
    ...
    console.log(
      '  saved b5-2d-tracked-cyan.png (tracked airliner cyan, follow camera)',
    );
    ...
    const flModelsUp = await page
      .waitForFunction(
        () => {
          const models = window
            .__collectModels()
            .filter((m) => m.ready && m.show);
      ...
      .catch(() => false);
    record(
      '3D flights: tracked model + 2 fleet cluster models rendered',
      flModelsUp,
      flModelsUp ? 'all up' : 'timed out',
    );

    if (flModelsUp) {
      const flModels = await page.evaluate(() =>
        window.__collectModels().filter((m) => m.ready),
      );
      const modelScaleBad = [];
      for (const [id, want] of [
        ...
        if (!m || Math.abs(m.scale - want) > 1e-9)
          modelScaleBad.push(`${id}: ${m ? m.scale : 'missing'} != ${want}`);
      }
      ...
      if (!trackedM || Math.abs(trackedM.scale - trackedWant) > 1e-9) {
        modelScaleBad.push(
          `tracked: ${trackedM ? trackedM.scale : 'missing'} != ${trackedWant}`,
        );
      }
      record(
        'B3a flights: model scales match the per-class registry (all GLBs meter-scale; shared @1×class)',
        modelScaleBad.length === 0,
        modelScaleBad.length
          ? modelScaleBad.join(' ')
          : 'all exact per registry',
      );
      console.log(
        `  world bounding radii (m): ${flModels
          .filter((m) => m.scale !== undefined)
          .map((m) => `${m.id ?? 'tracked'}=${m.radius?.toFixed(1)}`)
          .join(' ')}`,
      );
      ...
      console.log(
        '  saved b5-3d-flights-mid.png (900 m range — near the min-pixel clamp)',
      );
      ...
      console.log(
        '  saved b5-3d-flights-close.png (450 m range: widebody left, airliner center, fastjet right)',
      );
    }
    ...
    const milModelsUp = await page
      ...
      .catch(() => false);
    record(
      '3D military: tracked model + 3 fleet cluster models rendered',
      milModelsUp,
      milModelsUp ? 'all up' : 'timed out',
    );

    if (milModelsUp) {
      const milModels = await page.evaluate(() =>
        window.__collectModels().filter((m) => m.ready),
      );
      const modelScaleBad = [];
      for (const [id, want] of [
      ...
      record(
        'B3b military: model scales match the weight-class registry (real/heavy/jet assets @1×class)',
        modelScaleBad.length === 0,
        modelScaleBad.length
          ? modelScaleBad.join(' ')
          : 'all exact per registry',
      ...
      console.log(
        `  world bounding radii (m): ${milModels
          .filter((m) => m.scale !== undefined)
          .map((m) => `${m.id ?? 'tracked'}=${m.radius?.toFixed(1)}`)
          .join(' ')}`,
      );
      ...
      console.log(
        '  saved b5-3d-military-far.png (900 m range: C130/C17 left, F16 center, B744 right)',
      );
      await zoomToRange(450);
      await page.screenshot({
      ...
      console.log(
        '  saved b5-3d-military-close.png (450 m range: C17 left, F16 center, B744 right)',
      );
    }

    record(
      'B4: no console errors during QA run',
