...
const ANGLE = String(
  argv.angle || (process.platform === 'darwin' ? 'metal' : 'swiftshader'),
);
...
const SITE = {
  ...
  lon: Number.isFinite(Number(argv.lon)) ? Number(argv.lon) : -97.666,
};
const ICAO = 'aa0f01';
...
await page.evaluateOnNewDocument(
  (site, icao) => {
    ...
    window.fetch = (input, init) => {
      ...
      if (app && url.pathname === '/api/opensky-track')
        return Promise.resolve(json({ path: [] }));
      if (app && /^\/api\/adsbdb\//.test(url.pathname))
...
await page.evaluate(() => {
  document
    .querySelector(
      '#first-run-launcher:not([hidden]) [data-first-run-choice="explore"]',
    )
    ?.click();
});
...
record(
  'the billboard floor test is explicitly in 2D aircraft mode',
  billboardMode,
);
    ...
    for (let attempt = 0; attempt < 10 && mesh == null; attempt += 1) {
      ...
      for (const [pLat, pLon] of [
        ...
        for (const exclude of [[...sprites, ...models], []]) {
          try {
            const h = v.scene.sampleHeight(
              C.fromDegrees(pLon, pLat, 0),
              exclude,
            );
...
async function demTruth(lat, lon) {
  ...
  try {
    const res = await fetch(
      `${APP_URL}/api/terrain/heights?points=${lon.toFixed(5)},${lat.toFixed(5)}`,
    );
    if (res.ok) {
...
async function sample(phase, note) {
  ...
  samples.push(row);
  console.log(
    `  ${phase} ${note}: @${m.lat},${m.lon} sprite=${m.spriteH.toFixed(1)}` +
...
await sample('A', 'warm');
fs.writeFileSync(
  path.join(OUT_DIR, `hold-A-warm-${ANGLE}.png`),
...
fs.writeFileSync(
  path.join(OUT_DIR, `hold-B-outage-${ANGLE}.png`),
  await page.screenshot({ type: 'png' }),
);
await browser.close();
...
if (demRows.length) {
  ...
  record(
    'the contact never fell to the geoid (bare-earth DEM oracle)',
    worst >= -2,
    `worst clearance ${worst} m above bare earth; the geoid here is ~150 m below it`,
  );
}
fs.writeFileSync(
  path.join(OUT_DIR, `hold-${ANGLE}.json`),
