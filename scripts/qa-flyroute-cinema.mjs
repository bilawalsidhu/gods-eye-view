...
const ROUTE_POINTS = [
  { latitude: 30.262, longitude: -97.7431 },
  { latitude: 30.265, longitude: -97.7431 },
  { latitude: 30.265, longitude: -97.7397 },
  { latitude: 30.268, longitude: -97.7397 },
  { latitude: 30.268, longitude: -97.7363 },
...
page.on('pageerror', (e) =>
  console.log(`  [page error] ${String(e).slice(0, 160)}`),
);
...
try {
  ...
  const run = (name, args = {}) =>
    page.evaluate((n, a) => window.__gevVoiceCommands.runner(n, a), name, args);
  ...
  const routeInfo = await page.evaluate(() => {
    const route = (window.__gevAnnotations.list?.() || [])
      .filter((a) => a.type === 'route')
      .at(-1);
    return route
      ? { label: route.label, waypoints: route.path?.length ?? 0 }
      : null;
  });
  report(
    Boolean(routeInfo?.waypoints >= 2),
    'route drawn on the board',
  ...
  if (coldPath && coldPath !== 'module-unavailable') {
    report(
      true,
      'cold-path telemetry read from the live flight',
      `arming=${coldPath.arming} floorKnown=${coldPath.floorKnown} viaMeshProbe=${coldPath.floorFromMeshProbe}` +
        (TERRAIN_DELAY_MS
          ? ` (terrain proxy held back ${TERRAIN_DELAY_MS} ms, ${terrainRequests} request(s))`
          : ''),
    );
  }
  ...
  const stillForMs = () =>
    page.evaluate(() => {
      ...
      for (let i = rows.length - 2; i >= 0; i -= 1) {
        const moved =
          Math.abs(rows[i].lon - last.lon) +
          Math.abs(rows[i].lat - last.lat) +
  ...
  while (Date.now() - startedAt < budgetMs) {
    const file = path.join(
      OUT_DIR,
      `flight-${String(shotIndex).padStart(2, '0')}.png`,
    );
  ...
  report(
    ...
    `${shots.length} frames @ ${SHOT_EVERY_MS}ms`,
  );
  ...
  report(
    peakRoll <= 10.5,
    'bank never exceeds the 10° cap',
    `peak |roll| = ${peakRoll.toFixed(2)}°`,
  );
  report(
    peakRoll > 3,
    'turns actually bank',
  ...
  report(
    Math.abs(rolls[0]) < 1 && Math.abs(rolls.at(-1)) < 3,
  ...
  for (let i = span; i < rows.length; i += 1) {
    ...
    if (!(dt > 0)) continue;
    const headingRate =
      wrapDeg(rows[i].headingDeg - rows[i - span].headingDeg) / dt;
  ...
  report(
    agreement > 0.85,
    'the camera rolls INTO the turn (right turn → right bank)',
  ...
  const peakRate = (valueAt) => {
    ...
    for (let i = 0, j = 0; i < rows.length; i += 1) {
      while (j < rows.length - 1 && rows[j].t - rows[i].t < RATE_WINDOW_MS)
        j += 1;
      const dt = (rows[j].t - rows[i].t) / 1000;
      if (dt >= RATE_WINDOW_MS / 2000)
        worst = Math.max(worst, Math.abs(valueAt(j) - valueAt(i)) / dt);
    }
    return worst;
  };
  ...
  report(
    peakRollRate < 20,
    'the roll enters and exits smoothly, never snaps',
  ...
  const window1s = (from, to) => {
    ...
    return inWindow.length
      ? inWindow.reduce((sum, s) => sum + s.v, 0) / inWindow.length
      : Number.NaN;
  };
  const peakV = Math.max(...speeds.map((s) => s.v));
  const firstSecond = window1s(0, 1200);
  report(
    firstSecond < peakV * 0.5,
    'the dolly eases IN (no velocity step at the start)',
  ...
  report(
    decayMs > 800,
    'the dolly eases OUT over a real ramp, not a hard stop',
    `speed fell 90% → 10% of peak over ${decayMs.toFixed(0)} ms (a hard stop collapses to one frame)`,
  );
  const riseMs = (() => {
    const first = (fraction) =>
      speeds.find((s) => s.v >= peakV * fraction)?.t ?? Number.NaN;
    return first(0.9) - first(0.1);
  })();
  report(
    riseMs > 800,
    'and eases IN over one too',
    `speed rose 10% → 90% of peak over ${riseMs.toFixed(0)} ms`,
  );
  report(
    peakV < 40 * 1.35,
    'the easing keeps the shipped pace — the plateau IS the speed word',
  ...
  for (let i = 0, j = 0; i < speeds.length; i += 1) {
    ...
    if (dt >= 0.3)
      peakAccel = Math.max(peakAccel, Math.abs(speeds[j].v - speeds[i].v) / dt);
  }
  report(
    peakAccel < 40,
    'no velocity discontinuity anywhere on the route',
  ...
  if (probed.length > 20) {
    ...
    report(
      worst > 0,
      'the eye is never inside the RENDERED world',
      `min clearance over the rendered surface ${worst.toFixed(1)} m across ${probed.length} live probes` +
        (worstAt
          ? ` (worst at ${worstAt.lat.toFixed(5)}, ${worstAt.lon.toFixed(5)})`
          : ''),
    );
  } else {
    note(
      'rendered-surface clearance',
      `only ${probed.length} live mesh probes answered`,
    );
  }
  const warm = floors
    .map((f, i) => (Number.isFinite(f) ? heights[i] - f : null))
    .filter((v) => v !== null);
  if (warm.length > 20) {
    ...
    report(
      minAgl >= 90,
      'the eye always clears the rendered floor',
      `min AGL ${minAgl.toFixed(1)} m, max ${maxAgl.toFixed(1)} m over ${warm.length} warm samples`,
    );
  } else {
    note(
      'terrain clearance',
      `only ${warm.length} warm floor cells under the route — clearance clamp is pinned in npm test`,
    );
  }
  ...
  for (let i = 0, j = 0; i < cruiseRows.length; i += 1) {
    while (
      j < cruiseRows.length - 1 &&
      cruiseRows[j].t - cruiseRows[i].t < RATE_WINDOW_MS
    )
      j += 1;
  ...
  report(
    cruiseRangeM > 5,
    'altitude breathes rather than sitting flat',
    `${cruiseRangeM.toFixed(1)} m of vertical range in cruise`,
  );
  report(
    cruiseRangeM < 120,
    'and the cruise altitude never wanders far from its mean',
  ...
  report(
    peakDescentMps < 10.5,
    'the eye is never DROPPED — descent stays inside its cap',
    `peak descent ${peakDescentMps.toFixed(1)} m/s (cap 10 m/s)`,
  );
  report(
    peakClimbMps < 20,
    'and climbs stay a swell rather than a lurch',
  ...
  report(
    pitchSpread < 2,
    'the look-down angle stays locked (no pitch wobble)',
  ...
  report(
    Math.abs(rollBeforeCut) >= 3,
    'the dolly is genuinely banked before the cut',
  ...
  report(
    Math.abs(wrapDeg(cut.rollBefore)) >= 3 &&
      Math.abs(wrapDeg(cut.rollAfter)) < 0.01,
  ...
  if (cut.slotBefore && cut.slotBefore !== 'module-unavailable') {
    report(
      cut.slotAfter === null,
      'a manual pointerdown frees the motion slot synchronously',
  ...
  report(
    Math.abs(rollAfterSettle) < 0.01,
    'and the horizon STAYS level after the cut',
  ...
  const movementBefore =
    before.length > 2
      ? sampleDistanceM(before.at(-3), before.at(-1))
      : Number.NaN;
  let movementAfter = 0;
  for (let i = 1; i < after.length; i += 1) {
    movementAfter = Math.max(
      movementAfter,
      sampleDistanceM(after[i - 1], after[i]),
    );
  }
  report(
    movementBefore > 0.5 && movementAfter < 0.5,
  ...
  const nearestRow = (at) =>
    rows.reduce(
      (best, row) =>
        Math.abs(row.t - at) < Math.abs(best.t - at) ? row : best,
      rows[0],
    );
  const cols = 5;
  const tileW = 384;
  ...
  for (let i = 0; i < shots.length; i += 1) {
    ...
    composites.push({
      input: await sharp(shots[i].file)
    ...
    const sign = roll >= 0 ? '+' : '−';
    labels.push(
      `<text x="${left + 6}" y="${top + tileH + 18}" font-family="monospace" font-size="15" fill="#8fe9ff">` +
