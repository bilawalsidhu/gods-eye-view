...
async function section(name, body) {
  ...
  try {
    ...
    record(
      `[${name}] section completed without throwing`,
      false,
      error.message,
    );
  }
...
async function open(
  ...
  await page.goto(`${APP_URL}/${query}${hash}`, {
...
async function runArbitrationSection(page, { shots, consoleErrors }) {
  await section('esc-arbitration', async () => {
    ...
    record(
      'yielding is session-scoped, like any other dismissal',
      duringScene.session === 'dismissed',
      `session=${duringScene.session}`,
    );
    ...
    record(
      ...
      afterEsc.sceneStillOn
        ? 'scene class untouched by the launcher'
        : 'something else consumed the key',
    );
    await page.evaluate(() =>
    ...
    record(
      'a yielded launcher does not pop back when the scene ends',
      !afterScene.classVisible,
      `classVisible=${afterScene.classVisible}`,
    );
    ...
    record(
      'the launcher is up before the attribution lightbox opens',
      beforeLightbox.onScreen,
    );
    ...
    const attributionFocusRestored = await page.evaluate(
      () =>
        ...
        document.querySelector('#cesium-credits .cesium-credit-expand-link'),
    );
    record(
      'closing attribution restores its disclosure focus',
      attributionFocusRestored,
    );
    await sleep(300);
    const uncovered = await launcherState();
    record(
      'closing the lightbox hands the card back the key',
      uncovered.topmost,
      `topmost=${uncovered.topmost}`,
    );
    await page.keyboard.press('Escape');
    await sleep(600);
    ...
    record(
      ...
      beforeLightbox.onScreen &&
        !afterUncoveredEsc.classVisible &&
    ...
    record(
      'the launcher is up before the radio disclosure opens',
      beforeRadio.onScreen,
    );
    ...
    record(
      'ESC closes the disclosure',
      radioOpened && !radioAfterEsc,
      `disclosure still open=${radioAfterEsc}`,
    );
    record(
      'that SAME ESC does not also dismiss the launcher',
      beforeRadio.onScreen &&
        launcherAfterRadioEsc.classVisible &&
    ...
    record(
      ...
      beforeRadio.onScreen &&
        !escAfterRadioClosed.classVisible &&
    ...
    try {
      await page.goto(`${APP_URL}/?welcome=1`, {
        waitUntil: 'domcontentloaded',
      });
    } finally {
      await page.removeScriptToEvaluateOnNewDocument(
        earlyCockpitBlocker.identifier,
      );
    }
    await page
      .waitForFunction(() => !!document.body, { timeout: 45000 })
      .catch(() => {});
    const holdCockpit = async (ms) => {
      const until = Date.now() + ms;
      while (Date.now() < until) {
        await page
          .evaluate(() => document.body?.classList.add('cockpit-mode'))
          .catch(() => {});
        await sleep(200);
      }
    };
    ...
    const classHeld = await page.evaluate(() =>
    ...
    record(
      'waiting does not burn the session flag',
      withCockpitUp.session !== 'dismissed',
      `session=${withCockpitUp.session}`,
    );
    ...
    record(
      'the launcher appears once the surface clears',
      afterCockpitCleared.onScreen,
      `onScreen=${afterCockpitCleared.onScreen}`,
    );
    if (afterCockpitCleared.onScreen)
      shots.push(await shoot(page, 'arbitration-revealed-after-cockpit'));
  });
}
...
async function main() {
  ...
  console.log(
    `  Mode    : ${TEETH ? 'TEETH (launcher suppressed — expect RED)' : 'normal'}\n`,
  );
  ...
  try {
    ...
    page.on('console', (message) => {
      ...
      if (!/Failed to load resource.*(404|429|503)/i.test(text))
    ...
    await section('show-policy', async () => {
      ...
      const focused = await page.evaluate(
        () => document.activeElement?.dataset?.firstRunChoice ?? null,
      );
      record(
        'focus lands on the first mission tile',
        focused === 'contacts',
      ...
      record(
        'the hidden attribute still hides the flex card',
        hiddenHonored === 'none',
        `computed display while hidden = ${hiddenHonored}`,
      );

      const tiles = await page.$$eval('[data-first-run-choice]', (nodes) =>
        nodes.map((n) => n.dataset.firstRunChoice),
      );
      record(
      ...
      record(
        'ESC dismisses the launcher',
        freshVisible && !(await launcherVisible(page)),
        freshVisible
          ? 'visible → dismissed'
          : 'never appeared, so nothing was dismissed',
      );
      record(
        'ESC writes the SESSION flag only',
        state.session === 'dismissed' && state.durable === null,
      ...
      record(
        'a reload in the same session does not re-nag',
        freshVisible && !(await launcherVisible(page)),
        freshVisible
          ? 'seen this session, stayed away on reload'
          : 'never appeared, so the check is empty',
      );

      // New session (sessionStorage cleared, localStorage kept) → back.
      await open(page, { clearAll: false, clearSession: true });
      record(
        'the next fresh session gets it again',
        await launcherVisible(page),
      );
      ...
      record(
        'the checkbox writes durable suppression immediately',
        ticked.durable === 'suppressed',
        `durable=${ticked.durable}`,
      );
      await page.keyboard.press('Escape');
      await sleep(400);
      await open(page, { clearAll: false, clearSession: true });
      record(
        'a suppressed profile stays quiet in later sessions',
        ticked.durable === 'suppressed' && !(await launcherVisible(page)),
        ticked.durable === 'suppressed'
          ? 'suppressed and stayed away'
      ...
      await open(page, {
        ...
        clearSession: true,
        query: '?welcome=1',
        errorSink: consoleErrors,
      });
      record(
        '?welcome=1 replays past durable suppression',
        await launcherVisible(page),
      );
      await open(page, { query: '?welcome=0' });
      record(
        '?welcome=0 suppresses a fresh session',
        !(await launcherVisible(page)),
      );
      ...
      record(
        'a share link bypasses the launcher',
        shareState && !shareShowed,
    ...
    await section('mission-environmental', async () => {
      ...
      await pick(page, 'environmental', {
        settle: ['earthquakes', 'local-firms'],
      });
      state = await appState(page);
      record(
        'ENVIRONMENTAL leaves the detection override untouched',
        state.detectionOverridden === false,
        `_detectionUserOverridden=${state.detectionOverridden}`,
      );
      record(
        'a mission never auto-suppresses itself',
        state.durable === null,
        `durable=${state.durable}`,
      );
      record(
        'a mission never writes the detection-allocation pref',
        state.allocation === before.allocation,
        `${before.allocation} → ${state.allocation}`,
      );
      record(
        'ENVIRONMENTAL enables BOTH of its feeds',
        state.layers.earthquakes && state.layers['local-firms'],
        `quakes=${state.layers.earthquakes} fires=${state.layers['local-firms']}`,
      );
      record(
        'ENVIRONMENTAL loads real quake records',
        (state.counts.earthquakes ?? 0) > 0,
        `${state.counts.earthquakes} quakes`,
      );
      record(
        'ENVIRONMENTAL reaches the full-earth camera',
        state.heightKm !== null && state.heightKm > 12000,
        `${state.heightKm} km`,
      );
      ...
      console.log(
        `  \x1b[2m   FIRMS key state: ${keyless ? 'KEYLESS' : 'KEYED'} ` +
      ...
      record(
        ...
        !state.layers['local-datacenters'] &&
          !state.layers['local-dams'] &&
    ...
    await section('mission-contacts', async () => {
      ...
      state = await appState(page);
      record(
        'LIVE CONTACTS activates the Contacts context mode',
      ...
      record(
        'LIVE CONTACTS still leaves the detection override untouched',
        state.detectionOverridden === false,
        `_detectionUserOverridden=${state.detectionOverridden}`,
      );
    ...
    await section('mission-space', async () => {
      ...
      state = await appState(page);
      record(
        'SPACE MISSIONS activates its context mode',
        state.contextMode === 'space-missions',
        `contextMode=${state.contextMode}`,
      );
    ...
    await section('mission-explore', async () => {
      ...
      state = await appState(page);
      record(
        'EXPLORE MANUALLY enables no layer and no context mode',
        Object.values(state.layers).every((on) => !on) &&
      ...
      record(
        'EXPLORE MANUALLY never commandeers the camera',
        (state.heightKm ?? 0) < 12000 &&
          (state.heightKm ?? 1e9) <= (beforeExplore.heightKm ?? 1e9),
      ...
      const VIEWPORTS = [
        {
          ...
          height: 812,
          dpr: 2,
          label: '375 x 812 portrait phone',
        },
        {
          name: 'landscape-667',
          width: 667,
          height: 390,
          dpr: 2,
          label: '667 x 390 landscape phone',
        },
        {
          name: 'desktop-800',
          width: 800,
      ...
      for (const vp of VIEWPORTS) {
        ...
        const box = await page.evaluate((sel) => {
          ...
          return {
            ...
            checkboxOnScreen:
              !!cb && cb.top >= 0 && cb.bottom <= window.innerHeight,
          };
        }, LAUNCHER);
        const fits =
          !!box &&
        ...
        record(
          ...
          fits,
          box
            ? `${box.left},${box.top} → ${box.right},${box.bottom} in ${box.vw}x${box.vh}${box.listScrolls ? ' (list scrolls)' : ''}`
            : 'absent',
        );
        record(
          `"Don't show this again" stays on screen at ${vp.width}x${vp.height}`,
        ...
        const peek = await page.evaluate((sel) => {
          ...
          const visible = Math.max(
            0,
            Math.min(listBox.bottom, lastBox.bottom) -
              Math.max(listBox.top, lastBox.top),
          );
          return {
        ...
        const reach = await page.evaluate(async (sel) => {
          ...
          const targets = [
            ...node.querySelectorAll(
              '[data-first-run-choice], [data-first-run-suppress]',
            ),
          ];
          const offscreen = [];
          for (const target of targets) {
            ...
            if (
              ...
              offscreen.push(
                target.dataset.firstRunChoice || 'suppress-checkbox',
              );
            }
          }
        ...
        record(
          ...
          !!reach &&
            reach.count === expectedFocusable &&
            reach.offscreen.length === 0,
          reach
        ...
        await page.evaluate((sel) => {
          const list = document
            .querySelector(sel)
            ?.querySelector('.first-run-choices');
  ...
  const tally = sections.map((name) => {
    ...
    return {
      name,
      total: rows.length,
      red: rows.filter((entry) => !entry.ok).length,
    };
  });
  ...
  const toothless = tally.filter(
    (row) => row.red === 0 && !HYGIENE_SECTIONS.includes(row.name),
  );
  const unreached = EXPECTED_SECTIONS.filter(
    (name) => !sections.includes(name),
  );
  const healthy =
    failed.length > 0 && toothless.length === 0 && unreached.length === 0;
  if (unreached.length)
    console.log(`\n  UNREACHED sections: ${unreached.join(', ')}`);
  if (toothless.length)
    console.log(
      `  TOOTHLESS sections: ${toothless.map((row) => row.name).join(', ')}`,
    );
  console.log(
    healthy
