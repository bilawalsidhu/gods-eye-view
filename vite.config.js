...
import { LOCAL_VOICE_MODEL, VOICE_MODELS, isKnownVoiceTier, resolveVoiceModel } from './src/voice/voiceCost.js';
import {
...
/**
 * Human-readable voice-session trace for the `npm run dev` terminal.
 *
 * The JSONL file is the full record but useless to watch live. This prints the
 * handful of events you actually want while talking to the map: what it heard,
 * what it decided to call, what came back, and what it said. Set GEV_VOICE_LOG=0
 * to silence it.
 */
function logVoiceEvent(record) {
  if (process.env.GEV_VOICE_LOG === '0') return;
  ...
  const line = (label, text) => {
    ...
    console.log(`[voice] ${String(label).padEnd(9)} ${text}`);
  };
  ...
  switch (record?.event) {
    case 'session.starting':
      console.log('');
      line('session', 'starting…');
      return;
    case 'session.token.ready':
      line('backend', p.servedModel || 'unknown model');
      return;
    case 'data_channel.open':
      line('channel', 'open');
      return;
    case 'client.session.update': {
      const tools = p?.message?.session?.tools;
      line('tools', `${Array.isArray(tools) ? tools.length : 0} sent to the model`);
      return;
    }
    case 'client.function_call_output': {
      // The output lives on message.item.output — reading message.output logged
      // an empty string for every call and made working tools look broken.
      const item = p?.message?.item || {};
      let parsed = null;
      try { parsed = JSON.parse(item.output); } catch { /* not JSON */ }
      if (parsed && typeof parsed === 'object') {
        const status = parsed.ok === false ? 'FAILED' : 'ok';
        const detail = parsed.error ? ` — ${parsed.error}` : '';
        line('result', `${parsed.action || 'tool'} ${status}${detail}`);
      } else {
        line('result', compactText(item.output ?? '(no output)', 160));
      }
      return;
    }
    case 'error':
      line('ERROR', `${p.source || 'voice'}: ${p.message || p.errorDetail || 'unknown'}`);
      return;
    case 'session.stop':
      line('session', 'stopped');
      return;
    case 'server.event':
      break;
    default:
  ...
  // server.event — record.payload.type is the Realtime event, .payload its body
  const type = p.type;
  const body = p.payload || {};
  switch (type) {
    case 'input_audio_buffer.speech_started':
      line('listening', 'speech detected…');
      return;
    case 'conversation.item.input_audio_transcription.completed':
      line('heard', quote(body.transcript || ''));
      return;
    case 'response.created':
      line('thinking', '…');
      return;
    case 'response.function_call_arguments.done':
      line('tool', `${body.name}(${compactText(body.arguments || '', 200)})`);
      return;
    case 'response.output_audio_transcript.done':
      line('said', quote(body.transcript || ''));
      return;
    case 'error':
      line('ERROR', compactText(JSON.stringify(body), 200));
      return;
    default:
      return;
  }
...
export function openAiRealtimeProxy() {
  const localAi = createLocalAiRealtime({ environment: process.env });
  ...
  function install(middlewares) {
    ...
    middlewares.use('/api/realtime/token', async (req, res) => {
      ...
      // LOCAL PROVIDER SHORT-CIRCUIT (GEV_VOICE_PROVIDER=local).
      //
      // A self-hosted Realtime endpoint (LocalAI) speaks the same WebRTC
      // handshake as OpenAI: POST an SDP offer to /v1/realtime/calls. So the
      // whole client stays as-is and only the destination changes — we hand
      // the browser a callsUrl instead of rewriting the transport.
      //
      // No ephemeral token is minted (there is no upstream account to scope)
      // and the rate limiter is skipped: it guards SPEND, and local inference
      // spends nothing. `model` is the COST id resolved by voiceCost.js, not
      // the id LocalAI serves — that one rides in callsUrl's query string.
      // The UI toggle (?provider=) wins over the GEV_VOICE_PROVIDER env default,
      // so switching backends is a click rather than a dev-server restart. The
      // env var remains the default for headless/Pinokio runs with no UI.
      ...
      const provider = (requestedProvider || process.env.GEV_VOICE_PROVIDER || '').toLowerCase();
      if (provider === 'local') {
        const localModel = process.env.GEV_LOCAL_REALTIME_MODEL || GEV_LOCAL_REALTIME_MODEL_DEFAULT;
        res.statusCode = 200;
        res.setHeader('Content-Type', 'application/json');
        // Same echo headers the OpenAI path sets — fetchRealtimeToken reads the
        // served model/tier from these first, and `value` is where it looks for
        // the client secret. Matching the existing contract keeps the client
        // unchanged apart from reading callsUrl.
        res.setHeader('X-GEV-Voice-Model', LOCAL_VOICE_MODEL.id);
        res.setHeader('X-GEV-Voice-Tier', 'standard');
        res.end(JSON.stringify({
          value: 'local',
          // SAME-ORIGIN on purpose: the browser posts the SDP offer to our own
          // proxy below, which forwards it to LocalAI. Pointing the browser
          // straight at :8080 needs CORS configured on LocalAI, and without it
          // the offer fails as an opaque "Failed to fetch". Only signalling is
          // proxied — ICE media still flows directly to the backend.
          callsUrl: `/api/realtime/local-calls?model=${encodeURIComponent(localModel)}`,
          session: { model: LOCAL_VOICE_MODEL.id },
          // The OpenAI path bakes instructions + tools into the ephemeral
          // session at mint time. A self-hosted endpoint mints nothing, so the
          // agent would come up as a plain chatbot that never calls a tool.
          // Ship them here for the client to apply as a `session.update` once
...
/**
 * Voice-agent system instructions. Module scope because BOTH providers need
 * them: the OpenAI path bakes them into the minted session, and the local
 * path ships them to the client to send as a `session.update` over the data
 * channel (a self-hosted endpoint mints no session to bake them into).
 */
const GEV_REALTIME_INSTRUCTIONS = [
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    define: {
      ...
      'import.meta.env.GEV_VOICE_PROVIDER': JSON.stringify(env.GEV_VOICE_PROVIDER),
    },
    build: {
