...
// Same 90/300 one-minute bounds as Overpass: a GBFS city pair is two GETs, and
// the layer polls status once a minute, so a real session stays well under the
// cap while a scripted relay against an allowlisted host is cut off.
const _gbfsRateLimiter = makeRateLimiter({ windowMs: 60_000, max: 90, globalMax: 300 });
...
/**
 * Create the testable Connect middleware backing `/api/gbfs`.
 *
 * @param {{
 ...
 */
export function createGbfsProxyMiddleware({
  fetchImpl = fetch,
  rateLimiter = _gbfsRateLimiter,
} = {}) {
...
/**
 * Vite plugin: GBFS bike-share proxy with host allowlisting, size limits,
 * and the shared per-client/global rate limiter.
 *
 ...
 * caps response body at 5 MB, and throttles repeated client requests.
 *
 * @returns {import('vite').Plugin}
 */
function gbfsProxy() {
  return {
    name: 'gbfs-proxy',
    configureServer(server) {
      server.middlewares.use('/api/gbfs', createGbfsProxyMiddleware());
    },
  };
}
