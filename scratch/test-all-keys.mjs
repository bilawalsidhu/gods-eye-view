...
const keysToTest = [
  {
    ...
    test: async () => {
      ...
      return {
        status: res.status,
        ok: res.ok,
        text: (await res.text()).slice(0, 100),
      };
    },
  },
  {
  ...
  },
  {
    ...
    test: async () => {
      ...
      return {
        status: res.status,
        ok: res.ok,
        text: (await res.text()).slice(0, 150),
      };
    },
  },
  {
    ...
    test: async () => {
      const res = await fetch(
        `https://api.cesium.com/v1/me?access_token=${env.CESIUM_ION_TOKEN}`,
      );
      return {
        status: res.status,
        ok: res.ok,
  ...
  },
  {
    ...
    test: async () => {
      const res = await fetch(
        `https://firms.modaps.eosdis.nasa.gov/api/country/csv/${env.FIRMS_MAP_KEY}/MODIS_NRT/USA/1`,
      );
      return {
        status: res.status,
        ok: res.ok,
  ...
  },
  {
    ...
    test: async () => {
      const res = await fetch(
        `https://api.tomtom.com/traffic/services/4/incidentDetails/s3/10/10/10/10/-1/pbf?key=${env.TOMTOM_API_KEY}`,
      );
      return {
        status: res.status,
        ok: res.ok,
...
async function run() {
  ...
  for (const item of keysToTest) {
    try {
      ...
      console.log(
        `[${result.ok ? 'WORKING' : 'FAILED'}] ${item.name} (Status: ${result.status})`,
      );
      if (!result.ok) {
